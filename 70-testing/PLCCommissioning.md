...
**Last Updated:** September 9, 2026 (`G120` resolved) — **`C8` rewritten**: it was titled "AGC feed reaches the screen" and written for FL1/FL3 only because `A3` denied FL2 an instrument. It now runs **all three lines** and **asserts cadence rather than observing it** — ⚠ **4 s is the only cadence any line has on record**, so FL1/FL3's ~10 Hz is ours and `C8` is where it is confirmed. **`C1`** gains the two new FL2 paths and the `PLC-Q15` unit capture *(previously August 13, 2026 — split out of `06-TestPlanAndTestCases.md` in the ProjectPlan restructure. **Section numbers are unchanged**, so every `§n` citation still resolves; numbering inside this file is deliberately non-contiguous)*
...
| # | Test | Method | Pass |
|---|---|---|---|
| **C1** | **Tag paths resolve** | Read every registered tag path in turn. ⚠ **`GetOPCInfo` must return a non-empty `Tags` list first** — if it does not, the registration is missing (`G60`) and `C1` cannot start. ➕ **Two paths joined the list on 9 Sep 2026** — `FL2.PLC.AGC.Gauge` and `FL2.PLC.AGC.Width` (`A3` retired). This test is path-list-driven so it needs no rewrite, only the longer list; ⛔ **but resolve `PLC-Q19` first — if there are TWO gauging stands rather than one, `R6` makes these `AGC1`/`AGC2` and there are four paths to read, not two.** `PLC-Q15` also rides on this test: **record the unit the controller reports**, since gauge in mils rather than inches would pass every structural check and produce scrap | Every path resolves. **Correct any wrong path with an `UPDATE` on that line's `CommonDB.OPCTags` row — no file change and no redeploy** *(`D-44`; this read "correct any wrong path in `appsettings.json`" until 4 Sep 2026)*. Record each corrected `TagKey` → `TagName` so `[DEP §3.2]`'s version-stamped script can be regenerated |
...
| **C8** | **AGC feed reaches the screen — on ALL THREE lines, at each line's own cadence** *(rewritten 9 Sep 2026: this test was titled and written for FL1/FL3 only, because `A3` denied FL2 any gauge instrument. `A3` is retired and `FR-120` superseded — `G120`)* | Run each line at speed in turn. ⚠ **FL2 must be run explicitly** — it is the line this test never covered, and its two paths (`FL2.PLC.AGC.Gauge` / `.Width`) are new and unproven | Gauge and width stream to DB3 on **every** line; **the latency is measured and recorded per line — this is the number OI-34 needs**. ⛔ **Assert the cadence, do not just observe it: FL2 must arrive at ≈4 s and FL1/FL3 at their own rate.** A client-supplied figure of 4 s is the only cadence any line has on record (`PLC-Q11`, answered for FL2 only), so **FL1's and FL3's ~10 Hz is OUR figure and this test is where it is confirmed or corrected**. ⚠ **Record whether the FL2 stream arrives batched or unbatched** — batching a 4 s sample coalesces nothing and only adds latency, and `FW-N29` owes the unbatched send |
