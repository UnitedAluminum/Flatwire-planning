...
| Seed file | Content |
|---|---|
| `FlatWire_SampleData_Lookup.sql` | `Stand` with **fixed IDENTITY** 1=FM1 (12″), 2=FM2_S1 (8″), 3=FM2_S2 (6″), 4=FM2_S3 (6″, final) — **four rows; the former Id 5 is withdrawn** · `Drawer` **2 rows, `DB1`/`DB2`** — ⚠ *the 13 `DIE-0210`…`DIE-0340` size rows moved to* **`ToolingInventoryDie`** *(Id 1–14) with the die split, 2 Sep 2026* · **`ToolingInventoryRollSet`** Id 1–6 · **`ToolingInventoryEdger`** 1=Set `A` (Round), 2=Set `B` (Square) — ⭐ **the ids were carried across deliberately** when `Edger` was absorbed on 6 Sep 2026 (`D-53`), because the schedule seed pairs them with `EdgeType`. ⚠ **The rows carry no `Name` and no alpha** — removed 6 and 7 Sep because nothing read either, so `Set Number` is the only human-readable identifier; **`ToolingInventoryEdgerGauge`** 1-3 on set `A` only (`.045`/`.040`/`.035`) · `Spool` **45 articles** `SP-0001`…`SP-0045` — ⚠ *`SpoolConfiguration` was **merged into `Spool`** on 23 Aug 2026 (`Q60`) and no longer exists* · `AlloyProperty` for 1100 / 1350 / 3003 / 5052 / 6061 |
