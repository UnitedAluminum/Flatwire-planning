# PLC Tag Interface — Implementation Notes
...
**The parameter is `scheduleId`, not `passScheduleId`.** Where the ProjectPlan set names it at all it is `scheduleId` (`04-APIContract.md:463`, `:808`). The April-dated `DevelopmentPlan/APIContracts.md` still uses `passScheduleId`; it is declared superseded by `04-APIContract.md`, and the repo convention is to reconcile the April documents up to the roadmap rather than maintain both.
...
**Deployment:** `SimulatePLCTagPush` is `true` everywhere until commissioning completes (`[DR]:82`, restated in the go-live checklist at `:356`). Verification of a tag push happens **on a stopped line only** (`[DR §4.5]`, step V12). Rollback restores the file and recycles `OPCConnection_<Env>` — configuration only, nothing lost (`[DR §6.2.2]`).
...
**Audit log** (`FR-075` / `INT004`): one line per tag write and per clear, carrying path, value, operator, timestamp and result. Simulated writes log the write they *would* have made. Escalation policy is *any* failure — there is no routine failed tag write, because every one aborted a check-in (`[DR]:637`).
...
`PLCTagService` is **built in Phase 1, exercised first in Phase 4**, and consumed by five phases (`back-matter.md:37`):
...
| `C1` – `C11` | The commissioning sequence | `[PLC §12]` — reproduced; `[TP §8]` stays authoritative for pass criteria |
...
| File | Fix | Blocked on |
|---|---|---|
| `LatestDocument/ProjectPlan/02-SRS.md` `FR-073` | *speed limits* vs *speed targets* — the requirement text and the interface section disagree | **`PLC-Q06`.** Fixing it now would pick a side between a setpoint and a safety clamp |
...
`HMIAndSCADALayout.md` (deleted 4 Aug 2026 with the HMI/SCADA descope) contributed **`PLC-Q02`** and assumption **`A2`** to `[PLC]`; nothing else in it was unique. Both are now `[PLC]`'s only home for those items — **`A2` is read by open question `Q89` and `PLC-Q02` by `Q90`**, so neither can be dropped as stale. **`[PLC]` no longer names that document**, even as the source of `A2`: it is deleted, so a pointer to it is a dead reference. This note is where the attribution lives.

**Every tag path in `[PLC]` is `[PROPOSED]`, and that is now literally true (v1.0, 4 Aug 2026).** The `[CONFIRMED]` tag was **retired from the specification entirely** — the reading convention is two tags. It had rested on the second limb of its own definition, *"stated consistently across the source specifications,"* which let three internal copies of one unverified table agreeing with each other read as confirmation; **no client artifact contains a single tag path.** Two consequences for this file: the startup warning must be **one count, not one per path** (§2), and **the decision log `§12` is gone** — `D14`, `D15` and the rest were removed with the document's revision history, so cite `[PLC §8.1]` for the interlock rule and `Q92`/`Q93` in the register for the decisions behind it. Do not reintroduce a `D##` citation into `[PLC]`.

**`[PLC]` carries no history, by rule (4 Aug 2026).** Both client-facing artifacts — the markdown and `SRS/PLCTagSpecification.docx` — state the specification and nothing it replaced: no old→new tables, no `observed as …` footnotes, no struck-through rules. So when a path in `[PLC §5.2]` looks wrong at commissioning, **`[PLC]` cannot tell you what it used to be.** That trail is in [`Analysis/FlatWireOpenQuestions.md`](../Analysis/FlatWireOpenQuestions.md) — **`Q95`** for the measure segment (all nine measures, old→new) and **`Q94`** for the FM2 station names (`Stand8`, `Stand6S1`, `Stand6S2`) — and in git. **This file deliberately does not duplicate it**, because it carries no tag path strings at all; the register is the single home. `[PLC §4.2]` was renumbered to **R1–R8** in the same change, so any citation of R9 or of the old R5–R8 numbering is stale.
