...
> 3. **Rod uses the existing `coils` table** — the designed `Rod` table is **dropped**; the R-series rod lifecycle (receipt, status incl. `INFLAT`, chemistry/heat, lot) stays in the existing `coils`/`Lots` schema (per the Coil Receiving SRS). `coils` is therefore the single source of truth for rod material, and `FlatWireDB` drops to **21 tables**. Consequence: every `Rod.Alpha` reference (`RodCheckin`, `WeldEvent`, `RollOverride`, `DieChangeEvent`, `CoilTraceability`, `RodCheckout`, `Spool.ParentRodAlpha`) becomes a **cross-database logical reference** to `coils` (no enforced FK) — tracked as G2/G17 in the Gaps register.
