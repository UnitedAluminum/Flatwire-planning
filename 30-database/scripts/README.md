...
⛔ **`35` is the exception, and it is the one worth knowing.** `CREATE VIEW` does **not** defer name
resolution — it binds `CommonDB..WIPStations` at creation time — so `35` is the only file in here
that **fails outright** (`Msg 208`) when its other database is absent or invisible, where the five
procedures would create happily and fail later at run time. It therefore needs `20_` to have run and
`CommonDB` to be visible; its own pre-flight guard reports and skips rather than failing a
`FlatWireDB`-only run.
...
| **10** | *(withdrawn — no file in this folder)* | Reference data | `united_db` (`machines`), `CommonDB` (`WIPStations`, `MachineStationsConfiguration`) | **Rows, not objects** — FL1/FL2/FL3 machines, the WIP stations and their bindings. ⚠ **The rows are still required** — `D-32` keeps the `machines` rows and the `CommonDB` WIP-station registration in scope | `FW-241` *(was `FW-003`)* | ⛔ **WITHDRAWN 6 Sep 2026.** The draft was never run and is no longer delivered from here; authoring the seed **and its reverse** is [`FW-241`](../../10-requirements/features/FS-03-database-foundation/DB/FW-241.md)’s job, under `[DEP §4.2]` step 2. The retired draft — `D1`–`D11`, the `G21` guard, the three-views note — is kept for history at [`95-archive/design-notes/`](../../95-archive/design-notes/10_CommonDB_Insert_WIPStations_FlatWire.sql) and is **not citable as a requirement** | n/a — nothing to reverse until it is authored |
...
| **35** | `35_FlatWireDB_View_WIPStations.sql` | **View** (inbound read) | `FlatWireDB` (home), reads `CommonDB..WIPStations` | `FlatWireDB.dbo.WIPStations` — **two columns**, `Station` and `CoilNo`, both trimmed | `FW-N16` / `D-55` | Draft — **no open items.** It creates a READ and writes nothing, so `D-32` holds. ⛔ **The one file here that cannot be created without its other database present** — `CREATE VIEW` binds at creation time where `CREATE PROCEDURE` defers, so it needs `CommonDB` visible; its own guard reports and skips rather than failing. ⛔ **It does not project `MachineIdx`** — the column a reader reaches for first and the wrong key twice over (unindexed, and `FL1PO` shares FL1's value) | Yes — `99_…Teardown.sql` drops the view; **the shared table is untouched** |
| **36** | `36_FlatWireDB_Synonyms_SharedReads.sql` | **Synonyms** (inbound read) | `FlatWireDB` (home), reads `CommonDB..coils` and `united_db..Alloys` | `FlatWireDB.dbo.coils` and `FlatWireDB.dbo.Alloys` — **name aliases only; they reshape nothing** | `FW-N35` / `P-351`, `P-353` | Draft — **no open items.** Reads only, so `D-32` holds. ⛔ **Each targets a BASE TABLE, never `proddb..coils`** — that is itself a synonym, SQL Server forbids a synonym on a synonym, and the chain fails at *query* time, not at creation. ⚠ **The opposite hazard from `35`:** `CREATE SYNONYM` does not bind, so this deploys green anywhere and its checks come **after** the create, reporting rather than aborting. ⚠ **`20_` is a prerequisite, not a companion** — a synonym carries no rights across the boundary, so a missing `CommonDB` grant surfaces as an error naming *this* database | Yes — `99_…Teardown.sql` drops the synonyms; **the shared tables are untouched** |
