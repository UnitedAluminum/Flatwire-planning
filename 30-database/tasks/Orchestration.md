...
> | Open items and gaps, rolled up | the owning parent’s §6 — [`FS-03`](../../10-requirements/features/FS-03-database-foundation.md) |
> | What this functionality is, and what a change touches | [`FS-03`](../../10-requirements/features/FS-03-database-foundation.md) |
>
...
|---|---|---|---|---|---|
| **[`FW-241`](FW-241.md)** | Deploy step 2 — **author the seed and its reverse script**, run under sign-off | 1C | DB 8 | `[DEP §4.2]` step 2 — ⛔ **no script**; retired draft at [`95-archive/design-notes/`](../../95-archive/design-notes/10_CommonDB_Insert_WIPStations_FlatWire.sql) | ⛔ **The chain's only irreversible step, and the one that has never run.** The code half is now **both** scripts — the seed was withdrawn 6 Sep 2026 and the reverse never existed; the rest is an approval |
...
⚠ **The DB stream has nothing left to build for either event.** All six tables, their FKs, indexes
and **156 rows of inline production reference data** are in the DDL and pass
...
**Minted 3 Sep 2026 — the fourth Tooling Inventory tool type, `[TB §7]` Appendix `B.8`:**

| Story | Subject | Wave · Phase | h | Owning artifact | Closes |
|---|---|---|---|---|---|
| **[`FW-259`](FW-259.md)** | Reconcile `ToolingInventoryRollSet` with the client's roll-set grid when `Q92` returns | 1C | DB 5 | [`01_Lookup`](../sql/FlatWire_DDL_01_Lookup.sql) · [`FlatWireSchema_Lookup.md`](../schema/FlatWireSchema_Lookup.md) | **`G87`** · `Q92` |

⚠ **`FW-259` is the DB third of a set of three** — [`FW-260`](../../40-backend/tasks/FW-260.md) is
Backend's and [`FW-261`](../../50-frontend/tasks/FW-261.md) Frontend's. **No re-cost story:**
`FW-258` still owns the arithmetic and this set is a further input to it, not a second answer.

⚠ **Again the DB stream has nothing left to BUILD.** `ToolingInventoryRollSet`, its two foreign
keys — including **the first ever taken on `Drawer`** — its four indexes and its six seed rows all
landed with `D-42` on 3 Sep 2026, and the baseline is **40 / 64 / 86**, measured. ⛔ **What `FW-259`
exists for is authority, not construction:** dies, edgers and straighteners each arrived as a
**pictured grid with an ordered column list**; roll sets arrived as **one sentence**, so every
column in the table is ours and is marked `[PROPOSED]` at four sites. **`Q92` is the send-back and
`FW-259` is what removes the marks.**
...
| **2** | ⛔ **SIGN-OFF GATE — shared-schema rows.** Seeds `united_db..machines` and `CommonDB..WIPStations` / `MachineStationsConfiguration` | ⛔ **NEVER RUN, AND THERE IS NO SCRIPT** — the draft was withdrawn 6 Sep 2026. The rows are still required (`D-32`); `machine_type` (`D8`), the station set and `StationType` are still pending, and **no reverse script has ever existed**. Run by hand, after approval — `FW-241` |
...
## The edger register — `FW-268`, minted 6 Sep 2026 (`D-53`)

| Story | Subject | Wave · Phase | h | Owning artifact | Closes |
|---|---|---|---|---|---|
| **[`FW-268`](FW-268.md)** | Reconcile `ToolingInventoryEdger` with the client's edger grid when `Q95` returns | 1C | DB 4 | [`01_Lookup`](../sql/FlatWire_DDL_01_Lookup.sql) · [`FlatWireSchema_Lookup.md`](../schema/FlatWireSchema_Lookup.md) | `Q95` · `G104` |

⚠ **`FW-268` is the DB third of a set of three** — [`FW-269`](../../40-backend/tasks/FW-269.md)
is Backend's and [`FW-270`](../../50-frontend/tasks/FW-270.md) Frontend's. **No re-cost story:**
`FW-258` still owns the arithmetic.

⚠ **The DB stream again has nothing left to BUILD.** `ToolingInventoryEdger`,
`ToolingInventoryEdgerGauge`, the gauge foreign key, two indexes and five seed rows all landed with
`D-53` on 6 Sep 2026, `Edger` was dropped, `FK_PSC_Edger` was re-pointed keeping its name, and the
baseline is **46 / 68 / 89**, measured on a teardown-and-deploy against `DEV00164-001`.

⭐ **But this card is NARROWER than `FW-259`, and the reason is the opposite of that one's.**
`FW-259` exists because roll sets arrived as **one sentence** and every column is ours. The edger
grid **was supplied and was followed** — all fourteen columns, in order. What `FW-268` exists for
...
`.134`/`.184` range discrepancy is untouched.
