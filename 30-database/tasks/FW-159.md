...
id: FW-159
legacy_id:
title: RodStaging, the check-in write path and the INFLAT write
status: in-review
status_confirmed: false
status_note: "✅ **Schema BUILT.** ⚠ The write path is owed. ✅ **The stale blocker is identified and removed: `G21`** — resolved 15 Aug 2026 as one physical station and fixed in three layers, so it left `blocked_by` on 7 Sep 2026 *(the note said only that a blocker was stale, without naming which)*. ⛔ Still blocked by `G2` (check-in spans `FlatWireDB` and the shared schema in one transaction) and `G17` (rod→coils multiplies cross-DB logical FKs) [9 Sep 2026, client answers] Q68 - the rule this story already applies to the rod status now extends to all three remaining shared writes: they happen at check-in, never at staging. See FW-N24."
owner:
jira:
mvp: 1
phase: "4"
...
| Concern | Owner |
|---|---|
| The staging **commands** that write the table | [`FW-158`](../../40-backend/tasks/FW-158.md) |
| The check-in that sets `Rod.Status = 'INFLAT'` | [`FW-157`](../../40-backend/tasks/FW-157.md) |
| The shared-schema write-back | [`FW-220`](../../40-backend/tasks/FW-220.md) — `40_`, Draft |
...
| Object | Where | State |
|---|---|---|
| **`RodStaging`** | [`FlatWire_DDL_04_Runs.sql`](../sql/FlatWire_DDL_04_Runs.sql) **`:74`** | ✅ **Built** |
| **`UX_RodStaging_Bay`** | [`FlatWire_DDL_07_Indexes.sql`](../sql/FlatWire_DDL_07_Indexes.sql) **`:102`–`:103`** — filtered UNIQUE | ✅ **Built** |
| `UX_RodStaging_RodActive` | same file | ✅ Built |
| The listed indexes | `RodCheckin(RunId)`, `(RodAlpha)`, `(MachineName, PayoffPosition)`, `RodStaging(MachineName, Status)`, `(RodAlpha)` | ✅ Built |
| `PayoffPosition` lookup + 3 pinned rows | [`FlatWire_DDL_01_Lookup.sql`](../sql/FlatWire_DDL_01_Lookup.sql) `:257`–`:285` | ✅ Built — row `3` is `TraversingTakeup (FL2)` |
...
| **`FL1PO` seeded** | deploy step 2 — [`FW-241`](FW-241.md) | ⛔ **No script.** The draft was **withdrawn 6 Sep 2026**, having never run |
...
*"`FL1PO` WIP station seeded"* is [`FW-241`](FW-241.md)'s job — **deploy
...
3. The write path lands with
   [`FW-158`](../../40-backend/tasks/FW-158.md)
   (staging) and
   [`FW-157`](../../40-backend/tasks/FW-157.md)
   (check-in + `INFLAT`).
...
[`FW-158`](../../40-backend/tasks/FW-158.md) writes
the staging rows and **must not scope the bay conflict by line** (`P-183`).
[`FW-157`](../../40-backend/tasks/FW-157.md) makes the
