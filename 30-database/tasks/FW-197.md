---
id: FW-197
legacy_id:
title: Reference-data admin wiring
status: not-started
status_confirmed: true
owner:
jira:
mvp: 1
phase: "13"
stream: DB
streams: [DB]
priority: medium
hours: 8
sprint: S3
depends_on: [FW-005, FW-196]
blocked_by: [OI-43, OI-77]
has_plan: false
started:
completed:
---

# FW-197 - Reference-data admin wiring

> **No implementation plan has been written for this story yet.**
> The card below is the contract from `[TB]`. Before starting, replace this notice
> with the sections in the task template: *What to build* / *Context you need* /
> *Build order* / *Decisions made here* / *Verification* / *Handoff*.

## 1. What to build

**Hours:** 8 h DB · **Priority:** Medium · **Sprint:** S3 · **Phase:** 13 · **Stream:** DB

**As a** developer,
**I want** the non-die lookups editable through the admin surface,
**So that** stands, drawers, edgers and spool configurations are maintainable.

**Acceptance Criteria:**
- [ ] `Stand`, `Drawer`, **`ToolingInventoryEdger`** and `Spool` reachable through the admin endpoints — ⚠ **`Edger` was absorbed 6 Sep 2026 (`D-53`)**; the register is now the fourteen-column tooling table, and its gauge rows are reached **through it**, never on their own
- [ ] Audit columns populated on edit
- [ ] **`Drawer` is already seeded in Phase 1** — this exposes it, it does not create it
- [ ] **No die inventory table is created** — die inventory and lifecycle are owned outside MVP-1

**Rate-card basis:** non-die reference-data wiring 8 h (§2, §3b — this is the surviving DB line after the die-table carve)
**Dependencies:** FW-005, FW-196
**Blockers:** **OI-77 / OI-43** *(edger profiles and roll spares)*

---
