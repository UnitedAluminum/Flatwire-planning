...
id: FW-180
legacy_id:
title: SpoolCheckin table and the SpoolProcessing.OrderNo index
status: in-review
status_confirmed: false
status_note: "✅ **Table BUILT.** ⛔ **`OI-25`'s two footage coordinate systems are unreconciled**"
owner:
jira:
mvp: 1
phase: "8"
...
> [`FW-246`](FW-246.md) resolves it; `P-163` recommends **`3`**.
...
| Concern | Owner |
|---|---|
| The **write path** | ⚠ **Backend** — [`FW-179`](../../40-backend/tasks/FW-179.md) |
| `CK_SpoolCheckin_PayoffPos`'s wrong value | ⛔ [`FW-246`](FW-246.md) — `G55`, `P-163` |
| The spool this checks in | [`FW-202`](../../40-backend/tasks/FW-202.md) — nothing else creates it |
| The FL2 null-gauge contract | [`FW-181`](../../40-backend/tasks/FW-181.md) |
...
✅ **[`FW-246`](FW-246.md)'s `P-163` recommends `3`** and notes that
...
⚠ **This is the same blocker [`FW-202`](../../40-backend/tasks/FW-202.md)
...
3. ⛔ **Escalate `OI-25`** — it decides AC 4's trace projection, not just downstream arithmetic
   (§2.2).
4. The write path is [`FW-179`](../../40-backend/tasks/FW-179.md)'s.
...
| Check | Expected |
|---|---|
| Table | `SpoolCheckin` present; `MachineName` restricted to **FL2/FL3** |
...
[`FW-179`](../../40-backend/tasks/FW-179.md) owns the
write path — ⚠ **and `[API §4.6a]`'s worked example is stale**.
[`FW-246`](FW-246.md) fixes `G55` and `G50`'s hole (b), which is what
made `G55` possible. [`FW-202`](../../40-backend/tasks/FW-202.md)
creates the spool this checks in and shares `OI-25`.
[`FW-181`](../../40-backend/tasks/FW-181.md) governs what the
FL2 view shows.
...
**`SpoolOrder` has been re-grained since this story shipped.** `D-57` re-parented it from
`SpoolProcessing.Alpha` to **`SpoolTraceability.Id`** — one row per (segment, order) — because keyed on
the spool it named the rod nowhere, and its pound range could not be joined to `SpoolTraceability`'s
nullable feet. It now carries `AllocatedWeightLb`, a segment-local **pound** range and a spool-local
**footage** range, and its uniqueness moved from the inline `UQ_SpoolOrder_Key` to the filtered
`UX_SpoolOrder_Active`.

⛔ **Nothing here is edited** — this card records what was built on 22 Aug 2026 and stays as written.
For the current shape read [`FW-N20`](FW-N20.md), `[DBD §6.2]` and `FlatWireSchema_Materials.md`.
