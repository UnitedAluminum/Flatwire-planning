...
flowchart TB
  subgraph FLOOR["Shop floor"]
    P1["FL1 panel<br/>1920 × 1080 touch"]
    P2["FL2 panel"]
    P3["FL3 panel"]
...
>
> The **24–64 h reserve should be re-derived before S2 planning** rather than after: its cross-database portion is spent, its PLC portion stands. Phase 4 is the only phase already flagged provisional, so getting this netting wrong in either direction distorts it. See [`FW-220`](../10-requirements/features/FS-15-shared-schema-boundary/BE/FW-220.md) and `[INT §8.0]`.
...
>
> **`D-18`–`D-28` are NOT here** — they exist only in master spec §10.1. **`D-29`–`D-33` and `D-44`–`D-52` are here and nowhere else** *(this read "`D-44`" until 6 Sep 2026, when `D-52` was minted; `D-45`–`D-51` had already been added below without the sentence being updated)*. So neither table is complete on its own.
...
| **D-51** *(6 Sep 2026)* | **`FW-236` / `G94` must be present in the branch the deployed `OPCConnection` is BUILT FROM — not in `release/mvp-3.x`.** `FW-238` step 6 spent three passes gating on a merge into `release/mvp-3.x`, and that branch **does not contain `API/Domain/FlatWire` at all**: flat wire is built and deployed from `feature/UADEV-23146`, where `332060b7f` already lives. ⚠ **The gate was pointed at a branch that does not carry the module it was protecting.** The predicate becomes *"is `332060b7f` an ancestor of the branch this environment’s `OPCConnection` is built from"*, re-asked **per environment** rather than once. ✅ **`G94`’s diagnosis is unaffected and `G94` stays OPEN**: on `release/mvp-3.x`, `GetOpcUaTagName()` is wired into `SubscribeDataChange` (`:247`) only, so `ReadTag`/`WriteTag` there still address namespace **0** — measured 6 Sep 2026. It is simply not a statement about flat wire. ⛔ **This does NOT turn the write path on** — `P-320` is untouched and both simulate flags stay `true`. `FW-238` §3 step 6, `P-333` | `FW-238` | Client / delivery direction, 6 Sep 2026 |
| **D-52** *(6 Sep 2026)* | **`FW-145` — JWT role authorization policies — is MVP-2. MVP-1 ships on bare `[Authorize]` with no role policy and no `403`.** The six `[SEC §8]` roles exist as JWT claims on `ClaimTypes.Role` (`G6`, answered 15 Aug 2026); what has never arrived is the label→**value** mapping, and that gates *verifying* a policy, not writing one. Rather than hold an unverifiable story on the MVP-1 critical path, the whole of `FW-145` moves to MVP-2 and **`G6` leaves its `blocked_by`**. ⚠ **Two consequences, and the second is the reason the first is acceptable now.** **(1)** `[SEC §8]`'s authorisation matrix becomes an MVP-2 obligation — until it lands, any authenticated caller reaches any endpoint `FlatWire.API` exposes. ⛔ **This is a statement of the state that already exists, not a regression**: `[FW-146 §5.1]` measured *"there is no policy-gated endpoint … nothing in this service can return a `403`"*. **(2) `G72` closes for MVP-1** — `/sim/**` drops from `RequireAuthorization(SimulatorControl)` to bare `[Authorize]`, so the seven built simulator stories and the `DB-S1` console become driveable instead of denying every caller on six `TBD` constants. ⚠ **The `?access_token=` handler does NOT move** — `FW-080` built it inside its own hours and the hub depends on it; only the role policies travel. ⚠ **`FW-145` keeps `phase: "1B"`** because `check_docs.py` rule 5 requires a real phase file and MVP-2 has none — `mvp: 2` is the scope of record. Re-priced additively in `[CE §3h]`: Phase 1B `442 → 420 h`, MVP-1 all-in `3,358 → 3,336 h` | `FW-145`, `[SEC §8.8b]`, `[SIM §8.4]`, `G6`, `G72`, `[CE §3h]` | Delivery direction, 6 Sep 2026 |
...
| **`AlloyProperty` shadows `united_db..alloys`**, and FW-054 is concurrently pushing *more* alloy data upstream | The generator runs off a provisional seed while the maintained value sits upstream | Create the `FlatWireDB..Alloys` **synonym** (`[DBD §6.6]`, corrected 10 Sep 2026 — `P-353`; it read *view* until then) and read `Draw_max_reduction` across |
