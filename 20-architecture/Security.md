...
| **Authorisation** | **`Engineering/Maintenance`** / `Admin` only, **on top of** the above. Never `Operator` *(this said `Engineer`, which is not one of the matrix's six role names — and the constants class the guards bind to has to use the matrix's spelling)*. ⛔ **The built surface still carries the short spelling** — `G72`, owner `FW-215`: `SimControlSurface.cs` hardcodes `Roles = "Engineer,Admin"` instead of binding `FW-145`'s **`SimulatorControl`** policy, so it will keep denying after the claim is issued |
