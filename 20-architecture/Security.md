...
**Last Updated:** September 6, 2026 (`D-52`) — ⭐ **§8'S AUTHORISATION MATRIX IS NOW AN MVP-2 OBLIGATION.** `FW-145` — the only story that registers a role policy — is carved to MVP-2, so **MVP-1 authenticates and does not authorise**: bare `[Authorize]` everywhere, no policy, **no `403` reachable anywhere in this service**. ⛔ **A statement of the existing condition, not a new one** — `[FW-146 §5.1]` measured it on 27 Aug 2026 — but it is now the plan rather than a gap waiting on `G6`. ⚠ **Say it out loud at client review**: six columns of ticks read as delivered capability. ✅ **§8.8b's Authorisation row is superseded for MVP-1** — `/sim/**` drops to bare `[Authorize]`, `G72` closes for MVP-1 (by removing the guard, not satisfying it) and the built simulator becomes driveable; **`G66`'s `404` is the only control left on that surface**. ⚠ **Two things do NOT defer:** `FR-212` is client-side and still owes §8.8's three-option choice, and `FW-177`'s SignalR role targeting still needs `G6`'s claim **values**, where they fail *silent*. ⚠ **Also struck as stale, and stale before this pass:** `SimControlSurface.cs` has not hardcoded `"Engineer,Admin"` since `FW-215`'s build of 1 Sep 2026 (`P-310`). *(previously September 1, 2026 — **§8.8b's two enforcement rows corrected against the built surface, while reconciling the [`FW-215`](../10-requirements/features/FS-04-realtime-plc-backbone/BE/FW-215.md) card.** ⚠ **Registration named the wrong flag:** the key is **`FlatWireOpc:SimulateOpcFeed`**, the READ half of the pair — `SimulatePLCTagPush` is the write half and stays `true` in every environment until PLC commissioning, so gating on it would map the control plane against a **real** feed. `G70`'s widening (`FW-217`'s double needs `SimulateOpcFeed` false and is still synthetic) is recorded with it. ⛔ **New: `G72`** — `SimControlSurface.cs` hardcodes `Roles = "Engineer,Admin"`, and this document already recorded on 27 Aug 2026 that **`Engineer` is not one of the six role names**; it must bind `FW-145`'s **`SimulatorControl`** policy, or it keeps denying after the claim is issued. The prefix is also restated as **`/sim/**`** *(previously August 29, 2026 — **§8.8b restated for `D-33`: `DB-S1` is a standalone WinForms executable, not an Angular route.** *“The console route does not resolve when simulation is off”* becomes a **startup probe of `GET /sim/state` with a lock-out panel on 404**. ⚠ **This does not weaken §8.8b, and a new note says why**: the Registration row is enforced **server-side** — `MapFlatWireSimControlSurface` returns before mapping anything — so `/sim` is 404 to any caller regardless of client. What changed is that the tool no longer ships inside the operator bundle to machines that must never run it. Residual: **`G66`** *(previously August 27, 2026 — ⛔ **the `G6` residual is two problems, not one, and this document said it was one.** *"Gates verification, not construction"* is true of the server and **false of the client**: measured in `Second-Branch/ual-angular`, there is **no role field on `LoginStatusDetails`, no role vocabulary in `shared.constants.ts` and no JWT decoder anywhere**, so **`FlatWireRoleGuard` cannot be built** — §8 now splits the two sides and tables the three ways to give the client a role, which **this document owes**. ⚠ **`FlatWireRoleGuard`'s stated MVP-1 subject was DB9/DB9A, which are MVP-2** — the live MVP-1 gate is `FR-212` on DB11, as §8.9's own last row already said. ⚠ **The API citation was wrong** — `[API §9]` is Traceability; the per-endpoint roles are `[API §3.2]`'s. ⚠ **§8.8b's endpoint count was 30** against `[API §3.2]`'s **32**, and its `Engineer` role name is not one of the matrix's six. ⚠ **§8.9's Supervisor primary screen is also MVP-2** and only the Operations Manager's was marked. *(previously August 15, 2026 — **`OI-37`/`G6` confirmed**: all six roles exist as JWT claims on `ClaimTypes.Role`; §8's two *unverified* callouts restated. Residual: the claim **values** are coded and unmapped)*)*)*)*
...
> ### ⛔ THE MATRIX BELOW IS AN **MVP-2** OBLIGATION — `D-52`, 6 Sep 2026
>
> **`FW-145`, the only story that registers a role policy, is MVP-2.** Read every `✓`/`✗` in §8 as
> *what will be enforced*, not *what is enforced*.
>
> | | MVP-1 (now) | MVP-2 (`FW-145`) |
> |---|---|---|
> | **Authentication** | ✅ Unchanged. JWT throughout, `[Authorize]` on all 14 controllers, `?access_token=` on the hub — all **built** (`FW-138`, `FW-080`). No token still means `401` | same |
> | **Authorisation** | ⛔ **None.** No role policy is registered, so **any authenticated caller reaches any endpoint this service exposes**, and **nothing here can return a `403`** | The matrix below, enforced |
>
> ⛔ **This states an existing condition; it does not create one.** `[FW-146 §5.1]` measured it on
> 27 Aug 2026 — *"there is no policy-gated endpoint … nothing in this service can return a `403`"* —
> and `[FW-138]` shipped bare class-level `[Authorize]` by design, deferring policies here. What
> `D-52` changes is that it is now **the plan** rather than a gap waiting on `G6`.
>
> ⚠ **Say this out loud when the matrix is reviewed with the client.** A six-column table of ticks
> reads as delivered capability, and for MVP-1 it is not.
>
> ⚠ **Two things do NOT defer with it.** **(a) `FR-212`** — the one live MVP-1 role gate, on DB11 —
> is a **client-side** requirement and is unaffected by which MVP registers server policies; the
> three-option choice in §8.8's residual box is still owed. **(b) `FW-177`** targets SignalR groups
> by role name in MVP-1, so `G6`'s unmapped claim **values** still bite, and there they fail
> **silent** rather than closed.
...
(**`/sim/**`**, story `FW-215`) drives the machine model. With simulation **off**, the subsystem
it drives talks to a real mill.

| Control | Rule |
|---|---|
| **Registration** | ⚠ **When no line model is hosted the routes are not registered at all — `404`, not `403`.** The key is **`FlatWireOpc:SimulateOpcFeed`** *(this said `SimulatePLCTagPush` until 1 Sep 2026 — that is the **write** half of the flag pair and stays `true` in every environment until PLC commissioning, so gating on it would map the control plane against a real feed)*. ⛔ `G70` widens the test to include `FW-217`'s `OPCConnection` double, which needs `SimulateOpcFeed` **false** and is still a synthetic feed |
| **Authorisation** | **`Engineering/Maintenance`** / `Admin` only, **on top of** the above. Never `Operator` *(this said `Engineer`, which is not one of the matrix's six role names — and the constants class the guards bind to has to use the matrix's spelling)*. ⭐ **SUPERSEDED FOR MVP-1 BY `D-52` (6 Sep 2026): this row states the MVP-2 rule. In MVP-1 the surface carries BARE `[Authorize]` and no role check at all.** `FW-145` is MVP-2, so there is no `SimulatorControl` policy to bind; **`G72` closes for MVP-1 by removing the guard** and reopens in MVP-2 as a one-line swap. ⛔ **`Never Operator` is therefore NOT enforced by authorisation in MVP-1** — the Registration row above (`404` when no line model is hosted) is the whole of the control, which is exactly the trade `G66` says must be made knowingly. ⚠ **Two older claims in this row are struck as stale, and were already stale before `D-52`:** `SimControlSurface.cs` **no longer hardcodes** `Roles = "Engineer,Admin"` — `FW-215`'s build of 1 Sep 2026 (`P-310`) deleted the literal and bound `FlatWireRoles`, whose six constants are `TBD`; the *"will keep denying after the claim is issued"* consequence went with it |
| **Console** | ⚠ **`DB-S1` is a standalone WinForms executable since `D-33` (29 Aug 2026), not an Angular route.** It **probes `GET /sim/state` at startup and shows a lock-out panel on `404`**, which is the client half of the Registration row above. Installing it only where it is needed is a **procedure, not a build-time fact** — `G66`. See `[SIM §9.4]` |
...
> **`D-33` does not weaken this, and the reason is that the control was never on the client.** The
> Registration row is enforced in `FlatWire.API` — with simulation off, `MapFlatWireSimControlSurface` returns
> before mapping anything, so `/sim` is 404 to **any** caller, Angular or desktop. What changed is that the
> tool is no longer shipped inside the operator bundle to machines that must never run it.
