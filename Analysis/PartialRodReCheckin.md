...
**Question Ref:** Q47 — FlatWireOpenQuestions.md
**Priority:** High
**Owner:** Jaspreet / Tim O.
**Status:** In Progress — Partial answers received May 4, 2026; carry-forward design deferred
...
- **Footage-to-Weight Accounting (Q36)** — Because weight is derived from footage (not a scale), the carry-forward footage figure is the only way to calculate remaining rod weight correctly on the second run.
- **Rod Checkout Authorization (Q48 / Q50)** — The disposition authority question (operator vs. supervisor) only makes sense once it's settled whether the rod re-enters inventory as a tracked partial item or disappears from the system.
...
|---|---|---|
| Q36 | Footage-to-weight conversion factor | Remaining weight estimate calculation depends on this factor being defined per alloy/cross-section |
| Q48 | Mid-run rod checkout authorisation level | Who is permitted to remove a rod mid-run and trigger the carry-forward flow |
| Q49 | PLC tag behaviour on rod checkout | Whether PLC tags are cleared immediately or after a safe-stop handshake when checkout is confirmed |
| Q50 | Partial-run material disposition authority | Whether operator alone can accept partial spool footage or supervisor approval is required |
| Q55 | Spool alpha continuity through anneal or re-pass | Whether a partial spool alpha retains its identity after an anneal between runs |
...
- [x] **Q48 Resolved (May 4, 2026):** Supervisor approval required for mid-run checkout. The carry-forward screen will require supervisor sign-off before the new run is opened.
- [x] **Q50 Resolved (May 4, 2026):** Supervisor must approve material disposition (accept / hold / reject) before partial spool alpha is generated.
