...
**Total Questions:** 99 · **Shopfloor scope:** 72
...
**72 of 99 questions relate to Flat Wire Mill shopfloor changes** — the operator execution screens (Dashboards 1–12+) plus the reference data, equipment limits and validation rules those screens consume. The remaining 27 belong to adjacent modules (orders/quotes, pricing, costing, planning, scheduling, receiving, certification, maintenance) and are retained here marked `Scope = Other`.
...
| 79 | ~~Multi-order rod — sequencing rule and MVP1/MVP2 scope~~ — **DECIDED: full coils → partials → multi-order coils last** when welding is involved; pick list in planned order; validated at pre-check-in **and** check-in | `Shopfloor` | Medium | Srikanth / Tim O. | **Decided** | Aug 6, 2026 |
...
| 96 | Wire break where the **customer accepts no welds** — the disposition set, the supervisor gate and where the decision is persisted | `Shopfloor` | High | Tim O. / Shannon R. | Open | Principle Aug 6, 2026 |
| 97 | **FM2 dancer modes** (dancer vs tension) — who selects, per dancer or per line, scheduled or machine-side, read or written | `Shopfloor` | High | Tim O. / Engineering | Open | |
| 98 | **Edger roll-forming coefficients** — edging is a forming operation, not a trim; the formulas are owed (`PSG-D24`–`D26`) | `Shopfloor` | High | Tim O. / QC | Open | |
| 99 | Does **multi-order-last** survive when **no welding** is involved? Both readings are on the 6 Aug recording | `Shopfloor` | Medium | Srikanth / Tim O. | Open | |
...
**Q79** · `Medium` · Owner: Srikanth / Tim O. · ~~`Open`~~ **`DECIDED Aug 6, 2026`**
...
> 6. Where **no welding** is involved the sequence is free — subject to **Q99**.
...
>
> **MVP scope:** the MVP2 deferral floated on 30 Jul was **not** taken — the validation was assigned to the flat wire interface on this call *(“please make sure you capture these in the validations”)*, i.e. MVP1.
>
> **What this unblocks:** **G22** can now be written. `planning_routings` returns **orders (plural)**, "belongs to the established order" becomes membership in an **ordered set**, `RodStaging.OrderId` means *the order this staging is being consumed for*, and the ordering constraint is the three-tier sequence above. See [ClientCall_2026-08-06_SyncPlan.md](ClientCall_2026-08-06_SyncPlan.md) §3.1.

**Original question — superseded by the decision above, retained for the audit trail.**
...
~~**Until this closes, the staging validation is a known-wrong rule rather than a rule to edit.**~~ **Closed Aug 6, 2026.** [RodPreCheckin.md](../LatestDocument/RequirementDocuments/RodPreCheckin.md) refuses any rod whose order differs from the established one; Q69 made that wrong for the same-rod successor, and the replacement is now defined by the decision above. The minimum change, unchanged in shape: the `planning_routings` lookup returns **orders (plural)**, "belongs to the established order" becomes membership in an **ordered set**, and `RodStaging.OrderId` means *the order this staging is being consumed for*, with the successor visible in the queue — **plus** the three-tier ordering constraint.

**Answers to the three sub-questions.** (1) Staging R4 before R1–R3 is **refused**, not overridden — the multi-order rod is last by rule, and this is stricter than the `Q74` out-of-sequence authorisation. (2) Not put to the client explicitly; the pick-list model implies the rod is staged against the order **currently being consumed**, with the successor visible — carry forward under **Q99** if it bites. (3) **MVP1** — the validation was assigned on the call.

Related: **Q69**, **Q74**, **Q47** (partial rod carry-forward — the same physical remainder), **Q57**, **Q99** (the no-welding branch), gap **G22**.
...
**Q96** · `High` · Owner: Tim O. / Shannon R. · `Open` — principle decided Aug 6, 2026
**Wire break where the customer accepts no welds — what happens to both pieces?**

**Q79**'s companion case, raised by Yogender on the 6 Aug call. **Q27**/`OI-13` settle the normal break: weld and continue on the same alpha (see the decision recorded against `OI-13`). This asks what happens when the customer's specification **forbids welds**, so weld-and-continue is not available.
...
Related: `OI-13`, **G34**, **Q50** (partial-run disposition — the same decision shape), **Q65** (short close), **Q22** (weld footage attribution — why a no-weld customer is knowable at all).

---

**Q97** · `High` · Owner: Tim O. / Engineering · `Open`
...
1. **There is no dancer on FM2 anywhere.** The dancer appears only as an FM1 component ([FlatWire_MasterSpecification.md](../LatestDocument/FlatWire_MasterSpecification.md) §156, [FlatWireProcessWalkthrough.md](FlatWireProcessWalkthrough.md) step 19). No lookup table beside `Stand`/`Drawer`/`Edger`, no `PassScheduleComponent` row, no seed data.
2. **No tag element addresses a dancer** on any line, so a selectable mode has nowhere to be written and nothing to read back.
...
Related: **Q30** (roll gap validation — the other per-component setpoint), **Q86**/**G29** (the edgers, which sit in the same inter-stand positions), **`PSG-Q29`**/**`PSG-D27`** (the same question, client-facing), `PLC-Q##` to be raised once 3 is answered, gap **G35**.

---

**Q98** · `High` · Owner: Tim O. / QC · `Open`
**Edging is a forming operation, not a trim — the coefficients are owed**

Pointer entry. Tim's 6 Aug correction to [PassScheduleGenerationSpec.md](../LatestDocument/RequirementDocuments/PassScheduleGenerationSpec.md): *"Edging in this document is referred to as a knifing operation, where in our case we are using vertical profiling rollers to create our edge. It's not a cutting capability, it's an actual roll-forming capability. So what that means for the pass schedule generation is there's actually another subset of formulas that needs to be used and they need to be treated like their own operation."*

The mechanism as described: the edger **narrows width**; the displaced material expands **laterally and longitudinally**, so length increases and the section **bulges in the middle** (thickness rises slightly); the next stand flattens the bulge and the edge widens again; the second edger narrows it back. **The first edger also converts the section from round to square.** The final 6″ pass at S3 is a **skim pass** — light reduction, **no edging** — sized for a uniform cross-sectional area edge to edge.

**The physics lands in the generation spec's own registers**, which is why this entry is a pointer rather than the specification: **`PSG-D24`** (edger width-reduction limit per pass), **`PSG-D25`** (the spread/bulge partition coefficient — how removed width divides between length and thickness), **`PSG-D26`** (the round→square transition condition at the first edger), and **`PSG-Q27`**/**`PSG-Q28`**. All are trial-calibrated in the same way as `PSG-D08`.

This register carries the item so that the owed data is visible outside the spec — the spec is a client deliverable and its `PSG-` series is not tracked here otherwise.

**Widened Aug 6, 2026 by a gap audit of the specification.** The partition coefficient was found to be asked for **against an undefined function** — Step 9A read `Δt = φ · f(…)` with `f` unnamed, the only relation in the document without a named basis. The relation is now stated as volume-constant strain decomposition and tagged `[RECOMMENDED DEFAULT]`, so what is owed is a coefficient to a defined function; **whether that function is the right shape** is a separate question (`PSG-Q31`), because if the bulge is local to mid-width rather than distributed, no value of `φ` generalises. The same audit added **`PSG-D28`** (no-draw threshold — used in the algorithm but never defined), **`PSG-D29`** (strain→temper — `V16` had no method and target temper was a mandatory input nothing consumed), **`PSG-D30`** (density / footage-to-weight — **shares a factor with spool completion and the coil label, and overlaps `Q36`**, which is Critical here) and **`PSG-Q30`** (is FL2 ever scheduled independently — the engine had no path for a flat-spool input at all).

**Widened again the same day by a second audit** (spec issued as **v1.5**). Five more items are owed, and two of them are wrong-in-the-unsafe-direction rather than merely missing:

- **`PSG-D31` — rolling friction, separately from drawing friction.** `PSG-D04` had asked for **one** coefficient covering both die/wire and roll/material contact. The bite condition `Δh ≤ μ²R` is **quadratic in `μ`**, so across the 0.03–0.10 range quoted for drawing the admissible draft varies about **11×** — enough that the bite limit is either the binding constraint on every FM2 pass or invisible on all of them. **High.**
- **`PSG-Q32` — how the plant judges bite on the round → flat pass.** The flat-entry relation does not describe a round entry, and applied to it literally it demands `μ ≥ 0.176` and **rejects the spec's own nominal worked example**. **High.**
- **`PSG-D32` — the planned output multiple per line.** Step 12A had split orders against the take-up **ratings** (3,500 lb / 1,100 lb) rather than the weight units are actually produced at — which the client corroborated on this same call as **~1,800 lb on FL1 and 800/900 lb on FL2** (`D8`, and `Q60`/[Spool.md](Spool.md)). The two differ by roughly **2×**, so the reported unit count was about half the truth. **This figure already exists in planning and must not resolve to two values.**
- **`PSG-D33`** — the as-rolled incoming edge geometry, without which the FL2 spool path cannot compute the section it starts from; **`PSG-D34`** — the lever-arm coefficient, without which drive power (a stated limit with an Error-severity check) was never computed at all; **`PSG-D35`** — coefficient sanity bounds and revalidation interval, which two validations checked against data no item had ever requested.
- **`PSG-Q33`** — which edger bypass combinations are permitted, given that bypassing E1 removes the round→square transition and bypassing S2 leaves the two edgers adjacent; **`PSG-Q34`** — whether a requested line is a constraint the engine must honour or a default it may override.

The same audit found that **`PSG-D08` is not an FL1-only item**: there is no edger at the mill exit, so the **final S3 pass is unedged** and the delivered width of every FL2 and FL3 coil is set by free spread exactly as the FL1 spool's is. The flat→flat spread coefficient therefore governs finished-goods width, which raises the priority of instrumenting **S3's exit** in the same trial campaign as FM1's.

Related: **Q42** (edger blade profiles — the reference data), **Q86**/**G29** (edger tags), `PSG-D08` (free spread at FM1 — the other uncalibrated relation), **Q12** (edge as a critical attribute).

---

**Q99** · `Medium` · Owner: Srikanth / Tim O. · `Open`
**Does "multi-order coils last" survive when no welding is involved?**

The one branch **Q79** did not close cleanly. Srikanth's rule was scoped to welding: *"if welding is not involved, then… they can go in any which sequence, right? Because it's harmless."* Yogender then argued the multi-order case is different regardless (14:28): *"in that case as well, the multiple order should be processed at the end, because in a continuation one order should be processed and then the 2nd order."*

Srikanth agreed — but his confirming sentence carried the welding qualifier back in: *"That's right, that's right. That's why the multiple orders on the data that will be processed at the last **if welding is involved**. Because in that case the tail of order two will go into the next A-rod of order two."*

**Both readings are on the recording, and they differ by one validation branch:**

| Reading | Behaviour when no welding is involved |
|---|---|
| **A — Yogender's** | Full and partial coils in any order; **multi-order coils still last** |
| **B — Srikanth's literal words** | **No ordering constraint at all** — any coil, any sequence |

The argument for **A** is that it is not about the weld at all: an order boundary part-way down a rod means order 1 must complete before order 2 starts, whichever way the rod is joined to its neighbours. The argument for **B** is that without a weld there is no genealogy chain to break, so the rod can be interrupted and resumed.

**Ask it as one sentence:** *when no weld is involved, may an operator run a rod that carries two orders before the other rods of the first order?*

Related: **Q79** (the rule this qualifies), **Q69** (multi-order rods are legitimate), **Q22** (weld footage attribution).
