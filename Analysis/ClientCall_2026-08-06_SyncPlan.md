...
**Status:** **Registers applied 6 Aug 2026.** The eight decisions are recorded against `Q79`, `Q96`–`Q99`, `OI-13`, `G22`, `G34`, `G35` and the generation spec's edger section. Artifact-level propagation (SRS, contracts, DDL, mockups, phase files) is sequenced in §5 and **not yet executed**.
**Source:** Client call 6 Aug 2026, 10:59 UTC, 39m 50s — Tim O'Brien, Bob Scott, Shannon Riotte (United Aluminum); Srikanth Prabhala, Yogender Punia, Shray Anand, Divesh Malhotra, Waseem Khan, Ritika Raheja, Jaspreet Singh, Vicky Arora (Nagarro). Transcript: [`BaseDocuments/Shopfloor _ Review Priorities _ Discuss Open Items _ Use Time For Demos (35).docx`](../BaseDocuments/) — Teams meeting recording transcript, filed with the other read-only business inputs.
**Registers touched:** `Q##` ([FlatWireOpenQuestions.md](FlatWireOpenQuestions.md), 95 → 99) · `OI-##` ([FlatWire_MasterSpecification.md](../LatestDocument/FlatWire_MasterSpecification.md) §11) · `G##` ([back-matter.md](../DevelopmentPlan/ShopfloorPlan/back-matter.md), G33 → G35) · `PSG-D`/`PSG-Q` ([PassScheduleGenerationSpec.md](../LatestDocument/RequirementDocuments/PassScheduleGenerationSpec.md))
...
| | What it unblocks |
|---|---|
| **`Q79` answered** (rod consumption sequence) | `G22` has stood since 1 Aug as a **knowingly wrong rule** — staging refuses any rod whose order differs from the established one, which `Q69` made wrong on 30 Jul, and the correct replacement was explicitly waiting on this answer. [RodPreCheckin.md](../LatestDocument/RequirementDocuments/RodPreCheckin.md) §5.3 carries the wrong rule with a `[CLIENT INPUT REQUIRED]` note. **Phase 4 staging validation** |
| **`OI-13` flow specified** (wire break) | `FR-280`–`FR-282` are marked **"Not deliverable"** in [05-SprintPlanAndBacklog.md](../LatestDocument/ProjectPlan/05-SprintPlanAndBacklog.md), story `FW-N08` is **blocked**, and `TC-350`–`TC-352` are written-and-blocked in the test plan. The flow now exists; only the persistence target is missing (`G34`) |
| **Edging is roll-forming, not knifing** (new) | [PassScheduleGenerationSpec.md](../LatestDocument/RequirementDocuments/PassScheduleGenerationSpec.md) treats the edger purely as an **area correction** (§3.3.4, round vs square) and describes over-width material as *"trimmed or upset"*. Tim's correction adds a **whole formula subset** and makes the final stand a **skim pass** |
...
|---|---|---|---|---|
| **D1** | Rod consumption sequence: **full → partial → multi-order**, when welding is involved | **`Q79`** · new **`Q99`** | `Decided` | Closes `Q79`, resolves **`G22`**. Validation at pre-check-in **and** check-in. See §3.1 |
...
| **D3** | Wire break where the customer **accepts no welds** → Z-mill coil-break logic | new **`Q96`** | `Open` — principle decided | Supervisor disposition point on both pieces. See §3.3 |
| **D4** | FM2 stands re-confirmed **S1 = 8″, S2 = 6″, S3 = 6″** | **`Q93`** / `D-26` | Unchanged — corroborated | Independent client confirmation of the 4 Aug correction. No edit |
...
| **D6** | Edging is **vertical profiling roll-forming**, its own operation with its own formulas; **S3 is a skim pass** | new **`Q98`** · **`PSG-D24`–`D26`** · **`PSG-Q27`–`Q28`** | `Open` — data owed | New generation-spec section; the *"trimmed"* wording is wrong. See §3.4 |
| **D7** | FM2 carries **two dancers**, each with **dancer mode and tension mode** | new **`Q97`** · new **`G35`** | `Open` | Unmodelled in schema, tags and pass schedule; contradicts `PSM012`. See §3.5 |
| **D8** | Output multiples: **FL1 ~1,800 lb**, **FL2 800/900 lb** | `Q60` / [Spool.md](Spool.md) | Unchanged — corroborated | Matches the 30 Jul figures. Supplies the worked example in §3.1 |
...
6. **Where there is no welding**, sequence is free — subject to `Q99` below.
...
This is consistent with `Q27` Case 1 (a same-spec event mid-run keeps one alpha and records an event at a footage position). It does **not** change `Q27`.
...
**Why this is a formula subset and not a wording fix.** The spec presently handles the edge only through §3.3.4's area correction (`A = t·w − 0.2146·t²` for a round edge) and describes over-width material as *"trimmed or upset at the edger"* ([line 470](../LatestDocument/RequirementDocuments/PassScheduleGenerationSpec.md)). A roll-former **never trims** — there is no scrap stream at the edger — and it **feeds width back into length and thickness**, which changes the mass balance the allocation step depends on. Tim: *"there's actually another subset of formulas that needs to be used and they need to be treated like their own operation."*

Recorded as new **§3.3.11** in the spec — placed at the end of the 3.3 group rather than beside §3.3.4, so that **no existing §3.3.5–§3.3.10 cross-reference moves** — with **`PSG-D24`–`PSG-D26`** for the coefficients we do not have and **`PSG-Q27`–`PSG-Q28`** for the two questions that follow. **`PSG-D25` joins §12.3 as critical blocker `B5`**: without the partition coefficient the section entering S2 and S3 is unknown, so the finishing drafts are allocated against a guessed entry. `Q98` carries a pointer so the main register knows the item exists.
...
1. **There is no dancer on FM2 anywhere in the repository.** The dancer appears only as an FM1 component ([FlatWire_MasterSpecification.md](../LatestDocument/FlatWire_MasterSpecification.md) §156, [FlatWireProcessWalkthrough.md](FlatWireProcessWalkthrough.md) step 19). No `Stand`/`Drawer`/`Edger`-style lookup row, no `PassScheduleComponent` entry, no seed data.
...
`Q97` asks: who selects the mode, is it per dancer or per line, is it a **pass schedule parameter** or a machine-side setting, does the system **write** it at check-in acknowledgement or only **read** it, and — for `[PLC]` to answer — what carries it.
...
| **A4** | **Review [PLCTagSpecification.md](../LatestDocument/RequirementDocuments/PLCTagSpecification.md)** — not started; next after A1. This is what closes `Q90`/`Q94`/`Q95`, all Critical | **Tim O.** | After A1 |
...
|---|---|---|---|
| **W1 — Registers** ✅ | [FlatWireOpenQuestions.md](FlatWireOpenQuestions.md) (`Q79` decided; `Q96`–`Q99` added; index + change log), [FlatWire_MasterSpecification.md](../LatestDocument/FlatWire_MasterSpecification.md) (`OI-13` and its three citations), [back-matter.md](../DevelopmentPlan/ShopfloorPlan/back-matter.md) (`G22` resolved; `G34`, `G35` added; `G29` note) | 3 | L |
| **W2 — Generation spec** ✅ | [PassScheduleGenerationSpec.md](../LatestDocument/RequirementDocuments/PassScheduleGenerationSpec.md) **v1.2 + v1.3**. *v1.2 — the physics:* new §3.3.11, the *"trimmed or upset"* wording, the §2.3 route table, §3.3.9, `PSG-D24`–`D26`, `PSG-Q27`–`Q28`. *v1.3 — the propagation into the calculation:* **§6.3 Steps 9, 10, 11 and 12 rewritten plus new Step 9A (edger passes)**; **§3.3.5 allocation restructured** around the S3 skim; the **§3.3.6 tension omission** found and raised as `PSG-D27`/`PSG-Q29`; validations **V36–V38** added with V10/V20/V27/V30 amended; §6.1 flowchart, M4/M8 master rows, §8.7 blocked-validation table. `SRS/PassScheduleGenerationSpec.docx` re-rendered via [build_docx.py](../DevelopmentPlan/Tools/build_docx.py).<br><br>**Two gap audits followed the same day, taking the spec to v1.4 and then v1.5** — thirty defects, twelve of them algorithmic. They are recorded here because several were *caused* by W2: propagating the edger physics into a thirteen-step sequence written before edgers existed left five steps consuming quantities no step produced. The largest: **the width chain was never computed**, and with **no edger at the mill exit the final width leaves S3**, so `PSG-D08` governs finished-goods width and not just the FL1 spool's. **Eight further client items** are owed — `PSG-D31`–`D35`, `PSG-Q32`–`Q34` — of which two are High: **rolling friction stated separately from drawing friction**, and the **grip criterion for the round → flat pass**. Also **`PSG-D32`**, the planned output multiple, which is `D8` on this call: Step 12A had been splitting orders against take-up *ratings*, roughly halving the unit count | 1 (+1 generated) | M |
| **W3 — Requirement docs** | [RodPreCheckin.md](../LatestDocument/RequirementDocuments/RodPreCheckin.md) §5.3 order-membership rule, §6.2 sequence authorisation, §11 open-items table and the sign-off checklist (`Q79` row closes) · [RocCheckin.md](../LatestDocument/RequirementDocuments/RocCheckin.md) — the same validation at check-in · [WeldEvent.md](../LatestDocument/RequirementDocuments/WeldEvent.md) — the break-weld case and same-alpha rule | 3 | **H** |
| **W4 — SRS / requirement text** | [02-SRS.md](../LatestDocument/ProjectPlan/02-SRS.md) — `FR-280`–`FR-282` gain a persistence target once `G34` closes; new `FR-###` for the consumption sequence (**do not renumber; supersede in place**) · [FlatWire_MasterSpecification.md](../LatestDocument/FlatWire_MasterSpecification.md) §5 event table | 2 | M |
| **W5 — Contracts + schema** | [APIContracts.md](../DevelopmentPlan/APIContracts.md) and [04-APIContract.md](../LatestDocument/ProjectPlan/04-APIContract.md) — staging/check-in sequence-validation responses · `FlatWire_DDL_04_Runs.sql` (`RodStaging.OrderId` semantics) · `FlatWire_DDL_01_Lookup.sql` (a dancer component, if `Q97` says the mode is scheduled). **Re-run `FlatWire_DDL_RunAll.sql`; table count stays 27 unless `Q97`/`G34` add one** | 4 | **H** |
| **W6 — PLC surface** | [PLCTagSpecification.md](../LatestDocument/RequirementDocuments/PLCTagSpecification.md) — a dancer element and mode, pending `Q97`; **the client doc owns every path string** · [PLCTagImplementation.md](../DevelopmentPlan/PLCTagImplementation.md) — **carries no path strings by rule** | 2 | M |
| **W7 — Plans + tests** | [05-SprintPlanAndBacklog.md](../LatestDocument/ProjectPlan/05-SprintPlanAndBacklog.md) (unblock `FW-N08`), [06-TestPlanAndTestCases.md](../LatestDocument/ProjectPlan/06-TestPlanAndTestCases.md) (`TC-350`–`TC-352`), `phase-04`, `phase-06`, `phase-07` | 5 | M |
| **W8 — Mockups** | `pause_run.js` — the machine-stop prompt chain (stop reason → break → WIP reject or weld-and-continue) · `dashboard_2a_rod_precheckin.html` — pick-list ordering and the sequence refusal · `wip_rejection.js` — the no-weld disposition set | 3 | **H** |

**Blocked on client input:** W3 partly (`Q99`), W5's dancer column and all of W6 (`Q97`), W4's `FR-280`–`FR-282` (`G34`, which is ours to decide, not the client's).

**Convention reminders for every wave:** update **Last Updated** and append a **Change Log** row on each document touched; strike resolved register items through with a `DECIDED (date)` note and **never delete them**; keep `Q##` numbering contiguous; phase files must not restate foundations text; and **no tag path string may be written outside `PLCTagSpecification.md`**.
...
| 2 | **`Q99`** — does multi-order-last hold when **no welding** is involved? | Srikanth / Tim O. | One branch of the check-in validation |
| 3 | **`Q96`** — the no-weld disposition set, the supervisor gate and whether the concession path reuses the coil-break e-mail flow | Tim O. / Shannon R. | Wire-break screen scope |
| 4 | **`Q97`** — dancer mode: who selects it, per dancer or per line, scheduled or machine-side, read or written | Tim O. / Engineering | `G35`, the PLC dancer element, pass schedule shape |
...
| 5b | **`PSG-D32`** — the **planned output multiple** per line (`D8`: ~1,800 lb FL1, 800/900 FL2), as distinct from the take-up rating. Take the figure planning already uses; it must not resolve to two values | Tim O. / Planning | Step 12A's order split; overlaps `Q60` |
...
| 7 | **`Q90` / `Q94` / `Q95`** — confirmation of the tag paths, the FM2 station rename and the measure grammar (all Critical, pending A4) | Tim O. / Engineering | Phase 4 tag push, commissioning `C1`/`C11` |
| 8 | Carried from 30 Jul and **still owed**: tolerance values (`Q71`), the 1920×1080 answer (`Q80`), `Q78`, the `Q67` residual, the customer weight-range field (`Q60`), the PIN validation source (`OI-38`), the bundle gross weight (`Q81`) | Tim O. / Bob S. / IT | Seed data, Phase 1 canvas, Phase 4 |
...
| [ClientCall_2026-07-30_SyncPlan.md](ClientCall_2026-07-30_SyncPlan.md) | The precedent for this document; `Q79` was raised there |
| [PassScheduleGenerationSpec.md](../LatestDocument/RequirementDocuments/PassScheduleGenerationSpec.md) | Target of D5–D7 — W2 |
| [RodPreCheckin.md](../LatestDocument/RequirementDocuments/RodPreCheckin.md) | Carries the wrong order-membership rule D1 replaces |
...
| [back-matter.md](../DevelopmentPlan/ShopfloorPlan/back-matter.md) | `G##` register — `G22` resolved, `G34`/`G35` added |
| [PLCTagSpecification.md](../LatestDocument/RequirementDocuments/PLCTagSpecification.md) | Target of A4 and of the `Q97` dancer element |
