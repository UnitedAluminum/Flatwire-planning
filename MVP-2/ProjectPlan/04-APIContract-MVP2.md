...
> **Correction 1 of 4 — this is the defect.** `MVP-1/DevelopmentPlan/APIContracts.md` publishes this same example returning `preflattenDiameterIn: 0.265`, `areaReductionPct: 50.1`, `drawPasses: 2` and `routeMode: "Standalone"` with FM2 bypassed and no warnings. **All four are wrong.** The published `areaReductionPct` of 50.1 is internally consistent with its own wrong 0.265 diameter, not with the stated formula; and an aspect ratio of 7.0 must, by the algorithm's own step 6, force `Hybrid`. **Implementers must build to the formula in `[SRS §5.18]` `FR-381`–`FR-387`, not to any published example.**
