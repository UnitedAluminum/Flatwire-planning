...
|---|---|
| Request validation | **FluentValidation** per command, invoked by a MediatR pipeline behaviour — **shape, type and range only**. Field presence, enum membership, `machineName = FL2` rejected at `/staging/rod`. → **`400`** |
...
- **Request DTOs carry *nullable* enums.** A non-nullable enum binds a missing field to `default` — `MachineName.FL1` — instead of failing. Nullable plus a `NotNull` rule fails fast at the boundary.
