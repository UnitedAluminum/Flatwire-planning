...
status_note: "**Ready to build — three procedures are drafted and structurally verified; four values need IT sign-off before a shared environment, and one deployment prerequisite is not yet met** [9 Sep 2026, client answers] Q68 - the queue insert, the requirements summary and the WIP order insert all MOVE from pre-check-in into this transaction. The client adopted our position outright: pre-check-in is to have no effect outside the flat wire system at all. The write ordering and the estimate both move. See FW-N24."
owner:
jira:
mvp: 1
phase: "4"
...
**Artifacts:**
- [`../../30-database/scripts/40_FlatWireDB_Proc_FlatWire_CheckInRod.sql`](../../30-database/scripts/40_FlatWireDB_Proc_FlatWire_CheckInRod.sql)
- [`../../30-database/scripts/60_FlatWireDB_Proc_FlatWire_ReleaseStation.sql`](../../30-database/scripts/60_FlatWireDB_Proc_FlatWire_ReleaseStation.sql)
- [`../../30-database/scripts/70_FlatWireDB_Proc_FlatWire_ReverseReqsum.sql`](../../30-database/scripts/70_FlatWireDB_Proc_FlatWire_ReverseReqsum.sql)
- [`../../30-database/scripts/20_FlatWire_Grants.sql`](../../30-database/scripts/20_FlatWire_Grants.sql)
- [`../../30-database/scripts/99_FlatWireDB_Proc_FlatWire_Teardown.sql`](../../30-database/scripts/99_FlatWireDB_Proc_FlatWire_Teardown.sql)
...
`FlatWire_DDL_07_Indexes.sql` gains **`UX_FlatWireRun_ActiveLine`**, a filtered unique index on `MachineName` where `Status IN ('Running','Paused')`. `FlatWire_DDL_04_Runs.sql` gains **`RodCheckin.WipCoilOrdersWritten BIT NOT NULL DEFAULT 0`**.
...
`FW-157` (`CheckInService` — calls this), `FW-159` (the `FlatWireDB` check-in writes), `FW-141` (repositories), `FW-142` (Dapper + context + the unit of work that owns the transaction), `FW-144` (configuration — extended with the cross-database grants), `FW-003` (the `machines` rows, `machine_idx` 125/126/127), and **`FW-241`** (deploy step 2 — the station rows, which must exist before step 9 can claim one). ⛔ **The draft script that used to carry step 2 was withdrawn on 6 Sep 2026**, so the dependency is now on a story with nothing written, not on a script awaiting sign-off.
