...
| Enum-typed fields on the payloads — `MachineName`, `PayoffPosition`, `ComponentName`, `LineState`, `AlertSeverity` | same |
...
- **Nothing in `ual-angular` defines the set.** The `flat-wire` library **does not** exist in
  `Second-Branch/ual-angular` — `FW-N03`’s scaffold was built on 28 Aug 2026 and **reverted on
  31 Aug 2026** — and **no `.ts` file anywhere in that checkout
...
**All six share `MachineName · RunId · FootagePosition · Timestamp`**, and that base is the right one:
...
| **Nullability where it is load-bearing** | ✅ `GaugeReadingEvent.Value` and `WidthReadingEvent.Value` are **`decimal?`** — **a `0` cannot pass for a missing reading**. ⚠ **The member's `FR-120` reference is stale as of 9 Sep 2026** (superseded, `A3` retired): nullability is still load-bearing, but because **no measurement** is reachable on **any** line — a dropped feed — not because FL2 broadcasts null by design |
...
| Marker base | ✅ all six carry `MachineName · RunId · FootagePosition · Timestamp` — asserted, not assumed |
...
`machineName ← MachineName` · `station ← Station` · `runId ← RunId` · `rodAlpha ← RodAlpha` ·
...
handler injects **`IFlatWireBroadcaster`** (`P-101`) — one member, `Line(MachineName)`, returning
