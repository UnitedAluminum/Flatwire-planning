...
| `POST /sim/{lineId}/fault` | The **only** way to reach [`FW-205`](FW-205.md)'s conditions **5 and 3** — two or more consecutive recordings missing, and, held past `ITInhibitSilenceMs`, the escalation to condition 3. ⚠ **The card says condition 5 alone; the built handler reaches both**, which is why `FW-213` must not re-implement it (`P-286`) |
