...
id: FW-237
legacy_id:
title: Service identity for unattended PLC writes
status: blocked
status_confirmed: false
status_note: "**Buildable — blocked on a credential decision nobody owns yet.** `G59`'s **identity half only**"
owner:
jira:
mvp: 1
phase: "4"
...
hours: 12
sprint: S2
depends_on: [FW-143, FW-151, FW-205, FW-234]
blocked_by: []
has_plan: true
started:
completed:
...
| The audit trail's persistence target | [`FW-234`](FW-234.md) — **a dependency**, and it is what makes AC 3 checkable |
| Per-tag write status | `FW-236` (`G58`) |
| Registering flat wire with `OPCConnection` | `FW-238` (`G60`) |
| The interlock's conditions | [`FW-205`](FW-205.md) built 3–5; [`FW-206`](FW-206.md) owns 1–2 |
...
`P-15` is the finding that nothing can query it. **[`FW-234`](FW-234.md)
...
1. **Get the credential decision** (Blockers). ⚠ Frame it as §2.2 does — a reserved
   ...
   answer.
2. **Land [`FW-234`](FW-234.md) first** (§2.4), or AC 3 is
...
survives having no `HttpContext` (`P-120`). [`FW-234`](FW-234.md)
is the dependency that makes AC 3 material. [`FW-206`](FW-206.md) adds
