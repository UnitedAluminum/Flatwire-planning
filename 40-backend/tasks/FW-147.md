...
are **[`FW-207`](FW-207.md)'s** acceptance criteria, not this story's — none of
...
**Line eligibility is a shape rule, and it is *not* enum membership.** `MachineName` carries **all
three lines** in all three layers, and narrowing it would break the mirror — the TypeScript
union and the DB `CHECK` both carry `FL1`/`FL2`/`FL3`. So an FL2 value is a **valid** `MachineName`
...
All five return **`400`**, and all five are guarded `.When(x => x.MachineName.HasValue)` so the
`NotNull` rule owns the missing-field case and one bad request does not produce two errors.

> ### ⚠ Do not build the `machineName = FL2` **`422`** — it is withdrawn
>
...
> condition would be breaking. See [`FW-138 §8.2`](FW-138.md).
>
> ⚠ **The earlier reading here — *"an FL2 value fails enum membership, so no line-eligibility
> validator is needed"* — is withdrawn (27 Aug 2026).** It was wrong twice over: `MachineName`
...
|---|---|---|
| 1 | `MachineName` | `{FL1, FL2, FL3}` — **all three, never narrowed per endpoint** (`P-83`) |
...
> an error. **(2)** Request DTOs carry **nullable** enums; without that a missing `machineName`
> binds to `MachineName.FL1` rather than failing, and the `NotNull` rule in §2.1 never fires.
...
2. ✅ Validators per command in `FlatWire.Application/Validators/`, registered by
   ...
   `ValidatorBehavior` — [`FW-139`](FW-139.md)
...
> [`FW-138`](FW-138.md) when this scope was built early. Read those
...
### `P-83` — line eligibility is a per-endpoint shape rule, and `MachineName` is never narrowed

**Settled — 27 Aug 2026, and it corrects this plan's own earlier reading.**

`MachineName` carries all three lines in all three layers, so **no endpoint gets a line refusal for
...
- **The mirror.** A per-endpoint `MachineName` subtype has no TypeScript union and no DB `CHECK` to
...
| `MachineName` | **Three** members everywhere — a per-endpoint subset is a `Must`, not a narrower enum (`P-83`) |
...
| **DB `CHECK`** | ✅ present for **12** of the 14, across 24 constraints — `MachineName` alone is mirrored by 10 |
...
|---|---|---|---|
| `MachineName` | 3 | 10 — `CoilOutput` · `DieChangeEvent` · `FlatWireRun` · `PassSchedule` · `RodCheckin` · `RollOverride` · `SpcCheckpoint` · `SpoolProcessing` · `WeldEvent` · `WipRejection` | ✅ |
...
| **`G41`** | `CK_PSC_FM1NotBypassable` is **line-blind**, so a correct FL2 pass schedule cannot be authored — every FL2 schedule must mark `FM1` **Active**, engaging a stand the line does not have. It is a **`CHECK` that cannot be fixed as a `CHECK`**, so the fix lands as a domain invariant in [`FW-207`](FW-207.md), not as a validator here. Recorded because this story owns the enum/`CHECK` mirror and the mirror is where it looks like a defect |
...
| *"an FL2 `machineName` fails enum membership"* — this plan, to 26 Aug 2026 | `MachineName` carries all three lines; the refusal is an explicit **shape rule** | `P-83`, `CanonicalEnums.cs` |
