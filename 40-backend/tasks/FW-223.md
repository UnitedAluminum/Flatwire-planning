...
id: FW-223
legacy_id:
title: Rod ingestion — populating the FlatWire tables
status: not-started
status_confirmed: false
status_note: "**Ready to build — the procedure is written and parses clean; no sign-off items, one deployment prerequisite shared with `FW-220`**"
owner:
jira:
mvp: 1
phase: "4"
...
hours: 14
sprint: S2
depends_on: [FW-007, FW-142, FW-144, FW-157, FW-158, FW-159, FW-220]
blocked_by: []
has_plan: true
started:
completed:
...
**Artifacts:**
- [`../../30-database/scripts/30_FlatWireDB_Proc_sp_IngestRodFromCoils.sql`](../../30-database/scripts/30_FlatWireDB_Proc_sp_IngestRodFromCoils.sql) — it sits with the four cross-database procedures rather than in the schema runner, because it reads `proddb..coils` and `united_db..alloys` and so cannot be verified by a `FlatWireDB`-only deploy
- [`../../30-database/sql/FlatWire_SampleData_RunAll.sql`](../../30-database/sql/FlatWire_SampleData_RunAll.sql) — new, and the seed block removed from `FlatWire_DDL_RunAll.sql`
