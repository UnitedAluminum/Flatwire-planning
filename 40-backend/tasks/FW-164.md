...
status_note: "⚠ **A de-stub — and AC 3's order block is uncontracted as well as unmapped**"
owner:
jira:
mvp: 1
phase: "5"
...
| `sp_GetGaugeTrace` | ✅ Built ([`FW-165`](../../30-database/tasks/FW-165.md)) — ⛔ `G9` leaves its decimation parameter untunable |
...
| **`G51`** *(new here)* | `gaugeTolerance` / `widthTolerance` are **single-±** on this response while `Q22` decided **min/max pairs** — `[API §4.7a]`'s own note flags it, and [`FW-245`](../../30-database/tasks/FW-245.md) fixes the sibling column. **This DTO has the same defect and no owner** |
