...
id: FW-145
legacy_id:
title: JWT authentication and role authorization policies
status: not-started
status_confirmed: true
status_note: "⭐ **CARVED TO MVP-2 — `D-52`, 6 Sep 2026, and UNBLOCKED in the same act.** `G6` leaves `blocked_by`: its residual (the six claim **values**, coded rather than `[SEC §8]`'s labels) gates *verification* of role policies, and role policies are no longer MVP-1's to verify. ⛔ **MVP-1 therefore ships on bare `[Authorize]` with no role policy and no `403` anywhere** — which is what [`FW-146 §5.1`](FW-146.md) already measured as the built state, not a new concession. ✅ **`G72` closes for MVP-1**: `/sim/**` drops to bare `[Authorize]`, so the simulator control surface and `DB-S1` become driveable. ⚠ **The `?access_token=` handler does NOT travel** — [`FW-080`](FW-080.md) built it (§3.5, `P-77`) inside its own 28 h and it stays in MVP-1. ⚠ **Phase stays `1B` for tooling reasons only** — `check_docs.py` rule 5 requires a real phase file and MVP-2 has none; read `mvp: 2` as the scope of record. Re-priced additively in [`[CE §3h]`](../../60-delivery/CapacityAndEffortModel.md)"
owner:
jira:
mvp: 2
phase: "1B"
...
**Last Updated:** September 6, 2026 (`D-52`) — carved to MVP-2 and unblocked; the plan body below is unchanged and is still the plan
**Document Type:** Implementation plan for a single backlog story
**Status:** ⭐ **MVP-2 since 6 Sep 2026 (`D-52`), and no longer blocked.** *(Was ⚠ Buildable — one fact outstanding, from 15 Aug 2026; ⚠ Blocked before that)*
...
> ### ⭐ Read this first — this story is MVP-2 (`D-52`, 6 Sep 2026)
>
> **Everything below is still the plan and none of it is withdrawn.** What changed is *when* it is
> built and what MVP-1 does in the meantime.
>
> | | |
> |---|---|
> | **Scope** | **MVP-2.** `mvp: 2` in the front matter; `sprint` is cleared, because `S0` is an MVP-1 sprint. `phase` stays `1B` **only** because `check_docs.py` rule 5 requires every task to name a real phase file and MVP-2 has none |
> | **Blocked?** | **No.** `G6` is off `blocked_by`. Its residual — the six claim **values** are coded rather than `[SEC §8]`'s labels — gates the **verification** of role policies (`AC 4`, §6.1), and that verification is now MVP-2's. It was never a construction blocker |
> | **What MVP-1 does instead** | **Bare `[Authorize]` on every endpoint, no role policy, no `403`.** ⛔ This is not a new concession: [`FW-146 §5.1`](FW-146.md) already measured it as the built state — *"there is no policy-gated endpoint … nothing in this service can return a `403`"* |
> | **What that costs** | `[SEC §8]`'s authorisation matrix becomes an **MVP-2 obligation**. Until then any authenticated user reaches any endpoint this service exposes. ⚠ **State that plainly when the matrix is reviewed** — do not let it read as delivered |
> | **What it buys** | **`G72` closes for MVP-1** and `/sim/**` becomes driveable on bare `[Authorize]`, so the built simulator (`FW-210`–`FW-215`, `FW-217`, `FW-218`) and the `DB-S1` console can actually be run. That was the whole reason the carve was worth making now |
> | **What does NOT travel** | ⚠ **The `?access_token=` handler stays in MVP-1.** §3.5 / `P-77` specified it; [`FW-080`](FW-080.md) **built** it inside its own 28 h as the stated prerequisite, and the hub depends on it. Only the **role policies** move |
> | **Hours** | The 16 h leaves MVP-1 whole. Re-priced additively in [`[CE §3h]`](../../60-delivery/CapacityAndEffortModel.md) — Phase 1B `442 → 420 h` on §3's base, MVP-1 all-in `3,358 → 3,336 h`. ⛔ **`[CE §3]`/`§3b`/`§3c` are not re-derived in place** |
>
> ⚠ **`P-76` travels with the story and is still open** — the `ProductionTransaction` policy for the
> ten write endpoints `[SEC §8]` gives no row. It is an MVP-2 question now, not an MVP-1 one.
...
test on an **unauthenticated** probe. See [`FW-148`](FW-148.md).
...
the path is `/hubs/flat-wire` (`[SIG §4]`) — the standard SignalR pattern, because browsers cannot
set an `Authorization` header on a WebSocket handshake. Hub methods carry `[Authorize]`.
[`FW-080`](FW-080.md) depends on this story for exactly this.
    ...
    options =>
    {
        options.Events = new JwtBearerEvents
        {
            OnMessageReceived = context =>
            {
                ...
                if (!string.IsNullOrEmpty(accessToken) && path.StartsWithSegments("/hubs/flat-wire"))
                {
...
is present, so under IIS the segment to match is `/hubs/flat-wire` and **not** the deployed prefix.
...
6. Add the `?access_token=` `PostConfigure` for `/hubs/flat-wire` (§3.5).
...
| Hub auth | A hub connection with `?access_token=` succeeds; without it, fails. ⚠ **The handler is this story's; the evidence is not available until [`FW-080`](FW-080.md) maps `/hubs/flat-wire`** — until then there is nothing to connect to. Carry it to `FW-080`'s *Auth* verification row rather than recording an untested pass |
...
sees it — [`FW-146`](FW-146.md) §2 maps `401`/`403` to
...
[`FW-080`](FW-080.md) lists this story in its `Dependencies` — it cannot
...
- [`FW-177`](FW-177.md) targets *"the Supervisor role"* by joining a
...
- [`FW-218`](FW-218.md) applies `SimulatorControl` (§3.4) to `/sim/**`, on
