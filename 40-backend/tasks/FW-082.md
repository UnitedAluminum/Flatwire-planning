...
id: FW-082
legacy_id:
title: PLC tag group push on check-in acknowledgement
status: not-started
status_confirmed: false
status_note: "⚠ **Five blockers, and one dependency reaches into MVP-2** [9 Sep 2026, client answers] Q27 REMOVES PUSH GROUP 5 ENTIRELY - line speed is no longer sent from the application at all. The client: 'we will not feed the line speed to the Opc from the machine application. Speeds shall be controlled via the machine HMI'. Speed becomes a READ tag, and the safety consequence this story was raised for disappears with it. Q28 also CLOSES G29 - the edgers do have a position signal on FL2/FL3, so the push no longer has a group with nowhere to write. Blade profile is not a signal and never will be: they are grooved rolls. See FW-N25."
owner:
jira:
mvp: 1
phase: "4"
...
hours: 16
sprint: S2
depends_on: [FW-151, FW-157, FW-010]
blocked_by: [G2, G29, G30, OI-39, PLC-Q04, Q05]
has_plan: true
started:
completed:
---
...
is the CALLER's.** `PassSchedulePushRequest` is `PlcWriteContext(MachineName, OperatorId, RunAlpha)` +
