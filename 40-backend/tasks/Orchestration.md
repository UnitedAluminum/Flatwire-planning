...
**Last Updated:** September 5, 2026 — **[`FW-238`](FW-238.md) gains its plan; `P-318`–`P-321` minted and §4a extended to `P-321`.** All four are about **sequencing an already-written deliverable**: the story splits into an authored half (complete) and an activated half gated on three outside answers in a fixed order (`P-318`/`P-319`), closing it does **not** flip `SimulatePLCTagPush` (`P-320`), and the `UAL.Constants` enum change is proposed rather than depended on (`P-321`). ⚠ **No hour figure moved and no story was re-scoped.** *(previously September 3, 2026 - **Table C3 added: `FW-260`**, the roll-set register service for the fourth Tooling Inventory tool type (`D-42`). Deliberately thinner than `FW-252` - a roll set has **no footage counter**, so there is no per-run write path and no history log. Its one non-trivial invariant is `CK_TIRS_Mount`. *(previously August 31, 2026 - change history is in [`CHANGELOG.md`](../../CHANGELOG.md))*)*
...
|---|---|---|---|---|
| [FW-N04](FW-N04.md) | Solution + 4-project skeleton | 16 | **0** | ✅ **Built 25 Aug 2026** — wave 1 is unblocked · `P-01`, `P-02`, `P-50`, `P-51` |
| [FW-138](FW-138.md) | Fourteen thin controllers, 22 endpoints | 45 *(42 owed)* | 1 | ✅ **Built 25 Aug 2026** — 61/61 cases, 22/22 `401` · `P-06` ratified in the build · `P-55` minted |
| [FW-139](FW-139.md) | MediatR + behaviours | 16 | 1 | ✅ **Built 25 Aug 2026** — pipeline order read off the log · `P-59` bridge proven behaviourally · `P-61` applied · `P-10` still waits on `FW-142` |
| [FW-140](FW-140.md) | DI + `useMockData` swap | 12 *(understated)* | 1 | ✅ **Built 26 Aug 2026** — 12 interfaces / 12 stubs / 12 shells · 61/61 and **65/65 bodies byte-identical** · `P-02` re-verified · ⚠ **`P-65` still needs ratification** |
| [FW-141](FW-141.md) | Seven repositories | 28 *(gained `FW-207` scope)* | 1 | ✅ **BUILT 27 Aug 2026** — 6 alphas · 7 structural roots · 7 interfaces · **7 `sealed` repositories, all registered** · `ContextRepository` with `sp_GetGaugeTrace` · `G14` **verified at startup** · **13/13 accessors verified on the live DB** · `P-71` minted. ⚠ **Step 6 still waits on `OI-33`** |
| [FW-142](FW-142.md) | `FlatWireDbContext` + Dapper | 24 | 1 | ✅ **Built 27 Aug 2026** — 7 roots mapped, model validated, insert→select→rollback proven on the live DB · `P-13` holds (**no `DbSet<PassSchedule>`**) · `P-69`, **`P-70`** minted · **unblocks `FW-141` steps 2–3 and `FW-139`'s `P-10`**. ⛔ `SpoolProcessing` untestable — **deployed DB predates `Q60`**; teardown owed |
| [FW-143](FW-143.md) | Serilog + audit log | 12 | 1 | ✅ **Built 27 Aug 2026** — Serilog + Console sink, correlation verified end to end, `UseSerilogRequestLogging` (`P-73`) closes *"controllers log nothing"*, `IAuditLog` + `SerilogAuditLog`. ⚠ **`P-15` still open** — AC 3 met structurally, not materially |
| [FW-144](FW-144.md) | Configuration binding | 12 | 1 | ✅ **Built 27 Aug 2026** — bound map measures **72 paths** (FL1 17 · FL2 22 · FL3 33), settling the stale-`41` figure as a runtime fact; four boot assertions close §5's verification gap; `ITagPathResolver` seam built so an `OI-A` move is one class. `P-74` minted. ⚠ **Map *contents* still blocked on `PLC-Q05`/`G33`, shape is not.** ✅ **`OI-A` DECIDED 4 Sep 2026 — `D-44`: the paths live in the `CommonDB` `OPCTags` registration, not `appsettings`.** It was answered on 27 Aug by contradicting **both** candidates, and the decision followed the code. **This story is not reopened** — the swap is one class behind the seam and belongs to `FW-238`, gated on new gap **`G93`** *(this row read "Ready" until 29 Aug 2026)* |
...
| [FW-146](FW-146.md) | Exception middleware | 8 | 1 | ✅ **BUILT 27 Aug 2026** — `ExceptionHandlingMiddleware` live, filter removed and the removal **asserted at boot**; stack-on-the-wire 2,972 B → 173 B; 22/22 `401` intact; both inspection rows hold at once. `P-78`–`P-82` minted. ⚠ **`P-78`: §4's `RemoveType` sketch does not compile.** Two rows handed forward (§6) |
...
| `FW-236` | Per-tag write status from `OPCConnection` | BE · 14 | 16 | `G58`, **`G94`** *(ordered first)*, `FR-074` |
| **[`FW-237`](FW-237.md)** | Service identity for unattended PLC writes | BE · 4 | 12 | `G59` **identity half**, `P-127` |
| **[`FW-238`](FW-238.md)** | Register flat wire with `OPCConnection` | BE + DB · 14 | 12 | `G60`, `P-120` · ✅ **PLANNED 5 Sep 2026** — `P-318`–`P-323`. ✅ **`G97` RESOLVED (`D-49`)** — endpoint pair `OPCServersIdx` 1 and 2, both lines onto both. ⚠ **Scripts written and complete, nothing run**: three gates, `G94`/`FW-236` · `FW-241` *(`G100` cleared on `DEV00164-001` 5 Sep 2026 — see `08_`)* |
...
### 4a. Decision register — `P-01` to `P-321`
...
| **`P-106`** | [FW-151](FW-151.md) | **The operations carry operator and run context.** `[PLCC §1]`'s signatures are `PushPassSchedule(scheduleId, machineName, payoffPosition)` and `ClearPayoffTags(machineName, payoffPosition)`, and the built **`AuditEntry` requires `OperatorId` and queries on `RunAlpha`** — `FW-143` AC 3's two keys — while `FR-075` requires the operator on every write. **So as written the signatures cannot produce a compliant audit record**, and an unattributable audit trail has no compliance value. ⚠ Not via a static or an `IHttpContextAccessor` reach-around: operations 4 and 5 are raised from a hosted service with no HTTP context (`G59`). **Keep `scheduleId`/`machineName`/`payoffPosition` exactly as `[PLCC §1]` names them** — `phase-04`, `FW-082` and `FW-157` cite them | settled |
...
| `P-315` | [FW-238](FW-238.md) | **The tag paths are kept in two places and the copies are *checked*, not trusted.** `D-45` leaves `appsettings` as the resolution map and `CommonDB.OPCTags` as the registration, so the same 39 strings exist twice with nothing keying them together. The alternative — `D-44`'s `TagKey` column — cost a shared `ALTER` and another team's schedule to buy a property the ecosystem has never needed (`UnifiedSlitterService` addresses 313 tags without it). So the duplication is accepted and made **detectable**: `11_`'s verification block prints every registered path, and `[DEP §5]` carries a config-vs-rows diff. Fallback if drift ever bites — `OpcSubModuleTags` is the ecosystem's precedent for a side table, not an `ALTER` | settled |
| `P-316` | [FW-238](FW-238.md) | **`OPCTags` stores the BARE path; the `ns=2;s=` node-id form is applied only at the QuickOPC boundary.** `G94` measured 0 of 496 existing rows carrying a prefix, and `OPCConnection`'s four UA call sites all funnel through `OPCTag.GetOpcUaTagName()`. A prefixed row would be double-wrapped. `11_`'s verification block fails on any row matching `ns=%` | settled |
| `P-317` | [FW-238](FW-238.md) | **FL3 is left to fail loudly rather than made to half-work.** `D-47` gives FL3 no controller, and the two-controller push it implies is undesigned (`G99`). Two options were available: point FL3's rows at FL1/FL2 and let a push half-configure a line, or register nothing and let it fail by name. **Nothing is registered**: `Lines:FL3:MachineId` stays `0` so `FW-151`'s tripwire fires, `11_` writes no machine-127 row, and its verification block **asserts 127 is absent** so a later well-meaning edit is caught. An empty `GetOPCInfo` for FL3 is the designed answer, not a defect | settled |

**`P-318`–`P-321` — minted by [`FW-238`](FW-238.md)'s 5 Sep 2026 PLAN**, written after its scripts
rather than before them, so all four are about **sequencing an already-written deliverable** rather
than about what to build. Defined in [`FW-238 §4`](FW-238.md).

| Id | Plan | Subject |
|---|---|---|
| `P-318` | [FW-238](FW-238.md) | **The story has an AUTHORED half and an ACTIVATED half, and only the second closes `G60`.** The DB 8 h bought `11_`/`16_` and they are complete; a script that has never run registers nothing. Closing the card on the file existing misreports what the system can do, and holding it open in S3 waiting on a controls engineer misreports where the work is. So: authored = done, activated = a **scheduled commissioning-window event** gated on three outside answers, and `status` stays `in-progress` until it lands. ⚠ `G60`'s cost was never the typing — it was *"discovering the registration is absent inside the 40 h window"*, which is only avoided if the run is scheduled rather than assumed | settled |
| `P-319` | [FW-238](FW-238.md) | ⛔ **The activation order is fixed, because three of its four mis-orderings fail SILENTLY.** Ask `G97` → `FW-236`/`G94` merges → `10_` runs → `11_` runs → verify → probe `GetOPCInfo`. Running `11_` before `10_` is the **only** self-reporting mistake (its fifth guard aborts). Activating `21317` before `FW-236` merges writes to namespace 0 and reports `200`; seeding without the two system-error rows, or against a `machines` row at any status but `1`, both return an **empty list indistinguishable from `G60` itself**. That asymmetry — one guard, three silent failures — is why the order is a decision and not a runbook note | settled |
| `P-320` | [FW-238](FW-238.md) | ⛔ **Closing `FW-238` does NOT turn the write path on.** `SimulatePLCTagPush` and `SimulateOpcFeed` both stay `true`, and this story moves neither. Three reasons that all outlive it: **`G59`** — a background `GetOPCInfo` still fails with a `NullReferenceException` before the network, registration or not (`FW-237`); **`G33`** — a registered path is not a confirmed path, and only `C1`/`C11` settle that; and `[DEP §4.5]`'s rule that a tag push is verified **on a stopped line only**. ⚠ Stated because the opposite is the intuitive reading: `G60` was why the flag could not usefully go false, so retiring `G60` reads like permission to flip it. It is not | settled |
| `P-321` | [FW-238](FW-238.md) | **The `UAL.Constants.OPCModules` change is proposed as documentation and never becomes a dependency.** Measured 5 Sep 2026: **no line in the FlatWire codebase evaluates the enum** — every occurrence is a comment or a log string, `ResolveOpcInfoAsync` sends `options.OpcModuleId` (a plain config int), and `ual-window-service` does not reference the package at all. Adding `Slitter = 5` and `FlatWire = 6` is worth doing — an enum omitting a production module id is how `G96` happened — but it is **another team's shared-constants change**, and this story must not acquire their schedule as a blocker for something that gates nothing. Propose it; ship without it | settled |
...
| **Before Phase 14 opens** *(owner: **`FW-238`**)* | ⛔ **`G60`** — nothing registers flat wire with `OPCConnection` | **No `OPCModules` member and no `CommonDB` OPC registration** (`OPCModules`/`OPCServers`/`OPCTags`/`OPCTagApplicationMapping`), so `GetOPCInfo` cannot succeed and `WriteTag` cannot be called. **Not a build blocker** — simulate short-circuits before the resolve — but the resolve-and-write path is then **dead code until the 40 h commissioning window**, where `G33` and `G32` also have to be settled. ✅ **`OI-A` is decided (`D-44`, 4 Sep 2026) — `OPCTags` is in scope and the write-twice risk is gone.** ⛔ **`G93` is the new gate**: `OPCTags` has no logical-name column, so `TagKey varchar(64)` must ship before the rows | [FW-151](FW-151.md) · `FW-003` *(the `machines` rows it would key on)* · **`G93`** |
...
| 4 | Hub streams batched telemetry · **fourteen events + six markers** *(twelve until 22 Aug 2026 — `[SIG §5.2]`)* · **two** durable prompts survive reconnect · simulate logs an audit entry | [FW-080](FW-080.md) · [FW-149](FW-149.md) · [FW-150](FW-150.md) · [FW-151](FW-151.md) · [FW-143](FW-143.md) |
