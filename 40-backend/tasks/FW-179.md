...
id: FW-179
legacy_id:
title: POST /checkin/spool and GET /spools
status: not-started
status_confirmed: false
status_note: "⚠ **A de-stub** — and ⚠ **`[API §4.6a]`'s worked example uses the wrong fixture (§2.3)** [9 Sep 2026, client answers] Q15 - FL3 DOES NOT PRODUCE SPOOLS, it produces coreless oscillated coils. The hybrid-origin spool guard therefore has nothing to guard: no such spool ever reaches FL2 check-in. The premise of that requirement is void rather than merely unspecified. Q17 (FL2 pre-check-in) is still open and is now sized as FW-224."
owner:
jira:
mvp: 1
phase: "8"
...
hours: 18
sprint: S2
depends_on: [FW-139, FW-151, FW-180]
...
**Last Updated:** August 29, 2026 — ⚠ **Re-reviewed against the BUILT code: this is a DE-STUB.** ✅ **`G41` is mitigated in the domain** — `Fm1ScopedToRodFedLinesRule` refuses an FL2 schedule with `FM1` Active *and* an FL1/FL3 one without it, so component state is already read before anything is written; the DDL constraint stays line-blind. ✅ The `[API §4.6a]` fixture error is now flagged in the controller's own remarks too. ⛔ **Two ownership defects in the built code:** `CheckInService.CheckInSpoolAsync` attributes itself to **`FW-157`** (it is this story's), and `SpoolService.CompleteSpoolAsync` attributes itself to **this story** (`POST /spool/complete`, endpoint 16b, is [`FW-202`](FW-202.md)'s). Change history is in [`CHANGELOG.md`](../../CHANGELOG.md)
...
| Concern | Story |
|---|---|
| `SpoolCheckin` table + the `SpoolProcessing.OrderNo` index | [`FW-180`](../../30-database/tasks/FW-180.md), DB — ✅ **built** |
| DB5 / DB5A screens | `FW-178`, FE · `FW-124` deferred |
| The `SpoolProcessing` row this reads | [`FW-202`](FW-202.md) — ⚠ **must land first** (§4) |
| **`POST /spool/complete`** — endpoint **16b** | [`FW-202`](FW-202.md) — ⛔ **not this story, despite what the built shell says** (§1.2) |
| `PLCTagService` | [`FW-151`](FW-151.md) — ✅ **built** |
| The rod-side twin | [`FW-157`](FW-157.md) |
...
| `SpoolCheckin` table + `OrderNo` index | ✅ Built ([`FW-180`](../../30-database/tasks/FW-180.md)) |
...
`FW-179`'s"* — it is **[`FW-202`](FW-202.md)'s**, endpoint 16b. Same defect
...
decide what to write**, which is this story. See [`FW-207`](FW-207.md).
...
1. ⚠ **Both routes already exist** on [`FW-138`](FW-138.md)'s shell —
...
4. DTO joins: `CoilTraceability`/`WeldEvent` → source rods · the FL1 run → gauge/width ·
   **cross-database** → the order block *(the same unenforced-link basis as
   [`FW-164 §2.1`](FW-164.md); reuse its view)*.
...
[`FW-181`](FW-181.md) binds the FL2 Live/Profile view.
...
## 7. Added by `D-55`, 7 September 2026 — the spool's orders and its child alphas

⚠ **Adjacent scope, flagged rather than absorbed.** `D-55` is about the Active Run screen; these two
belong to the FL2 check-in flow (DB5). They are recorded here because the FL1→FL2 traceability chain
surfaces to the operator *through this endpoint*, and **the built contract cannot express it**.

- [ ] **An order collection, not a single order.** `SpoolsResponse.Order` is one
      `SpoolOrderContext?` and `SpoolListItem.OrderNo` a single `string?` — but the order **set**
      exists precisely because *"a spool coming off FL1 may carry **two or more** orders"* (client,
      20 Aug 2026) while FL2 runs **one at a time**. So the requirement *"the orders assigned to the
      spool are retrieved and displayed"* is **not satisfiable by the response as built**. Return the
      set alongside the resolved selection.
- [ ] **The child segment alphas.** `SpoolListItem.SourceRodAlphas` returns the source **rods**
      (`R00041`, `R00042`); the chain's identities are `SpoolTraceability.ChildAlpha` — one alpha per
      segment, e.g. `R00001C` (`Q57`, 22 Aug 2026). ⚠ **They are different identifiers for the same
      material.** Surface both, or state in the plan which one the FL2 screen shows and why.

⛔ **Both wait on a `Critical` upstream gap.** `G54` / `OI-138`: nothing writes `SpoolTraceability`
and nothing registers an FL1 segment alpha in `proddb..coils`, so until **`FW-230` and `FW-231` ship
together** there are no child alphas to return — and `FW-230` alone **reissues `R00001A` on every
spool**.

⚠ **And multiple orders per spool is itself contested.** The 20 Aug multi-voice call confirmed *"many
rods and many orders"*; Tim O'Brien on 3 Sep 2026: *"I do not believe that we want to have multiple
orders on a spool."* `FW-207` holds both open. ⛔ **If Tim's position is ratified the first criterion
above disappears entirely** — do not build the collection before that closes.
