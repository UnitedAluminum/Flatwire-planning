...
> ### ⭐ Re-pointed 7 September 2026 by `D-55`
>
...
>
> ⛔ **The rule, unchanged and still the point of this story:** a `null` gauge or width reading
...
>
> ⚠ **`[SIG §5.3]` suppresses only FL2's batched gauge and width.** `SpeedFPM`, `FootageCounter`,
> `ComponentStatus` and `LineStatus` still flow on FL2, so an absent reading is **not** an absent
> line. ⛔ Treating the absence of `GaugeReading` as a fault is a defect.
>
> ⚠ **Open, and above this story:** the uniform treatment collides with `FR-120`, which is `Must` and
> `[CONFIRMED]`, plus `ActiveRunMonitor` §3.2 and `TC-140`. That reconciliation is a **client
...
> client falls back to `machineName` and is wrong on FL3.
...
⭐ **MOOT AS OF 9 Sep 2026 — and this paragraph is why.** It recorded that FL2 has live gauge when
running as part of an FL3 hybrid but not standalone. **That distinction was never physical**: FL3 is
FL1 feeding FL2 on the **same FM2**, through the same gauging stands, so the instrument cannot appear
and disappear with the upstream feed. **This paragraph turned out to be the internal disproof of
assumption `A3`**, which is now retired and `FR-120` superseded — see `G120`. FL2 has live gauge in
**both** modes, so there is no mode-versus-`machineName` trap left to avoid here: a client may key on
neither, because every line behaves the same way.
