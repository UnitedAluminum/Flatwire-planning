...
**Send through `IFlatWireBroadcaster.Line(MachineName)`**, which `FW-080` built as
...
| Layer | What happens |
|---|---|
| Ingest | ✅ **`[PLC §5.2.2]` now publishes `FL2.PLC.AGC.Gauge` and `.Width`** (9 Sep 2026), so `FW-N05` produces FL2 samples once the two paths are bound in the FL2 `Tags` block — a **values-only** config change. *(This row read "`[PLC §5.2.2]` publishes no `AGC` row for FL2, so `FW-N05` never produces an FL2 gauge or width sample. Assumption `A3`: 'FL2 has no live measurement'" — `A3` is retired.)* ⚠ **Cadence is 4 s, not ~10 Hz: do not batch, and re-choose any consecutive-reading threshold per line** |
