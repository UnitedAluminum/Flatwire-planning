...
status_note: "**Ready to build on the server side** — the six streams and four of six markers exist. 🟡 **Part of the FE 4 h is already built**: maximise renders the trace in a popup on DB3, and the chart shell it maximises (`lib-trace-panel`) is in `projects/shared`. ⛔ **The RT 24 h and the runtime `isLive` switch are untouched**"
owner:
jira:
mvp: 1
phase: "5"
...
> (`P-45`); `WeldJoinEvent` needs [`FW-166`](FW-166.md) and
> `RodCheckoutEvent` needs [`FW-174`](FW-174.md).
> ⛔ **REVERSED 9 Sep 2026: FL2 broadcasts LIVE gauge and width at 4 s** — `A3` retired, `FR-120` superseded (`G120`). ~~**FL2 broadcasts `null` gauge and width — deliberately.**~~ ✅ The empty-state rule survives for a **dropped feed on any line**, not as FL2's normal state. The chart must render an **empty
> state**, not zeros. That is [`FW-181`](FW-181.md), *"the single most
...
> - [ ] **Each panel maximizes to full screen** — backdrop, ESC and backdrop-click restore — 🟡 **maximise is built** on DB3 as a popup rendering the same `lib-trace-panel` at `modal-size xl` ([`[UIC §3.14]`](../../50-frontend/UIConventions.md)). ⚠ **ESC and backdrop-click do not restore, by construction**: `shared`'s `CommonPopupService` opens every modal with `backdrop: 'static'` and `keyboard: false`, which `[VAL §7.5]` requires. **This AC and that rule conflict — decide which wins** rather than "fixing" the popup service
...
| Concern | Story |
|---|---|
| The broadcast loop producing the six streams | [`FW-150`](FW-150.md) — ✅ built |
| Four of the six markers | [`FW-172`](FW-172.md) — `P-45` |
| `WeldJoinEvent`'s producer | [`FW-166`](FW-166.md) |
| `RodCheckoutEvent`'s producer | [`FW-174`](FW-174.md) |
| The FL2 null-gauge **contract** | [`FW-181`](FW-181.md) — this story **renders** it |
| The historical profile query | [`FW-164`](FW-164.md) — `gaugetrace` |
...
| FL2 branch | ⛔ **None, deliberately** — and the reason is now **stronger**: every line publishes gauge and width identically, so there is nothing line-specific to branch on. *(This read "`[SIG §5.3]` 'entirely' + no `AGC` path = nothing to suppress" until 9 Sep 2026; §5.3 is withdrawn and FL2 has `AGC` paths.)* ⚠ **Cadence differs per line and is DATA, not a branch** |
...
| `DieChangeEvent`, `PauseEvent`, `SPCCheckpoint` + one more | markers | ⚠ [`FW-172`](FW-172.md), unbuilt — **four of six** (`P-45`) |
| **`WeldJoinEvent`** | marker | ⛔ [`FW-166`](FW-166.md), unbuilt |
| **`RodCheckoutEvent`** | marker | ⛔ [`FW-174`](FW-174.md), unbuilt |
...
⛔ **9 Sep 2026: FL2 broadcasts live gauge and width at 4 s**, so this no longer describes FL2. ~~FL2 broadcasts **`null`** gauge and width.~~ ✅ `P-124`'s server rule is unchanged and now applies to every line: **null means no
sample — skip the channel, never send `0`.**

⛔ **So the chart must distinguish "no data" from "zero"** and render an empty state with the
Profile view available. [`FW-181`](FW-181.md) is called *"the single most
...
6. Maximize per panel: backdrop, ESC, backdrop-click restore.
   ...
   `maximizeLabel`**. Consume it; do not build a second one.
   ⚠ **ESC and backdrop-click do not restore** — `CommonPopupService` sets `backdrop: 'static'` and
   `keyboard: false` for every modal, per `[VAL §7.5]`. That conflict is this story's to settle.
...
[`FW-203`](FW-203.md)'s simulator — **visibly synthetic**, not a silent stub.
...
[`FW-181`](FW-181.md) (Phase 8) is the FL2 Live/Profile control this
story's runtime toggle exists for — ⛔ **and `P-49` requires the server to publish route mode.**
[`FW-164`](FW-164.md) supplies the historical `gaugetrace`
the Profile view renders. [`FW-172`](FW-172.md),
[`FW-166`](FW-166.md) and
[`FW-174`](FW-174.md) are the marker producers.
...
- [ ] **The trace treatment is uniform across all three lines** — the same panel titles (`Gauge` and
      ...
      in `FW-062` and `[UIC §5]`.
- [ ] **`isLive` is driven by the data, not by the line.** The runtime-switchable contract this story
      already owes is exactly what makes one path serve a live FL1 run, an FL2 profile and a hybrid
      FL3 run that has both.
- [ ] ⛔ **A `null` reading renders a no-measurement state — never a flat line at target.** The rule
      is **`FW-181`**'s; this story is where the chart honours it.
- [ ] ⚠ **Open, and above this story:** the uniform treatment collides with `FR-120` (`Must`,
      `[CONFIRMED]`), `ActiveRunMonitor` §3.2 and `TC-140`, all of which still specify an FL2
      profile view. That reconciliation is a **client decision**.
