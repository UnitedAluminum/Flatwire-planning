...
`FlatWireHub` joins **line groups only** — `JoinLineGroup(MachineName)` builds the name server-side from
an enum — and `IFlatWireBroadcaster`'s entire surface is **`IFlatWireClient Line(MachineName)`**. There
...
2. **`IFlatWireBroadcaster`** gains a second addressing method beside `Line(MachineName)`, implemented
...
1. ✅ **All four are already typed on `IFlatWireClient`** ([`FW-149`](FW-149.md)) —
   confirm, do not add.
2. **Declare the two missing domain events** (§1.1a), each carrying `MachineName`, and write their
...
`IFlatWireBroadcaster` gains one addressing method beside `Line(MachineName)`.** Handlers keep
