...
status_note: "**Buildable — and it CANNOT produce a number today.** `Q10` is unanswered and `LbPerFtFactor` is seeded `NULL` [9 Sep 2026, client answers] Q30/Q33 - THERE ARE NO LOAD CELLS anywhere on the take-ups or payoffs, so the derived basis is not one option of two, it is the only one. Coil outside diameter is itself derived from encoder footage against take-up speed and exposed as a read tag, which INVERTS the OD-to-weight question: if OD comes from footage then weight comes from footage directly and OD is not an input to it. Confirm that reading before building a second basis."
owner:
jira:
mvp: 1
phase: "4"
