...
id: FW-167
legacy_id:
title: POST /diechange and DieChangeService
status: not-started
status_confirmed: false
status_note: "⛔ **`D4` reverted to PER-TOOL validation on 2 Sep 2026 (`Q91`) — §2.3's size-level restatement is retired, and `TC-274` is executable**"
owner:
jira:
mvp: 1
phase: "6"
...
hours: 12
sprint: S2
depends_on: [FW-139, FW-171, FW-169, FW-251]
blocked_by: [OI-12]
has_plan: true
started:
completed:
...
**Status:** ⛔ **`D4` reverted to PER-TOOL validation on 2 Sep 2026 (`Q91`) — §2.3's size-level restatement is retired**
...
> ⛔ **RETIRED 2 SEPTEMBER 2026 BY THE DIE SPLIT (`Q91`), AND THIS IS THE DETAIL MOST LIKELY TO
> BE BUILT WRONG.** The die master **is** in MVP-1: `ToolingInventoryDie` is the per-tool
> register, `DieHistory` its append-only life log, and `Drawer` now holds only the two draw boxes
> `DB1`/`DB2`. **The 13-row size catalogue this callout tells you to validate against no longer
> exists.** `FR-233` / `D4` revert to their per-tool form — the service rejects a die that is
> **not registered**, no longer merely an unrecognised size — `DieChangeEvent` gained
> **`OldDieId` / `NewDieId`** with real FKs, and **`OI-41` closed after five months**, retiring
> *"two dies of one diameter share a counter."* **`TC-274` is executable for the first time.**
> ⚠ **The lifecycle half is [`FW-252`](FW-252.md)**, not this story: this one validates and writes
> the event; that one owns registration, reset, retire, threshold edits, the `RunFootage` accrual
> and `GET /die/{dieAlpha}/history`. ⚠ **`OI-12` is now a live defect** — the 60/85 % and 65/80 %
> bands derive from one table; owned by [`FW-199`](../../60-delivery/tasks/FW-199.md).
> **It auto-creates a linked override** — `DieChangeEvent.LinkedOverrideId →
> RollOverride.OverrideId` — so [`FW-169`](FW-169.md) is a
...
| Concern | Owner |
|---|---|
| The `DieChangeEvent` table | [`FW-171`](../../30-database/tasks/FW-171.md) — ✅ built |
| The override this creates | [`FW-169`](FW-169.md) — **hard predecessor** |
| The SPC checkpoint the gate waits on | [`FW-168`](FW-168.md) |
...
| `RollOverrideService` | — | ⛔ **Absent** — [`FW-169`](FW-169.md) |
...
[`FW-234`](FW-234.md)'s audit trail** once that exists, not only to
Serilog.
...
1. ⛔ **[`FW-169`](FW-169.md) first** — this service calls
...
7. Read die life from `Drawer.LastGrindingFeet` / `TotalFeetAllowed`, **60/85 % bands only**
   (§2.3).
8. Raise the domain event so [`FW-172`](FW-172.md) can mark the trace —
...
[`FW-169`](FW-169.md) is the hard predecessor and owns the
override and its PLC write. [`FW-168`](FW-168.md) supplies the `PostDieChange`
...
[`FW-172`](FW-172.md) marks the die change on the trace.
[`FW-234`](FW-234.md) gives the logged exception a queryable home.
