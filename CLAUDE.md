...
3. **`DevelopmentPlan/ShopfloorPlan/back-matter.md`** — dependency chain, milestone calendar, and the **Gaps register (G1–G24)**.
...
| Path | Contents |
|---|---|
| `Analysis/` | Cross-cutting analysis: end-to-end process, the process walkthrough, the umbrella dashboard spec (`FlatWireShopfloorDashboards.md`), the open-questions register, the implementation plan, the Dashboard 2A UX review, partial-rod re-check-in, operations-manager role, spool material reference, client questions/sync notes. Prose specs. |
| `LatestDocument/RequirementDocuments/` | **Per-screen requirement docs, one screen or screen family each** (moved out of `Analysis/` on 1 Aug 2026): `RodPreCheckin` (DB2A) · `RocCheckin` (DB2) · `RodCheckout` (DB12) · `WeldEvent` (DB4) · `SPCCheckpoint` (DB6) · `DieChangeAndManagement` · `HMIAndSCADALayout` (DB13/14) · `SpoolCompletionNotification` · `PassScheduleManagement` (DB9/9A) · `LineStatusOverview` (DB1 — **still a 0-byte file**, OI-91).<br><br>**Not per-screen:** `PassScheduleGenerationSpec.md` (1 Aug 2026) — a **client-facing** FDD/SRS for the pass schedule generation engine, deliberately written implementation-agnostic (no tables, endpoints, screens, or story IDs). It carries its own `PSG-D##` data-item and `PSG-Q##` question series, distinct from the `Q##` register. It is the engineering basis FW-013 should be built to, and it supersedes the physics in `PassScheduleManagement.md` §"Pass Schedule Generation Algorithm" — see the divergence note below. **This `.md` is the source of truth; the rendered `PassScheduleGenerationSpec.docx` lives in `SRS/`.** |
