...
3. **`DevelopmentPlan/ShopfloorPlan/back-matter.md`** — dependency chain, milestone calendar, and the **Gaps register (G1–G28)**.
...
- **The generation algorithm exists in four copies — know which one you are reading.** In authority order: **(1)** `LatestDocument/RequirementDocuments/PassScheduleGenerationSpec.md` (1 Aug 2026) is the **engineering basis** — compounding reduction, lateral spread, inter-stand pass allocation, edge geometry, mill spring, drawing-vs-rolling separation. It wins on *physics*. **(2)** `FlatWire_MasterSpecification.md` FR-380–FR-391 + its `POST /passschedule/generate` section wins on *contract shape and arithmetic*; it already corrects the published worked example. **(3)** `APIContracts.md:604-663` is **known broken** (REVIEW Tier 1 #1 — the example contradicts its own formula). **(4)** `PassScheduleManagement.md` §"Pass Schedule Generation Algorithm" (Apr 25) was the original narrative, with wrong Step 3 pass-count branches — **deleted 1 Aug 2026** (recoverable at `2a0426b`), so there are now three copies, not four. Copy 3 is retained for history; do not implement from it.
...
| Path | Contents |
|---|---|
| `Analysis/` | Cross-cutting analysis: end-to-end process, the process walkthrough, the umbrella dashboard spec (`FlatWireShopfloorDashboards.md`), the open-questions register, the implementation plan, partial-rod re-check-in, operations-manager role, spool material reference, client questions/sync notes. Prose specs.<br><br>`Dashboard2A_UXReview.md` (17 findings against the pre-check-in screen) was **deleted 1 Aug 2026** — recoverable at `2a0426b`. Its decisions were already propagated before deletion: **Q72**, **Q76**, **Q77** in the open-questions register, **G21** in `back-matter.md`, and the `201`/`Blocked` staging contract. Nothing in it is load-bearing that is not also recorded at those destinations. |
| `LatestDocument/RequirementDocuments/` | **Per-screen requirement docs, one screen or screen family each** (moved out of `Analysis/` on 1 Aug 2026): `RodPreCheckin` (DB2A) · `RocCheckin` (DB2) · `RodCheckout` (DB12) · `WeldEvent` (weld capture — DB4 retired 1 Aug 2026, now the DB2A *Mark as welded* dialog) · `SPCCheckpoint` (DB6) · `DieChangeAndManagement` · `HMIAndSCADALayout` (DB13/14) · `SpoolCompletionNotification` · `PassScheduleManagement` (DB9/9A) · `LineStatusOverview` (DB1).<br><br>**All ten were reissued as client-review deliverables on 1 Aug 2026** — a common format (change history, `[CONFIRMED]`/`[PROPOSED]`/`[CLIENT INPUT REQUIRED]` tags, confirmed-decisions table, open-items table, assumptions, client sign-off checklist), superseded rules and internal schema/interface/mockup detail stripped. `LineStatusOverview` was written from scratch in that pass (it had been a 0-byte file, OI-91) by consolidating DB1 content out of `FlatWireShopfloorDashboards.md` and the HMI/pre-check-in docs — **it is the only one of the ten never previously reviewed.**<br><br>**Not per-screen:** `PassScheduleGenerationSpec.md` (1 Aug 2026) — a **client-facing** FDD/SRS for the pass schedule generation engine, deliberately written implementation-agnostic (no tables, endpoints, screens, or story IDs). It carries its own `PSG-D##` data-item and `PSG-Q##` question series, distinct from the `Q##` register. It is the engineering basis FW-013 should be built to, and it is now the **only** generation-engine description in `LatestDocument/` — the rival algorithm in `PassScheduleManagement.md` was deleted on 1 Aug 2026 rather than corrected (git history at `2a0426b`). **This `.md` is the source of truth; the rendered `PassScheduleGenerationSpec.docx` lives in `SRS/`.** |
| `DevelopmentPlan/` | `ShopfloorAndRealTimePlan.md` (roadmap index), `REVIEW.md`, `APIContracts.md`, `FlatWireJiraStories.md`, `FlatWireTables.md`, `TechStackRecommendation.md`, `Checkin*` plan+prompt, `PassScheduleGenerationSpecPrompt.md`, `Flat Wire.code-workspace`. |
...
| `SRS/` | **Client-facing `.docx` deliverables.** `Shopfloor_Flat_wireSRS.docx` — the delivered SRS. The consolidated v3 was removed from the repo on 1 Aug 2026 (in git history at `6096921`). The surviving SRS has **no pre-check-in content**, so it is *not* the requirement source for `PCI`/`PRC`/`CHK`/`WLD`/`TRV` IDs; those rules live in `LatestDocument/ProjectPlan/02-SRS.md` as `FR-###`.<br><br>`PassScheduleGenerationSpec.docx` (1 Aug 2026) — the Word rendering of the pass-schedule spec, moved here from `RequirementDocuments/` because this folder is where client deliverables live. **The markdown in `LatestDocument/RequirementDocuments/` is the source; this `.docx` is generated output** — edit the `.md` and re-render, never the `.docx`. It also supplies the header/footer branding (Nagarro logos) that the renderer lifts out of `Shopfloor_Flat_wireSRS.docx`. |
| root `Shopfloor_Flat_wireSRS_Consolidated_v2.docx` | Untracked working copy at root. **It is not a substitute for the removed v3** — treat requirement text in `LatestDocument/ProjectPlan/02-SRS.md` as authoritative. *(`Flat Wire Mockups.xlsx` was tracked here but is absent from the working tree as of 1 Aug 2026 — restore with `git restore` if it is wanted.)* |
...
- **`RodStaging` (pre-check-in / payoff staging)** is the newest table and carries two invariants worth knowing before touching it: `Blocked` is a **derived** bay state (`Status='Staged'` + any inspection column `='Fail'`), never a fourth `Status` value; and `IsWelded` is a **flag on a `Staged` row**, not a status — so any code branching on "staged" also matches welded and blocked rods unless it says otherwise. See `LatestDocument/RequirementDocuments/RodPreCheckin.md` and gap **G21** (the `(LineId, PayoffPosition)` uniqueness scope is unresolved across FL1/FL3).
