...
**Last Updated:** September 10, 2026 — ⭐ **§5.0a added: four recorded departures on Dashboard 2's material band** (`FW-061` `Step-7`), of which two matter most — **two of the six *Incoming Bundle Information* cells render *Not available* because no contract carries them** (`OI-117`, `G130`), and the **payoff selector is a placeholder** because occupancy is `FW-158`'s. ✅ **No new class, style or colour was created**, and the mockup's `.input-readonly` / `.payoff-option` were verified absent from the application before being avoided. Change history is in [`../CHANGELOG.md`](../CHANGELOG.md) · **8 Sep 2026 (`D-56`): `LineId` is renamed `MachineName` throughout** — same `VARCHAR(5)` shape, same `CHECK` values, operator-visible labels unchanged. `FW-N17`/`FW-N18`/`FW-N19`.
...
**Part of:** `ProjectPlan/Frontend/` — index: [`tasks/Orchestration.md`](spool-notification/Orchestration.md)
...
| A **Live** badge on a context bar | ✅ | same claim — ⚠ **DB1's does not pulse and should.** It is a plain `fa-circle text-success` with no `live-dot`; **the code is wrong here, not this table.** Deferred to [`FW-060`](../10-requirements/features/FS-06-line-visibility-alerting/FE/FW-060.md) with the height budget (§4.1), because DB1 is the last screen scheduled |
...
[`FW-060`](../10-requirements/features/FS-06-line-visibility-alerting/FE/FW-060.md) rather than now. ⛔ **Until then, copy DB3's budget — never DB1's
layout.**
...
### 5.0a Departures on Dashboard 2's material band — `FW-061` `Step-7`, 10 September 2026

The rod scan row, the *Incoming Bundle Information* grid and the diameter field are built. Four
departures from `dashboard_2_rod_checkin.html`, each recorded rather than silently taken:

| Departure | The mockup shows | What is built | Why |
|---|---|---|---|
| **Two bundle cells** | `Heat / Cast no.` = `HC-2024-0892` and `Supplier` = `Southwire`, as data | **`Not available`**, in italic muted text | ⛔ **No contract carries either.** `coils` has no heat column (`OI-117`) and the supplier name is unmapped (`G130`) — `coil_origin_code` is a one-character origin flag and `vendor_no` a two-character code. `P-352`: this feeds welding-wire certificates, where a fabricated value is not noticed and a blank one is |
| **Gross and net weight** | a `disabled` input and an `.input-readonly` div — neither typeable | **pre-filled from the lookup**, and enterable at `Step-12` | `[RCI §3.1]` check 4 requires them *entered*. ⛔ The mockup is a **defect** here, not a specification |
| **The payoff selector** | two always-clickable `div`s with a fake radio dot, `data-payoff` never read | a **labelled placeholder** | Occupancy is `[API §4.4]`'s and `FW-158` has not started. ⛔ Two enabled bays would reproduce the cold-start dead end **DB2A's own script comments record**. When it lands, port **2A's** `#pci-payoff` — `role="radiogroup"`, `aria-checked`, keyboard — ⛔ not DB2's |
| **The scan affordance** | a `.btn-scan` class that **no element uses** | one pre-focused field, resolving on the terminating keypress **and** a debounce | `FR-098`: no submit control. The mockup's empty affordance slot is not a control |

✅ **No new class, style or colour was created.** Every class was verified first: `fs-14`/`fs-12`
are generated by `_dynamic-classes.scss`'s `generate-font-classes(10, 16, 1)`; `is-invalid` and
`invalid-feedback` by Bootstrap's `$form-validation-states` loop; `fst-italic` is a Bootstrap
utility. ⛔ The mockup's `.input-readonly` and `.payoff-option` **do not exist in the application**
and were not used — they are exactly the shape of §2's three shipped no-ops.
