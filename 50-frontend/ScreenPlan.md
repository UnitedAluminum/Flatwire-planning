...
> ⚠ **RE-MEASURED 8 Sep 2026: the folder holds `44` files, `25` of them HTML — the same figure as 4 Sep, but NOT the same composition, and it was wrong in between.** `dashboard_3_active_run_ual.html` was **deleted on 5 Sep 2026** (`f85d3a0`) and neither count site was updated, so both read one too high from 5 Sep until `line_layout_isometric.html` was added on 8 Sep and coincidentally restored the total. ⛔ **Two consequences of that deletion are still open and are not fixed here:** its **five generated assets remain in the folder, now orphaned** (`ual-app.styles.css`, `fw-app-compat.css`, `fontawesome.app.css`, `fa-regular-400.woff2`, `fa-solid-900.woff2`), and **seven live documents still reference the deleted file** — this one, `[VAL]`, `FW-130`, `Orchestration.md`, `Phase-01A-ImplementationPlan.md` and `DOCUMENTS.md`. ⚠ **`line_layout_isometric.html` is not a screen** — it is an isometric equipment-layout reference for FL1/FL2/FL3, with no `DB##` number, no story and no route; it is not part of the screen inventory below. *(previously RE-MEASURED 4 Sep 2026: `44` files, `25` HTML — 43 / 24 before `ASK` was added.)* The published figures below were **already wrong before `dashboard_ask.html` was added** — they read "38 files / 19 HTML", which described the *pre-re-tree* split, when the five MVP-2 mockups lived in a separate folder. The 29 Aug 2026 re-tree folded them into this flat folder and no count followed. **Nothing measures these numbers**: `verify_schema_counts.py` counts database objects only, `linkcheck.py` is additive, and `check_docs.py` reads task front-matter — so screen counts are hand-maintained prose and have drifted accordingly. ⚠ **The same drift is uncorrected in roughly a dozen other restatements** — `[VAL]`, the `50-frontend/tasks/` files, `[CMP]`, `[ARC §2]`, `[MS §7.1]` and `tools/pathmap.py` — and reconciling all of them is a separate pass, deliberately not bundled into a screen addition.
>
...
2. ⛔ **WITHDRAWN — do not consume the mockups' token stylesheet.** This rule used to read *"consume `flat-wire-shopfloor.styles.scss` as-is … Angular components need `ViewEncapsulation.None` or `:host` scoping so the tokens resolve."* **The token stylesheet is not shipped into the repository** ([`Orchestration §4`](spool-notification/Orchestration.md), `F-03` withdrawn), the mockups' `--color-*` palette **exists nowhere**, and the standing rule is now **create no new classes, styles or colours** — a flat wire screen is built from the application's existing classes. See [`[UIC §2]`](UIConventions.md). The `--fw-*` half stands: that prefix is stale (gap **G18**) and there is nothing to migrate.
