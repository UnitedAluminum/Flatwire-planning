...
**Status:** Active — ⬜ **wave 0 is unbuilt.** The 28 Aug 2026 `FW-N03` build was **reverted on 31 Aug 2026**, so `projects/flat-wire/` no longer exists and `FW-130`, `FW-131`, `FW-132` and `FW-135` wait on the scaffold again
...
| **`projects/`** | **31 `angular.json` entries** — 30 libraries + the host app `united-aluminum`. ⬜ **No `flat-wire`.** It was entry 32 from 28 Aug 2026 until the `FW-N03` revert on 31 Aug 2026 removed it, and this row reads as it did before that build |
...
|---|---|---|---|---|
| **0** | `FW-N03` · library scaffold, routing, configuration | [FW-N03.md](FW-N03.md) | 24 | ⬜ **Not started** — built 28 Aug 2026 and **reverted 31 Aug 2026**; the execution record went with it |
...
⚠ **The technique mattered as much as the scope.** `../../90-registers/Gaps.md` and `phase-01a` are **pure CRLF**
...
**Found on executing `FW-N03`, 28 Aug 2026 — that build was reverted on 31 Aug 2026 but this gap is the checkout's, not the story's, and it still blocks runtime verification for every 1A story.**
...
[`FW-130 §3 step 2c`](FW-130.md); **whichever is taken must be written down**,
