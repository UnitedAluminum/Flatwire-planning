...
id: FW-130
legacy_id:
title: Shell layout and the 1280×1024 shopfloor canvas
status: not-started
status_confirmed: false
status_note: "⬜ **Waiting on `FW-N03` again — its 28 Aug 2026 build was reverted on 31 Aug 2026** (wave 1). The library, the empty app `styles` array slot and `FlatWirePlaceholderComponent` **no longer exist in any checkout**; §1a is dormant until the scaffold lands"
owner:
jira:
mvp: 1
phase: "1A"
...
#### 1a. ⚠ You may be replacing something that already exists — the reverted build shipped a placeholder

**Measured in the checkout after `FW-N03` landed (28 Aug 2026).** ⛔ **That build was reverted on
31 Aug 2026**, so nothing described below exists in any checkout today — re-measure once the
scaffold lands again, because a rebuild need not reproduce it:
...
(recorded in `FW-N03`'s execution record, which the revert removed).
...
1. **Delete `FlatWirePlaceholderComponent`** — the `.ts`, `.html` and `.spec.ts` — and drop it from `FlatWireModule`'s `declarations`/`exports` and from `public-api.ts`, **if the rebuilt scaffold ships it again**. ⚠ The reverted build **exported** it, so removing it is a public-API change; nothing outside the library consumed it.
2. **Remove `projects/flat-wire/src/lib/styles/.gitkeep`** when the stylesheet of step 2 lands — the folder is no longer empty.
3. ⚠ **The shell component needs a spec on the day it is written.** `collectCoverageFrom` globs `*.component.ts` and the gate is **95 %** (`F-08`); the 28 Aug 2026 `FW-N03` build hit exactly this — its first suite run failed at **36 %** because a guard stub had no spec.
...
and register it in `angular.json`'s app `styles` array — the slot `FW-N03` leaves empty. ✅ **Verified
31 Aug 2026, after the revert: the array holds no flat-wire entry**, and the precedent
...
The 28 Aug 2026 build, since reverted, skipped it on the grounds that `dist/` looked populated; `dist/shared` was stale from 23 Jul
