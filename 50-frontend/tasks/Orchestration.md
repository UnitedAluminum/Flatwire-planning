...
⛔ **The un-priced-mockup problem repeats, and here it is sharper**: there is no client screenshot
of this grid either, so there is nothing to author a mockup *from* until `Q92` returns.

| Story | Subject | h | Owning document | Status |
|---|---|---|---|---|
| [`FW-261`](FW-261.md) | **Tooling Inventory — the fourth *Choose Tool* option** and the roll-set grid | 12 | `D-42` · [`FlatWireSchema_Lookup.md`](../../30-database/schema/FlatWireSchema_Lookup.md) *(the columns, all `[PROPOSED]`)* · ⛔ **no mockup, and no client grid to build one from** | Blocked on **`Q92`**, **`OI-141`** |

> ⚠ **[`FW-003`](FW-003.md) was re-pointed rather than replaced** — its Tooling Inventory
> acceptance criterion moves from **three** tool types to **four**, and gains the FL1/FL2-only rule
> (`D-42`). Its stale claim that *"`Straightener` exists nowhere else in this repository"* is
> corrected in the same pass: the name reached the **delay-code lookup** on 2 Sep and now has three
> seeded codes and two mockup rows — just no equipment table, which is still `G77`.
>
> ⚠ **Build the one-option form and keep the split cheap.** Whether capstan rolls are their own
> *Choose Tool* entry is `Q92`'s second question; the schema takes mill and capstan rolls as one
> discriminated register, and the screen may or may not follow.
>
> ⛔ **Dancers, entry guides, payoffs and spools must not appear on this tab.** The client excluded
> all four **explicitly** (`D-42`), and each already has its own register.
...
| No date | **`G27`** *(open)* / **`G28`** ✅ *(**RESOLVED 3 Sep 2026**)* — the weld screen's rod queue and traceability chain lost their host **and that half is still open**; the other half — *FL2 may have no way to record a weld at all* — is **answered: FL2 never welds**, so no entry point is needed and `G28` is off `FW-063`'s `blocked_by` | `FW-063`. DB4 was retired 1 Aug 2026 into DB2A's *Mark as welded* dialog, and **two things did not move with it** | [`WeldEvent.md`](../../10-requirements/screens/WeldEvent.md) |
