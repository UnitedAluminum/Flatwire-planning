---
id: FW-063
legacy_id:
title: Weld capture — fw-mark-welded-dialog
status: not-started
status_confirmed: true
status_note: "⬜ **Not started, and it has no host screen yet.** Weld capture lives inside DB2A since DB4 was retired, and **DB2A itself is not built** (`FW-N01`). ⛔ **The endpoint is absent too** — `POST /staging/rod/mark-welded` is specified in `[API §3.1]` and implemented by no controller, and `FW-166` is `blocked`. ⚠ `G27` still stands: the rod queue and the traceability chain that DB4 carried have no host. the dialog scaffold to copy is `trace-graph-popup.component` — `CommonPopupService` + `NgbActiveModal` + `cdkDrag`, with the action button carrying an icon and the dismiss button not. ✅ **`Q6` DROPPED 9 Sep 2026** — footage-split at the weld point confirmed, question closed to `[QD]`. ⛔ **`OQ-6` is NOT the same id and stays** — `G61` says the Aug 2026 retarget was by subject, not by number. ⚠ **`Q72` adds a precondition**: the weld flag cannot be set before check-in or pre-check-in, and there is no clear-in-place path — see `FW-N27`."
owner:
jira:
mvp: 1
phase: "6"
stream: FE
streams: [FE]
priority: high
hours: 20
sprint: S2
depends_on: [FW-N01, FW-166]
blocked_by: [G26, G27, OI-59, OQ-6]
has_plan: false
started:
completed:
---

# FW-063 - Weld capture — fw-mark-welded-dialog

> **No implementation plan has been written for this story yet.**
> The card below is the contract from `[TB]`. Before starting, replace this notice
> with the sections in the task template: *What to build* / *Context you need* /
> *Build order* / *Decisions made here* / *Verification* / *Handoff*.

## 1. What to build

**Hours:** 20 h FE · **Priority:** High · **Sprint:** S2 · **Phase:** 6 · **Stream:** FE

**As an** FL1 operator,
**I want** to record the induction weld joining the running rod to the staged one,
**So that** output-coil footage can be attributed to the right source rod on a customer certificate.

**Acceptance Criteria:**
- [ ] Dialog raised from **Dashboard 2A's staged bay card**; outgoing alpha and weld footage auto-populated
- [ ] **Incoming rod defaults to the `Staged` rod on the idle bay** rather than free entry (`PCI008`), with scan-to-override still available
- [ ] **Induction only** — laser was dropped
- [ ] Quality Pass/Fail with a **reason mandatory on Fail** (six reasons); **neither result pre-selected**; confirm gated on *material match* **AND** *quality chosen* **AND** *(Pass OR reason selected)*
- [ ] Confirm links `Rout → Rin`; all later footage attributed to the incoming rod; weld marker on the trace; WELD-SOON/NOW alert cleared
- [ ] **Operator comes from the signed-in session** — no badge re-entry (`WLD003`)
- [ ] **No edit path** — reversing a recorded weld is `WLD011` and is unspecified
- [ ] Read-only **Welds this run** dialog over `GET /run/{runId}/weldevents`, **enabled at a count of 0** (the empty state is the answer), and **absent rather than disabled at cold start** when there is no active card

**Rate-card basis:** modal/dialog 12 h + quality-gate and weld-list uplift 8 h = 20 h (§2)
**Dependencies:** FW-N01, FW-166
**Blockers:** **OQ-6** · **G26** *(this de-stubs the Phase-4 screen)* · **G27** *(the weld screen's re-sequenceable rod queue and traceability chain lost their host when DB4 was retired)* · ~~**G28**~~ ✅ *(**RESOLVED 3 Sep 2026 — and the answer is that FL2 never welds at all.** Tim O'Brien: *"Once the material has been Flattened, it is no longer able to be welded due to a lack of capability at UA"*, plus *"Correct"* on welding being induction at FL1 only. **Build no FL2 weld entry point** — an FL2 coil inherits its spool's weld markers, which `WeldEvent.md` §6 now states normatively. Removed from `blocked_by`. [`ClientEmail_2026-09-03_RodOrderAllocation_SyncPlan.md`](../../95-archive/source-documents/ClientEmail_2026-09-03_RodOrderAllocation_SyncPlan.md))* · **OI-59 / Q6** *(a fail-then-remake writes several `WeldEvent` rows for one physical join; whether a superseded attempt reaches the certificate is undecided)*

---
