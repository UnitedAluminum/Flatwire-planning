...
status_note: "⬜ **Not started.** ✅ **`POST /rolloverride` answers today.** ⚠ **FL2 and FL3 only** — `FR-107` gives FL1 no Roll Adjust; ⛔ **but the built FL1 rail contains a Roll Adjust entry**, which `FW-062`'s AC now records as a live contradiction to settle before the rail becomes active. ⚠ Per-stand controls are `FM2_S1` / `S2` / `S3` — **three stands, position-only**, roll diameter is data. ⛔ `OI-103` sets no bound on a roll-gap change, neither per step nor cumulative. the dialog scaffold to copy is `trace-graph-popup.component` — `CommonPopupService` + `NgbActiveModal` + `cdkDrag`, with the action button carrying an icon and the dismiss button not."
owner:
jira:
mvp: 1
phase: "6"
