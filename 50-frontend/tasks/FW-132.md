...
status_note: "⬜ **Waiting on `FW-N03` again — its 28 Aug 2026 build was reverted on 31 Aug 2026** (wave 1). `services/`, `models/` and the `useMockData` config keys **no longer exist**. ⚠ Two model decisions are unresolved (`G14`)"
owner:
jira:
mvp: 1
phase: "1A"
...
**Status:** ⬜ **Waiting on `FW-N03` again — its 28 Aug 2026 build was reverted on 31 Aug 2026** (wave 1). `services/`, `models/` and the `useMockData` config keys **no longer exist**. ⚠ Two model decisions are unresolved (`G14`)
