...
status_note: "✅ **Buildable now — `FW-N03` landed 28 Aug 2026** (wave 1). `services/` and `models/` exist empty and `useMockData` is live in both config files. ⚠ Two model decisions are unresolved (`G14`)"
owner:
jira:
mvp: 1
phase: "1A"
