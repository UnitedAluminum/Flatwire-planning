...
status_note: "⛔ **Cannot connect.** `flatWireHubUrl` is `FlatWire/hubs/flat-wire`, but the running hub is mapped at `/hubs/flatwire` — `FW-080 §0` owes the re-map. ⚠ One package must still be added (`G10`), and this story also waits on `FW-N03` to land"
owner:
jira:
mvp: 1
phase: "1A"
...
**Status:** ⛔ **Cannot connect** — `flatWireHubUrl` is `FlatWire/hubs/flat-wire` and the running hub is mapped at `/hubs/flatwire` (`FW-080 §0`). ⚠ One package must still be added (`G10`)
...
⛔ **The running hub does not answer there, so this story cannot connect.**
[`FW-080`](../../40-backend/tasks/FW-080.md) maps `app.MapHub<FlatWireHub>("/hubs/flatwire")` and
owes the re-map (`FW-080 §0`), together with
[`FW-145 §3.5`](../../40-backend/tasks/FW-145.md)'s `?access_token=` handler, which matches the same
segment and would otherwise reject a valid token with **401**. **Nothing here is owed by this
story** — do not work around it with a hard-coded URL.
