 ...
 * of the target spool weight. See MVP-1/ProjectPlan/Business/Screens/SpoolCompletionNotification.md for the requirement.
 *
 * Behavior:
 ...
 * exceeds the TKUP-2 ceiling of 1,100 lb, and it is WITHDRAWN. Still open (Q18): which order
 * field carries the customer min/max.
 *
 * SHORT CLOSE (Q79, decided 30 Jul 2026) — closing below target is an UNPLANNED STOP on the
...
(function () {
  ...
  var CFG = {
    ...
    /* Customer max from the order. 900 lb is the figure the client gave as an example, not a
       default to rely on — the real value comes from the order (Q18). */
    targetLb:     user.targetLb     || 900,
    /* Customer minimum. A spool closed below this is OUT OF RANGE and needs a supervisor
       override + production hold, or a concession offer to the customer (Q79). */
  ...
  /* When no supervisor is on the floor, fall back to the remote approval model (Q75). */
