...
|---|---|---|---|
| **A** — milestone alerts | `§2`, `§3` — `FR-130`–`FR-137`, **`Should`** | [`FW-N02`](../../10-requirements/features/FS-11-spool-lifecycle-fl2.md) | `blocked`, 4–8 h |
| **B** — stop confirmation | `§4` — `FR-140`–`FR-157`, **`Must`** | [`FW-202`](../../10-requirements/features/FS-08-active-run-monitoring.md) | `blocked`, 98 h, critical |
| **C** — short close | `§5` — `FR-130b`–`FR-130d`, **`Must`** | no owner | see §5 |

The card, the pill and the accent map were built by
[`FW-062`](../../10-requirements/features/FS-08-active-run-monitoring.md) as part of the DB3 shell.
...
| [`FW-N02`](../../10-requirements/features/FS-11-spool-lifecycle-fl2.md) | ⚠ its build order evaluates the ladder over `payoffWeight$`; `[SIG §5.2]` names **`FootageCounter`** as spool progress's source |
