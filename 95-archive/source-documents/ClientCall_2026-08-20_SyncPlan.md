...
| | What it does |
|---|---|
| **`D1` — FL2 gets pre-check-in** | **The first outright reversal of a requirement that is asserted in the DDL.** `FR-031` reads *"the system shall **not** support pre-check-in on FL2 — a `lineId` of `FL2` is rejected"*, and the rule is asserted across **eighteen documents** — **sixteen citing `PCI002` in thirty places**, plus two that assert it without citing the rule at all ([`PLCTagSpecification.md`](../../20-architecture/PLCTagSpecification.md) line-capability table and [`VisionAndScope.md`](../../00-overview/VisionAndScope.md) line-capability table), **so a grep for `PCI002` alone will miss two of them**. Among the eighteen: a `CHECK` constraint in [`FlatWire_DDL_04_Runs.sql`](../../30-database/sql/FlatWire_DDL_04_Runs.sql), a *"deliberately NOT created"* comment in [`10_CommonDB_Insert_WIPStations_FlatWire.sql`](../../30-database/scripts/10_CommonDB_Insert_WIPStations_FlatWire.sql), a `422` branch in the contract, a test case, two phase files, the whole rationale of [`SpoolQueue.md`](../../10-requirements/screens/SpoolQueue.md) §1.2, and comments inside three mockups. The 30 Jul call carried three reversals; **none of them reached a constraint** |
