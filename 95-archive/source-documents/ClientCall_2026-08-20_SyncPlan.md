...
**Registers touched:** `Q##` ([FlatWireOpenQuestions.md](../../90-registers/Questions.md), 36 → 47) · `OI-##` ([FlatWire_MasterSpecification.md](../../10-requirements/MasterSpecification.md) §11, → `OI-122`) · `G##` ([Development/GapsRegister.md](../../90-registers/Gaps.md), `G42` updated, `G46` added) · `FR-###` ([BusinessRequirements.md](../../10-requirements/BusinessRequirements.md), `FR-031` reversed — W3)
...
| | What it does |
|---|---|
| **`D1` — FL2 gets pre-check-in** | **The first outright reversal of a requirement that is asserted in the DDL.** `FR-031` reads *"the system shall **not** support pre-check-in on FL2 — a `lineId` of `FL2` is rejected"*, and the rule is asserted across **eighteen documents** — **sixteen citing `PCI002` in thirty places**, plus two that assert it without citing the rule at all ([`PLCTagSpecification.md`](../../20-architecture/PLCTagSpecification.md) line-capability table and [`VisionAndScope.md`](../../00-overview/VisionAndScope.md) line-capability table), **so a grep for `PCI002` alone will miss two of them**. Among the eighteen: a `CHECK` constraint in [`FlatWire_DDL_04_Runs.sql`](../../30-database/sql/FlatWire_DDL_04_Runs.sql), a *"deliberately NOT created"* comment in [`10_CommonDB_Insert_WIPStations_FlatWire.sql`](../design-notes/10_CommonDB_Insert_WIPStations_FlatWire.sql), a `422` branch in the contract, a test case, two phase files, the whole rationale of [`SpoolQueue.md`](../../10-requirements/screens/SpoolQueue.md) §1.2, and comments inside three mockups. The 30 Jul call carried three reversals; **none of them reached a constraint** |
...
[`BusinessRules.md`](../../10-requirements/BusinessRules.md) §3 and `Q18`; no edit.
...
> | **Visual inspection before unbanding** — oxidation, surface defects, water stains | **Not performed.** [`RocCheckin.md`](../../10-requirements/screens/RocCheckin.md) §4.3: *"Not required. The material was inspected at FL1 before it was drawn and flattened"* |
...
**Where it should live.** [`SpoolQueue.md`](../../10-requirements/screens/SpoolQueue.md) — Dashboard 5A
...
> **This supersedes the May 2026 description** in [`Spool.md`](../design-notes/Spool.md) §"Status
...
**What is open — `Q44`.** [`SpoolCompletionNotification.md`](../../10-requirements/screens/SpoolCompletionNotification.md)
...
|---|---|---|---|
| **W1 — Registers** ✅ | [FlatWireOpenQuestions.md](../../90-registers/Questions.md) — **`Q37`–`Q40` backfilled** (the `G46` repair) and **`Q41`–`Q47` added**, index counts, priority table, Quick Reference log and the *"no numbering holes"* paragraph all realigned · [FlatWire_MasterSpecification.md](../../10-requirements/MasterSpecification.md) §11 — **`OI-115`/`OI-116` backfilled**, **`OI-118`–`OI-122`** added · [GapsRegister.md](../../90-registers/Gaps.md) — **`G42` updated** (client-confirmed, weight-per-alpha, spool↔order sibling, window closes at `S1`), **`G46` added** | 3 | L |
| **W2 — The two specifications** ✅ | [SpoolQueue.md](../../10-requirements/screens/SpoolQueue.md) **v1.4** — §1.2 rationale, §3.2 order-set, §3.5 carrier-vs-material, §4 fields, §6 confirmed decisions, §7 item 1 **closed**, §8 assumption 1 **replaced**, and the stray change-history row at the foot removed · [SpoolCompletionNotification.md](../../10-requirements/screens/SpoolCompletionNotification.md) **v2.3** — next-carrier capture as a hard gate, mandrel diameter, and the **label content that this document has never stated** | 2 | M |
| **W3 — Requirement text** | [BusinessRequirements.md](../../10-requirements/BusinessRequirements.md) — **`FR-031` superseded in place, never renumbered**; new **`FR-533`+** for the FL2 pre-check-in; `FR-172`'s spool-side genealogy given a table at last · [FlatWire_MasterSpecification.md](../../10-requirements/MasterSpecification.md) **§2.2** (line-capability table), **§4.1** (`FR-031`), **§5.3a** (the DB5A rationale), the **`RodStaging` schema block** (its `LineId` CHECK note), **§5.12** (`FL2PO` *"deliberately not created"*), the **`POST /staging/rod`** block (the `422`) · [VisionAndScope.md](../../00-overview/VisionAndScope.md) line-capability table and [PLCTagSpecification.md](../../20-architecture/PLCTagSpecification.md) line-capability table — ⚠ **these two are the ones that do not cite `PCI002`**, so they will not appear in the obvious grep; `[PLC]` is **prose only**, it owns every tag path string and this change adds none · [FlatWireShopfloorDashboards.md](../design-notes/FlatWireShopfloorDashboards.md) — not a requirements source, but it states the exclusion twice and is read as background | 4 | **H** |
| **W4 — Schema + DDL** | ⚠ **The table count moves 28 → 29** once `OI-118` resolves to a `SpoolStaging` table, and **again** for `SpoolTraceability` and the spool↔order association (`G42`, `OI-119`, `OI-121`) — so plan for **28 → 31** and re-derive in one sweep, not incrementally. *"34 tables · 57 FKs · 69 index statements · 1 procedure · 1 trigger"* is a **published, verified** figure cited in **20+ files** (`CHANGELOG.md` ×10, master spec ×8, `Architecture.md` ×4, `DatabaseDesign.md` ×4, `SQL/README.md` ×4, `FlatWire_DDL_RunAll.sql` ×2, `CLAUDE.md` ×2, `MVP2-SCOPE.md` ×3, `FlatWireSchema_Mapping.md`, `GapAnalysis.md`, `VisionAndScope.md`, `Orchestration.md`, `FW-142`, `FW-143`, `FW-223`, `TrialRunPlan.md`). Files: `FlatWire_DDL_01_Lookup.sql` (the `Spool` registry), `_03_Materials.sql` (`SpoolProcessing.OrderNo`, `ParentRodAlpha`), `_04_Runs.sql` (`CK_RodStaging_LineId`, the new staging table), `_06_ForeignKeys.sql`, `_07_Indexes.sql`, the seed files, and [FlatWireSchema_Runs.md](../../30-database/schema/FlatWireSchema_Runs.md) / [FlatWireSchema_Materials.md](../../30-database/schema/FlatWireSchema_Materials.md). **Verify by teardown → `RunAll` → idempotent re-run**, and publish the new counts once | 10+ | **H** |
| **W5 — Contracts + shared schema** | [APIs.md](../../40-backend/APIs.md) — `POST /precheckin` loses its `lineId = FL2` → `422` branch; `GET /spools` returns an order **set**; `POST /checkin/spool` selects one order from the set · [Integration.md](../../20-architecture/Integration.md) §8.0 — **`OI-115` narrowed**: the FL2 station claim is the **lead rod alpha**, not `SP-00021` · [`10_CommonDB_Insert_WIPStations_FlatWire.sql`](../design-notes/10_CommonDB_Insert_WIPStations_FlatWire.sql) — **`FL2PO` must now be created**, against a comment that says it is deliberately absent. ⚠ **Interacts with `OI-26`** (does FL3 share FL1's VPS?) and **`G21`** (`UX_RodStaging_Bay` scope), neither closed | 4 | **H** |
...
| **W7 — Plans, tests, effort** | [TaskBreakdown.md](../../60-delivery/TaskBreakdown.md) — **`FW-224`+ minted, additive**; line 1629's *"FL1/FL3 only — `PCI002` excludes FL2"* · [CapacityAndEffortModel.md](../../60-delivery/CapacityAndEffortModel.md) §3b — **a new additive sheet, never an in-place edit of a total** · `phase-04` line 50 and **`phase-08`** (the FL2 spool check-in phase, which states the exclusion and is where an FL2 pre-check-in would land) · [TrialRunPlan.md](../../60-delivery/TrialRunPlan.md) §1.5 (`OI-122`) · [TestCases.md](../../70-testing/TestCases.md) — an existing case asserts the `422`, and new cases are owed for the FL2 pre-check-in and the multi-order spool | 6 | M |
...
| Document | Why |
|---|---|
| [FlatWireOpenQuestions.md](../../90-registers/Questions.md) | Authoritative `Q##` register — W1; `Q37`–`Q47` |
| [FlatWire_MasterSpecification.md](../../10-requirements/MasterSpecification.md) | `OI-##` register §11 and the reconciliation authority — W1, W3 |
| [GapsRegister.md](../../90-registers/Gaps.md) | `G42` client-confirmed, `G46` added — W1 |
| [SpoolQueue.md](../../10-requirements/screens/SpoolQueue.md) | The most affected specification — its §7 item 1 is what `D3`–`D6` answer |
| [SpoolCompletionNotification.md](../../10-requirements/screens/SpoolCompletionNotification.md) | Where `D5` and `D6` land — and it has never stated what the label prints |
| [RodPreCheckin.md](../../10-requirements/screens/RodPreCheckin.md) | §1.3 excludes FL2; `D1` reverses it, and §3.1 explains why it cannot simply be pointed at FL2 |
| [RocCheckin.md](../../10-requirements/screens/RocCheckin.md) | §4.3 is the source of *"spool inspection not required"*, which shapes `Q41` |
| [Spool.md](../design-notes/Spool.md) | Its May 2026 *"at the start of the FL1 job"* is superseded by `D5` |
| [Integration.md](../../20-architecture/Integration.md) | §8.0 / §8.1 — where `OI-115` and `Q37`–`Q40` were minted without reaching a register |
