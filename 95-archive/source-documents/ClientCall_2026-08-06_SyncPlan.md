...
**Registers touched:** `Q##` ([FlatWireOpenQuestions.md](../../90-registers/Questions.md), 95 → 99) · `OI-##` ([FlatWire_MasterSpecification.md](../../10-requirements/MasterSpecification.md) §11) · `G##` ([Development/GapsRegister.md](../../90-registers/Gaps.md), G33 → G35) · `PSG-D`/`PSG-Q` ([PassScheduleGenerationSpec.md](../../10-requirements/screens/PassScheduleGenerationSpec.md))
...
| | What it unblocks |
|---|---|
| **`Q73` answered** (rod consumption sequence) | `G22` has stood since 1 Aug as a **knowingly wrong rule** — staging refuses any rod whose order differs from the established one, which `Q70` made wrong on 30 Jul, and the correct replacement was explicitly waiting on this answer. [RodPreCheckin.md](../../10-requirements/screens/RodPreCheckin.md) §5.3 carries the wrong rule with a `[CLIENT INPUT REQUIRED]` note. **Phase 4 staging validation** |
| **`OI-13` flow specified** (wire break) | `FR-280`–`FR-282` are marked **"Not deliverable"** in [05-SprintPlanAndBacklog.md](../../60-delivery/SprintPlan.md), story `FW-N08` is **blocked**, and `TC-350`–`TC-352` are written-and-blocked in the test plan. The flow now exists; only the persistence target is missing (`G34`) |
| **Edging is roll-forming, not knifing** (new) | [PassScheduleGenerationSpec.md](../../10-requirements/screens/PassScheduleGenerationSpec.md) treats the edger purely as an **area correction** (§3.3.4, round vs square) and describes over-width material as *"trimmed or upset"*. Tim's correction adds a **whole formula subset** and makes the final stand a **skim pass** |
...
| **D8** | Output multiples: **FL1 ~1,800 lb**, **FL2 800/900 lb** | `Q18` / [Spool.md](../design-notes/Spool.md) | Unchanged — corroborated | Matches the 30 Jul figures. Supplies the worked example in §3.1 |
...
Tim's notes on [PassScheduleGenerationSpec.md](../../10-requirements/screens/PassScheduleGenerationSpec.md), given verbatim on the call:
...
**Why this is a formula subset and not a wording fix.** The spec presently handles the edge only through §3.3.4's area correction (`A = t·w − 0.2146·t²` for a round edge) and describes over-width material as *"trimmed or upset at the edger"* ([line 470](../../10-requirements/screens/PassScheduleGenerationSpec.md)). A roll-former **never trims** — there is no scrap stream at the edger — and it **feeds width back into length and thickness**, which changes the mass balance the allocation step depends on. Tim: *"there's actually another subset of formulas that needs to be used and they need to be treated like their own operation."*
...
1. **There is no dancer on FM2 anywhere in the repository.** The dancer appears only as an FM1 component ([FlatWire_MasterSpecification.md](../../10-requirements/MasterSpecification.md) §156, [FlatWireProcessWalkthrough.md](../../10-requirements/ProcessWalkthrough.md) step 19). No `Stand`/`Drawer`/`Edger`-style lookup row, no `PassScheduleComponent` entry, no seed data.
...
| **A4** | **Review [PLCTagSpecification.md](../../20-architecture/PLCTagSpecification.md)** — not started; next after A1. This is what closes `PLC-Q02`/`PLC-Q04`/`PLC-Q05`, all Critical | **Tim O.** | After A1 |
...
> ### ⚠ Re-pointed 11 Aug 2026 after the `Analysis/` → `Business/Screens/` consolidation
>
> The consolidation completed the per-screen migration: **five new owning specifications** were created — [`ActiveRunMonitor.md`](../../10-requirements/screens/ActiveRunMonitor.md) (DB3), [`OutputCoilCompletion.md`](../../10-requirements/screens/OutputCoilCompletion.md) (DB7), [`WipRejection.md`](../../10-requirements/screens/WipRejection.md) (DB8), [`ShiftSummary.md`](../../10-requirements/screens/ShiftSummary.md) (DB10) and [`RollAdjust.md`](../../10-requirements/screens/RollAdjust.md) (DB11) — and `FlatWireShopfloorDashboards.md` ceased to be a requirements source. **Three consequences for the waves below:**
>
...
|---|---|---|---|
| **W1 — Registers** ✅ | [FlatWireOpenQuestions.md](../../90-registers/Questions.md) (`Q73` decided; `Q31`–`Q73 item 6` added; index + change log), [FlatWire_MasterSpecification.md](../../10-requirements/MasterSpecification.md) (`OI-13` and its three citations), [Development/GapsRegister.md](../../90-registers/Gaps.md) (`G22` resolved; `G34`, `G35` added; `G29` note) | 3 | L |
| **W2 — Generation spec** ✅ | [PassScheduleGenerationSpec.md](../../10-requirements/screens/PassScheduleGenerationSpec.md) **v1.2 + v1.3**. *v1.2 — the physics:* new §3.3.11, the *"trimmed or upset"* wording, the §2.3 route table, §3.3.9, `PSG-D24`–`D26`, `PSG-Q27`–`Q62`. *v1.3 — the propagation into the calculation:* **§6.3 Steps 9, 10, 11 and 12 rewritten plus new Step 9A (edger passes)**; **§3.3.5 allocation restructured** around the S3 skim; the **§3.3.6 tension omission** found and raised as `PSG-D27`/`PSG-Q29`; validations **V36–V38** added with V10/V20/V27/V30 amended; §6.1 flowchart, M4/M8 master rows, §8.7 blocked-validation table. `../../deliverables/PassScheduleGenerationSpec.docx` re-rendered via [build_docx.py](../../tools/deliverables/build_docx.py).<br><br>**Two gap audits followed the same day, taking the spec to v1.4 and then v1.5** — thirty defects, twelve of them algorithmic. They are recorded here because several were *caused* by W2: propagating the edger physics into a thirteen-step sequence written before edgers existed left five steps consuming quantities no step produced. The largest: **the width chain was never computed**, and with **no edger at the mill exit the final width leaves S3**, so `PSG-D08` governs finished-goods width and not just the FL1 spool's. **Eight further client items** are owed — `PSG-D31`–`D35`, `PSG-Q32`–`PSG-Q34` — of which two are High: **rolling friction stated separately from drawing friction**, and the **grip criterion for the round → flat pass**. Also **`PSG-D32`**, the planned output multiple, which is `D8` on this call: Step 12A had been splitting orders against take-up *ratings*, roughly halving the unit count | 1 (+1 generated) | M |
| **W3 — Requirement docs** | [RodPreCheckin.md](../../10-requirements/screens/RodPreCheckin.md) §5.3 order-membership rule, §6.2 sequence authorisation, §11 open-items table and the sign-off checklist (`Q73` row closes) · [RocCheckin.md](../../10-requirements/screens/RocCheckin.md) — the same validation at check-in · [WeldEvent.md](../../10-requirements/screens/WeldEvent.md) — the break-weld case and same-alpha rule | 3 | **H** |
| **W4 — SRS / requirement text** | [02-SRS.md](../../10-requirements/BusinessRequirements.md) — `FR-280`–`FR-282` gain a persistence target once `G34` closes; new `FR-###` for the consumption sequence (**do not renumber; supersede in place**) · [FlatWire_MasterSpecification.md](../../10-requirements/MasterSpecification.md) §5 event table | 2 | M |
| **W5 — Contracts + schema** | [04-APIContract.md](../../40-backend/APIs.md) and [04-APIContract.md](../../40-backend/APIs.md) — staging/check-in sequence-validation responses · `FlatWire_DDL_04_Runs.sql` (`RodStaging.OrderId` semantics) · `FlatWire_DDL_01_Lookup.sql` (a dancer component, if `Q32` says the mode is scheduled). **Re-run `FlatWire_DDL_RunAll.sql`; table count stays 27 unless `Q32`/`G34` add one** | 4 | **H** |
| **W6 — PLC surface** | [PLCTagSpecification.md](../../20-architecture/PLCTagSpecification.md) — **a read-only dancer element and mode — authored 12 Aug 2026** (`PLC-Q18`, test `C12`). **The write surface is still blocked**: `Q32` is now `Q65`, and the 23 Jul meeting contradicts the 6 Aug position on whether the mode is scheduled at all; **the client doc owns every path string** · [PLCTagImplementation.md](../../20-architecture/PLCCommunication.md) — **carries no path strings by rule** | 2 | M |
| **W7 — Plans + tests** | [05-SprintPlanAndBacklog.md](../../60-delivery/SprintPlan.md) (unblock `FW-N08`), [06-TestPlanAndTestCases.md](../../70-testing/TestStrategy.md) (`TC-350`–`TC-352`), `phase-04`, `phase-06`, `phase-07` | 5 | M |
| **W8 — Mockups** | `pause_run.js` — the machine-stop prompt chain (stop reason → break → WIP reject or weld-and-continue) · `dashboard_2a_rod_precheckin.html` — pick-list ordering and the sequence refusal · `wip_rejection.js` — the no-weld disposition set.<br><br>**Since 11 Aug 2026 each of these has an owning specification that must be updated with it:** `pause_run.js` → [`ActiveRunMonitor.md`](../../10-requirements/screens/ActiveRunMonitor.md) §6 · `wip_rejection.js` → [`WipRejection.md`](../../10-requirements/screens/WipRejection.md) §3–§4 · `dashboard_2a_rod_precheckin.html` → [`RodPreCheckin.md`](../../10-requirements/screens/RodPreCheckin.md) | 3 (+3 specs) | **H** |
...
| [PassScheduleGenerationSpec.md](../../10-requirements/screens/PassScheduleGenerationSpec.md) | Target of D5–D7 — W2 |
| [RodPreCheckin.md](../../10-requirements/screens/RodPreCheckin.md) | Carries the wrong order-membership rule D1 replaces |
| [FlatWire_MasterSpecification.md](../../10-requirements/MasterSpecification.md) | `OI-##` register and the reconciliation authority |
| [Development/GapsRegister.md](../../90-registers/Gaps.md) | `G##` register — `G22` resolved, `G34`/`G35` added |
| [PLCTagSpecification.md](../../20-architecture/PLCTagSpecification.md) | Target of A4 and of the `Q32` dancer element |
