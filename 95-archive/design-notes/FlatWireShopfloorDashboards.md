...
|---|-----------|:---:|-------------|---------|----------------------|
| 1 | Line Status Overview | MVP-1 | Supervisor / Foreman | Always visible | [LineStatusOverview.md](../../10-requirements/screens/LineStatusOverview.md) |
| 2A | Rod Pre-Check-in Station (FL1/FL3) | MVP-1 | FL1 Operator | Staging the next rod while the current coil still runs | [RodPreCheckin.md](../../10-requirements/screens/RodPreCheckin.md) |
| 2 | Rod Check-in & Pre-Run Setup (FL1/FL3) | MVP-1 | FL1 Operator | Start of each rod | [RocCheckin.md](../../10-requirements/screens/RocCheckin.md) |
| 3 | Active Run Monitor | MVP-1 | FL1 / FL2 / FL3 Operator | During every run | [ActiveRunMonitor.md](../../10-requirements/screens/ActiveRunMonitor.md) |
| ~~4~~ | ~~Weld Event Logger~~ — **retired 1 Aug 2026** | *(retired)* | — | Weld capture moved to DB2A *Mark as welded* | [WeldEvent.md](../../10-requirements/screens/WeldEvent.md) |
| 5 | FL2 Spool Check-in | MVP-1 | FL2 Operator | Loading each spool onto TPO | [RocCheckin.md](../../10-requirements/screens/RocCheckin.md) §4 |
| 5A | FL2 Spool Queue | MVP-1 | FL2 Operator | Selecting the next spool, ahead of check-in | [SpoolQueue.md](../../10-requirements/screens/SpoolQueue.md) |
| 6 | SPC Checkpoint Entry | MVP-1 | Any operator | Pre-run, post die-change | [SPCCheckpoint.md](../../10-requirements/screens/SPCCheckpoint.md) |
| 7 | Output Coil Completion & Label | MVP-1 | FL2 Operator | When TKUP-2 coil is complete | [OutputCoilCompletion.md](../../10-requirements/screens/OutputCoilCompletion.md) |
| 7B | **Packing Station** | MVP-1 | **Packing operator** | Coil arrives from the line; skid seated and staged | [OutputCoilCompletion.md](../../10-requirements/screens/OutputCoilCompletion.md) **§8** |
| 8 | WIP Rejection | MVP-1 | Any operator | Material fails at any stage | [WipRejection.md](../../10-requirements/screens/WipRejection.md) |
| 9 | Pass Schedule Management | **MVP-2** | Operations / Maintenance | Before a new product campaign | [PassScheduleManagement.md](../../10-requirements/screens/PassScheduleManagement.md) |
| 9A | Pass Schedule List ("All Schedules") | **MVP-2** | Operations / Maintenance | Via Dashboard 9 "← All schedules" button; direct navigation | [PassScheduleManagement.md](../../10-requirements/screens/PassScheduleManagement.md) |
| 10 | Supervisor Shift Summary | **MVP-2** | Supervisor / Shift Manager | End of shift / on-demand | [ShiftSummary.md](../../10-requirements/screens/ShiftSummary.md) |
| 11 | Roll Adjust | MVP-1 | **FL2 / FL3 operator** (not FL1) | Roll gap adjustment during run | [RollAdjust.md](../../10-requirements/screens/RollAdjust.md) |
| 12 | Rod Checkout | MVP-1 | FL1 / FL3 Operator | Rod removed from payoff before run completes naturally | [RodCheckout.md](../../10-requirements/screens/RodCheckout.md) |
| DC | Die Change | MVP-1 | FL1 / FL3 Operator | Drawing die replaced mid-run (planned, gauge drift, failure, or size change) | [DieChangeAndManagement.md](../../10-requirements/screens/DieChangeAndManagement.md) |
| — | *(no dashboard)* | — | — | Machine / OPC tag surface | [PLCTagSpecification.md](../../20-architecture/PLCTagSpecification.md) |
| — | *(no dashboard)* | — | — | Pass schedule generation engine | [PassScheduleGenerationSpec.md](../../10-requirements/screens/PassScheduleGenerationSpec.md) |

> **DB13 and DB14 are descoped** (4 Aug 2026, `PLC-Q02` superseded) and were never in this inventory. Their tag map
> lives in [PLCTagSpecification.md](../../20-architecture/PLCTagSpecification.md).
>
...
> [RollAdjust.md](../../10-requirements/screens/RollAdjust.md) §1.5.

---

## Alloy Reference Data

> **Moved 13 Aug 2026, and again on 23 Aug 2026 — it now lives with the column definitions it seeds, as the **Seeded values** block of [`../../30-database/schema/FlatWireSchema_Lookup.md`](../../30-database/schema/FlatWireSchema_Lookup.md#alloyproperty).** The intermediate `ReferenceData.md` was retired: holding the seed values apart from the `AlloyProperty` column list is what let its column names drift out of date. It is MVP-1 seed data feeding `AlloyProperty` and the
> Phase-13 admin grid, and had been sitting inside this file's MVP-2 Dashboard 9 section. Cited from
> `../../30-database/schema/FlatWireSchema_Lookup.md` and `phase-13`.

## Dashboard 1 — Line Status Overview
> **Scope: MVP-1.**

> **Owning specification: [LineStatusOverview.md](../../10-requirements/screens/LineStatusOverview.md)**. DB1 content was consolidated out of this file on 1 Aug 2026 — that document was written from this section. Reproduced here as the design record only; the owning document is authoritative.
...
**Mockup:** `../../50-frontend/mockups/dashboard_2a_rod_precheckin.html`
**Full analysis:** [RodPreCheckin.md](../../10-requirements/screens/RodPreCheckin.md)
...
See [RodPreCheckin.md](../../10-requirements/screens/RodPreCheckin.md) — notably whether pre-check-in sets coil status to `INFLAT` (SRS) or `STAGED` (walkthrough), and whether `CHK005` removes the payoff selector from Dashboard 2.
...
> **Owning specification: [RocCheckin.md](../../10-requirements/screens/RocCheckin.md)**. The owning document covers both this screen and Dashboard 5 (spool check-in), and reduces the PLC push detail to a pointer into the tag specification. Reproduced here as the design record only; the owning document is authoritative.
...
> **Owning specification: [WeldEvent.md](../../10-requirements/screens/WeldEvent.md)**. **This screen is retired (1 Aug 2026).** Weld capture is the *Mark as welded* action on Dashboard 2A, because the rod being welded in is the staged one. Retained here as the design record of a withdrawn screen; gaps G27 and G28 record the two capabilities that were not rehoused. Reproduced here as the design record only; the owning document is authoritative.
...
> **Owning specification: [SpoolQueue.md](../../10-requirements/screens/SpoolQueue.md)**. Added 2 Aug 2026 as the eleventh per-screen document; this section and that document were authored together. Reproduced here as the design record only; the owning document is authoritative.
...
> **Owning specification: [RocCheckin.md](../../10-requirements/screens/RocCheckin.md)**. Covered as section 4 of the check-in specification, alongside rod check-in, rather than as a separate document. Reproduced here as the design record only; the owning document is authoritative.
...
> **Owning specification: [SPCCheckpoint.md](../../10-requirements/screens/SPCCheckpoint.md)**. **This is no longer a screen** — it is a dialog over the run being measured. The owning document also flags that the "Post DB1" option offered on screen is not one of the five recorded checkpoint types (OI-10). Reproduced here as the design record only; the owning document is authoritative.
...
> **Owning specification: [PassScheduleManagement.md](../../10-requirements/screens/PassScheduleManagement.md)**. The *Generate from Specs* algorithm reproduced in this section is **superseded**: the engineering basis is [PassScheduleGenerationSpec.md](../../10-requirements/screens/PassScheduleGenerationSpec.md) v1.5, which now wins on physics and arithmetic both. Do not implement the generator from this section. Reproduced here as the design record only; the owning document is authoritative.
...
> **Owning specification: [PassScheduleManagement.md](../../10-requirements/screens/PassScheduleManagement.md)**. Covered by the same owning document as Dashboard 9. Reproduced here as the design record only; the owning document is authoritative.
...
**Mockup:** [dashboard_9a_schedule_list.html](../../50-frontend/mockups/dashboard_9a_schedule_list.html)
...
> **Owning specification: [RollAdjust.md](../../10-requirements/screens/RollAdjust.md)** (consolidated out of this file 11 Aug 2026). **This is no longer a screen** — it is a dialog over the paused run.
>
...
> **Owning specification: [RodCheckout.md](../../10-requirements/screens/RodCheckout.md)**. **This is no longer a screen** — it is a dialog. The owning document defines three modes (A pre-run, B mid-run, P pre-check-out) and records that rod checkout is a *resume outcome* rather than a pause reason (OI-14). Reproduced here as the design record only; the owning document is authoritative.
...
**Full analysis:** [RodCheckout.md](../../10-requirements/screens/RodCheckout.md)
...
See [RodCheckout.md](../../10-requirements/screens/RodCheckout.md) — Open Questions OQ-A through OQ-D for items that require customer confirmation before implementation.
...
> diagram covering every table. **Not divided — a half-ER is worse than none**"* (the quoted line said *"all 34 tables"*, a figure superseded since; the count is `[DBD §6.2]`'s and the principle is what is cited here)
> ([`../../30-database/sql/MVP2-SCOPE.md`](../../30-database/sql/MVP2-SCOPE.md)).
>
...
| [FlatWireOpenQuestions.md](../../90-registers/Questions.md) | Open questions register — **99 items** (authoritative for decisions) |
...
| [FlatWireEndToEndProcess.md](../../10-requirements/EndToEndProcess.md) | End-to-end process reference — 11 stages. **Partially superseded**: FM2 description predates the May 21 and Aug 4 corrections |
| [FlatWireProcessWalkthrough.md](../../10-requirements/ProcessWalkthrough.md) | Sequential operator walkthrough, and the `Known Source Conflicts` arbitration table |
| [`../../10-requirements/MasterSpecification.md`](../../10-requirements/MasterSpecification.md) | Reconciliation authority when two documents disagree (§10), and the `OI-##` register |
