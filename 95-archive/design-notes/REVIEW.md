...
> | `FlatWireTables.md` | `../../30-database/schema/FlatWireSchema_Mapping.md`, appendix |
...
The DevelopmentPlan is thorough and largely self-aware — its own [`ShopfloorPlan/back-matter.md`](../../90-registers/Gaps.md) **Gaps register** (`G1`–`G36` at the time of review; `G1`–`G52` today) already tracks many of the issues below as "Open." The dominant problem is **doc split-brain**:
...
**Timeline note (authoritative):** **Phase 1 must be complete by 14 Aug 2026** (current directive). ~~This is *earlier* than the roadmap's own W1 (Aug 17–23 in [`back-matter.md`](../../90-registers/Gaps.md)), so the roadmap calendar, milestone M1 (Aug 23) and QA1 need re-baselining against the Aug-14 gate.~~ **Re-baselined Jul 30 2026** — the grid now opens with **W0 = to Aug 14**, M1 moved to Aug 14, QA0 added (#31). Phase-1 work has to be underway well before the gate — the [Capacity & Effort Model](../../60-delivery/CapacityAndEffortModel.md) finds Phase 1 needs **1,027 h — 10.7 FTE across the 12 working days to Aug 14**, so the gate is at risk on headcount, not on sequencing. **Phase 1 is unaffected by the MVP split**: none of its three layer specs carries deferred work, so the 1,027 h figure means MVP-1 as it stands. *(Phase 1 has been split into three execution-ready layer specs — [1A Angular](../../60-delivery/phases/phase-01a-angular-foundation.md) · [1B Backend](../../60-delivery/phases/phase-01b-backend-foundation.md) · [1C Database](../../60-delivery/phases/phase-01c-database-foundation.md) — that carry the Aug-14 gate and bake in the Phase-1-relevant fixes below.)*
...
2. **Reconcile the three timeline/sprint models and re-baseline to the Aug-14 Phase-1 gate** (Tier 4 / G1) — the plan has no single calendar. *(Calendar re-baselined and the capacity model published Jul 30 2026 — see [`CapacityAndEffortModel.md`](../../60-delivery/CapacityAndEffortModel.md) and #31/#53. The sprint→phase crosswalk (#33) is still open, but `FlatWireJiraStories.md` is now explicitly superseded **as a schedule**; its points survive only as a cross-check. **New top-priority consequence:** the model finds **MVP-1 is 3,292 hours against 44 working days** (352 h/person) → **9.4 FTE sustained and 24.5 in W7** — the 30 Sep date needs a programme decision, not a doc fix. *(Apportioned 11 Aug 2026, model §3b; the same pass corrected a stale TOTAL row that had read 3,727 where the phase rows sum to 3,660.)*)*
3. **Persist AGC readings** (Tier 2 / **G3**) — add a `RunReading` table; FL2 profile + Gauge-Trace/Cut-Traceability reports currently have no data source. *(Done in [1C](../../60-delivery/phases/phase-01c-database-foundation.md).)*
4. **Add FK/`RunId` indexes and correct the doc↔DDL precision drift** (Tier 3) — hub-join performance + a regeneration hazard. *(Indexes/precision addressed in [1C](../../60-delivery/phases/phase-01c-database-foundation.md).)*
...
> **The 25 Aug 2026 `LatestDocument/` → `MVP-1/ProjectPlan/` sync did NOT close any Tier 1 or Tier 2
> finding, and that is the expected result.** Those tiers are correctness and data-model defects; the
> sync corrected *drift* — stale counts, an unfinished rename, and self-descriptions. The two places
> it touched this document are **item 50** (re-closed: its closure named a figure that has since moved
> twice) and **item 24** (cross-referenced against the new `OI-127`, which must not be merged into it).
>
> **Three Tier 1 items remain open and none is a documentation problem:** **#1** is out of MVP-1 scope,
> **#5** is `RodCheckin` requiring `NOT NULL` fields the check-in API never sends, and **#6** is the
> 3-vs-4 inspection-item conflict (`G14`), which still blocks the Phase-4 build.
>
> ⚠ **This document deliberately keeps the pre-13-Aug document names** — its findings are statements
> about those files as they stood, and the header carries the old→new map. The sync's old-name sweep
> **explicitly exempted this file**; do not "fix" the names here.

> **Three PLC-surface arbitrations, added 4 Aug 2026 when the tag surface was consolidated into [`../../20-architecture/PLCTagSpecification.md`](../../20-architecture/PLCTagSpecification.md) (`[PLC]`).** Each names a winner, so no one has to re-derive it:
>
...
> it. The arbitration is published as **`../../10-requirements/MasterSpecification.md` §10.5**; in one line each —
>
...
18. **Systematic bare `decimal` in schema docs vs scaled `DECIMAL` in DDL** — the `FlatWireSchema_*.md` docs declare many numeric columns as bare `decimal` (= `decimal(18,0)`, zero fraction) where the DDL correctly uses `DECIMAL(8,2)` (weights), `DECIMAL(8,4)` (gauges), `DECIMAL(10,2)/(10,4)` (footage/measures) — across `Spool`, `Rod`, `SpoolProcessing`, `RodCheckin`, `SpoolCheckin`, `FlatWireRunDetail`, `WipRejection`, `CoilOutput`, `RodCheckout`. Regenerating DDL from the docs would round money/measurements to whole numbers. **Largest sync hazard. Fix:** the DDL is authoritative — correct the docs. **[P1✓ 1C states DDL authoritative]**
...
24. **Weld↔coil footage in two coordinate systems** — run events use cumulative *run* footage; `CoilTraceability.FootageFrom/To` are *coil-local*. Mapping source-rod→coil-footage needs an unstated coil-start offset. **Fix:** document/normalise the frame. Registered as **`OI-25`**. ⚠ **Do not merge this with `OI-127`** (minted 25 Aug 2026): that one is about the *moment of capture* — the encoder is read when the operator joins at the payoff, but the join passes the counter later — whereas this is about two *coordinate frames*. Settling either leaves the other open, and both must be closed to attribute footage correctly.
...
27. **ERD overstates audit columns** — the finding stands as a statement about `FlatWire_ERDiagram_Documentation.md:227-230`, which claimed the audit quad was "Present in Most Entities" when only `PassSchedule` carried it. ⚠ **That file was absorbed into `[DBD]` §6–§7 on 13 Aug 2026 and deleted, so the line citation no longer resolves** — this row is retained per this document's convention of describing artifacts *as they stood*. **Before re-raising, verify against `[DBD]` §7.3–§7.7**, which were written from the DDL rather than relocated from the ERD; the overstatement did not necessarily survive the absorption.
...
50. ⚠ **RE-CLOSED 25 Aug 2026, against a baseline that has moved twice since.** The 13 Aug closure pointed at **25 MVP-1 / 28 in the full design**; `D-31` then took it to **34** and the 23 Aug `SpoolConfiguration` merge (`Q60`) to **33 tables · 55 FKs · 69 index statements · 1 procedure · 1 trigger**, which is the live figure. ⚠ **A closure that names a figure goes stale when the figure moves** — this one read as closed while pointing at a number wrong by eight. The counts are now asserted at **exactly one site, `[DBD §6.2]`**, and checked by [`verify_schema_counts.py`](../../tools/deliverables/verify_schema_counts.py); the 25 Aug sync corrected ~35 live restatements across 24 files. **Do not re-state the figure here.** ~~**Table-count drift**~~ — 20 (`FlatWireJiraStories.md:1466`) vs 21 (`back-matter.md`, post-`Rod`-drop) vs 22 (ERD); the roadmap change log is itself self-inconsistent (58/58 vs 44/44; 22 vs 21). *(With the new `RunReading` table the current count is **22**.)*
...
53. ~~**No document-control block on any of the 14 phase files** — no owner/version/change-log/effort.~~ **✅ ADDRESSED (Jul 30 2026)**, but **this entry was wrong on two counts until 14 Aug 2026.** (a) The count was **17**, naming a combined `phase-01` that no longer exists and `phase-02`, which is wholly MVP-2 — **MVP-1 holds 15 phase files**. (b) `phase-01a`, `phase-01b` and `phase-01c` carried **no `Scope call` line at all**; all three were given one on 14 Aug 2026, along with the `OQ blockers:` / `Stories:` trailers that every phase 03–14 already had and the foundation trio never did. With that corrected, the claim now holds: every phase spec carries `Project` / `Last Updated` / `Status` / `Layer` / **`Owner`** / **`Effort`** / **`Scope call`**. Owner names a delivery **stream**; the named-owner roster is in [`CapacityAndEffortModel.md`](../../60-delivery/CapacityAndEffortModel.md) §1 and is **still unfilled**. Per-phase change logs were **not** added (the roadmap index carries the shared change log).
...
[04-APIContract.md](../../40-backend/APIs.md) attributes to this document. A read-only **Welds this run** dialog was built on
...
>
> This document states that its `Q-W#` questions were *"to be added to `../../90-registers/Questions.md`"*. **Only
...
>
> **It needs registering in `../../90-registers/Questions.md`.** Flagged rather than done, because that register is
...
| Gap | Outcome |
|---|---|
| ~~**G25**~~ | **WITHDRAWN 1 Aug 2026 — built rather than deferred** (the *Welds this run* dialog). It was withdrawn before it was ever registered, so nothing cited it, and **the ID was reused on 13 Aug 2026** for the requirement-coverage gap now in [`back-matter.md`](../../90-registers/Gaps.md) |
