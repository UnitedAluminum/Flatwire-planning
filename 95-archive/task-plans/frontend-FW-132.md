...
status_note: "⬜ **Not started — and it is now reconciliation, not greenfield authoring.** ⚠ A type surface already exists in the library and this story must converge with it rather than author beside it: `interfaces/` holds **12 exported view-model interfaces** (`RunHeader`, `MachineLine`, `Payoff`, `ComponentSetting`, `InfoTable`, `SpoolReading`, `SpoolAccent`, `TraceGraphModalData`, `ShiftStat`, `LineMetric`, `FlatWireLine`, `LineAlert`), `enums/` holds `LineState` and `SpoolMilestone`, and `constants/api-methods.constants.ts` already declares the first method name. ⚠ **Those 12 are the contract any API mapper must satisfy** — DB3 binds them directly. ✅ **Both config keys are already wired**: `useMockData` is `true` in `local-config.json` and `false` in `environment.js`. ✅ **23 of the 31 endpoints answer today** behind the service's own stub/real DI swap, so this story can be built against live HTTP with no database. ⚠ Two model decisions remain unresolved (`G14`)"
owner:
jira:
mvp: 1
phase: "1A"
...
> - [ ] ⭐ **A view-model mapper per screen** — a pure function in a `.model.ts` sibling with its own spec, DTO → the **12 existing interfaces** in `src/lib/interfaces/` *(folded in 7 Sep 2026)*. ⛔ **Map onto those interfaces; do not redefine them** — the templates already bind them. ⚠ **This is what replaces DB3's seven private `build*()` methods**, and it is what turns the reference screen from a layout into a pattern

⚠ **Two of those criteria are now reconciliation rather than authoring** *(annotated 7 Sep 2026)*:

- **"Models authored"** — the library already carries **12 view-model interfaces** in `interfaces/`,
  which DB3 binds directly and which are therefore the **contract a mapper must return**. Author the
  wire DTOs *beside* them and map, or the two sets diverge; do not redefine what the templates bind.
- **"Canonical enums defined once"** — `enums/` already holds `LineState` and `SpoolMilestone`,
  keyed into presentation maps (`LINE_STATE_ICONS`, `SPOOL_MILESTONE_TONES`). `State` and `EdgeType`
  are genuinely new; **the "once" in this criterion now has to account for the two that exist.**
- ⚠ **`line-context.service` / `run-state.service` are named here and in `[CMP §5.4]` but exist
  nowhere** — `projects/flat-wire` has no `services/` folder at all. They are the seam DB3's seven
  hardcoded `build*()` builders currently stand in for.
>
...
| **`MachineName` is never narrowed** | line eligibility is a per-endpoint *shape* rule, so no screen gets an FL2 refusal free from enum membership — `P-58` / `P-83` |
...
**and** its last segment is a positive number; `[CMP §5.2]`'s `:machineName` (`FL1`/`FL2`/`FL3`) satisfies
...
this service is to resolve `:machineName` → machine id and set `CURRENT_MACHINE`, decide before you write
