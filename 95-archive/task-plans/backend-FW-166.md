...
status_note: "⚠ **A de-stub.** The controller and service interface exist; ✅ `Q6` answered 8 Sep 2026 (footage-split at the weld point, confirmed); ⛔ `G26` and `OI-59`'s rework-weld half are still open [9 Sep 2026, client answers] Q72 - the weld flag gains a PRECONDITION: it cannot be set unless the rod is checked in or pre-checked in, and there is no clear-in-place path - a mis-scan is reversed out of staging and rejected under supervisor override. Built as FW-N27."
owner:
jira:
mvp: 1
phase: "6"
...
**Status:** ⚠ **A de-stub.** The controller and service interface exist; ✅ `Q6` answered 8 Sep 2026 (footage-split at the weld point, confirmed); ⛔ `G26` and `OI-59`'s rework-weld half are still open
...
| **`WeldRecorded` domain event** | `FlatWire.Domain/Events/RunEvents.cs:103` | ✅ Built — ⚠ **`FW-208` had to ADD `MachineName` to it** (`P-139`), because without it the weld was **unbroadcastable** |
...
6. **Raise `WeldRecorded` on Pass only**, so `FW-160`'s broadcast follows (§2.1).
   ⚠ **`WeldRecorded` carries `MachineName`** because `IFlatWireBroadcaster.Line(MachineName)` is the only
...
| `MachineName` populated | `WeldRecorded.MachineName` is set — without it the weld is unbroadcastable (`P-139`) |
...
| **`P-139`** | `WeldRecorded` gained `MachineName` because the weld was otherwise unbroadcastable |
