...
status_note: "**Ready to build** — and it is **eight**, not nine: **B7 is largely already built** (§2.2) [9 Sep 2026, client answers] Q30 - no load cells exist, so a stored scale weight has no source on the machine and the weight basis vocabulary needs re-deriving around a derived-only figure plus the new coil outside diameter read."
owner:
jira:
mvp: 1
phase: "1C"
