...
id: FW-142
legacy_id:
title: Dapper/EF data access and FlatWireDbContext
status: done
status_confirmed: false
status_note: "✅ **BUILT 27 Aug 2026** (§6) — two of the four acceptance criteria were reversed by `D-31` (§2.1); `FW-141` had already landed part of §4 (§7)"
owner:
jira:
mvp: 1
phase: "1B"
...
hours: 24
sprint: S0
depends_on: [FW-N04, FW-006, FW-007]
blocked_by: []
has_plan: true
started:
completed:
...
| Concern | Story |
|---|---|
| The seven repositories that sit on this | [`FW-141`](FW-141.md) |
| The aggregates being mapped | [`FW-207`](FW-207.md) |
| Raising and dispatching the domain events — this story provides the hook | `FW-208` |
| The connection string binding | [`FW-144`](FW-144.md) |
...
`Stand`, `Drawer`, **`ToolingInventoryEdger`** *(`Edger`, absorbed 6 Sep 2026 — `D-53`)*, `Dancer`, `Spool`, `AlloyProperty`, `PayoffPosition`, the **seven**
...
⚠ **Superseded 5 Sep 2026 — `D-50`.** The .NET unit level is reinstated, but **nothing above
changes**: AC 3's smoke pass was always manual and QA0 is not retro-amended. **What changes is
downstream** — `FW-267` tests `BusinessKeyGenerator` and the services' guard clauses against an
in-memory provider. ⛔ **That is not an integration suite against a seeded `FlatWireDB`**, which
stays withdrawn, and ⛔ **`FlatWireDbContext` is deliberately NOT put behind an interface** — seven
repositories bind it as a generic type argument and `MediatorExtension` needs its `ChangeTracker`.
...
> `RowVersion`; the select rehydrated `RunAlpha`/`RodAlpha` and the `MachineName` enum; rollback left
...
| **`FW-219` vs `FW-220`** | ⚠ **Two sibling plans disagree on the cross-database transaction, and this story supplies the mechanism both would use.** [`FW-219 §70`](FW-219.md) says the shared writes *"are not in the same transaction… They cannot be: different database"*; `[INT §8.0]`, `[ARC §10]` and `FW-220` all say the opposite — co-located on one instance, one `SqlConnection`, one `SqlTransaction`, no MSDTC. **Not this story's to settle.** Build the unit of work per §1.1 and let each caller choose; do not bake either answer into the context |
