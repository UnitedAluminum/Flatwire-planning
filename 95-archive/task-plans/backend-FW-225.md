...
id: FW-225
legacy_id:
title: Rod ↔ order allocation — schema and domain model
status: blocked
status_confirmed: false
status_note: "⛔ **Blocked on `Q48`** — and the **DB half is already built**, so this is the domain mapping"
owner:
jira:
mvp: 1
phase: "4"
...
> **`UX_RodOrderConsumption_Station` is keyed on `Station`, not `MachineName`**, because FL1 and FL3
...
| Concern | Story |
|---|---|
| The entity types and EF configurations | [`FW-240`](FW-240.md) — ⛔ **blocked on `[SVC §3.2a]`, and a hard predecessor** |
| The four-tier sequence validation | [`FW-226`](FW-226.md) |
| The order-boundary handoff and its state machine | [`FW-227`](FW-227.md) |
| Footage-to-weight | [`FW-228`](FW-228.md) |
| `POST /order/{orderNo}/complete`'s host | [`FW-232`](FW-232.md) |
| Fulfilment rollup and order status | `FW-229`, Phase 9 |
| `G52`'s `Sole` tier | [`FW-246`](../../30-database/tasks/FW-246.md) — ⚠ it may cost no DDL |
...
| Thing | Where | State |
|---|---|---|
| **`RodOrderAllocation`** | [`FlatWire_DDL_03_Materials.sql`](../../30-database/sql/FlatWire_DDL_03_Materials.sql) **`:412`** | ✅ **Built, seeded, in the MVP-1 runner** |
| **`RodOrderConsumption`** | [`FlatWire_DDL_04_Runs.sql`](../../30-database/sql/FlatWire_DDL_04_Runs.sql) **`:733`** | ✅ **Built** |
| **`UX_RodOrderConsumption_Station`** | [`FlatWire_DDL_07_Indexes.sql`](../../30-database/sql/FlatWire_DDL_07_Indexes.sql) **`:337`** — `UNIQUE … ON ([Station]) WHERE [State] IN ('InProgress','ThresholdReached')` | ✅ **Built, keyed on `Station`** exactly as specified |
...
| The design document | `../../95-archive/design-notes/RodOrderAllocation.md` | ⚠ **Rationale — applied, and NOT citable as a requirement** |
...
### 2.4 `Station`, not `MachineName` — this is `G21` on another table

`UX_RodOrderConsumption_Station` is keyed on **`Station`** because **FL1 and FL3 share one
physical VPS**. Keying it on `MachineName` would let FL1 and FL3 each hold an `InProgress`
...
[`FW-226`](FW-226.md) validates sequences over these allocations —
⚠ **and `G52`'s `Sole` tier is unresolved**, which is
[`FW-246`](../../30-database/tasks/FW-246.md)'s.
[`FW-227`](FW-227.md) runs the boundary handoff and is **gated by the same
`Q48`**. [`FW-228`](FW-228.md) supplies the weight each consumption
row persists. `FW-229` (Phase 9) rolls fulfilment up. [`FW-240`](FW-240.md)
