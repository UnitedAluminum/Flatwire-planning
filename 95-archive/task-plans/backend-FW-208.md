...
id: FW-208
legacy_id:
title: Domain events and post-commit dispatch
status: done
status_confirmed: false
status_note: "✅ **BUILT — steps 1–10; 27 Aug 2026 (the mechanism) and 29 Aug 2026 (step 8, the broadcast handlers).** The only thing outstanding is `CoilCompleted`, which has no hub member to send on — a contract gap (`P-137`, `OI-140`), not unfinished wiring. **Previously — BUILT 27 Aug 2026, steps 1–7, 9 and 10.** The dispatch mechanism is in `ual-api`: the two lane wrappers, capture-then-replay hooked into `"
owner:
jira:
mvp: 1
phase: "1B"
...
hours: 8
sprint: S0
depends_on: [FW-207, FW-142, FW-080]
blocked_by: []
has_plan: true
started:
completed:
...
| Concern | Story |
|---|---|
| The aggregates that raise the events | [`FW-207`](FW-207.md) |
| `FlatWireDbContext` and its `SaveEntitiesAsync` hook | [`FW-142`](FW-142.md) |
| `IFlatWireClient` and its payloads | [`FW-149`](FW-149.md) |
| The hub the handlers send through | [`FW-080`](FW-080.md) |
...
`PayoffStateChanged`, which [`FW-150`](FW-150.md) sends **immediately, never in the
~100 ms batch**.
...
review note, costs nothing, and matches how [`FW-205`](FW-205.md) `P-25`
...
`IFlatWireClient Line(MachineName)`, implemented in `FlatWire.API` over the real hub context. ⚠ **It has
...
|---|---|---|
| `WeldRecorded` | **`MachineName Line`** | ⛔ **A hard blocker, not a nicety.** `IFlatWireBroadcaster.Line(MachineName)` is the only way to address a group, so without it the weld could not be broadcast **at all**. `WeldEvent` has carried `MachineName` since `FW-207`; only the event was missing it |
...
> `WeldRecorded` without a `MachineName` — was unbroadcastable rather than merely thin: there was no group
> to address. `P-139`.
