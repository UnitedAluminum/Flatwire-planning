...
status_note: "⛔ **Blocked on `Q49` and `G52`** — and `G52` means one `PinRole` value matches **no tier** [9 Sep 2026, client answers] Q73 ANSWERS THIS STORY'S BLOCKER IN SUBSTANCE - the client: 'continue to utilize the three-tier rule. This keeps things consistent and clean. The rules apply all the time, regardless of welding.' So the no-weld branch takes the stricter reading and the rule is unconditional, enforced at both entry points. Q49 itself is still Open in the register and should be closed against Q73 before this story is unblocked - the id, not the answer, is what is missing."
owner:
jira:
mvp: 1
phase: "4"
