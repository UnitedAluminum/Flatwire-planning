...
status_note: "⚠ **A de-stub — all four routes exist.** Two acceptance criteria are **stale on the card** [9 Sep 2026, client answers] Q68 - un-staging becomes a PURE LOCAL DELETE. With the three shared writes moved to check-in, pre-check-out has nothing outside the flat wire database to undo. Q72 separately fixes the weld flag's precondition: it cannot be set before check-in or pre-check-in, and there is no clear-in-place path. See FW-N24 and FW-N27."
owner:
jira:
mvp: 1
phase: "4"
...
| Concern | Story |
|---|---|
| `POST /weldevent` — where DB2A's weld actually posts | [`FW-172`](FW-172.md) / `FW-166`, **Phase 6** |
...
| WIP rejection releasing a blocked bay | [`FW-174`](FW-174.md), **Phase 7** — ⚠ `OQ-23`'s cross-phase link |
...
on the line. **A handler that scopes the conflict check by `MachineName` re-opens the gap with the
...
[`FW-157`](FW-157.md)'s problem, not this story's** — but the
...
3. **Let the index enforce the conflict** (§2.1) — no pre-read. Confirm `FW-146` maps
   ...
   ⛔ **Do not scope the conflict by `MachineName`** — the station is shared (`G21`).
...
7. The two queries — `GET /payoff/status`, `GET /staging/queue`.
   ⚠ **FL2 has no payoff bays**; agree with [`FW-154`](FW-154.md)'s
...
station. A handler that filters by `MachineName` before writing would find the bay free for FL3 while
...
[`FW-157`](FW-157.md) consumes a staged row — ⚠ **and carries
`G14`'s check-in half, which still blocks Phase 4**.
[`FW-174`](FW-174.md) releases a blocked bay in Phase 7
