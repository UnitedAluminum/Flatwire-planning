...
| `WeldRecorded`, `RunPaused`, `RunResumed` domain events | ✅ Built — `WeldRecorded` gained `MachineName` because the weld was otherwise unbroadcastable (`P-139`) |
| `IFlatWireBroadcaster.Line(MachineName)` | ✅ Built — **the only addressing this story has**, and the reason handlers live in Infrastructure (`P-101`) |
...
⚠ **`WeldRecorded` carries `MachineName` and the others must too.** `IFlatWireBroadcaster.Line(MachineName)`
...
⛔ **`G67` makes the weld re-broadcast wrong today, and the weld is the worst case.** `FW-150`'s
...
3. **Declare `SpcCheckpointRecorded` and `DieChangeRecorded`** (§2.0), each carrying `MachineName`, and
...
5. The two re-broadcasts (§2.1) — ⚠ the weld one is blocked behind `G67`.
...
| Re-broadcasts | `PayoffWeight` after a weld; `ComponentStatus` after a roll override — ⚠ the weld one carries a **stale denominator until `G67` closes** |
| `LineStatus` | RUNNING ↔ PAUSED reaches Dashboard 1 **without `LineStateMap` being configured** *(`P-256`)* |
| **Inert handlers declared** | `SpcCheckpointRecorded` / `DieChangeRecorded` exist, carry `MachineName`, and are recorded as **wired and inert** until their writers land |
...
| ⛔ **`G67`** *(new here)* | `stagedWeights` has no eviction path, so the post-weld `PayoffWeight` re-broadcast reports the **previous** rod's percentage. Owned by [`FW-239`](FW-239.md) |
