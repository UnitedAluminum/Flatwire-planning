...
id: FW-203
legacy_id:
title: OPC feed simulator — a stand-in for the real ingest
status: done
status_confirmed: false
status_note: "✅ **Built — `OpcFeedSimulator` is live, 0 errors and no analyzer warning from the changed files, and the whole RT spine now runs end to end: simulator → channel → `FW-150` drain → broadcast + `RunReading` → `FW-205`'s watchdog.** ⚠ **Steering, the stop edge and the drop-readings fault are built but NOT exercisable until [`FW-218`](FW-218.md) exposes them** — that is `G43`, and `P-36` chose it deli"
owner:
jira:
mvp: 1
phase: "1B"
...
**Status:** ✅ **Built — `OpcFeedSimulator` is live, 0 errors and no analyzer warning from the changed files, and the whole RT spine now runs end to end: simulator → channel → `FW-150` drain → broadcast + `RunReading` → `FW-205`'s watchdog.** ⚠ **Steering, the stop edge and the drop-readings fault are built but NOT exercisable until [`FW-218`](FW-218.md) exposes them** — that is `G43`, and `P-36` chose it deliberately
...
**That is `G43`, and [`FW-218`](FW-218.md) is its resolution.** Build the
...
| Concern | Story |
|---|---|
| The control endpoints that steer this | [`FW-218`](FW-218.md) |
| The console `DB-S1` | `FW-214`, FE — **a standalone WinForms tool since `D-33`**, not an Angular screen |
| The channel and the contract | [`FW-N05`](FW-N05.md) |
| Draining and broadcasting | [`FW-150`](FW-150.md) |
...
⚠ **`null` is not silence — and it no longer describes FL2's normal state.** ⛔ This read *"`FR-120`
requires FL2's live gauge and width to be **`null`**… the Live view must render 'No live gauge on FL2
· see Profile'"*. **`FR-120` is superseded and `A3` retired** (9 Sep 2026, `G120`): FL2 broadcasts
live gauge and width at 4 s, and that empty state is retired with it.
   ✅ **The rule that matters survives and is now broader:** a reading with **no measurement**, on
**any** line, **must not draw a flat line at target**, which reads as a real in-spec measurement of
nothing. `[TRP §7]`'s *"single most likely thing to ship wrong"* now points at **cadence** rather
than absence — see `FW-N29`.

⚠ ~~**The suppression is conditioned on FL2 *in standalone mode*, not on FL2 as a line.**~~ ⭐ **This
distinction turned out to be the disproof of `A3`, not a nuance of it:** the same FM2 and the same
gauging stands serve FL2 standalone and FL2-within-FL3, so an instrument cannot appear with the
upstream feed. There is no mode condition left — every line, in every mode, measures live.
...
1. A publisher writing `Reading` values into
   [`FW-N05`](FW-N05.md)'s **bounded channel** — same type,
...
[`FW-218`](FW-218.md) lands.**

---

## 6. Handoff

`FW-218` puts four endpoints over this. `FW-214` (FE) puts a console over those — **a WinForms
desktop tool since `D-33`, not an Angular screen**. `FW-N05`
