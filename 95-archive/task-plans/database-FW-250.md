...
id: FW-250
legacy_id:
title: build_development_plan_xlsx.py silently drops every multi-stream story
status: not-started
status_confirmed: false
status_note: "**Ready to build** — the defect is confirmed in the source, at two line numbers"
owner:
jira:
mvp: 1
phase: "14"
...
| The schema count guard's blind spot | [`FW-248`](FW-248.md) — sibling defect, same `S1`, found the same way |
...
| Thing | Where | State |
|---|---|---|
| The generator | [`build_development_plan_xlsx.py`](../../tools/deliverables/build_development_plan_xlsx.py) | ✅ Built and in use — produces a **client-facing** deliverable |
...
| The `.xlsx` | `../../deliverables/FlatWire_DevelopmentPlan.xlsx` | ⚠ **Committed, and missing eleven stories.** Regenerated during `FW-241`–`FW-249`'s verification and then **restored to its committed state** on purpose |
...
[`FW-248`](FW-248.md) is the sibling tooling defect in the same sprint;
