...
> ⭐ **Superseded on one detail only, 10 September 2026 (`D-59`) — the recorded text above stands.**
> **The address is `#/flat-wire/flatline/{line}`**, and the line is still read from that segment and
> nowhere else. `home` was chosen in passing by `D-55`; `flatline` names what the operator is looking
> at. ⛔ **Nothing else in this section changes** — one component, three lines, exactly four things
> varying by line, all as confirmed on 7 September 2026.
...
| [Shift Summary](./ShiftSummary.md) | Consumes the pause and rejection data this screen produces |
| [PLC Tag Specification](../../20-architecture/PLCTagSpecification.md) | The machine read surface behind the traces and status, and the write surface used on pause |
