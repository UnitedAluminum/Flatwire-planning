...
## 3.3 Operations Manager and Supervisor are distinct roles `[CONFIRMED]`

They are frequently the same person in a small operation, and the system still treats them as separate permission levels.

| Dimension | Supervisor | Operations Manager |
|---|---|---|
| Focus | People and shift management | Process configuration and machine parameters |
| Primary screen | Shift summary | This screen |
| Approves | Run approvals, mid-run rod checkout | Schedule overrides and activation |
| May edit a pass schedule | No | Yes |

**Operations Manager is defined by what it can do that the other two cannot.** In practice it maps to a Production or Process Engineer, or a Line Superintendent — someone who owns the pass schedules and machine configuration without necessarily being on the floor for every run.

## 3.4 What an Operations Manager does *not* control
...
| **Reverting a roll gap override** | Operations Manager — operators may apply one but not undo it |

The alloy table restriction matters because it holds the material property values the generation engine calculates from. A wrong figure there is wrong on every schedule generated afterwards, which is a different class of error from a wrong value on one schedule.
...
| [PLC Tag Specification](../../20-architecture/PLCTagSpecification.md) | What of this record is pushed to the machine, and the rule that approving a schedule configures nothing |
