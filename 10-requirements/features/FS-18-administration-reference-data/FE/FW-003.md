...
- [ ] ⚠ **Two of these tabs now have a schema, and it is NOT in this card's 12 h.**
      [`FW-262`](../../FS-03-database-foundation/DB/FW-262.md) (10 h DB, 4 Sep 2026) built the five
...
- [x] ✅ **FL3's two disputed Setup/Handling rows are SETTLED — `Q94` legs 2–3, client 10 Sep 2026.**
      **Render what the seed now holds.** They had **one** root cause, not two: FL3's `H1AA` SPC
      row named FL1's **takeup** where it should have named FL1's **mill**.
      - `FL3`/`H1AA` seq 8 is now **`SPC: FL1-Stand 1`** — *"This is my mistake, it should have
        contained SPC FL1.FM1 not Takeup1."*
      - The **absence** of `SPC: FL1-Stand 1` from `FL3`/`H1B` was **correct** — post-stop
        dimensions *"would not represent final gauge/width"*. ⭐ Which is also why FL1
        legitimately keeps it in its own `H1B`: **FL1's output IS final gauge.**
      - `FL2`/`H1B` and `FL3`/`H1B` seq 2 are now **`SPC: FL2-Stand 3`**.
      ⚠ **Two parts of that are OUR reading and are in the send-back** (`Q110` item 6): that
      *"FL2.FM2"* means stand **S3**, and that the instruction is a **replace** rather than an
      **add** — replace is what holds the element counts at `FL1 33 · FL2 29 · FL3 47`.
      *(Superseded criterion:)* ⛔ **seeded AS PICTURED and must render that way** —
      `SPC: Takeup-1` present under `H1AA` though FL3 has no Takeup-1, and `SPC: FL1-Stand 1`
      absent from `H1B` though FL3 runs through that stand. Do not "correct" either in the UI
