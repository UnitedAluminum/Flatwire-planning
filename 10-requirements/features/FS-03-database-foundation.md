...
**21 stories in this category, each retained as its own file.** One activity per story. **Activities are expected to merge downward** as work proceeds — `FW-N17`/`N18`/`N19` are one rename, not three.
...
| `FW-268` | Reconcile ToolingInventoryEdger with the client's edger grid when  | DB | ✅ done | FW-251 | — |
...
| `FW-268` | ✅ done | **DONE 10 September 2026 — `Q95` returned and all five legs were applied.** `EdgeType` is `NOT NULL` again and widened to `varchar(20)` with a **three**-value CHECK (`Natural Round Edge` is excluded — no set is ground for *no edgi |
...
**12 register items cited by 7 of 21 activities** - **10 open**, 1 closed but still cited, **1 resolving to no register** (`G61`).
