...
- [ ] ⛔ **`screenKey` does NOT follow the route constant.** It is stamped into every element id
      (`btn-{item}-{screenKey}`, `act-{key}-{screenKey}` — [`[UIC §3.22]`](../../../../50-frontend/UIConventions.md)), so it
...
|---|---|
| **`D-55`** ([`[MS §10.2]`](../../../MasterSpecification.md)) | The decision this story implements — one component, four profile rows, the URL, the station rule |
| [`ActiveRunMonitor.md §1.4a`](../../../screens/ActiveRunMonitor.md) | The four varying things, in the client's own document |
| [`[UIC §3.22]`](../../../../50-frontend/UIConventions.md) | `screenKey`'s effect on element ids, and the shared controls to bind rather than rewrite |
| [`[CMP §5.2]`](../../../../50-frontend/Components.md) | ⚠ **Superseded by `D-55` on route shape** — it is written line-first (`/flat-wire/line/:machineName/...`), this is screen-first |
