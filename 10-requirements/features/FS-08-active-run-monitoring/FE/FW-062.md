...
status_note: "🟡 **The screen is built** — `active-run.component`, reached at `#/flat-wire/flatline/:machineName`. ⛔ **This note described a UI skeleton with 'no API client, no live stream' and named it the default child of `#/flat-wire`; all three are stale (10 Sep 2026, `D-59`)** — the component now carries an API client and a hub subscription, and `#/flat-wire` redirects to `supervisor-dashboard` (DB1). ⚠ **Re-measure this story against the working tree before planning it** — what remains of the eight hardcoded builders is not established here, and `FW-N15` §6 is the pointer. ⭐ **Re-scoped 7 Sep 2026 by `D-55`: this is now ONE machine-driven component for FL1/FL2/FL3 at `#/flat-wire/flatline/:machineName`, with only FOUR per-line rows** — the FL1/FL2/FL3 'variants' are retired, and six things that read as configuration are data or uniform. ⛔ **It now depends on two new stories that did not exist**: `FW-N15` (route, line resolver, `LINE_PROFILES`) and `FW-N16` (the station-claim read). Still owed: the data path, the eight hardcoded builders replaced by a mapper, and the `PS-1100-FL1-003` negative fixture removed"
owner:
jira:
mvp: 1
phase: "5"
...
>
> **DB3's screen exists** as `active-run.component` in `projects/flat-wire/`, built ahead of
> this story as the reference every other flat wire screen is copied from, and it is the source of
> [`[UIC]`](../../../../50-frontend/UIConventions.md). This story inherits it.
>
> ⛔ **It is NOT the landing screen** *(corrected 10 Sep 2026, `D-59`)*. This note said
> *"`#/flat-wire` redirects to `#/flat-wire/flat-wire-landing`"*, which is wrong twice over: the
...
> and `lib-trace-panel` live in `projects/shared` ([`[UIC §3.22]`](../../../../50-frontend/UIConventions.md)). Do not
...
> popup** ([`[UIC §3.14]`](../../../../50-frontend/UIConventions.md)) · the **spool notification card and its acknowledged
> pill** ([`[UIC §3.16]`](../../../../50-frontend/UIConventions.md)) · a **budgeted height** that fits the window at any
> resolution without scaling ([`[UIC §4.1]`](../../../../50-frontend/UIConventions.md)).
>
> ⛔ **The blue palette is final** and the screen carries **no theme mechanism** — a light-theme
> demo toggle was built and removed. Do not reintroduce one ([`[UIC §3.17]`](../../../../50-frontend/UIConventions.md)).
>
...
- [ ] `dashboard-3-active-run` built from `../../../../50-frontend/mockups/dashboard_3_active_run.html`, plus the FL3 variant `dashboard_3_active_run_fl3.html` — 🟡 **FL1 renders; FL3 does not exist**
...
- [ ] **The line comes from the URL segment** (`#/flat-wire/flatline/fl1|fl2|fl3` — `D-59`) and from nowhere else
...
- [ ] ⛔ **Two departures from the mockups are recorded here and in [`[UIC §5]`](../../../../50-frontend/UIConventions.md)** —
