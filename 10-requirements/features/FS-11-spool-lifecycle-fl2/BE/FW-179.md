...
**Last Updated:** August 29, 2026 — ⚠ **Re-reviewed against the BUILT code: this is a DE-STUB.** ✅ **`G41` is mitigated in the domain** — `Fm1ScopedToRodFedLinesRule` refuses an FL2 schedule with `FM1` Active *and* an FL1/FL3 one without it, so component state is already read before anything is written; the DDL constraint stays line-blind. ✅ The `[API §4.6a]` fixture error is now flagged in the controller's own remarks too. ⛔ **Two ownership defects in the built code:** `CheckInService.CheckInSpoolAsync` attributes itself to **`FW-157`** (it is this story's), and `SpoolService.CompleteSpoolAsync` attributes itself to **this story** (`POST /spool/complete`, endpoint 16b, is [`FW-202`](../../FS-08-active-run-monitoring/FE/FW-202.md)'s). Change history is in [`CHANGELOG.md`](../../../../CHANGELOG.md)
...
| Concern | Story |
|---|---|
| `SpoolCheckin` table + the `SpoolProcessing.OrderNo` index | [`FW-180`](../DB/FW-180.md), DB — ✅ **built** |
| DB5 / DB5A screens | `FW-178`, FE · `FW-124` deferred |
| The `SpoolProcessing` row this reads | [`FW-202`](../../FS-08-active-run-monitoring/FE/FW-202.md) — ⚠ **must land first** (§4) |
| **`POST /spool/complete`** — endpoint **16b** | [`FW-202`](../../FS-08-active-run-monitoring/FE/FW-202.md) — ⛔ **not this story, despite what the built shell says** (§1.2) |
| `PLCTagService` | [`FW-151`](../../FS-04-realtime-plc-backbone/RT/FW-151.md) — ✅ **built** |
| The rod-side twin | [`FW-157`](../../FS-07-rod-checkin-plc-config/BE/FW-157.md) |
...
| `SpoolCheckin` table + `OrderNo` index | ✅ Built ([`FW-180`](../DB/FW-180.md)) |
...
`FW-179`'s"* — it is **[`FW-202`](../../FS-08-active-run-monitoring/FE/FW-202.md)'s**, endpoint 16b. Same defect
...
decide what to write**, which is this story. See [`FW-207`](../../FS-02-backend-service-foundation/BE/FW-207.md).
...
1. ⚠ **Both routes already exist** on [`FW-138`](../../FS-02-backend-service-foundation/BE/FW-138.md)'s shell —
...
4. DTO joins: `CoilTraceability`/`WeldEvent` → source rods · the FL1 run → gauge/width ·
   **cross-database** → the order block *(the same unenforced-link basis as
   [`FW-164 §2.1`](../../FS-08-active-run-monitoring/BE/FW-164.md); reuse its view)*.
5. `CheckInService` spool path — **FL2 tags only** (§2.2), through
   [`FW-151`](../../FS-04-realtime-plc-backbone/RT/FW-151.md).
...
[`FW-181`](../RT/FW-181.md) owns the **no-measurement state** on the shared trace path — *(it read
"binds the FL2 Live/Profile view" until 9 Sep 2026; that binding is gone, FL2's trace is live at 4 s
— `G120`)*.
