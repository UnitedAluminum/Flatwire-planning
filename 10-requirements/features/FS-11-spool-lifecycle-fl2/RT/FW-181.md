...
| Concern | Story |
|---|---|
| The broadcast loop that applies the suppression | [`FW-150`](../../FS-04-realtime-plc-backbone/RT/FW-150.md) |
| The `gaugetrace` REST read Profile uses | [`FW-164`](../../FS-08-active-run-monitoring/BE/FW-164.md) |
...
4. ✅ **`RouteMode` is already on the active-run DTO**
   ([`FW-164`](../../FS-08-active-run-monitoring/BE/FW-164.md)) — **verify it is populated**, and that
   `FW-164`'s de-stub fills it (`P-49`).
5. Confirm Profile's source is the **REST** `gaugetrace` read, not the hub.
6. ⚠ **Check the simulator drives FL2** ([`FW-203`](../../FS-04-realtime-plc-backbone/RT/FW-203.md)) — speed and
...
> line."* What remains is **populating** it ([`FW-164`](../../FS-08-active-run-monitoring/BE/FW-164.md))
...
`FW-081` / `FW-163` (FE) render both views. [`FW-203`](../../FS-04-realtime-plc-backbone/RT/FW-203.md) **must
drive FL2** or both FL2 trial screens are dead.
[`FW-164`](../../FS-08-active-run-monitoring/BE/FW-164.md) serves Profile.
