...
> [`FW-246`](../../FS-03-database-foundation/DB/FW-246.md) resolves it; `P-163` recommends **`3`**.
...
| Concern | Owner |
|---|---|
| The **write path** | ⚠ **Backend** — [`FW-179`](../BE/FW-179.md) |
| `CK_SpoolCheckin_PayoffPos`'s wrong value | ⛔ [`FW-246`](../../FS-03-database-foundation/DB/FW-246.md) — `G55`, `P-163` |
| The spool this checks in | [`FW-202`](../../FS-08-active-run-monitoring/FE/FW-202.md) — nothing else creates it |
| The FL2 null-gauge contract | [`FW-181`](../RT/FW-181.md) |
...
✅ **[`FW-246`](../../FS-03-database-foundation/DB/FW-246.md)'s `P-163` recommends `3`** and notes that
...
⚠ **This is the same blocker [`FW-202`](../../FS-08-active-run-monitoring/FE/FW-202.md)
...
3. ⛔ **Escalate `OI-25`** — it decides AC 4's trace projection, not just downstream arithmetic
   (§2.2).
4. The write path is [`FW-179`](../BE/FW-179.md)'s.
...
[`FW-179`](../BE/FW-179.md) owns the
write path — ⚠ **and `[API §4.6a]`'s worked example is stale**.
[`FW-246`](../../FS-03-database-foundation/DB/FW-246.md) fixes `G55` and `G50`'s hole (b), which is what
made `G55` possible. [`FW-202`](../../FS-08-active-run-monitoring/FE/FW-202.md)
creates the spool this checks in and shares `OI-25`.
[`FW-181`](../RT/FW-181.md) governs what the
FL2 view shows.
...
For the current shape read [`FW-N20`](../../FS-03-database-foundation/BE/FW-N20.md), `[DBD §6.2]` and `FlatWireSchema_Materials.md`.
