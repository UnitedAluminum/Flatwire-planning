...
**23 stories in this category, each retained as its own file.** One activity per story. **Activities are expected to merge downward** as work proceeds — `FW-N17`/`N18`/`N19` are one rename, not three.
...
| `FW-N34` | Widen the EdgeType enum across all three mirror legs, and replace  | BE | ⬜ not-started | FW-147 | **OI-05** |
...
| `FW-142` | ✅ done | ⚠ **THE EDGER ENTITY SHAPE MOVED ON 10 Sep 2026 AND THIS STORY IS NOT REOPENED.** `ToolingInventoryEdger` — in this story's context/CRUD surface — gained `AllocatedPosition`, lost `GrooveNo`, took the natural key `(MachineName, Se |
...
| `FW-147` | ✅ done | ⚠ **`EdgeType` IS SUPERSEDED BY `D-61` (10 Sep 2026) AND THE CHANGE IS **`FW-N34`**, NOT HERE.** The client's four-value edge-profile vocabulary replaces `{Round, Square}`. ⛔ **That removes an enforcement this story deliberately b |
...
| `FW-207` | 🔵 in-review | ✅ **BUILT — all 8 build-order steps are in the branch; 308/308 unit tests pass. Nothing here is left to BUILD (§0).** Seven roots as `Entity, IAggregateRoot`, 13 child types, 13 value objects + `PassScheduleSnapshot`, **22 rules** |
...
**4 register items cited by 3 of 23 activities** - **4 open**.
...
| **`OI-05`** | ⛔ open | `FW-N34` | Bevel edge has no domain value. The Dashboard 9 / 9A Generate modal offers Round / Flat / Bevel; the domain and the DB C |
...
- ⚠ **`G126` — four of this category's cards carried a stale acceptance criterion whose
  ...
  **`FW-145`** five roles → **six, the omission being QA**; **`FW-147`** all three sample rules in
  ...
  ✅ **`G126` is closed** — all 97 correction rows across the fourteen plans adjudicated, and
  `FW-207` and `FW-208` each carried further stale criteria, now corrected on their cards. The
  plans are back in `*/tasks/` and citable again, so read each beside its card. See [`Gaps.md`](../../90-registers/Gaps.md).
