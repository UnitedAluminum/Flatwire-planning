...
> **Read [`[P1A §2]`](../../../../95-archive/orchestration-boards/frontend-Phase-01A-ImplementationPlan.md) first.** Decisions: **`F-02`** (`useMockData`
...
> - [ ] **Canonical enums defined once:** `State = 'Active' | 'Bypass' | 'Skip'` and ⚠ **`EdgeType` is now the client's FOUR profiles — `'Natural Round Edge' | 'Full Round Edge' | 'Square Edge' | 'Broken Edge'` (`D-61`, 10 Sep 2026), not `'Round' | 'Square'`.** ⭐ **THIS STORY OWNS THE TYPESCRIPT LEG OF `FW-147`'s THREE-WAY MIRROR, AND IT DOES NOT EXIST YET** — `ComponentState` and `MachineName` each have a real `src/lib/enums/*.enum.ts`; `EdgeType` has none, only `edgeType?: string | null` on the model and a bare `'Square'` string in a fixture. That is **unbuilt work, not a defect**. ⛔ **And the display pipe is very likely unnecessary now** — the client's values already read as operator labels, so letting the domain values *be* the labels removes the `Flat Edge`/`Square` translation `G76` was about. **Decide it here.** ⚠ The enum widening is gated on `OI-05` (`Q110` item 1)
...
- ⛔ ~~**`null` live gauge and width for FL2** — its absence is specified behaviour (`FR-120`, `[SIG §5.3]`), and treating it as a fault is a defect.~~ **REVERSED 9 Sep 2026 (`G120`):** `A3` retired, `FR-120` superseded, `[SIG §5.3]` withdrawn — **FL2 sends both, live, at 4 s.** ⚠ **The stub must now do the opposite**: emit FL2 gauge and width **at ≈4 s**, and emit a `null` **only** as an occasional no-measurement sample, on **any** line — which is still the case whose mis-rendering is a defect (`FW-181`).
...
[`FW-135`](../../FS-04-realtime-plc-backbone/RT/FW-135.md) and `[P1A §6.12]`.
