---
id: FW-065
legacy_id:
title: SPC checkpoint dialog
status: not-started
status_confirmed: true
status_note: "⬜ **Not started — and it is the first dialog, so it sets the pattern for six more.** ✅ **`POST /spc` answers today.** ⚠ the dialog scaffold to copy is `trace-graph-popup.component` — `CommonPopupService` + `NgbActiveModal` + `cdkDrag`, with the action button carrying an icon and the dismiss button not. ⚠ **Build the confirmation gate and the transaction summary here, once** — `FW-067`, `FW-070`–`FW-073` and `FW-256` all copy them. ⛔ `tolerance-viz` is owed by `FW-133`, and `OI-57` leaves the published tolerance bands per alloy and temper undefined. ⚠ Its DB3 rail entry renders but is inert."
owner:
jira:
mvp: 1
phase: "6"
stream: FE
streams: [FE]
priority: medium
hours: 24
sprint: S2
depends_on: [FW-133, FW-067, FW-168]
blocked_by: [OI-57]
has_plan: false
started:
completed:
---

# FW-065 - SPC checkpoint dialog

> **No implementation plan has been written for this story yet.**
> The card below is the contract from `[TB]`. Before starting, replace this notice
> with the sections in the task template: *What to build* / *Context you need* /
> *Build order* / *Decisions made here* / *Verification* / *Handoff*.

## 1. What to build

**Hours:** 24 h FE · **Priority:** Medium · **Sprint:** S2 · **Phase:** 6 · **Stream:** FE

**As an** operator,
**I want** to record a measurement against tolerance and be routed correctly when it fails,
**So that** out-of-spec material is suspended rather than shipped.

**Acceptance Criteria:**
- [ ] Built as a `MatDialog` from `../../../../50-frontend/mockups/spc_checkpoint.js` — `openSpcCheckpoint(ctx)`, CSS scoped under `.fwspc`
- [ ] Checkpoint types: **Pre-Run · Post DB1 · Post Die Change · Manual Spot Check · Post-Run**; measurements vary by type
- [ ] **Marker maths:** `pct = 50 + ((measured − target) / (tol × 1.67)) × 50`, **clamped 4–96 %**
- [ ] **Submit · Continue** when in spec; **Submit · Suspend material** when out of spec → **opens the WIP rejection dialog with the failing reading carried over**, coil to `SPC-HOLD`
- [ ] **Read-only mode** — Dashboard 1's "SPC · Last check … · View →" reviews a recorded checkpoint; it must not open a blank form
- [ ] Opens pre-loaded with the die change as its trigger when raised from FW-073

**Rate-card basis:** modal/dialog 12 h + five checkpoint types, marker maths and read-only mode 12 h = 24 h (§2)
**Dependencies:** FW-133 *(`tolerance-viz`)*, FW-067, FW-168
**Blockers:** **OI-57** *(tolerance bands)*

---
