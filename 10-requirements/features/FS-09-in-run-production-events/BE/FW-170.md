...
> [`FW-254`](../../FS-18-administration-reference-data/BE/FW-254.md).
>
...
>
> ⚠ **The 8 h is not re-priced here** — [`FW-258`](../../FS-18-administration-reference-data/BA/FW-258.md) owns that.
...
| `RunPauseEvent` the table | [`FW-171`](../DB/FW-171.md), DB — ✅ **built** |
| `PLCTagService`'s hold/idle and restore operations | [`FW-151`](../../FS-04-realtime-plc-backbone/RT/FW-151.md) — ✅ **built and harness-verified 28 Aug**; first called here |
| The `PauseEvent` marker on the trace | [`FW-172`](../RT/FW-172.md) — ✅ **its two handlers are already built** |
| Rod checkout, the fourth outcome's destination | [`FW-174`](../../FS-10-exceptions-off-ramps/BE/FW-174.md) / `FW-072` — **deferred from the trial** |
...
| `RunPauseEvent` table, `PauseDurationSeconds` computed | ✅ Built ([`FW-171`](../DB/FW-171.md)) |
...
1. ⚠ **Both actions already exist** on [`FW-138`](../../FS-02-backend-service-foundation/BE/FW-138.md)'s
...
7. ✅ **`RunPaused` / `RunResumed` are already raised by the aggregate**, and their broadcast
   handlers are built ([`FW-208`](../../FS-02-backend-service-foundation/BE/FW-208.md) step 8) — so this
...
[`FW-172`](../RT/FW-172.md) broadcasts the `PauseEvent` marker and the
...
| **`D-30`** | `FlatWireRun` carries a `ROWVERSION`; two concurrent pauses on one run are a concurrency conflict, not a last-write-wins — see [`FW-243`](../../FS-14-order-allocation-fulfilment/BE/FW-243.md) for the three roots that do **not** |
