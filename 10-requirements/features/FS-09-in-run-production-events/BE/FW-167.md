...
> bands derive from one table; owned by [`FW-199`](../../FS-18-administration-reference-data/BA/FW-199.md).
...
| Concern | Owner |
|---|---|
| The `DieChangeEvent` table | [`FW-171`](../DB/FW-171.md) — ✅ built |
...
[`FW-234`](../../FS-02-backend-service-foundation/BE/FW-234.md)'s audit trail** once that exists, not only to
Serilog.
...
7. Read die life from `Drawer.LastGrindingFeet` / `TotalFeetAllowed`, **60/85 % bands only**
   (§2.3).
8. Raise the domain event so [`FW-172`](../RT/FW-172.md) can mark the trace —
...
[`FW-172`](../RT/FW-172.md) marks the die change on the trace.
[`FW-234`](../../FS-02-backend-service-foundation/BE/FW-234.md) gives the logged exception a queryable home.
