...
**Part of:** category [`FS-06` Line Visibility and Alerting](../../FS-06-line-visibility-alerting.md) — the parent story for this functional area
...
| Concern | Story |
|---|---|
| Exception broadcasts and supervisor notification | [`FW-177`](../../FS-10-exceptions-off-ramps/RT/FW-177.md), Phase 7 — ⚠ **it consumes `AlertRaised`** |
| The telemetry loop | [`FW-150`](../../FS-04-realtime-plc-backbone/RT/FW-150.md) — built, and ⛔ these events stay out of its batch |
| `PayoffStateChanged` itself | `FW-160`, Phase 4 — this story **consumes** it |
| Bay occupancy writes | [`FW-158`](../../FS-07-rod-checkin-plc-config/BE/FW-158.md), Phase 4 |
| Reading alerts for page load | [`FW-154`](../BE/FW-154.md)'s `ActiveAlertDto` |
| The `ITInhibit` interlock | [`FW-205`](../../FS-04-realtime-plc-backbone/RT/FW-205.md) — ⚠ **a different mechanism**; do not conflate (§2.4) |
...
**Take the second**, the same shape as [`FW-206`](../../FS-07-rod-checkin-plc-config/BE/FW-206.md)'s `P-179`.
...
4. **Rule 2** — gauge outside ±tolerance → Warning. ⚠ **Reuse `FW-150`'s per-run cached band**
   ...
   built and unexercised, like [`FW-245`](../../FS-03-database-foundation/DB/FW-245.md).
...
[`FW-177`](../../FS-10-exceptions-off-ramps/RT/FW-177.md) (Phase 7) consumes `AlertRaised` for supervisor
notification — ⚠ **and `FW-175` is deferred, so that notification is transient.**
[`FW-154`](../BE/FW-154.md)'s `ActiveAlertDto` reads the current
alert set for page load and must return an empty list until this story lands. `FW-160` supplies
`PayoffStateChanged`; [`FW-158`](../../FS-07-rod-checkin-plc-config/BE/FW-158.md) writes the
