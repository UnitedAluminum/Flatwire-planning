...
status_note: "⚠ **`EdgeType` IS SUPERSEDED BY `D-61` (10 Sep 2026) AND THE CHANGE IS **`FW-N34`**, NOT HERE.** The client's four-value edge-profile vocabulary replaces `{Round, Square}`. ⛔ **That removes an enforcement this story deliberately built** — §L195: *'the absence is the enforcement, so a helpful addition breaks it'*, and `Bevel` is *'rejected at model binding by not being enum members'*. So it is not a cosmetic edit, and it is **blocked on `OI-05`** (is `Bevel` simply `Broken Edge`? `Q110` item 1). ⚠ **The DB leg of the mirror has already moved** — both `CHECK`s re-valued and deployed 10 Sep — so the mirror is temporarily two-of-three by design. ✅ **This story stays `done`**; the enum-mirror inventory it owns is `FW-N34`'s checklist. *(Previously:)* ✅ **EXECUTED — 27 Aug 2026.** All five build-order steps are complete or accounted for: the fourteen enums and fourteen validators were built 25 Aug (`P-57`/`P-58`), step 4's `P-19` handoff was **already in `FW-207`** in three places, and **`TC-020` has now been run** (§6.1) — **12 of 14 pass C# ↔ DDL with zero mismatches**. ⚠ **It is a TWO-way diff: the TypeScript leg does not exist** (`FW-132` u"
owner:
jira:
mvp: 1
phase: "1B"
