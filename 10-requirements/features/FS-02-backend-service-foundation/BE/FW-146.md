...
`OI-46`'s *"alert Operations"* side effect is [`FW-157`](../../FS-07-rod-checkin-plc-config/BE/FW-157.md)'s
...
([`FW-157`](../../FS-07-rod-checkin-plc-config/BE/FW-157.md), [`FW-179`](../../FS-11-spool-lifecycle-fl2/BE/FW-179.md)).
...
|---|---|---|
| `409` **constraint-name** resolution — `BAY_OCCUPIED` vs `ROD_ALREADY_STAGED` vs `RUN_ALREADY_ACTIVE` (`P-82`) | No endpoint reaches `FlatWireDB`; both indexes exist and nothing inserts against them. **Written and unexercised** | [`FW-157`](../../FS-07-rod-checkin-plc-config/BE/FW-157.md) / [`FW-179`](../../FS-11-spool-lifecycle-fl2/BE/FW-179.md) — first real insert |
...
| **`OI-46`** | **b** | ✅ **This story's obligation is complete**: emit `SCHEDULE_NO_MATCH` + `422` + `errors[]`. The *"block and alert Operations"* half is a **side effect**, and exception middleware must not carry business behaviour on an already-failing path | `OI-46` stays open. [`FW-157`](../../FS-07-rod-checkin-plc-config/BE/FW-157.md) owns the behaviour, via `FW-208` → `AlertRaised`. Must close before the de-stub pass (`[API §7.3]`) |
| **`G2` / `OI-39`** | **b** | ✅ **No exposure to the outcome.** §2.3's wording — `500` means *the transaction aborted and compensating writes were issued*, never *"rollback"* — holds whichever strategy `G2` picks, saga/outbox or `INFLAT` mirror | `G2` stays **Critical**, **before T2**, 24–64 h reserve, **settle `G30` first** — owned by [`FW-157`](../../FS-07-rod-checkin-plc-config/BE/FW-157.md) and [`FW-151`](../../FS-04-realtime-plc-backbone/RT/FW-151.md), where the recovery is built |
