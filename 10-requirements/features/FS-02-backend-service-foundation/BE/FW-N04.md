...
| Concern | Story | Hours |
|---|---|---|
| Endpoints on the controllers + stub fixtures — **fourteen since `P-53`** | [`FW-138`](FW-138.md) | 45 h *(42 h owed)* |
| MediatR handler registration + validation/logging pipeline behaviours | [`FW-139`](FW-139.md) | 16 h |
| DI registration and the stub/real service swap (`useMockData`) | [`FW-140`](FW-140.md) | 12 h |
| The seven repositories | [`FW-141`](FW-141.md) | 28 h |
| `FlatWireDbContext` and Dapper data access | [`FW-142`](FW-142.md) | 24 h |
| Serilog structured logging and the audit log | [`FW-143`](FW-143.md) | 12 h |
| Configuration binding, validated at startup | [`FW-144`](FW-144.md) | 12 h |
| JWT authentication and role authorization policies | [`FW-145`](FW-145.md) | 16 h |
| Global exception middleware and the response envelope | [`FW-146`](FW-146.md) | 8 h |
| FluentValidation, value objects, the canonical enums | [`FW-147`](FW-147.md) | 12 h |
| Health checks | [`FW-148`](FW-148.md) | 8 h |
| `FlatWireHub` | [`FW-080`](../../FS-04-realtime-plc-backbone/RT/FW-080.md) | 28 h |
| The seven aggregate roots (`D-29`) | [`FW-207`](FW-207.md) | 32 h |
| The typed event contract | [`FW-149`](../../FS-04-realtime-plc-backbone/RT/FW-149.md) | 16 h |
| OPC ingest + bounded channel | [`FW-N05`](../../FS-04-realtime-plc-backbone/RT/FW-N05.md) | 32 h |
| The broadcast loop | [`FW-150`](../../FS-04-realtime-plc-backbone/RT/FW-150.md) | 16 h |
| `PLCTagService` + `SimulatePLCTagPush` | [`FW-151`](../../FS-04-realtime-plc-backbone/RT/FW-151.md) | 16 h |
| **`ITInhibitService`** — `phase-01b` **exit criterion 5** | [`FW-205`](../../FS-04-realtime-plc-backbone/RT/FW-205.md) | 16 h |
| Domain events + post-commit dispatch | [`FW-208`](FW-208.md) | 8 h |
...
> trial-scope stories [`FW-203`](../../FS-04-realtime-plc-backbone/RT/FW-203.md) and
> [`FW-218`](../../FS-04-realtime-plc-backbone/BE/FW-218.md) — twenty-two. **That set is complete and
...
> [`Orchestration.md`](../../../../50-frontend/spool-notification/Orchestration.md) — which is why they are absent from the table above
...
2. **⚠ CORRECTED 25 Aug 2026 — `app.UseAuthorization()` IS required, between `UseRouting()` and
   ...
   **`P-55`** in [`FW-138`](FW-138.md) §5 carries the measurement.
...
> repository-wide. `P-06`–`P-08` are in [`FW-138`](FW-138.md) §5.
...
**[`FW-138`](FW-138.md)** (endpoints on the shells this story
...
| Item | Effect here |
|---|---|
| **`G6` / `OI-37`** | ✅ **Largely closed, 15 Aug 2026** — the six roles **do** exist as JWT claims, on the standard `ClaimTypes.Role`. Nothing here changes: the skeleton was never blocked. The residual (the six claim *values* are coded, mapping unsupplied) belongs to [`FW-145`](FW-145.md) §5. **Note it needed no `JWTClaims` addition** — had it, this story's local-feed repack dance would have been on `FW-145`'s critical path |
