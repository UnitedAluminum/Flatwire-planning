...
| The mock SignalR stream — `[API §7.2]`'s fifth obligation (§2.3) | [`FW-080`](../../FS-04-realtime-plc-backbone/RT/FW-080.md) |
...
| ~~`null` live gauge and width for FL2~~ ⛔ **REVERSED 9 Sep 2026 (`G120`)** — FL2 sends both live at 4 s; the fixture owes an **occasional no-measurement sample on any line** instead | **This story** |
...
> ## ✅ CLOSED 10 September 2026 — **option 3, re-specify.** Neither endpoint is built.
>
> **Direction:** rod and coil receiving are another team's, and so is the data these two reads
> return. DB2 and DB2A are re-specified so that **neither needs a rod lookup from this service**,
> and both sections below are **withdrawn** rather than re-homed. ⛔ `FW-233`, whose only job was to
> host them, **closes with no build** and releases its 6 h — which is exactly what its own `Step-3`
> was written to do.
>
> ✅ **Why option 3 rather than 1 or 2.** The five fields `[API §4.3]` promises beyond the coil
> master — `orderId`, `scheduledMachineName`, `footageRunToDate`, `remainingWeightEstimateLb` and
> `stagedPayoffPosition`/`isWelded` — are projected from **`RodStaging`**, and the screens can read
> that row directly through `GET /payoff/status` (`FW-158`), which this service already owns. So the
> single round trip `§4.3` existed to guarantee was never buying anything the staging read cannot
> already answer. Re-homing (option 1) would have kept a controller alive to re-serve data two other
> surfaces hold, and splitting by source (option 2) would have paid two round trips for the same.
>
> ✅ **And `§4.20` is planning's data by rule.** **`FR-542`** (*Must*) — *"the pairing, the
> per-pairing allocated weight and the planned rod sequence shall be **supplied by planning**; the
> shopfloor shall not author them."* A service that never authors an allocation set has no claim to
> host the read of it either.
>
> ⛔ **Two residuals, and neither is discharged by this closure.**
>
> 1. **`FR-547` still requires check-in to ENFORCE membership** of the active allocation set, and
>    with `§4.20` withdrawn nothing says where the shopfloor reads that set from. Planning owns it;
>    the surface is unnamed. **Raised as `G129`**, and it is the reason
>    [`FW-226`](../../FS-14-order-allocation-fulfilment/FE/FW-226.md) (20 h) and
>    [`FW-227`](../../FS-14-order-allocation-fulfilment/FE/FW-227.md) (26 h) are affected — ⚠ they
>    are **not** DB2/DB2A screens, so option 3's *"re-specify the screens"* wording did not cover
>    them and their 46 h now depends on `G129`.
> 2. **`Q24`'s station switching loses `scheduledMachineName`.** `§8.1 F5` already recorded that
>    `P-54` was *necessary and not sufficient* for that field and for `orderId`, the second blocker
>    being **`OI-33`** (`planning_routings`' column mapping is unmapped). Closing `P-54` leaves
>    `OI-33` standing and makes it the whole of the problem rather than half of it.
>
> ⚠ **The endpoint counts are NOT re-derived here.** `[API §3.1]`/`§3.2` carry them and they
> propagate; the two rows are marked **withdrawn** and the count restatement is owed to whoever owns
> `[API]`, exactly as `P-53` left it. ⛔ Do not read a withdrawn row as a deleted one.
>
> ---
>
> ### ⚠ QUALIFIED 10 September 2026 by `P-351` — *appended, and the decision above is unchanged*
>
> ⛔ **The direction above named a surface that does not exist.** *"The receiving team's own
> surface"* is not an endpoint, a service, a screen or a named contact in any of the four
> checkouts, while `FR-064` / `CHK006` — validate the rod alpha against `coils` — **survived this
> closure**, as `[API §4.3]`'s own kept-shape note says. A requirement with no host is not
> re-specified; it is deferred without a record.
>
> ✅ **So the check-in gate is hosted by flat wire after all**, as `[API §4.22]`
> `GET /checkin/rod/{alpha}`: a **read-only** existence check plus the coil master, over a
> `FlatWireDB` **synonym** on `CommonDB..coils`, built by
> [`FW-N35`](../../FS-15-shared-schema-boundary/BE/FW-N35.md). ⛔ It never calls
> `sp_IngestRodFromCoils` (`FR-530`), so this endpoint **creates nothing** — which is the property
> `P-53` and this decision were protecting.
>
> ⚠ **What is NOT reversed:** rod and coil **receiving** remain another team's. No `/rod/**`
> surface, no `RodReceivingController`, no ingestion here, and `§4.3` / `§4.20` stay **withdrawn** —
> `§4.20`'s allocation-set read is not re-homed, and `G129` still stands. The line this decision got
> right is that receiving is not shopfloor; the line it got wrong is that the *check-in gate* was
> therefore someone else's too.
>
> ⛔ **One premise of this decision was measurably false**, and it is quoted in three places
> (`[API §3.2]`, and twice below). `CoilCheckin`'s `getCheckinCoilInfo` returns `MfgOrderNo`,
> `SeqNo`, `OpLetter` and three booleans; `getCoilStatus` returns one `VARCHAR(6)`. Between them
> they serve **status alone** — not alloy, temper, weights, location or received-at. Recorded as
> **`G132`**. ⚠ The decision still closes as option 3; only its reasoning is corrected.

**Was open — escalated 25 Aug 2026.** `P-53` removes the host; it does not remove the requirement.
...
| # | Item | Owning plan |
|---|---|---|
| 1 | `[API §7.2]`'s five obligations against every stub endpoint | **this story** (4 of 5) · [`FW-080`](../../FS-04-realtime-plc-backbone/RT/FW-080.md) (the mock SignalR stream) |
| 2 | Hub behaviour by observation — join, cadence, reconnect re-join, prompt re-delivery (`TC-173`) | [`FW-080`](../../FS-04-realtime-plc-backbone/RT/FW-080.md) |
...
| 7 | `TC-013`–`TC-017a` — `ITInhibit` conditions 3–5, line-scoped, no operator clear path | [`FW-205`](../../FS-04-realtime-plc-backbone/RT/FW-205.md) |
