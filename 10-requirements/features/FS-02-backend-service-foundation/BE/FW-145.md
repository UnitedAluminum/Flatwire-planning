...
> | **What does NOT travel** | ⚠ **The `?access_token=` handler stays in MVP-1.** §3.5 / `P-77` specified it; [`FW-080`](../../FS-04-realtime-plc-backbone/RT/FW-080.md) **built** it inside its own 28 h as the stated prerequisite, and the hub depends on it. Only the **role policies** move |
...
[`FW-080`](../../FS-04-realtime-plc-backbone/RT/FW-080.md) depends on this story for exactly this.
...
| Hub auth | A hub connection with `?access_token=` succeeds; without it, fails. ⚠ **The handler is this story's; the evidence is not available until [`FW-080`](../../FS-04-realtime-plc-backbone/RT/FW-080.md) maps `/hubs/flat-wire`** — until then there is nothing to connect to. Carry it to `FW-080`'s *Auth* verification row rather than recording an untested pass |
...
[`FW-080`](../../FS-04-realtime-plc-backbone/RT/FW-080.md) lists this story in its `Dependencies` — it cannot
...
- [`FW-177`](../../FS-10-exceptions-off-ramps/RT/FW-177.md) targets *"the Supervisor role"* by joining a
...
- [`FW-218`](../../FS-04-realtime-plc-backbone/BE/FW-218.md) applies `SimulatorControl` (§3.4) to `/sim/**`, on
