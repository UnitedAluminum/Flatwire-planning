...
`[DBD §6.6]`: `united_db..alloys` is surfaced as a **synonym** named `Alloys` in the consuming
databases. ⚠ **That paragraph said *view* until 10 Sep 2026** and was measurably wrong —
`P-353` corrects it, and a synonym absorbs nothing, so the mismatches are handled in the one
Dapper query that reads it rather than in the object.
