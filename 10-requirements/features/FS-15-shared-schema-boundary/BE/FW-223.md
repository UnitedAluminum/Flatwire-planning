...
**Last Updated:** September 10, 2026 — ✅ **`FR-530`'s acceptance criterion is EXERCISABLE AGAIN.** `P-351` hosts the check-in gate as `[API §4.22]` `GET /checkin/rod/{alpha}`, built by [`FW-N35`](../../FS-07-rod-checkin-plc-config/BE/FW-N35.md), so *a read creates nothing* can finally be tested — against that endpoint, ⛔ **not** against `GET /rod/{alpha}`, which stays withdrawn. ⛔ **`FW-N35` is not an amendment to this story**: a read-only lookup carries none of this story's `depends_on`, and phase 4 files it under `FS-07`. *(previously August 25, 2026 — flagged that `GET /rod/{alpha}` left the service (`FW-138` `P-53`): **`FR-530` stands as a rule** but its acceptance criterion cannot be exercised here until `P-54` closes.)* Change history is in [`CHANGELOG.md`](../../../../CHANGELOG.md)
...
>
> ✅ **SUPERSEDED 10 September 2026 — the read has a host again, and the criterion is exercisable.**
> `P-54` closed as *option 3, re-specify*, but the surface it named — *"the receiving team's own"* —
> exists nowhere, while `FR-064` / `CHK006` survived. `P-351` therefore hosts the **check-in gate**
> here after all, as [`[API §4.22]`](../../../../40-backend/APIs.md) `GET /checkin/rod/{alpha}`,
> built by [`FW-N35`](../../FS-07-rod-checkin-plc-config/BE/FW-N35.md).
> ⛔ **It is a third entry in THIS register, not a third call site** — a read that must **not** ingest.
> ⚠ **`GET /rod/{alpha}` itself is still withdrawn and is not coming back**; the criterion below is
> now exercised against `§4.22` instead. ⛔ `FW-N35` is **not** an amendment to this story: a
> read-only lookup has none of this story's dependencies, and `FS-07` owns it because phase 4 does.
...
| 3 | **The alloy is a lookup, not a cast — ⚠ but the KEY was wrong until 10 Sep 2026 (`G133`).** `coils.coil_alloy` is `smallint`; `Rod.Alloy` is `varchar(10)` holding `'1100'` | Resolve through `united_db..alloys` on the **NAME** — `alloys.alloy = CAST(coil_alloy AS VARCHAR(10))`. ⛔ Keyed on `alloy_idx` it matched **0 of 1,776** real rows. Storing the code as text does **not** fail — it silently stops every alloy comparison downstream from matching. Throws `55005` when the lookup misses |
...
- [ ] ~~`GET /rod/{alpha}` creates nothing (`FR-530`)~~ → ✅ **exercisable again, against `[API §4.22]` `GET /checkin/rod/{alpha}`** (`P-351`, `FW-N35`). ⚠ `GET /rod/{alpha}` stays withdrawn; the **rule** is unchanged and the endpoint under it is not
