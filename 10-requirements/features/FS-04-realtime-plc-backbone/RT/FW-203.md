...
status_note: "✅ **Built — `OpcFeedSimulator` is live, 0 errors and no analyzer warning from the changed files, and the whole RT spine now runs end to end: simulator → channel → `FW-150` drain → broadcast + `RunReading` → `FW-205`'s watchdog.** ⚠ **Steering, the stop edge and the drop-readings fault are built but NOT exercisable until [`FW-218`](../BE/FW-218.md) exposes them** — that is `G43`, and `P-36` chose it deli"
owner:
jira:
mvp: 1
phase: "1B"
...
**Status:** ✅ **Built — `OpcFeedSimulator` is live, 0 errors and no analyzer warning from the changed files, and the whole RT spine now runs end to end: simulator → channel → `FW-150` drain → broadcast + `RunReading` → `FW-205`'s watchdog.** ⚠ **Steering, the stop edge and the drop-readings fault are built but NOT exercisable until [`FW-218`](../BE/FW-218.md) exposes them** — that is `G43`, and `P-36` chose it deliberately
...
**That is `G43`, and [`FW-218`](../BE/FW-218.md) is its resolution.** Build the
...
| Concern | Story |
|---|---|
| The control endpoints that steer this | [`FW-218`](../BE/FW-218.md) |
...
|---|---|---|
| **FL1** | ✅ driven | ✅ driven |
| **FL2** | ➕ **driven, at 4 s and UNBATCHED** *(read "❌ suppressed, live values `null`" until 9 Sep 2026 — `G120`)*. ⛔ **A simulator ticking FL2's gauge at the FL1 rate is not modelling the machine** | ✅ **driven** — this is the criterion that gets missed |
| FL3 | ✅ driven end to end — **one series, two cadences** (`[PLC §5.2.3]`) | ✅ |
