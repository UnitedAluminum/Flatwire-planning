...
| The cadence values as configuration | [`FW-144`](../../FS-02-backend-service-foundation/BE/FW-144.md) |
...
> [`FW-177`](../../FS-10-exceptions-off-ramps/RT/FW-177.md)'s. **Do not add it to `ComponentStatus`** — that would
...
`[SIG §5.3]` says the two channels are suppressed *"entirely"*; [`FW-181`](../../FS-11-spool-lifecycle-fl2/RT/FW-181.md)
...
| Client | A skipped channel still leaves the field **`null`** — ⚠ **but between samples now, not always** — and `FW-181` requires the **no-measurement** state and **never a flat line at target**. ⛔ *(This row required the text "No live gauge on FL2 · see Profile" until 9 Sep 2026; it is **retired** with `FR-120`, and the state is generic to all three lines — `G120`.)* |
...
1. A drain loop on a **fixed** cadence from `FlatWireSignalROptions.DrainCadenceMs` (default
   **100 ms**, validated positive at boot by [`FW-144`](../../FS-02-backend-service-foundation/BE/FW-144.md)), reading
...
| Value | Invalidate on |
|---|---|
| `RunId`, and the band that follows from it | **The run's own lifecycle domain events**, dispatched post-commit by [`FW-208`](../../FS-02-backend-service-foundation/BE/FW-208.md). They fire from the check-in and checkout commands, so they exist today and do not wait on `C2` |
...
[`FW-148`](../../FS-02-backend-service-foundation/BE/FW-148.md) and is withdrawn there.)*
