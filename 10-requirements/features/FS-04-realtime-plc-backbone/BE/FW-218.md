...
| `POST /sim/{machineName}/fault` | The **only** way to reach [`FW-205`](../RT/FW-205.md)'s conditions **5 and 3** — two or more consecutive recordings missing, and, held past `ITInhibitSilenceMs`, the escalation to condition 3. ⚠ **The card says condition 5 alone; the built handler reaches both**, which is why `FW-213` must not re-implement it (`P-286`) |
...
| Concern | Story |
|---|---|
| The mechanisms being exposed | [`FW-203`](../RT/FW-203.md) `P-36` |
...
> [`FW-138`](../../FS-02-backend-service-foundation/BE/FW-138.md), `phase-01b` L82 and `[TRP]`.
...
2. `Engineer`/`Admin` policy from [`FW-145`](../../FS-02-backend-service-foundation/BE/FW-145.md) — **never
   `Operator`**.
3. The four endpoints of §1.1, each calling a
   [`FW-203`](../RT/FW-203.md) capability and adding no logic of its own.
...
6. Use [`FW-138`](../../FS-02-backend-service-foundation/BE/FW-138.md)'s envelope and
   [`FW-146`](../../FS-02-backend-service-foundation/BE/FW-146.md)'s middleware — the surface is
...
[`FW-214`](../FE/FW-214.md) (FE, **36 h** — 52 h base; a **standalone WinForms
...
| **`G68`** *(raised 31 Aug 2026 by [`FW-214`](../FE/FW-214.md))* | `[SIM §9.2]` requires the console to display `lbPerFt` and the seed; `[SIM §8.1]`'s five endpoints — and `[TB §7]`'s `FW-215` card — provide **no route to either**, and `SimLineState` may not be widened. **Nothing owed by this story**; recorded because §6 previously claimed the readout was live |
