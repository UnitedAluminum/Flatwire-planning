...
> [`FW-172`](../../FS-09-in-run-production-events/RT/FW-172.md) does not, and between them the six of `[SIG §5.4]`
...
Per [`FW-172 §4`](../../FS-09-in-run-production-events/RT/FW-172.md) `P-45` — the split is stated in neither card:

| Marker | Owner |
|---|---|
| `WeldJoinEvent` · `DieChangeEvent` · `SPCCheckpoint` · `PauseEvent` | [`FW-172`](../../FS-09-in-run-production-events/RT/FW-172.md) |
...
| Thing | State |
|---|---|
| `AlertRaised` · `AlertCleared` · `AlertEvent` · `RodCheckoutEvent` · `LineStatus` on `IFlatWireClient` | ✅ Built ([`FW-149`](../../FS-04-realtime-plc-backbone/RT/FW-149.md)) |
| `LineState.Idle` | ✅ Built — AC 4 has a value to send |
| Post-commit dispatch and the two-lane pattern | ✅ Built ([`FW-208`](../../FS-02-backend-service-foundation/BE/FW-208.md)) |
...
⚠ **Declare the two events here and let [`FW-174`](../BE/FW-174.md)
...
| Concern | Story |
|---|---|
| The rejections and checkouts being broadcast | [`FW-174`](../BE/FW-174.md) |
| The durable queue behind AC 3 | [`FW-175`](../BE/FW-175.md) — ⚠ **deferred from the trial** |
| The typed contract | [`FW-149`](../../FS-04-realtime-plc-backbone/RT/FW-149.md) |
...
[`FW-150`](../../FS-04-realtime-plc-backbone/RT/FW-150.md)'s ~100 ms batch. `LineStatus` is **on-change only**.

The route from handler to hub is a **domain event dispatched after commit**
([`FW-208`](../../FS-02-backend-service-foundation/BE/FW-208.md)), because `[SVC §3.2]` bars SignalR
...
**The state is still recorded** — [`FW-174`](../BE/FW-174.md)
...
([`FW-145`](../../FS-02-backend-service-foundation/BE/FW-145.md) §5), never from a literal. The claim *values*
...
1. ✅ **All four are already typed on `IFlatWireClient`** ([`FW-149`](../../FS-04-realtime-plc-backbone/RT/FW-149.md)) —
   confirm, do not add.
2. **Declare the two missing domain events** (§1.1a), each carrying `MachineName`, and write their
   ...
   ([`FW-208`](../../FS-02-backend-service-foundation/BE/FW-208.md) `P-35`), fired **after commit**.
3. `AlertRaised` on a WIP hold; `RodCheckoutEvent` on checkout — raised by
   [`FW-174`](../BE/FW-174.md), inert until it ships.
4. `LineStatus → IDLE` on checkout — on-change only, **from the checkout event, not from the
   telemetry loop** ([`FW-172`](../../FS-09-in-run-production-events/RT/FW-172.md) `P-256`; the loop's channel is dark
   until `C2`).
...
[`FW-172`](../../FS-09-in-run-production-events/RT/FW-172.md) covers the other four markers **and owns `P-256`, the
`LineStatus` source this story reuses**. [`FW-175`](../BE/FW-175.md) makes this
durable when scheduled. `FW-060` / [`FW-154`](../../FS-06-line-visibility-alerting/BE/FW-154.md) bring
DB1 back after the trial, restoring `AlertRaised`'s consumer.
[`FW-N06`](../../FS-06-line-visibility-alerting/RT/FW-N06.md)'s rules engine adds `AlertCleared` and the lifecycle.
...
| **`FW-175` deferred** | The notification is transient in the trial (§3) — [`FW-175`](../BE/FW-175.md) is planned and still out of trial scope |
