...
> `WipRejectionReason` in `01_Lookup` and read through [`FW-254`](../../FS-18-administration-reference-data/BE/FW-254.md).
>
...
> precedent `D-35` cites for `LineDowntimeEvent` ([`FW-255`](../../FS-09-in-run-production-events/BE/FW-255.md)). Do not tighten it.
...
| The `WipRejection` / `RodCheckout` tables | [`FW-176`](../DB/FW-176.md), DB — ✅ **built, approval columns included** |
| The durable supervisor queue this notifies into | [`FW-175`](FW-175.md) — ⚠ **deferred from the trial** |
| The broadcasts | [`FW-177`](../RT/FW-177.md) |
| `ClearPayoffTags` itself | [`FW-151`](../../FS-04-realtime-plc-backbone/RT/FW-151.md) — ✅ **built**; `ClearPayoffTagsAsync(PayoffClearRequest)` |
...
([`FW-150 §2`](../../FS-04-realtime-plc-backbone/RT/FW-150.md)) — a bay changing hands is an operator-visible
...
**Both handlers are already built** ([`FW-208`](../../FS-02-backend-service-foundation/BE/FW-208.md)
...
3. `ClearPayoffTags(machineName, payoffPosition)` — [`FW-151`](../../FS-04-realtime-plc-backbone/RT/FW-151.md)
...
| **`G67`** *(new here, 29 Aug 2026)* | Releasing a bay is one of the restage paths whose `stagedWeights` entry is **never evicted**, so a later `PercentRemaining` is computed against the departed rod. Owned by [`FW-239`](../../FS-04-realtime-plc-backbone/RT/FW-239.md) |
| **`D-30`** | `RodCheckout` and `WipRejection` are two of the three roots with **no `ROWVERSION`** — [`FW-243`](../../FS-14-order-allocation-fulfilment/BE/FW-243.md) |
