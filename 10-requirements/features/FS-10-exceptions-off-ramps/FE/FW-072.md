...
status_note: "⬜ **Not started.** ✅ **`POST /checkout` answers today.** ⚠ Three modes (A, B and P), with Mode B entered from `FW-071`'s pause dialog carrying the frozen footage. ⛔ **The supervisor gate is unbacked**: `G24` says supervisor approvals are decided but unpersisted, `OI-38` leaves the PIN validation source undecided, and `G7` notes the approval currently relies only on transient SignalR. ⚠ Entry is footage-gated — behaviour DB3 does not implement, since its rail is inert. the dialog scaffold to copy is `trace-graph-popup.component` — `CommonPopupService` + `NgbActiveModal` + `cdkDrag`, with the action button carrying an icon and the dismiss button not."
owner:
jira:
mvp: 1
phase: "7"
