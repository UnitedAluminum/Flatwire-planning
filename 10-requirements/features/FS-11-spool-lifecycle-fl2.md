...
**13 absorbed stories**, seeded one activity each. **Activities are expected to merge downward** as work proceeds — `FW-N17`/`N18`/`N19` are one rename, not three.
...
| `FW-179` | POST /checkin/spool and GET /spools | BE | ⬜ not-started | FW-139 FW-151 FW-180 | **OQ-15** **OQ-17** |
| `FW-180` | SpoolCheckin table and the SpoolProcessing.OrderNo index | DB | 🔵 in-review | FW-007 | **OI-25** |
| `FW-181` | The shared trace path — the no-measurement state | RT | ⬜ not-started | FW-081 FW-150 | — |
| `FW-224` | FL2 pre-check-in - spool staging | BE·FE | ⛔ blocked | FW-179 FW-158 | **Q17** **Q41** |
| `FW-230` | FL1 segment alpha — one namespace | DB·BE | 🔵 in-review | — | — |
| `FW-N02` | Spool completion weight milestones and machine-stop confirmation | RT | ⛔ blocked | FW-150 | **Q96** |
...
This category's 76 criteria across 13 cards were measured, and **only 7 % of the 1,222 in the
