...
**13 stories in this category, each retained as its own file.** One activity per story. **Activities are expected to merge downward** as work proceeds — `FW-N17`/`N18`/`N19` are one rename, not three.
...
| `FW-230` | FL1 segment alpha — one namespace | DB·BE | ⛔ blocked | FW-231 | **G54** |
...
**11 register items cited by 8 of 13 activities** - **7 open**, **4 resolving to no register** (`G61`).
...
| **`G54`** | ⛔ open | `FW-230` | ⛔ The 26 Aug alpha design has an unbuilt precondition: nothing registers an FL1 segment alpha in proddb..coils. Every mi |
