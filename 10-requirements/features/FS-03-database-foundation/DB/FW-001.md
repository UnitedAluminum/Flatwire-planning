...
status_note: "⛔ **Cancelled by `D-32`, 18 Aug 2026 — there is no shared-schema migration.** The existing `coils` / scheduling schema is read and written as it stands and never altered, so the column renames and new columns this story existed to add are not required. ⚠ The flattening operation letter `F`, the FL1/FL2/FL3 `machines` rows and the `CommonDB` WIP-station registration all still stand — they are other stories. Anything describing the `Coil/Bundle…` renames as work to be done is stale"
owner:
jira:
mvp: 1
phase: "1C"
