...
| Moving the DDL into `ual-database` | [`FW-242`](../../FS-19-integration-testing-golive/DB/FW-242.md) |
...
| The `ua_user` **principal itself**, in six databases | [`20_FlatWire_Grants.sql`](../../../../30-database/scripts/20_FlatWire_Grants.sql) — **deploy step 3**, outside the `00`–`08` chain | Its procedures grant to **`[public]`**, *"the UAL convention"*, stated in the script's own header |
...
> The `P-##` series belongs to [`40-backend/tasks/`](../../../../50-frontend/spool-notification/Orchestration.md) and is
...
> The de-stub of [`FW-157`](../../FS-07-rod-checkin-plc-config/BE/FW-157.md) had taken the same three ids in code the same
...
first in `S1`, before anything that moves the baseline. [`FW-242`](../../FS-19-integration-testing-golive/DB/FW-242.md) moves this DDL into
