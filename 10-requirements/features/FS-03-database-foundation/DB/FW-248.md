...
| `build_development_plan_xlsx.py`'s multi-stream bug | [`FW-250`](../../FS-19-integration-testing-golive/DB/FW-250.md) — a sibling tooling defect, same `S1` |
...
| **Contradiction 1** | `../../../../30-database/DatabaseDesign.md` **`:34`** — *"the current scripts measure … **69** index statements"*, against **`:24`** and **`:327`** which both say **70**. ⛔ **Same file, three claims, two values** |
...
| ⛔ **A live `C2` failure** | **`../../../../30-database/sql/FlatWire_DDL_02_Schedule.sql` creates tables but has no `"Tables :"` header block** — the run **FAILS** today. See below |
...
one re-opens the same blind spot. [`FW-250`](../../FS-19-integration-testing-golive/DB/FW-250.md) is
