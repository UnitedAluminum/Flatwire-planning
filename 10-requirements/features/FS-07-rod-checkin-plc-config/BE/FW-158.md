...
| Concern | Story |
|---|---|
| `POST /weldevent` — where DB2A's weld actually posts | [`FW-172`](../../FS-09-in-run-production-events/RT/FW-172.md) / `FW-166`, **Phase 6** |
...
| WIP rejection releasing a blocked bay | [`FW-174`](../../FS-10-exceptions-off-ramps/BE/FW-174.md), **Phase 7** — ⚠ `OQ-23`'s cross-phase link |
...
7. The two queries — `GET /payoff/status`, `GET /staging/queue`.
   ⚠ **FL2 has no payoff bays**; agree with [`FW-154`](../../FS-06-line-visibility-alerting/BE/FW-154.md)'s
...
[`FW-174`](../../FS-10-exceptions-off-ramps/BE/FW-174.md) releases a blocked bay in Phase 7
