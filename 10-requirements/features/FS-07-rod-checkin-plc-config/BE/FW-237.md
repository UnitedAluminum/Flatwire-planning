...
| The audit trail's persistence target | [`FW-234`](../../FS-02-backend-service-foundation/BE/FW-234.md) — **a dependency**, and it is what makes AC 3 checkable |
| Per-tag write status | `FW-236` (`G58`) |
| Registering flat wire with `OPCConnection` | `FW-238` (`G60`) |
| The interlock's conditions | [`FW-205`](../../FS-04-realtime-plc-backbone/RT/FW-205.md) built 3–5; [`FW-206`](FW-206.md) owns 1–2 |
...
`P-15` is the finding that nothing can query it. **[`FW-234`](../../FS-02-backend-service-foundation/BE/FW-234.md)
...
1. **Get the credential decision** (Blockers). ⚠ Frame it as §2.2 does — a reserved
   ...
   answer.
2. **Land [`FW-234`](../../FS-02-backend-service-foundation/BE/FW-234.md) first** (§2.4), or AC 3 is
...
survives having no `HttpContext` (`P-120`). [`FW-234`](../../FS-02-backend-service-foundation/BE/FW-234.md)
