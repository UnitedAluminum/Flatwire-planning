...
This document **does not introduce new requirements**. It is a navigational overlay on the existing analysis set — [FlatWireEndToEndProcess.md](./EndToEndProcess.md) remains the stage-by-stage process reference, and the docs listed under [Related Documents](#related-documents) remain authoritative for their own subject areas.

**Equipment facts used here follow the May 21, 2026 client corrections** recorded in [`../20-architecture/Architecture.md`](./BusinessRules.md) §0.3, not the April-dated equipment descriptions still present in `./EndToEndProcess.md`. See [Known Source Conflicts](#known-source-conflicts).
...
10. **Operator opens the 6-step guided wizard** (`../50-frontend/mockups/dashboard_2_rod_checkin.html`): ① Visual Inspection → ② Pass Schedule → ③ Pre-run SPC → ④ Die Block (DB1 · DB2) → ⑤ Rolling Mill (FM1) → ⑥ Lube & Safety. Tabs unlock in sequence.
...
[FlatWireEndToEndProcess.md](./EndToEndProcess.md) is April 28, 2026-dated and predates the client equipment corrections. Where it disagrees with this walkthrough, the **May 21, 2026 corrections win** ([`../20-architecture/Architecture.md`](./BusinessRules.md) §0.3):
...
| [RodCheckout.md](screens/RodCheckout.md) | Pre-run and mid-run checkout, PLC line-state gatekeeper rule (step 27) |
| [PartialRodReCheckin.md](../95-archive/design-notes/PartialRodReCheckin.md) | Carry-forward design for re-checking-in a partially run rod |
