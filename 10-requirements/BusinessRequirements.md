...
**Shortcode:** `[REQ]`
**Part of:** `ProjectPlan/Business/` — index: [README.md](../DOCUMENTS.md)
...
> **Do not hand-edit these figures.** They are generated — run [`../tools/deliverables/build_coverage_matrix.py`](../tools/deliverables/build_coverage_matrix.py), which counts the rows in this document and cross-checks them against `[TCS §5]`. The **363** and **366** previously quoted here and in `00-README.md` both predate the 11 Aug 2026 MVP-2 split, which withdrew §5.10, §5.18, §5.19, §5.23 and §5.24; neither was reproducible from the document. Corrected 13 Aug 2026.

Requirement numbers are **carried forward unchanged** from [`./MasterSpecification.md`](./MasterSpecification.md) §4. Nothing has been renumbered, merged or dropped.
...
| Reference | Role |
|---|---|
| [`./MasterSpecification.md`](./MasterSpecification.md) | The reconciled source this SRS re-cuts. Authoritative where two older artifacts disagree |
...
| [`../95-archive/design-notes/REVIEW.md`](../95-archive/design-notes/REVIEW.md) | Catalogue of known contradictions between older documents |
| [`../90-registers/Questions.md`](../90-registers/Questions.md) | The open-questions register (`OQ-##`) |
...
| `OL`, `PCI`, `CHK`, `WLD`, `TRV`, `ORD`, `PSM`, `GWT`, `SPC`, `ALT`, `STP`, `WBK`, `PR`, `FRT`, `DAT`, `WRJ`, `PKG`, `RCO`, `PRC`, `LST`, `ARM`, `PSL`, `SHS`, `RAJ`, `DCH`, `DMG`, `HMI`, `SCD`, `OEE`, `PRN`, `DM`, `INT`, `NFR` | Source requirement IDs from the delivered SRS | ⚠ **`Shopfloor_Flat_wireSRS_Consolidated_v3.docx`, which is NOT in the working tree** — removed 1 Aug 2026, recoverable at commit `6096921`. The surviving `../deliverables/Shopfloor_Flat_wireSRS.docx` is **not** a substitute: it carries no pre-check-in content, so it cannot own `PCI` or `PRC`. **Where a family has been restated in-repo, the in-repo statement wins** — `PCI009`–`PCI022` in [Screens/RodPreCheckin.md](screens/RodPreCheckin.md), `ORD003`–`ORD017` in §5.28 here, `SQ-1`–`SQ-14` in [Screens/SpoolQueue.md](screens/SpoolQueue.md) |
...
| `OQ-##` | Open question | `../90-registers/Questions.md` |
| `OI-##` | Open issue blocking a build | Master spec §11, carried to §11 here |
| `G#` | Gap register entry | `../90-registers/Gaps.md` |
...
| Dimension | Value |
|---|---|
| Client | Touch panel at the machine, **1920 × 1080** (`Q26`, 7 Sep 2026), gloved operation, no physical keyboard; Angular PWA in a modern Chromium browser |
...
**Screen:** [`dashboard_2a_rod_precheckin.html`](../50-frontend/mockups/dashboard_2a_rod_precheckin.html)
...
| **FR-047** | The override flag, the authorising supervisor, the timestamp and the reason shall be persisted on the staging record, and the bay card shall keep showing the authorisation for as long as the rod is there. *(`RodStaging.OffScheduleOverride` and `ScheduledMachineName` were **dropped** 1 Aug 2026 with the off-schedule case; `OverrideBy`/`OverrideAt`/`OverrideReason` are retained and shared.)* | Must | OQ-24 |
...
**Screen (approved):** [`dashboard_2_rod_checkin.html`](../50-frontend/mockups/dashboard_2_rod_checkin.html) — a guided 6-step tab wizard.
**FL3 variant:** [`dashboard_2_rod_checkin_fl3.html`](../50-frontend/mockups/dashboard_2_rod_checkin_fl3.html) *(still on the older single-page layout; a wizard-shaped FL3 variant is outstanding — **OI-16**)*.
...
**Screen:** [`dashboard_5_spool_checkin.html`](../50-frontend/mockups/dashboard_5_spool_checkin.html)
...
**Screen:** [`dashboard_5a_spool_queue.html`](../50-frontend/mockups/dashboard_5a_spool_queue.html)
...
**Screens:** [`dashboard_3_active_run.html`](../50-frontend/mockups/dashboard_3_active_run.html) (FL1 — grouped action cluster + spool-completion overlay; **the sole FL1 layout since 1 Aug 2026**, when the earlier left-rail layout that held this filename was withdrawn; this file was named `dashboard_3_active_run_v2.html` until 11 Aug 2026) · [`dashboard_3_active_run_fl2.html`](../50-frontend/mockups/dashboard_3_active_run_fl2.html) · [`dashboard_3_active_run_fl3.html`](../50-frontend/mockups/dashboard_3_active_run_fl3.html)
...
| **FR-120** | ~~FL2 in standalone mode shall render the **historical profile**, not a live streaming trace, because the server broadcasts `null` live gauge and width for it.~~ ⛔ **SUPERSEDED 9 Sep 2026.** Its sole basis was assumption **`A3`** of [`[PLC §14]`](../20-architecture/PLCTagSpecification.md), now **retired**: no client source ever supported it; [`Decisions.md`](../90-registers/Decisions.md) Case 4 contradicted it with client confirmation (*"both FL1 and FL2 mills have automatic gauge control… via output of gauge/width trace devices"*, *"Tim confirmed"*); `Q19` and `Q1` contradict it again; and it was **incoherent with the FL3 route**, where the same FM2 and the same downstream gauging stands are real-time (`CLAUDE.md`, `[PLC §5.2.3]`). **FL2 now falls under `FR-101` like every other line**, reading `FL2.PLC.AGC.Gauge` / `.Width` at a **4 s** update rate. ⚠ **The historical profile is retained as context, not as the value of record** — `GET /run/{runId}/gaugetrace` still serves the Gauge-Trace / CPK / Cut-Traceability reports (`G3`, `D-12`). ⛔ **This does not change where a spool's *incoming* profile comes from**: the FL1 profile reviewed at FL2 **check-in** is a different artefact and is unaffected. `G120`. | — | ~~`INT010`~~ |
...
**Component:** [`spool_notification.js`](../50-frontend/mockups/spool_notification.js), hosted in `dashboard_3_active_run.html`
...
| **FR-137** | Actual spool weight shall be derived as `(current footage − footage at spool start) × lb-per-ft`, where `lb-per-ft = A(in²) × 12 × ρ` — `A` applying the round-edge correction where applicable, and ρ read from **`united_db..alloys.alloy_density`**. **For FL2, gauge and width shall come from the pass schedule / order, not live measurement** — ⚠ **because the dimensional basis is *nominal* for every line (`Q10`, `D17`), NOT because FL2 cannot measure.** *(This clause previously read "because FL2 broadcasts `null`". That justification was retired with `A3` and `FR-120` on 9 Sep 2026; the **behaviour is unchanged**, because `Q10` settled the basis as nominal independently and for all three lines. **Live FL2 gauge drives the trace, the tolerance alerting and SPC — not the weight.**)* Worked reference: 1100 at 0.110″ × 0.625″ gives **0.0809 lb/ft** square edge, so a **1,800 lb** spool is ≈ 22,250 ft. *(The 2,000 lb figure this reference previously used was the default withdrawn by `D5`.)* | Must | Analysis; see `[DBD §6.6]` |
...
**Screen:** [`dashboard_2a_rod_precheckin.html`](../50-frontend/mockups/dashboard_2a_rod_precheckin.html) — *Mark as welded* dialog
...
**Dialog:** `spc_checkpoint.js` — `openSpcCheckpoint(ctx)`, a popup over the run being measured (converted from a screen 1 Aug 2026; [`dashboard_6_spc_checkpoint.html`](../50-frontend/mockups/dashboard_6_spc_checkpoint.html) is now its launcher)
...
**Screen:** [`dashboard_11_roll_adjust.html`](../50-frontend/mockups/dashboard_11_roll_adjust.html)
...
**Dialog:** `die_change.js` — `openDieChange(ctx)`, a popup over the paused run (converted from a screen 1 Aug 2026; [`dashboard_die_change.html`](../50-frontend/mockups/dashboard_die_change.html) is now its launcher)
...
⚠ **SUPERSEDED 2 Sep 2026 by the die split (`Q91`) — `FR-233` reverts to its original per-tool form and the size-level restatement above is withdrawn.** The per-tool die inventory now exists: `ToolingInventoryDie` is built and seeded in Phase 1, so an incoming die resolves to a **registered physical tool** and an unregistered die is rejected, exactly as first written. `DieChangeEvent` gained `OldDieId` / `NewDieId`. **`OI-41` closes**, **Phase 6 depends on Phase 1 rather than Phase 13**, and `TC-274` becomes executable. Read what follows as the Aug-11 interim, not as current behaviour. *(Was: "Resolved Aug 11, 2026.")* ~~`FR-233` requires a die inventory that does not exist as a table anywhere in the schema — only the `Drawer` lookup and `DieChangeEvent`. This is why Phase 6 depends on Phase 13 (**OI-41**).~~ **Die inventory and lifecycle are owned outside MVP-1**, so no such table is expected here and **Phase 6 no longer depends on Phase 13** — `Drawer` is seeded in Phase 1. The die change reads size, `LastGrindingFeet` and `TotalFeetAllowed` from `Drawer`, and `FR-233` is restated at size level above.
...
> **Five screen sections sit in [`../60-delivery/02-SRS-MVP2.md`](../60-delivery/02-SRS-MVP2.md)** — Die Management, Pass Schedule Management (DB9), Pass Schedule List (DB9A), Shift Summary (DB10) and the OEE Dashboard. **Copied verbatim; no `FR-###` was renumbered.**
>
...
**Component:** [`pause_run.js`](../50-frontend/mockups/pause_run.js) — a shared dialog for the FL1/FL2/FL3 active-run screens
...
**Dialog:** `wip_rejection.js` — `openWipRejection(ctx)`, a popup raised by whichever screen rejects the material (converted from a screen 1 Aug 2026; [`dashboard_8_wip_rejection.html`](../50-frontend/mockups/dashboard_8_wip_rejection.html) is now its launcher)
...
**Screen:** [`dashboard_12_rod_checkout.html`](../50-frontend/mockups/dashboard_12_rod_checkout.html)
...
**Screen:** [`dashboard_7_coil_completion.html`](../50-frontend/mockups/dashboard_7_coil_completion.html)
...
**Screen:** [`dashboard_7b_packing_station.html`](../50-frontend/mockups/dashboard_7b_packing_station.html)
...
**Screen:** [`dashboard_1_line_status.html`](../50-frontend/mockups/dashboard_1_line_status.html)
...
| **FR-424** | The data source for "Payoff 2 not loaded" shall be **`RodStaging`** — a `Staged` row on `(MachineName, PayoffPosition)` means loaded. `PayoffWeight` alone cannot distinguish an empty bay from a sensor reading zero; the `PayoffStateChanged` event keeps the evaluation live. | Must | Analysis |
...
| Concern | Now |
|---|---|
| The **machine tag map** both screens consumed | [`PLCTagSpecification.md`](../20-architecture/PLCTagSpecification.md) §3, split per line — it is now the only tag map in the repository |
...
`UnstageKind`, two bay states, `PayoffPosition NOT NULL` and a `MachineName` CHECK admitting only FL1 and
...
(`[API]` — `POST /precheckin` still refuses `machineName = FL2` in five places), the screen
...
[`ClientCall_2026-08-20_SyncPlan.md`](../95-archive/source-documents/ClientCall_2026-08-20_SyncPlan.md).
...
| Authored canvas | **1920 × 1080**, renders complete at 1:1 | `[VAL §7.5]` — confirmed by the client 7 Sep 2026 (`Q26`) |
