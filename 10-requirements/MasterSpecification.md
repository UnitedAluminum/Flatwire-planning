...
| **OI-74** | **OQ-18 basis RESOLVED (client, 30 Jul 2026) — the customer weight range, not a default.** ~~Order *Max Wgt of Spool* versus take-up capacity, with a 2,000 lb default assumed.~~ The customer specifies a **min–max weight** (e.g. 900 max / 800 min) and completion is graded against **that range, by weight**; spools are sized ~**1,800 lb** so two finished coils can be cut at FL2. **The 2,000 lb default is withdrawn** and must come out of the mockup and `spool_notification.js`. **Still open:** *which order field* carries the customer min/max; whether an **over-target M4 state** is required; whether the ladder applies to **finished coils at TKUP-2** (OQ-19); and supervisor mirroring plus the acknowledgement audit record's home (OQ-20). ⚠ **THE TKUP-2 HALF HAS AN ANSWER SHAPE, 3 Sep 2026.** Asked what FL2 should do when an order boundary lands a few pounds past a coil cut, Tim O'Brien: *"**Cut at 900 lb. and keep the remainder, a WIPREJ would be forced for the overage**, essentially a peel-back of the coil to put with in the allowable range."* So at a **finished coil** the rule is cut at the maximum and reject the excess — not an alert ladder. That is a **different mechanism from M3/M4**, so *"whether the ladder applies to finished coils at TKUP-2"* is answered **no — a forced WIPREJ applies instead**, and it has no reason code or requirement (**`G88`**). ⚠ **He did not set the threshold** — our proposal was *"keep the remainder below a threshold you set"* and no number came back. The other three halves of this item are untouched (audit record [`ClientEmail_2026-09-03_RodOrderAllocation_SyncPlan.md`](../95-archive/source-documents/ClientEmail_2026-09-03_RodOrderAllocation_SyncPlan.md)) | The spool alert's central comparison basis | spool completion feature | Tim O. / Operations / IT |
