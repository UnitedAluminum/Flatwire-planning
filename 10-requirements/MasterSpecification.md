...
**Last Updated:** September 3, 2026 (tooling) — **`D-42` minted: the Tooling Inventory tab carries FOUR tool types**, not three — Dies · Edgers · Straighteners · **Roll Sets**, and dancers, entry guides, payoffs and spools are explicitly **not** tooling. §5 gains **`ToolingInventoryRollSet`**; the `Drawer` note's *"nothing holds an FK to it"* is **corrected** — `ToolingInventoryRollSet.DrawerId` is the first ever. **`OI-77` partly answered** on the roll half only, **`OI-141`** now spans four types. *(previously August 29, 2026 — ⛔ **New open item `OI-140` (§11.2): a completed output coil is NEVER BROADCAST, because `IFlatWireClient` has no member to send it on.** `FW-207` raises `CoilCompleted` and `FW-208` assigned it the post-commit lane, but `[SIG §5.2]`'s fourteen events and `[SIG §5.4]`'s six markers carry no completed coil — so the operator who completes it on DB7 sees the result, and **every other client on the line learns nothing until it refreshes**. Adding a member is breaking under `[API §8]`; **cheap now, dear later**. **Previously, August 26, 2026 — ⚠ **`GenerateCoilAlpha`'s six-character root is the exclusion sweep's `LIKE` filter, NOT the string the suffix is appended to.** A seven-character input returns a **child** (`R00002A` → `R00002AA`), verified on the live instance; the recorded reason for rejecting segment-rooted alphas — *"returns a sibling of the segment"* — was **false**. The verdict is unchanged and now rests on **collision**: `R00002AA` is also suffix 27 of the rod-rooted sequence. Ceiling corrected **26 → 702** — `OI-135`'s headline and its *"~13 of 26"* headroom corrected; the evidence is `F13` in `[RodOrderAllocation.md §9.1]`. ⚠ **`OI-136` minted — the shipped coil-completion procedure passes a BLANK ignore list where every design document says `@ignoreList`.** Latent, not live: harmless until FL1's segment mint exists, then a silent duplicate onto the welding-wire certificate. Fix rides wave `S5`'s `[S]` rewrite, not a standalone patch; evidence is `F14`. *(previously August 25, 2026 — **the 20 Aug FL2 pre-check-in reversal applied to all six sites** (`FR-031` superseded, line-capability table, DB5A rationale, `RodStaging` CHECK note, `FL2PO`, the `422`); **`OI-127` minted** (weld footage vs material boundary); `Q87` anchored on `FR-336`; object counts 34 → 33 including the post-deployment gate; the `D-##` register duplication made explicit *(previously August 23, 2026 — **`Spool` and `SpoolCarrier` are SWAPPED (`Q60`).** The reusable stencilled article is now **`Spool`** in `01_Lookup`; the material record is now **`SpoolProcessing`** in `03_Materials`; `CarrierNo` → `SpoolNo`. ⚠ **A stale `Spool` reference is now *silently wrong*, not obviously stale** — see `[DBD §6.2a]`, the naming convention this closed. Object counts: the merge took the baseline to **33 tables · 55 FKs · 69 index statements** — `[DBD §6.2]` is the defining site. *(previously August 20, 2026 — **§11 gains seven open items, and two of them are a register repair.** From the [20 Aug client call](../95-archive/source-documents/ClientCall_2026-08-20_SyncPlan.md): **`OI-118`** (`RodStaging` cannot host an FL2 pre-check-in — the client reversed `FR-031`, and a published table count moves **28 → 31**), **`OI-119`** (`SpoolProcessing.OrderNo` and `SpoolCheckin.OrderId` are scalar against…)*)*)*)*
...
One row per **tool**, not per size. The name is the client's own term, from the Machines Application **Tooling Inventory** tab; that tab carries **four** tool types (`D-42`) and this is the first — edger and straightener inventory are **`G77`**, not this table, and the fourth is `ToolingInventoryRollSet` below.
...
| `StandId` / `EdgerId` | INT | NULL | FKs → `Stand` / `Edger`. **`DrawerId` was dropped 2 Sep 2026** with the die split — a `DB1`/`DB2` row carries neither and states its die *size* in `ParameterValue` |
...
**WIP station registration** (`../30-database/scripts/10_CommonDB_Insert_WIPStations_FlatWire.sql`) creates:
...
| `../30-database/scripts/10_CommonDB_Insert_WIPStations_FlatWire.sql` | 28 Jul 2026 (D2 revised 29 Jul) | current — Draft; `machine_type` and StationType pending sign-off | §5.12, §11 |
