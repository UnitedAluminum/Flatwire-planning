...
|---|---|---|---|
| **Lookup / Reference** | `01_Lookup` | 7 | `Stand` · `Drawer` · `Edger` · `Dancer` · `Spool` · `AlloyProperty` · `PayoffPosition` — *`SpoolConfiguration` merged into `Spool` 23 Aug 2026 (`Q60`)* |
...
| `TotalFeetAllowed` | DECIMAL(10,2) | NULL | Die life — scheduled life, the maximum footage before the die is pulled; set **lower** on a reconditioned die. NULL until the client supplies thresholds (**OI-77 / OQ-83**). `CK_Drawer_TotalFeetAllowed`: > 0 when set |
...
> object counts is `[DBD §6.2]`, now **33 tables · 55 FKs**.
