...
**Last Updated:** September 9, 2026 (`G120` resolved) — the seven verbatim `[REQ]` duplicates of the FL2 no-live-gauge claim are corrected: **`FR-120` superseded**, `FR-137`'s reason replaced, `FR-421` simplified, plus the line-comparison table and the `RunReading` / `[API]` / `[SIG]` restatements. ⚠ **`OI-74`'s "not an alert ladder" inference is superseded** by `Q19`'s direct answer, and its other three legs closed by `Q18` / `Q20` *(previously September 7, 2026 (`G23` closed — the canvas is 1920 × 1080) — **§7.5's *Authored canvas* moves to 1920 × 1080, and `OI-96` is CLOSED.** The client confirmed the shopfloor panel (`Q26`), which closed gap `G23` in the same pass. ⛔ **The residual is not resolution but *size*:** the panel's physical diagonal is recorded nowhere, and the 14 px floor and 48 px tap targets track **dpi, not resolution**. ⚠ §7.3's `flat-wire-fit.js` description is unchanged **on purpose** — that script really does hard-code the mockups' 1280 box, and it was never the authority for either figure. *(previously August 29, 2026)* · **8 Sep 2026 (`D-56`): `LineId` is renamed `MachineName` throughout** — same `VARCHAR(5)` shape, same `CHECK` values, operator-visible labels unchanged. `FW-N17`/`FW-N18`/`FW-N19`.)*
...
*This document's change log — the former Appendix B, together with the entries that followed it — lives in [`../CHANGELOG.md`](../CHANGELOG.md) under `./MasterSpecification.md`. Per repository convention there is one change log for the whole repository; no document keeps its own.*
...
United Aluminum is entering a new market: **oscillate-wound flat wire** produced from aluminum rod, instead of traditional pancake (strip) coils. Three new **Flattening Lines** — FL1, FL2 and the hybrid FL3 — convert aluminum rod into coreless oscillated flat-wire coils through wire drawing, controlled flattening, optional annealing and finish rolling [REQ §1.3; `./EndToEndProcess.md`].
...
| Data model | The standalone `FlatWireDB` schema (**33 tables**) plus the named legacy integration points in the shared databases |
...
[Scope split per `../00-overview/Roadmap.md` scope note and SRS §1.2 / Appendix D.]
...
- **No printed traveler.** The traveler is fully digital; the "Print Traveler" action is disabled for flat wire. Coil, spool and skid **labels are still printed** [decision Apr 28 2026, `../95-archive/design-notes/FlatWireShopfloorDashboards.md` design principle 8].
...
- **No software stop command to the PLC.** The application is a gatekeeper that reads line state; the operator always stops the machine physically [`RCO007`; `screens/RodCheckout.md`].
...
| Dimension | State |
|---|---|
| Requirements | Consolidated SRS v3 (`MVP-1/SRS/Shopfloor_Flat_wireSRS_Consolidated_v3.docx`) — **removed from the repository 1 Aug 2026**, in git history at `6096921`; §4 below and `./BusinessRequirements.md` now carry its rules. ~29 feature areas, requirement IDs `OL`/`PCI`/`CHK`/`WLD`/`TRV`/`ORD`/`PSM`/`GWT`/`SPC`/`ALT`/`STP`/`WBK`/`PR`/`FRT`/`DAT`/`WRJ`/`PKG`/`RCO`/`PRC`/`LST`/`ARM`/`PSL`/`SHS`/`RAJ`/`DCH`/`DMG`/`HMI`/`SCD`/`OEE`/`PRN`/`DM`/`INT`/`NFR` |
| Schema | Designed, scripted, and validated on SQL Server 2019 — 35 tables, 58 FK constraints, 75 index statements, one trigger, one read proc, idempotent build+seed. `[DBD §6.2]` is the defining site |
...
| Gauge trace | **Real-time** (live AGC feed), ~10 Hz | **Real-time** (live AGC feed from the gauging stands downstream of S3), **4 s** | **Real-time**, end to end |
...
| Pre-check-in supported | **Yes** | **Yes** — *client reversal, 20 Aug 2026 (`FR-533`, `§5.29` of `[REQ]`)*. ⚠ It is a **validation queue, not a staging bay**: `PCI002`'s premise stands — one traversing payoff, no staging space — and only the inference from it was reversed. Shape is `[PROPOSED]` pending **`Q41`** | **Yes** |
...
**Authoritative equipment configuration is `./BusinessRules.md` §3**, as corrected by the client on May 21 2026 (FL1 has no edger) and **Aug 4 2026 (FM2 roller sizes)**. Superseded, in any source of any date: an edger on FL1; FM2 shown as `8" → 6"S1 → 6"S2` with S2 final; **FM2 shown with four stands, with a separate `8" Roller` component, or with a 6" stand named S1**. FM2 has **three** stands — **S1 (8"), S2 (6"), S3 (6", final)**.
...
| **TKUP-2** traversing take-up | FL2 / FL3 | Finished coreless coil | **1,100 lb** (OQ-77, revised up from 1,000 lb May 4 2026) | No |
...
| Output coil, mid-run child | `FW-#####-C##-A` | `FW-00421-C01-A` | On a product-spec change mid-run | Per OQ-61 cases 2, 3, 5 |
...
**Material status** — `Rod.Status`, `SpoolProcessing.Status`, `CoilOutput.Status`, `RodCheckout.NewRodStatus`, and the shared `coils.coil_status`. ⚠ **Since `D-32` (18 Aug 2026) the two vocabularies are no longer the same set.** The shared column keeps exactly the values it has today, so **`INFLAT` exists only in the `FlatWireDB`-local columns**. The module may still write the shared column with values that already exist there — a rejection putting the rod on `HOLD`/`SCRAP`, for instance — because that is a data write, not a migration:
...
| `INFLAT` | Rod, Spool | **In-process on a flattening line.** ⚠ **`FlatWireDB`-local only** since `D-32` (18 Aug 2026) — it lives on `Rod.Status`, `SpoolProcessing.Status` and `RodCheckout.NewRodStatus`. `FW-002` would have added it to the **shared** scheduling schema as well; that story is cancelled, so the shared `coils.coil_status` vocabulary never gains it |
...
**Edge type** — domain values are the client's **four product edge profiles** (`D-61`, 10 Sep 2026): **`Natural Round Edge`** (no edging) · **`Full Round Edge`** · **`Square Edge`** · **`Broken Edge`**, enforced by `CK_PSC_EdgeType`. ⚠ **The TOOL takes only three of them** — `CK_TIE_EdgeType` on `ToolingInventoryEdger` excludes `Natural Round Edge`, because *no edging* means no set is ground for it. ⛔ **Do not merge or align the two constraints**; they differ in cardinality on purpose. ⚠ **`CK_Edger_EdgeType` no longer exists** — removed with `[dbo].[Edger]` by `D-53`; the survivor is `CK_TIE_EdgeType`. ⭐ **These values are already operator-readable**, so the `"Round Edge" / "Flat Edge"` display pipe that `[API §2.1]` specifies may be unnecessary — `FW-132` owns that decision, and its own ACs still hardcode the old pair. The `Bevel edge` option in the Dashboard 9 / 9A Generate modal still has **no domain value**; it may be `Broken Edge` under another name — see `OI-05` and `Q110` item 1.

**Spool working states** (`ACTIVE` / `IN-PLAN` / `IN-USE` / `COMPLETED`) appear in the planning narrative and are a **second, unreconciled vocabulary** against the material statuses above. OQ-17 owns the reconciliation; see OI-06.
...
| **Dancer** | Tension-management roller between components. **FM2 carries two** — between S1/S2 and between S2/S3 — each running in **dancer mode** (compensating speed control) or **tension mode** (`D-28`, 6 Aug 2026). `PSM012`'s *"tension is derived from speed, never entered manually"* holds **in dancer mode**; whether tension mode takes a setpoint, and from where, is **Q32**. **The same question is asked client-facing as `PSG-Q29`, with the setpoints as `PSG-D27`** — one answer closes both, and the client should not be asked twice. Tension also has an engineering consequence the register does not carry: applied tension **reduces roll separating force**, so it moves both the force check and the roll gap (`[PSG §3.3.6]`) |
...
| **Skid** | Packaging unit carrying exactly two finished coreless coils (R48) |
...
| 4 | On success, rod alpha `R#####` assigned (no gaps, per lot). `coils` entry created: gauge populated; **width, OD, ID and surface finish blank** — rods have no width. Inventory type TBD (OI-49) | Coil Receiving | `coils` row, status `RECEIVED` |
...
> **DECIDED (client, 30 Jul 2026): `INFLAT` is set only at check-in.** ⚠ **Overtaken by `D-32` (18 Aug 2026): `INFLAT` is now set only in `FlatWireDB`, on `Rod.Status`. Neither pre-check-in nor check-in writes `coils.coil_status` at all** — `FW-002` is cancelled with the shared-schema migration. The 30 Jul answer still governs the **timing** (check-in, not staging) and *"no intermediate status for a welded-but-not-checked-in rod"* still holds; only the column it is written to has changed. ~~Whether pre-check-in itself sets `coils.coil_status = INFLAT` (SRS 4.2 data note) or leaves it `STAGED` (process walkthrough) is OQ-68, Critical.~~ Pre-check-in does **not** commit the shared status, and there is **no intermediate status** for a welded-but-not-checked-in rod. The process walkthrough wins and the SRS §4.2 `PCI` data note is superseded on this point. **Residual:** whether the reqsum / `wip_coil_orders` insert from that note stays at staging is unanswered (OI-01). ~~The interim design follows the SRS and treats `RodStaging.Status` (bay occupancy) as orthogonal to `coils.coil_status`, which makes rod status `STAGED` effectively vestigial for FL1. See OI-01.

**Spool**: `RECEIVED → STAGED → INFLAT → COMPLETE`, plus `HOLD` and `SCRAP`. An intermediate anneal **modifies the existing alpha** — no child alpha is issued (OQ-78). Re-passing a spool through FL1 is not possible (no capability). The full transition set is OQ-17, In Progress.
...
| Approval | Operator *(OQ-69 open)* | Operator | **Supervisor** (OQ-74, Decided) |
...
10. **Certification and shipment:** C of C generated with chemistry, mechanical properties, dimensional data, alloy, temper and traceability to source rod heat. Welding-wire customers additionally require every weld join traceable, and may impose a contractual maximum weld count per coil (limit TBD — OQ-7).
...
The **Scrap Box vs Scrap Skid** outlet selection is a new option that also benefits Conveyors and Inspection. Banding material (steel vs aluminium alloy) is unresolved (OI-83).
...
> ⚠ **This section is a restatement, and it has drifted. `[REQ]` is the register of record.**
> [`BusinessRequirements.md`](./BusinessRequirements.md) self-declares as *"the
> `FR-###` register"*, and §4 here is a grouped narrative over it — useful for reading by workflow, not
> authoritative on membership.
>
> **As of 25 Aug 2026 §4 runs to `FR-508`; the register runs to `FR-560`.** **Fifty-two** requirements
> are not restated here — enumerated, so the figure cannot drift again:
>
> | Block | Count | Added | Subject |
> |---|---|---|---|
> | `FR-509`–`FR-518` | 10 | 18 Aug 2026 | FL2/FL3 run-end write-back |
> | `FR-561`–`FR-570` + `FR-561a`–`e` | 15 | 26 Aug 2026 | **Welded-coil part alphas and the shared coil master** (`Q88`, `Q89`) — ⛔ **`FR-512` deleted**, `FR-509`/`510`/`511`/`513`/`514`/`516`/`518` re-specified per-record, `FR-335`/`339`/`515` counting basis moved to **physical** coils |
> | `FR-519`–`FR-532` | 14 | 19 Aug 2026 | Check-in write-back and inbound ingestion |
> | `FR-533`–`FR-540` | **8** | **25 Aug 2026** | **FL2 pre-check-in** — the `FR-031` reversal |
> | `FR-541`–`FR-560` | 20 | 22 Aug 2026 | Rod ↔ order allocation |
> | | **52** | | |
>
> ⚠ **This note read “Fifty-two … twenty-four of them predate this wave” on 22 Aug 2026, when the
> true figure was 44.** It over-counted by exactly eight, and the `FR-533`–`FR-540` block added on
> 25 Aug 2026 has made it accidentally correct. Enumerated above rather than left as a bare total
> for precisely that reason.
>
> **None of it is transcribed here, deliberately** — copying one block into a drifted restatement
> would duplicate the register and deepen the inconsistency rather than close it.
>
> **Read §4 for the workflow grouping and `[REQ]` for the requirement set.** Reconciling the two is a
> separate pass and should be done in one sweep, not one block at a time.
...
**Screen:** [`../50-frontend/mockups/dashboard_2a_rod_precheckin.html`](../50-frontend/mockups/dashboard_2a_rod_precheckin.html)
...
| **FR-044** | Staging shall **refuse** a rod with no `planning_routings` allocation, a rod that is not available (`coils.coil_status` in `COMPLETE`/`HOLD`/`SCRAP`, **or `Rod.Status = 'INFLAT'`**, or already staged elsewhere), and a rod belonging to a **different order** once an order is established (welding across orders would break coil genealogy). ⚠ **Changed by `D-32` (18 Aug 2026):** `INFLAT` is no longer a shared `coil_status` value, so the *"already on a flattening line"* limb of this test is served from **`FlatWireDB`-local** state — `Rod.Status`, or an open `RodCheckin` — not from the shared row. The `COMPLETE`/`HOLD`/`SCRAP` limbs are unchanged. |
...
**Screen (approved):** [`../50-frontend/mockups/dashboard_2_rod_checkin.html`](../50-frontend/mockups/dashboard_2_rod_checkin.html) — a guided 6-step tab wizard. **FL3 variant:** [`../50-frontend/mockups/dashboard_2_rod_checkin_fl3.html`](../50-frontend/mockups/dashboard_2_rod_checkin_fl3.html) *(still on the older single-page layout; a wizard-shaped FL3 variant is outstanding — OI-16)*.
...
| **FR-077** | On successful check-in the system shall update the `coilno` field in WIP stations, perform reqsum and insert `wip_coil_orders` if the rod is not yet reqsummed, and update `actual_start_date` in `planning_routings` and `routings`. It shall set **`Rod.Status = 'INFLAT'`** in `FlatWireDB`. ⚠ **Changed by `D-32` (18 Aug 2026):** ~~set `coils.coil_status = INFLAT`~~ — struck. `FW-002` is cancelled and the shared status vocabulary never gains the value, so in-process state is recorded locally and **nothing writes `INFLAT` to `coils.coil_status`**. Shared-column writes using values that already exist there (a rejection setting `HOLD`/`SCRAP`) are unaffected, as are the three other shared writes; all land in existing columns. **`OI-111`** carries the consequence for upstream visibility. [`CHK019`, `DM002`] |
...
| **FR-083** | For **FL3**, one acknowledgement shall push **all FM1 and FM2 tags in a single batch**; `FlatWireRun.RouteMode` shall be `Hybrid`; **no `SpoolProcessing` row shall be created**. |
...
**State changes on success:** `FlatWireRun` created with `Status='Running'` and `StartedAt`; `RodCheckin` written; `SpcCheckpoint(PreRun)` + measurements written; `RodStaging → CheckedIn`; **`Rod.Status = 'INFLAT'`** *(`FlatWireDB`-local — `coils.coil_status` is **not** written, `D-32`)*; PLC tags pushed; run timer started.

**Error paths:** line already has an active run → `409` · pass schedule is `Draft` → `422` · PLC push failed → `500` with the check-in aborted · inspection fail → routed to Dashboard 8 · no matching active schedule → **undefined, OQ-14 Critical** (the stub assumes a single active schedule).
...
**Screen:** [`../50-frontend/mockups/dashboard_5_spool_checkin.html`](../50-frontend/mockups/dashboard_5_spool_checkin.html)
...
| **FR-096** | On acknowledgement the system shall push FM2-specific tags (**S1/S2/S3 roll gaps and stand states, edger activation and edge type at S2 and S3**), set `SpoolProcessing.Status = INFLAT`, create the FL2 `FlatWireRun` linked to the source spool and its source rod alphas, and start the FL2 run. |
...
**Screen:** [`../50-frontend/mockups/dashboard_5a_spool_queue.html`](../50-frontend/mockups/dashboard_5a_spool_queue.html)
**SRS:** `CHK012`; **Q17**
**Actors:** FL2 operator
**Preconditions:** none — usable on opening

*Added 2 Aug 2026. FL1 has a pre-check-in station listing the rods planned for the running order; FL2 had none, because `PCI002` was read as excluding it — **reversed 20 Aug 2026, see `FR-533`** — so the FL2 operator had **no view of waiting material at all**. `FR-090` has the operator scan the FL1-printed label and **Q17** records the client stating the operator "selects it by spool number for check-in"; both stand, and only the scan had a screen. This is the selection half, and the first thing actually named "the spool queue" — a phrase `FR-326`, `TC-389`, `RodCheckout.md` and phase 7 all use with no table, endpoint, screen or status behind it.*

| ID | Requirement |
|---|---|
| **FR-097** | Dashboard 5A shall, **on opening and without requiring a scan**, list every spool **available for processing irrespective of order** — identifier, order, source FL1 run and source rod alphas, gauge × width, net weight, origin route mode, status — with a rollup of spool count, ready count and total weight. **Gauge and width shall come from the source FL1 run**, since `SpoolProcessing.GaugeIn`/`WidthIn` are null until check-in. |
...
**Open:** which statuses count as "available" is **OI-55/Q17**; the two status vocabularies are **OI-06**; identifier and format are **OI-50** / **OI-02**; the hybrid consequence is **OI-47**. **`SpoolProcessing.OrderNo` must be populated from planning or FR-098 cannot resolve anything** — the one dependency that would invalidate the screen.

**Deliberately absent:** spool age (`SpoolProcessing` has no creation timestamp) and location (`SpoolProcessing.Location` has no writer).

### 4.4 Active Run Monitor — Dashboard 3 (FL1 / FL2 / FL3)

**Screens:** [`../50-frontend/mockups/dashboard_3_active_run.html`](../50-frontend/mockups/dashboard_3_active_run.html) (FL1 — grouped action cluster + spool-completion overlay; **the sole FL1 layout since 1 Aug 2026**, when the earlier left-rail layout that held this filename was withdrawn; this file was named `dashboard_3_active_run_v2.html` until 11 Aug 2026), [`../50-frontend/mockups/dashboard_3_active_run_fl2.html`](../50-frontend/mockups/dashboard_3_active_run_fl2.html), [`../50-frontend/mockups/dashboard_3_active_run_fl3.html`](../50-frontend/mockups/dashboard_3_active_run_fl3.html)
...
| **FR-120** | ~~FL2 in standalone mode shall render the **historical profile**, not a live streaming trace, because the server broadcasts `null` live gauge and width for it.~~ ⛔ **SUPERSEDED 9 Sep 2026** — its sole basis was assumption `A3` of `[PLC §14]`, now retired. **FL2 falls under `FR-101` like every other line**, reading `FL2.PLC.AGC.Gauge` / `.Width` at **4 s**. Full supersession note at `[REQ]` `FR-120`; `G120`. |
...
**Component:** [`../50-frontend/mockups/spool_notification.js`](../50-frontend/mockups/spool_notification.js), hosted in `dashboard_3_active_run.html`
**Source:** `screens/SpoolCompletionNotification.md` (Parts A and B)
...
| **FR-137** | Actual spool weight shall be derived as `(current footage − footage at spool start) × lb-per-ft`, where `lb-per-ft = A(in²) × 12 × ρ` — `A` applying the round-edge correction where applicable, and ρ read from **`united_db..alloys.alloy_density`** (§5.4). **For FL2, gauge and width shall come from the pass schedule / order, not live measurement** — ⚠ **because the dimensional basis is *nominal* for every line (`Q10`, `D17`), NOT because FL2 cannot measure** *(that justification retired with `A3` on 9 Sep 2026; the behaviour is unchanged)*. Worked reference: 1100 at 0.110″ × 0.625″ gives **0.0809 lb/ft** square edge, so a **1,800 lb** spool is ≈ 22,250 ft *(the 2,000 lb figure was the default withdrawn by `D5`)*. |
...
**Screen:** [`../50-frontend/mockups/dashboard_2a_rod_precheckin.html`](../50-frontend/mockups/dashboard_2a_rod_precheckin.html) — *Mark as welded* dialog
**SRS:** `WLD001`–`WLD017`
...
| **FR-171** | The system shall validate the number of weld joints against the **maximum permitted per finished coil** (a customer contractual limit) at weld confirmation. **The limit is TBD — OQ-7.** [`WLD016`] |
...
**Dialog:** `spc_checkpoint.js` — `openSpcCheckpoint(ctx)`. Converted from a screen to a popup 1 Aug 2026; [`../50-frontend/mockups/dashboard_6_spc_checkpoint.html`](../50-frontend/mockups/dashboard_6_spc_checkpoint.html) is now a launcher that opens it. Raised over the run being measured, and over the die change that mandated it.
**SRS:** `SPC001`–`SPC015`
...
| **FR-197** | **Camber** shall be available as an SPC measurement where the customer has camber specifications — the field is available but not mandatory for all orders. [OQ-81, Decided] |

### 4.8 Roll Adjust — Dashboard 11

**Screen:** [`../50-frontend/mockups/dashboard_11_roll_adjust.html`](../50-frontend/mockups/dashboard_11_roll_adjust.html)
**SRS:** `RAJ001`–`RAJ022`
...
**Dialog:** `die_change.js` — `openDieChange(ctx)`. Converted from a screen to a popup 1 Aug 2026; [`../50-frontend/mockups/dashboard_die_change.html`](../50-frontend/mockups/dashboard_die_change.html) is now a launcher that opens it. Raised over the paused run it is logged against.
**SRS:** `DCH001`–`DCH028`
...
**Screen:** [`../50-frontend/mockups/dashboard_die_management.html`](../50-frontend/mockups/dashboard_die_management.html)
**SRS:** `DMG001`–`DMG017`
...
| **FR-255** | Cumulative footage per die shall be incremented from the **PLC footage counter** on each completed or partial run — **no new sensor is required**. On a mid-run swap the system closes accumulation on the outgoing die and starts a new counter on the incoming die. [OQ-83, Decided] |
...
**Component:** [`../50-frontend/mockups/pause_run.js`](../50-frontend/mockups/pause_run.js) — a shared dialog for the FL1/FL2/FL3 active-run screens
...
> **✅ Resolved 1 Aug 2026 — four outcomes. OI-14 closed.** The contract (`POST /run/{runId}/resume`), the schema (`CK_RunPauseEvent_Outcome`) and `../95-archive/design-notes/FlatWireShopfloorDashboards.md` all specified four; only `pause_run.js` dissented. It now implements four and `FR-262` is superseded. Beyond the contract, the deciding argument is that a reason which uniquely does **not** pause, presented like fourteen that do, is a trap on a touch panel — and Mode B needs the line stopped anyway, so reaching it *through* a pause is the truthful sequence.
...
**Dialog:** `rod_checkout.js` — `openRodCheckout(ctx)`. Converted from a screen to a popup 1 Aug 2026; [`../50-frontend/mockups/dashboard_12_rod_checkout.html`](../50-frontend/mockups/dashboard_12_rod_checkout.html) is now a launcher that opens it. The **caller states the mode**: Mode A from the check-in station (no footage), Mode B from a paused run, which passes the footage frozen at that pause straight through.
**SRS:** `RCO001`–`RCO051`
...
**Screen:** [`../50-frontend/mockups/dashboard_7_coil_completion.html`](../50-frontend/mockups/dashboard_7_coil_completion.html)
...
| **FR-338** | The system shall write the **pass schedule ID, version and effective configuration snapshot** to the output coil record at coil creation, for technical traceability and quality audits — and shall **not** print that data on the customer label. [`PSM024`, OQ-64 Decided] |
...
**Screen:** [`../50-frontend/mockups/dashboard_7b_packing_station.html`](../50-frontend/mockups/dashboard_7b_packing_station.html)
**SRS:** `PKG001`–`PKG004`
...
**Screen:** [`../50-frontend/mockups/dashboard_9_pass_schedule.html`](../50-frontend/mockups/dashboard_9_pass_schedule.html)
**SRS:** `PSM001`–`PSM024`
...
> `screens/PassScheduleGenerationSpec.md` v1.5 (6 Aug 2026) contradicts them
...
>
> ⚠ **A SIXTH item, 2 Sep 2026 — the entry-diameter *solve*, not just `FR-381`'s expression.** `D-43`
> supplies the client's own round→flat spread correlation, so `[PSG §6.3 Step 2]` no longer stops at
> the zero-elongation bound. On the `FW-013` acceptance case (rod 0.375″, 0.125″ × 0.875″) the solved
> entry diameter is **0.3823″ — larger than the rod** — and `[PSG §6.3 Step 3A]`'s rod-adequacy margin
> moves **+0.98% → −3.93%**, so the correct output is a **rejection**, not the schedule the criterion
> asserts. See `[05-Backlog-MVP2.md]` and `G90`.
...
> **Worked example, corrected.** `04-APIContract.md` publishes a worked example for alloy 1100, rod 0.375", gauge 0.125", width 0.875" that is **internally inconsistent on three counts** (REVIEW Tier 1 #1). The corrected numbers are:
>
...
These values require Process Engineering sign-off and are maintained via the Phase-13 alloy-lookup admin grid. The matching copy is [`../95-archive/design-notes/FlatWireShopfloorDashboards.md` § **Alloy Reference Data**](../95-archive/design-notes/FlatWireShopfloorDashboards.md) — a **top-level MVP-1 section** since 11 Aug 2026, when it was lifted out of the MVP-2 Dashboard 9 section it had been buried in. Both copies must be changed together, and both map onto `AlloyProperty`.

### 4.19 Pass Schedule List — Dashboard 9A

**Screen:** [`../50-frontend/mockups/dashboard_9a_schedule_list.html`](../50-frontend/mockups/dashboard_9a_schedule_list.html)
**SRS:** `PSL001`–`PSL020`
...
**Screen:** [`../50-frontend/mockups/dashboard_1_line_status.html`](../50-frontend/mockups/dashboard_1_line_status.html)
**SRS:** `LST001`–`LST019`
...
|---|---|
| The **machine tag map** | [`../20-architecture/PLCTagSpecification.md`](../20-architecture/PLCTagSpecification.md) §3, split per line — now the only tag map |
...
**Screen:** [`../50-frontend/mockups/dashboard_10_shift_summary.html`](../50-frontend/mockups/dashboard_10_shift_summary.html)
**SRS:** `SHS001`–`SHS015`
...
**Screen:** [`../50-frontend/mockups/dashboard_oee.html`](../50-frontend/mockups/dashboard_oee.html)
**SRS:** `OEE001`–`OEE017`
...
**Table count: the counted object baseline is `[DBD §6.2]`**, which is the only site that states it. Other counts appear across the repo (21, 22, 25, 27, 28, 32) and are all superseded — see §10 decision D-03 for the history. *(`FlatWire_ERDiagram_Documentation.md` was absorbed into `[DBD]` §6–§7 on 13 Aug 2026 and deleted; it is no longer the as-built description.)*

| Group | Script | Count | Tables |
|---|---|---|---|
| **Lookup / Reference** | `01_Lookup` | 8 | `Stand` · `Drawer` · `ToolingInventoryDie` · `Edger` · `Dancer` · `Spool` · `AlloyProperty` · `PayoffPosition` — *`SpoolConfiguration` merged into `Spool` 23 Aug 2026 (`Q60`)* |
| **Schedule** | `02_Schedule` | 3 | `PassSchedule` · `PassScheduleComponent` · `PassScheduleChangeLog` |
| **Materials** | `03_Materials` | 3 | `Rod` · `FlatWireRun` · `SpoolProcessing` |
...
erDiagram
    ...
    ToolingInventoryDie ||--o{ DieChangeEvent : ""
    ToolingInventoryDie ||--o{ DieHistory : ""
    ToolingInventoryEdger ||--o{ PassScheduleComponent : ""
    ToolingInventoryEdger ||--o{ ToolingInventoryEdgerGauge : ""
...
#### `ToolingInventoryDie` — the register of physical dies

One row per **tool**, not per size. The name is the client's own term, from the Machines Application **Tooling Inventory** tab; that tab carries **four** tool types (`D-42`) and this is the first — the second is **`ToolingInventoryEdger`** (`D-53`, 6 Sep 2026, which **absorbed** `Edger`), the fourth is `ToolingInventoryRollSet` below, and only the **straightener** is still unbuilt (`G77`).
...
| `LubricationType` | VARCHAR(50) | NULL | Client `Lubrication Type` |
| `DieType` | VARCHAR(20) | NULL | `TC Mono` · `TC Poly` · `Natural diamond` (`FR-247`) |
| `LastGrindingFeet` | DECIMAL(10,2) | NOT NULL | Die life — feet run **since** the last grinding/reconditioning; a resettable counter, **not** the reading *at* that grind. Default 0 |
| `TotalFeetAllowed` | DECIMAL(10,2) | NULL | Die life — scheduled life, the maximum footage before the die is pulled; set **lower** on a reconditioned die. NULL until the client supplies thresholds (`OQ-83`) |
| `LifecycleStatus` | VARCHAR(20) | NOT NULL | `Active` · `In Service` · `In Grinding` · `Retired`. Default `In Service` |
| `InUse` | BIT | NOT NULL | Client `In Use`. Default 0 |
| `Source` / `Condition` / `InspectionDate` / `Notes` / `LastResetBy` / `LastResetAt` | — | NULL | `FR-247`, `FR-245`, `FR-248` |
| `IsActive` | BIT | NOT NULL | default 1 |

> **Die life moved here 2 Sep 2026, unchanged in meaning.** `LastGrindingFeet` and `TotalFeetAllowed` are §4.10's *"footage on die"* and *"scheduled life"*; the screen derives `Remaining` and `Life used %`. There is deliberately **no** `LastGrindingFeet ≤ TotalFeetAllowed` constraint — *overdue* is a real operating state, not a data error. What changed is the **grain**: the counter accumulates against a physical tool, so **`OI-41` closes** and its accepted consequence — *"two dies of one diameter share a counter"* — is retired. `DieChangeEvent` gained `OldDieId` / `NewDieId`, so a run event can attribute footage to a tool and **`FR-255`** is implementable; **`FR-233` / `D4` revert to their pre-11-Aug per-tool form**.

> ⚠ **Two status vocabularies, resolved by derivation.** The client's grid has three values (`Active` · `In Service` · `In Grinding`); `FR-253` has five. Three of `FR-253`'s five are **percentage bands** — so `LifecycleStatus` stores the client's three plus `Retired`, and `Nearing` / `Overdue` / `Spare` are **derived** from `LastGrindingFeet` against `TotalFeetAllowed`, with `Spare` as `InUse = 0 AND LastGrindingFeet = 0`. **Both vocabularies contain `Active` meaning different things**; the client's value is quoted, not edited. `ID(MM)` is not stored — it is a derived display value (`0.343 × 25.4`, shown as `8,700`).

> ⚠ **`OI-141` is open on ownership.** §4.10 puts Die Management on the Machines Application → Tooling Inventory tab while `FR-254` makes it the runtime source of truth, and the client's grid carries **none** of the four values `FR-254` names. The column set above is the **union** of both field sets. If `OI-141` resolves to *two* registers, this is the `FlatWireDB` one and `FR-254`'s exclusivity needs restating. ⚠ **`OI-12` escalates**: the 60/85 % and 65/80 % band sets now derive from one table.

#### `DieHistory` — one log for a die's whole life

Serves **both** of `FR-252`'s tabs — Run history and Replacement log.

| Column | Type | Null | Notes |
|---|---|---|---|
| `Id` | INT IDENTITY | NOT NULL | PK |
| `DieId` | INT | NOT NULL | FK → `ToolingInventoryDie.Id` |
| `EventType` | VARCHAR(20) | NOT NULL | `Install` · `Reset` · `Retire` · `ThresholdEdit` · `RunFootage` |
| `RunId` | VARCHAR(20) | NULL | FK → `FlatWireRun.RunId`. **NULL when made outside a run** |
| `FootageAddedFt` | DECIMAL(10,2) | NULL | `RunFootage` rows only |
| `OldValue` / `NewValue` | VARCHAR(100) | NULL | Threshold edits and resets |
| `ReasonCode` / `ReasonNotes` | — | NULL | Retire reason (`FR-250`) or reset disposition (`FR-248`); `FR-249` requires a reason on an edit |
| `RemovedFromLineDate` / `ReturnedReadyDate` / `InspectionDate` / `DieRoomSource` | — | NULL | `FR-248` |
| `OperatorId` | VARCHAR(50) | NOT NULL | |
| `Timestamp` | DATETIMEOFFSET | NOT NULL | default `SYSDATETIMEOFFSET()` |

> **Why one table.** `PassScheduleChangeLog` already sets the pattern — a discriminated append-only log with a nullable `RunId` and the same *"NULL when made outside a run"* semantics. Two CHECKs recover the typing that costs: `CK_DieHistory_RunFootageHasRun` and `CK_DieHistory_FootageOnlyOnRunFootage`.
>
> **Why it exists.** `FR-248`/`FR-250` are die-room actions with **no run**, and `DieChangeEvent.RunId` is `NOT NULL` — so they cannot live there; and **nothing else records which physical die ran a given run**, so `FR-252`'s per-run footage is not derivable. `FR-252`'s *order* and *line* are **not** columns — derived through `RunId`.
>
> ⚠ **`LastGrindingFeet` is denormalised** against `SUM(FootageAddedFt)`, deliberately: `FR-254` wants one cheap column, `FR-252` wants the rows. The total is authoritative for the screens; the log explains it. The invariant lives in the application beside `FR-255`'s increment, **not** in a trigger (`G41` — `RunId` is nullable).

#### `ToolingInventoryRollSet` — the register of physical roll sets

The **fourth** Tooling Inventory tool type (`D-42`, 3 Sep 2026). `Stand` and `Drawer` are the **positions**; this is the physical **tool** fitted to one of them — the same split the die took on 2 Sep.

| Column | Type | Null | Notes |
|---|---|---|---|
| `Id` | INT IDENTITY | NOT NULL | PK |
| `RollSetAlpha` | VARCHAR(20) | NOT NULL | UNIQUE. `RS-{position}-{seq}` — **ours**; no alpha appears on any client tooling grid (`OI-141`) |
...
| `OdIn` / `MinOdIn` / `IdIn` | DECIMAL(8,4) | NULL | The **grind** life model |
| `SerialNo` / `PartNo` / `Location` | VARCHAR | NULL | Client grid. `SerialNo` unique when set, **filtered** |
| `LifecycleStatus` | VARCHAR(20) | NOT NULL | `Active` · `In Service` · `In Grinding` · `Retired` |
| `IsRefurbishable` | BIT | NOT NULL | *"they can be refurbished"* — the client's word for the capstans. Default `0` |
| `DateOfChange` / `DateOfLastGrind` | DATE | NULL | Client grid |
| `InUse` / `Notes` / `IsActive` | — | — | As on the die |

> **`CK_TIRS_Mount` is the whole design.** Exactly one mount, agreeing with the discriminator: a `Mill` row has `StandId` and no `DrawerId`, a `Capstan` row the reverse. Two parents would normally argue for two tables; the client named both in one breath as one tab option, and `DieHistory` set the precedent for one discriminated table serving two shapes.
>
> ⚠ **`Stand.RollDiameterIn` remains authoritative for the machine** (`D-26`, `[PLC §5.4]`). `NominalDiameterIn` is the tool's own size. Neither is stale.
>
> ⚠ **The life model is grind, not footage** — a roll is reground to a minimum OD. **No footage counter**, deliberately; `G77` records that the two models differ.
>
> ⛔ **Every column is `[PROPOSED]` until `Q92` returns — `G87`.** Die, Edger and Straightener each arrived as a screenshot grid; roll sets arrived as **one sentence**.

#### `ToolingInventoryEdger` — the register of physical edger roll sets

The **second** of the four Tooling Inventory tool types (`D-42`), built 6 Sep 2026 from the client's
fourteen-column grid of 31 Aug 2026. **`D-53`.**

> ⭐ **This ABSORBS `Edger`; it does not sit beside it.** `Edger` held five columns against fourteen,
> its `IsActive BIT` could not express `In Grinding`, and its seed (`EDGE-ROUND-A`) named a **profile**
> while its `ToolingSetNo` named a **physical set** — exactly `G77`'s grain complaint.
> [`FlatWire_DDL_02_Schedule.sql`](../30-database/sql/FlatWire_DDL_02_Schedule.sql) already recorded
> that `EdgerId` *"identifies the fitted **TOOL**, not the STATION"*, so `Edger` was already meant to
> be this register and was simply too thin to be one. **`PassScheduleComponent.EdgerId` re-points here
> and `FK_PSC_Edger` keeps its name.**

> ⚠ **Not the die split's shape.** That split kept `Drawer` as the two draw **boxes**. There is no
> edger equivalent to keep: `E1`/`E2` exist nowhere as rows, and `CK_PSC_ComponentName` still offers a
> single `EdgeSet` value for two physical stations. **That station gap is untouched and stays open.**
...
| `SerialNo` | VARCHAR(50) | NULL | **`[PROPOSED]`** — the edger grid has **no** `S/N` column (`G104`). Filtered-unique |
| `RollQty` | INT | NOT NULL | Client grid `Roll Qty`. Default `2` |
| `StdRemovalFromOdIn` | DECIMAL(8,4) | NULL | Client grid `STD Removal From OD(")` — `.100` per grind |
| `OdIn` / `MinOdIn` / `IdIn` | DECIMAL(8,4) | NULL | Client grid `OD(")` / `Min OD(")` / `ID(")` — `6.00` falling to `4.75` |
| `DateOfChange` / `DateOfLastGrind` | DATE | NULL | Client grid, both |
| `LifecycleStatus` | VARCHAR(20) | NOT NULL | Client grid `Status` + `Retired`. Default `In Service`. **A `BIT` could not express `In Grinding`** |
| `InUse` | BIT | NOT NULL | default 0 |
| `Notes` / `IsActive` | VARCHAR(500) / BIT | — | As elsewhere |

> ⚠ **`EdgeType` is relaxed to NULL, and it is ours.** The client's fourteen columns do not classify a
> set by edge profile in a way we can rely on. Requiring it would invent a mandatory classification the
> client has never supplied — the `G87` mistake. `CK_PSC_EdgeTypeReq` still forces the **schedule** to
> state its profile, which is where the requirement belongs. `Q95` asks.

> ⚠ **`EdgerType` carries the grid's `Type` cell separately, and whether the two are one field is
> open.** They are not merged on our own reading, because `ToolingInventoryDie` reads the same heading as
> the die **material** (`DieType`) — so it demonstrably does not mean one thing across the grids. `Q95`.

> ⭐ **No key but the `IDENTITY`.** `Name` was removed on 6 Sep 2026 and **`EdgerToolAlpha` on 7 Sep**,
> both once it was **verified, not assumed**, that nothing read them: no procedure, view, join or seed
> row referenced `Name`, and the schedule seed resolves `EdgerId` by `Id`; no client tooling grid has
> ever shown an alpha (`OI-141`). The client identifies a set by `Machine Name` + `Set Number` + `P/N`.
>
...
> ⚠ **The life model is grind, not footage.** `.100` off `OD 6.00` down to `Min OD 4.75` is about
> twelve grinds. **No footage counter** — do not add die-life columns here by analogy.

#### `ToolingInventoryEdgerGauge` — one row per groove

| Column | Type | Null | Notes |
|---|---|---|---|
| `Id` | INT IDENTITY | NOT NULL | PK |
| `EdgerToolId` | INT | NOT NULL | FK → `ToolingInventoryEdger`. `NO ACTION`, like every key in the schema |
| `GaugeIn` | DECIMAL(8,4) | NOT NULL | One groove: `.0450` / `.0400` / `.0350`. `CK_TIEG_GaugeIn > 0` |
| `GrooveNo` | TINYINT | NULL | **`[PROPOSED]`** — the grid neither numbers nor orders the grooves (`G104`) |
| `IsActive` | BIT | NOT NULL | default 1 |

> ⭐ **Why a child table — `G77` asked for this in terms.** `Gauge Range(")` is multi-valued in one
> client cell (`.045, .040, .035`, three grooves on one roll). The failure mode a delimited `VARCHAR`
> causes is specific: **a pass schedule could never select a groove**, only a whole tool.
> `UQ_TIEG_ToolGauge (EdgerToolId, GaugeIn)` leads on the FK, so no separate index is created.

#### ~~`SpoolConfiguration`~~ — **merged into `Spool`, 23 Aug 2026 (`Q60`)**

> The size class held **one** meaningful row against 30–45 articles, so its six `Min/Max` dimensional
> columns and its `Name` (now `SizeClass`) were folded into `Spool` itself. **The table no longer
> exists.** The column list below is retained as the record of what merged. Defining site for the
> object counts is `[DBD §6.2]`, now **35 tables · 58 FKs**.

##### Former columns — spool type constraints
...
| `SpeedRangeMinFpm` / `SpeedRangeMaxFpm` | Flattening-line speeds, trial-derived (OQ-66) |
...
| `StandId` / `EdgerId` | INT | NULL | FKs → `Stand` / **`ToolingInventoryEdger`** (re-pointed 6 Sep 2026, `D-53`; `FK_PSC_Edger` keeps its name). **`DrawerId` was dropped 2 Sep 2026** with the die split — a `DB1`/`DB2` row carries neither and states its die *size* in `ParameterValue` |
...
This table satisfies OQ-62 and backs the Dashboard 9 Change History tabs (Overrides / Schedule edits / Acknowledgments).
...
| `InventoryType` | VARCHAR(20) | NULL | Planning/cost classification — **OI-49 PROVISIONAL** |
...
#### `SpoolProcessing` — pre-drawn intermediate spools

| Column | Type | Null | Notes |
|---|---|---|---|
| `Id` | INT IDENTITY | NOT NULL | PK |
| `Alpha` | VARCHAR(20) | NOT NULL | **UNIQUE** — e.g. `SP-00021` |
| ~~`SpoolTypeId`~~ | INT | — | **Dropped 23 Aug 2026** with the `SpoolConfiguration` merge; the article and its limits are reached through the nullable `SpoolId` |
...
#### `RodOrderAllocation` — the rod ↔ order plan `[PROPOSED]`

**Added 22 Aug 2026.** The rod ↔ order many-to-many, which nothing in the repository persisted — it existed
only implicitly in `united_db..planning_routings`, read and never written. One row per (rod, order) pairing.

| Column | Type | Null | Notes |
|---|---|---|---|
| `Id` | INT IDENTITY | NOT NULL | PK |
| `RodAlpha` | VARCHAR(20) | NOT NULL | FK → `Rod.Alpha` |
| `OrderNo` · `RelLetter` | VARCHAR(50) · VARCHAR(10) | NOT NULL · NULL | **No FK** — orders are shared-schema and `D-32` forbids altering it. Same basis as `PlanId` / `SkidId` |
| `OrderSeqNo` | SMALLINT | NOT NULL | the order's position in the station's queue — what makes *which boundary* computable |
| `RodSeqNoInOrder` | SMALLINT | NOT NULL | planning's rod sequence within the order |
| `AllocatedWeightLb` | DECIMAL(8,2) | NOT NULL | planning's number, never derived |
| `RodWeightFrom` · `RodWeightTo` | DECIMAL(8,2) | NOT NULL | rod-local cumulative **pounds**, half-open. The split point is the outgoing row's `To` = the incoming row's `From` |
| `PinRole` | VARCHAR(12) | NOT NULL | `Sole` · `PinnedFirst` · `Free` · `PinnedLast` · `PinnedBoth` |
...
| `SupersededByAllocationId` · `IsActive` | INT · BIT | NULL · NOT NULL | re-planning is **additive**: superseded, never updated |

> **Why the split is in pounds and not footage.** Weight is conserved through drawing and rolling; footage is
> not — the same 900 lb is ≈ 11,100 ft at FL1 gauge and ≈ 76,300 ft at FL2, the 7× cross-section difference
> `[DBD §6.6]` records. A rod-local footage figure could not be compared with the line's counter without
> re-deriving through weight. It is also the client's own unit: their planner has no footage variable in 561
> lines. **And it buys an enforceable constraint** — `CK_RodOrderAllocation_WeightRange` asserts
> `To − From = AllocatedWeightLb`, which the footage version could not express.

**`SpoolTraceability` gains `ChildAlpha`** — the segment identity (`R00001C`), one alpha per segment,
minted through `CommonDB.dbo.GenerateCoilAlpha` (`Q57`). **`SpoolOrder` gains `SpoolWeightFrom`/`To`** so a
spool crossing an order boundary can say **where** the boundary is (`G48`).
...
| `ReasonCode` / `ReasonCategory` | VARCHAR(10) | NOT NULL | The client's **delay code** and **delay bucket** — e.g. `RUN13` / `RunTime`. `FK_RunPauseEvent_DelayCode` is **composite** on the pair, targeting `DowntimeReason`. `CK_RunPauseEvent_Bucket` excludes `Downtime`, which belongs to `LineDowntimeEvent` |
| `Notes` | VARCHAR(500) | NULL | **`CK_RunPauseEvent_NotesOther`: required on the per-bucket `Other` codes `SET23` / `RUN12` / `HDL15`.** ⚠ Rewritten 2 Sep 2026 — it keyed on `ReasonCategory = 'Other'`, a category the delay-code model does not have, so it was vacuously true on every row |
...
| `GaugeIn` | DECIMAL(8,4) | NULL | Gauge reading (in). **Nullable for a dropped or not-yet-arrived feed on any line** — *not* an FL2 marker: this column read *"NULL for the FL2 standalone live feed"* until 9 Sep 2026, when `A3` was retired and FL2 began measuring live |
...
#### `RodOrderConsumption` — the actual, and the handoff state machine `[PROPOSED]`

**Added 22 Aug 2026.** One row per pairing actually run; created when the pairing starts, never updated
after `Closed`.

**The cardinality is the client's rule.** `RodCheckinId` is a plain FK, so **one check-in has many
consumption rows** — which *is* the rule that a rod carrying an order boundary is checked in once and stays
mounted across it.
...
| `LatchedWeightAtThresholdLb` · `WeightAtAcknowledgementLb` | **two latches, not one.** The first at the crossing instant, the second when the operator confirms |
| `OverrunWeightLb` · `VarianceVsAllocationLb` | **PERSISTED computed** — the overrun the client requires be captured rather than discarded |
| `ConversionBasis` · `LbPerFtUsed` · `ConverterVersion` | the row states its own conversion, so a later `Q10` answer cannot retro-change history |
| `ClosureReason` · `RodCheckoutId` · `ShortfallWeightLb` | `Acknowledged` · `AcknowledgedEarly` · `RodExhausted` · `RodAbandoned` · `Superseded`. Abandonment reuses `RodCheckout` **Mode B** |

> **`UX_RodOrderConsumption_Station` is rule 2 as a constraint rather than a `409`.** At most one pairing may
> be `InProgress` or `ThresholdReached` per payoff. Enforced in the database for the same reason
> `UX_RodStaging_Bay` is: a rule checked only in application code is not enforced under concurrent clients —
> which is what `Q17` asks for on the spool side. Its predicate must stay a literal `IN` list, because SQL
> Server forbids a computed column in a filtered-index predicate.

> ⚠ **One run now spans two orders, so `FlatWireRun.OrderId` narrows to *"the order at check-in"*.** That is
> forced, not chosen: a run is one row per check-in event and `RodCheckin.RunId` is a single `NOT NULL`
> column, so closing and reopening the run at a boundary would require a second check-in. **Anything reading
> `FlatWireRun.OrderId` as "the order this run produced" is silently wrong at a boundary** — `OI-123` / `G47`.
...
| `FootageFt` | DECIMAL(10,2) | NOT NULL | `CK`: > 0 |
| `PassScheduleId` | VARCHAR(30) | NULL | FK → `PassSchedule.ScheduleId` — the schedule effective at creation (OQ-64) |
| `PassScheduleSnapshot` | NVARCHAR(MAX) | NULL | **JSON snapshot of the configuration at coil creation** (`NFR013`) |
| `CoilNo` | VARCHAR(9) | NULL | **The shared-schema identity of this coil** → `proddb..coils.coil_no`, minted by `CommonDB.dbo.GenerateCoilAlpha` off the source rod. `FlatWireDB`-local, filtered-UNIQUE, and the **retry contract** for the run-end write-back (`FR-509`, `[INT §8.1]`) ⚠ **AMENDED 26 Aug 2026 (`Q89`).** This is now the **LEAD** part's identity — the coil's one scalar shared face, retained deliberately. ⛔ **It is no longer the coil's only shared identity:** every source rod contributes its own, on `FlatWire_CoilTraceability.ChildAlpha`, and **every one is written to `proddb..coils`** with its own weight |
| `ChildAlpha` | VARCHAR(20) | NULL | ⚠ **New 26 Aug 2026.** On `FlatWire_CoilTraceability`: **one shared identity per (coil × source rod)**, minted by `CommonDB.dbo.GenerateCoilAlpha` rooted on **that row's** rod. **Every one is written to `proddb..coils`** with its own `SegmentWeightLb` (`FR-561`, `FR-562`, `Q89`). Filtered-UNIQUE. **Opaque** — never parsed, never ordered by (`FR-565`) |
| `SourceSegmentAlpha` | VARCHAR(20) | NULL | ⚠ **New 26 Aug 2026.** Which **segment** of that rod the coil part came from — `(RodAlpha, SpoolAlpha)` only implies it. ⛔ **Not a foreign key and cannot be one:** the parent index `UX_FlatWire_SpoolTraceability_ChildAlpha` is **filtered**, and SQL Server will not point an FK at a filtered index. Enforced in the domain model and by test (`FR-561e` / `ORD022`) |
| `SharedWrittenAt` | DATETIMEOFFSET | NULL | ⚠ **New 26 Aug 2026.** The **retry contract** under `Q89`: NULL until this part's `proddb..coils` row commits. A retry writes only the parts still NULL and reports all of them (`FR-569`). **A column, not a new table** — `ChildAlpha` already stores the N identities; the only thing the old scalar contract could not express is *which* committed |
| `SharedSkidNo` | VARCHAR(9) | NULL | The legacy skid number → `united_db..wip_skids.skid_no`, from `proddb..generate_new_skid_no` (`FR-514`) |
| `SkidId` | VARCHAR(20) | NULL | **External reference**, no local FK. ⚠ **The skid table is `united_db..wip_skids`** — `OI-104` closed 18 Aug 2026; `SharedSkidNo` carries the resolved legacy number |
...
| `RodAlpha` | VARCHAR(20) | NOT NULL | **FK → `Rod.Alpha`** |
| `SpoolAlpha` | VARCHAR(20) | NULL | **FK → `SpoolProcessing.Alpha`** — source spool for this footage range. **NULL on a rod-fed run** (FL1 standalone; FL3 fed directly from rod). Filtered index `IX_CoilTraceability_SpoolAlpha` |
...
### 5.9 Foreign keys (added last, in `06_ForeignKeys` — one script)
...
| `DieChangeEvent` | `OldDieId` / `NewDieId` | `ToolingInventoryDie.Id` | NULL | Per-tool footage attribution (`FR-255`); nullable so an unregistered tool cannot lose the run record |
| `DieHistory` | `DieId` | `ToolingInventoryDie.Id` | NOT NULL | The die this history entry belongs to |
| `DieHistory` | `RunId` | `FlatWireRun.RunId` | NULL | NULL when the action happened outside a run |
| `PassScheduleComponent` | `EdgerId` | `ToolingInventoryEdger.Id` | NULL | EdgeSet only. **Re-pointed 6 Sep 2026 (`D-53`)** — constraint name unchanged |
| `FlatWireRun` | `PassScheduleId` | `PassSchedule.ScheduleId` | NOT NULL | Schedule governing the run |
| ~~`SpoolProcessing`~~ | ~~`SpoolTypeId`~~ | ~~`SpoolConfiguration.Id`~~ | — | **Dropped 23 Aug 2026** — `SpoolConfiguration` merged into `Spool` (`Q60`) |
| `SpoolProcessing` | `ParentRodAlpha` | `Rod.Alpha` | NULL | Rod drawn into this spool |
| `SpoolProcessing` | `SourceRodAlpha` | `Rod.Alpha` | NULL | Partial-run source rod |
| `SpoolProcessing` | `SourceRunId` | `FlatWireRun.RunId` | NULL | FL1 run that produced it |
...
| `SpoolCheckin` | `RunId` / `SpoolAlpha` / `PassScheduleId` | `FlatWireRun.RunId` / `SpoolProcessing.Alpha` / `PassSchedule.ScheduleId` | NOT NULL | 3 FKs |
...
| `CoilTraceability` | `SpoolAlpha` | `SpoolProcessing.Alpha` | NULL | Source spool; NULL on a rod-fed run |
...
**Per-file table counts are deliberately absent from this table.** They are `[DBD §6.2]`'s, and this is not one of the three sites permitted to restate them. Every figure this table used to carry — *"6 lookup tables"*, *"9 run tables"*, *"64 non-clustered indexes"*, *"2 read procs"* — was wrong by 23 Aug 2026.
...
| 01 | `FlatWire_DDL_01_Lookup.sql` | the lookup group **+ the three pinned `PayoffPosition` rows** |
| 02 | `FlatWire_DDL_02_Schedule.sql` | the schedule group — **MVP-1 since `D-31`**, 15 Aug 2026 |
| 03 | `FlatWire_DDL_03_Materials.sql` | `Rod`, `FlatWireRun`, `SpoolProcessing`, `SpoolTraceability`, `SpoolOrder`, `RodOrderAllocation` |
| 04 | `FlatWire_DDL_04_Runs.sql` | the run-tracking group, including `SpoolStaging` and `RodOrderConsumption` |
| 05 | `FlatWire_DDL_05_QualityOutput.sql` | the quality/output group |
| 06 | `FlatWire_DDL_06_ForeignKeys.sql` | **All foreign keys — one script.** `06b` was folded in on 23 Aug 2026, retiring the "47 of the 57" split-count, which had to be maintained in two places and drifted through "41" and "43" before that. Count: `[DBD §6.2]` |
| 07 | `FlatWire_DDL_07_Indexes.sql` | **All index statements — one script.** `07b` was folded in on 23 Aug 2026 |
| 08 | `FlatWire_DDL_08_Programmability.sql` | `trg_CoilTraceability_NoOverlap` + `sp_GetGaugeTrace` + its grant. **One procedure, not two** — `sp_IngestRodFromCoils` is a `FlatWireDB` object but ships in `Database/Scripts/`, and `sp_ShiftSummary` is MVP-2's, in `09` |
| 09 | `FlatWire_DDL_09_Programmability_MVP2.sql` | `sp_ShiftSummary` (DB10). **MVP-2 — not in the MVP-1 runner** |
| 99 | `FlatWire_DDL_99_Teardown.sql` | drop everything. In **neither** runner, by design |
...
| Seed | Content |
|---|---|
| Lookup | `Stand` with **fixed IDENTITY** 1=FM1 (12"), 2=FM2_S1 (8"), 3=FM2_S2 (6"), 4=FM2_S3 (6", final) — **four rows; the former Id 5 is withdrawn** · `Drawer` **2 rows, `DB1`/`DB2`** — ⚠ *the 13 `DIE-0210`…`DIE-0340` size rows moved to* **`ToolingInventoryDie`** *(Id 1–14) with the die split, 2 Sep 2026* · **`ToolingInventoryRollSet`** Id 1–6 · **`ToolingInventoryEdger`** 1=Set `A` (Round), 2=Set `B` (Square) — ⚠ *no `Name` and no alpha: removed 6 and 7 Sep 2026, nothing read either, so `Set Number` is the only human-readable identifier* · **`ToolingInventoryEdgerGauge`** Id 1–3, on set `A` only (`.045`/`.040`/`.035`) · `Spool` **45 articles** `SP-0001`…`SP-0045` — ⚠ *`SpoolConfiguration` was **merged into `Spool`** on 23 Aug 2026 (`Q60`) and no longer exists* · `AlloyProperty` for 1100 / 1350 / 3003 / 5052 / 6061 |
...
|---|---|---|---|
| ~~`coils.coil_status = INFLAT`~~ | ~~`proddb` / shared~~ | **STRUCK — `D-32`, 18 Aug 2026.** `FW-002` is cancelled, so the shared vocabulary never gains `INFLAT`. In-process state is `FlatWireDB`-local (`Rod.Status`, `SpoolProcessing.Status`) | **No** |
...
| **`wip_skids`** | `united_db` | **opened / closed at FL2/FL3 coil completion — §5.13a** | **Yes** — *this is the skid table; `OI-104` closed 18 Aug 2026* |
| **`wip_skid_coils`** | `proddb` | **one row per output coil at completion** | **Yes** |
| **`coils` finished-coil row** | `proddb` | **at FL2/FL3 coil completion** | **Yes** — a child of the rod, `char(9)` alpha from `GenerateCoilAlpha` |
| **`coil_gen_history`** | `united_db` | **at FL2/FL3 coil completion** | **Yes** — one parent only, `OI-113` ✅ **AMENDED 26 Aug 2026 — `OI-113` CLOSED.** No longer *one parent only*: each part alpha is its own `child_coil_no`, so the per-child guard permits **one correctly-parented row each** and the shared tree stops disagreeing with the flat wire chain. ⚠ **Conditional — each alpha must name ITS OWN rod** (`FR-568`) |
| **`coil_cost`** | `united_db`, via `CoilCost_UpdateInsert` | **at FL2/FL3 coil completion** | **Yes** — omit it and the coil leaves cost and yield |
| **`coil_slit_cuts`** | `SlitterDB` | **one row per output coil at completion** | **Yes** — a flat wire coil is one cut |
| **`wip_log`** | `wiplogdb`, via `proddb..wip_log_view` | **at FL2/FL3 coil completion** | **Yes** — all 44 columns `NOT NULL` |
...
**WIP station registration** (deploy step 2, [`FW-241`](features/FS-03-database-foundation/DB/FW-241.md) — ⚠ **the draft script was withdrawn 6 Sep 2026 and never ran**) creates:
...
⚠ **`FL2PO` must now be created** — the 20 Aug 2026 reversal (`FR-533`) gives FL2 pre-check-in, and `FR-536` has it claim and release that station within the transaction. *(This read "`FL2PO` is deliberately not created — `PCI002` excludes FL2 from pre-check-in" until 25 Aug 2026.)* The script change is wave W5 of the 20 Aug ledger and is **not yet applied**. **There is no `FL3PO`**: the script never mentions one, while the schema, DDL and API all scope staging to FL1 **and FL3**. The working assumption is that FL3 posts to `FL1PO` (one physical VPS shared between FL1 and FL3 modes) — **OI-26 / OI-26**.
...
### 5.13 FW-001 — the shared-schema renames, **cancelled**

> ⚠ **CANCELLED — decision `D-32`, 18 Aug 2026. There is no shared-schema migration and nothing in this section is to be built.** The existing scheduling / `coils` schema is **read and written as it stands and is never altered**. The section is retained because the rename table below is quoted in a dozen documents, and a reader who meets it elsewhere needs one place that says it is dead. `FW-001` and `FW-002` are both cancelled; **−60 h base**, Phase 1C **221 → 138 h**.

Story **FW-001** *would have applied* **slash dual-naming** renames to the **existing shared scheduling / `coils` schema** — not to `FlatWireDB`. It had been the single highest-blast-radius change in the project.
...
**Also cancelled:** the new columns `OutgoingCoil/BundleWidth` and `IncomingWireDia`, and the **new status value `INFLAT`** (story FW-002 — it survives only as a `FlatWireDB`-local value on `Rod.Status` / `SpoolProcessing.Status`, see §3's status vocabulary).

**Not cancelled, because neither is a schema change** — both are rows and values in columns that already exist:
**New machine rows:** FL1, FL2, FL3 with unique `IdNo` values (story `FW-003`, machine_idx 125/126/127).
**New operation letter:** `F` in `PrevOpLetter`, `RemainingOps`, `RootRemainingOps`, `OpLetter` — so **`OI-27` stands**, `AccountingDB.dbo.GetMachineTypeFromOpLetter` still has no case for `F`.

**Blast radius — the reason the change was cancelled rather than deferred.** These columns are read by upstream receiving, planning, scheduling, reporting, yield and cost. A **full stored-procedure / view / report / query audit had to precede the migration** — a discrete 40 h line item — with a regression pass over renamed-column reports at QA4. **All of that goes with the change:** the audit, the QA4 regression pass in `FW-201`, the `[RB §6.3]` rollback treatment, and the migration script that was never written. The related web/report renames that would have ridden along are cancelled with it: "Outgoing Gauge" → "Outgoing Gauge/Diameter", "Coil #" → "Coil/Bundle #", "Gauge" → "Gauge/Diameter", and the removal of the "# Cuts" column from flat-wire report layouts.
...
**Base URL:** `/api/v1/flatwire`. **Hub URL:** `/hubs/flat-wire`.
...
`activeOrderId` is **`null` while a line is `Idle`** — the station must not display an order it has not started. `currentGauge`/`currentWidth` are `null` **while a line is idle, on any line** — *they were `null` for FL2 in all states until 9 Sep 2026, when `A3` was retired and FL2 began measuring live (`RA-AMB09`)*. `payoffs[].state` carries bay occupancy from `RodStaging` (`FR-424`); `weightLb`/`percentRemaining` come from the live `PayoffWeight` feed and are `null` on a bay that is not drawing.
...
- **All three lines publish gauge and width; FL2 does so at 4 s rather than ~10 Hz, and is therefore not batched.** *(This read "FL2 standalone suppresses the batched gauge/width channels" until 9 Sep 2026 — `[SIG §5.3]` is withdrawn.)*
...
> **Everything else in this section is superseded by [`../20-architecture/PLCTagSpecification.md`](../20-architecture/PLCTagSpecification.md) (4 Aug 2026)** — the write surface (`[PLC §7]`), the **per-line tag map** (`[PLC §5.2]`), the naming convention (`[PLC §4]`), `ITInhibit` (`[PLC §8]`), the sixteen-moment tag lifecycle (`[PLC §9]`), feet consumption (`[PLC §5.1]`), the commissioning sequence (`[PLC §12]`) and the `PLC-Q##` open-item register (`[PLC §13]`).
>
...
| **DB2** | Rod Check-in & Pre-Run Setup (FL1/FL3) | **`dashboard_2_rod_checkin.html`** | FL1 operator | Start of each rod |
...
- `dashboard_2_rod_checkin - Old.html` — the original grid + inline-SVG progress-ring layout, with a 9-step footer counter. **Retired, and deleted from the repository 11 Aug 2026** (recoverable at `d79ce78`).
- the **interim single-page rod-scan-row layout** with an 8-step counter, which held the `dashboard_2_rod_checkin.html` filename. **Superseded** by the wizard, and **deleted from the repository 11 Aug 2026** (recoverable at `d79ce78`). The wizard — `dashboard_2_rod_checkin - New.html` until 11 Aug 2026 — took that filename the same day, so **the name now means the approved wizard, not this retired layout**.
...
**23 of the 25 screens include it.** The two that do not: `coil-spinner.html` (a component demo) and **`dashboard_2_rod_checkin.html`, which inlines its own app bar**. When starting a new screen, **clone Dashboard 12's skeleton, not Dashboard 2's**.
...
**`spool_notification.js`** is the shared spool-progress component (Part A milestone card + docked pill, and the Part B PLC-stop modal). Wired into `dashboard_3_active_run.html`. It keeps the host screen's `#fw-spool-lb` / `#fw-spool-target` readout in step so the screen and the notification never disagree.
...
> **Confirmed 15 Aug 2026 — this is no longer an assumption.** All **six** roles exist as JWT claims in the `Login` service, carried on the standard `ClaimTypes.Role`. **No provisioning story is needed**, and the earlier *"it could block the build"* reading of gap **G6** / OI-37 is spent. ⚠ **One residual:** the six claim **values** are abbreviated or coded rather than the labels used in the matrix above, and the mapping has not been supplied — it gates verification rather than construction. ⚠ **And a separate item survives untouched:** the `Engineering/Maintenance` and `QA` role *definitions* remain **inferred** from source usage rather than explicitly specified [REQ §2.8 open item]. Confirming that a claim exists is not confirming that the capabilities attributed to it are right.
...
**Superseded targets — do not use.** The April-dated documents carry "machine setup and PLC commissioning complete end of June 2026 · trials 1 July 2026 · production 1 August 2026 · ~10-week window · 5 sprints". Every one of those is dead. `05-SprintPlanAndBacklog.md`, `04-APIContract.md`, `FlatWireSchema_Mapping.md` and `03-HLD-and-ERDiagram.md` §14 still print them.
...
|---|---|---|---|---|---|
| **1** | **Core Platform Setup** (1A Angular · 1B Backend · 1C Database, in parallel) | Stand up the reusable platform so every later phase is pure feature work | Angular library scaffold + mock API + mock SignalR; `FlatWire` 4-project solution with stubbed controllers; `FlatWireHub` skeleton; `PLCTagService` in simulate mode; `FlatWireDB` created with 33 tables, FKs, indexes, seed; ~~FW-001/FW-002 migrations~~ *(cancelled, `D-32`)* | — | ~~FW-001, 002~~ *(cancelled)*, 004, 005, 006, 007; scaffolds of 080/082 |
...
> **Dependency-graph correction.** `../90-registers/Gaps.md` draws Phase 8 → 9 → 10, implying FL3 hybrid depends on the FL2 **spool check-in**. It does not — **FL3 has no intermediate spool**. FL3 depends on Phases 4, 5, 6 and 9. Corrected in the table above [REVIEW Tier 4 #35].

> ~~**Phase 6 depends on Phase 13.** Die-change validation (Phase 6) needs the die inventory that Die Management (Phase 13) creates. Either pull a minimal die reference forward into Phase 6 or resequence.~~ ✅ **RESOLVED 2 Sep 2026 by the die split (`Q91`) — the option taken was the first: the die inventory is built and seeded in Phase 1.** `ToolingInventoryDie` and `DieHistory` are `01_Lookup` and `04_Runs` objects, so **Phase 6 depends on Phase 1**, not Phase 13. `OI-41` closes with it [REVIEW Tier 4 #34].
...
Working days are **counted, not assumed**: **Labor Day falls on Mon 7 Sep 2026** (inside W4), and W7 holds only 3 days. Effort is in dev-days from `../60-delivery/CapacityAndEffortModel.md`; peak FTE = that week's days ÷ its working days.
...
**Post-gate window = 32 working days**; whole window **44** (inclusive of Thu 30 Jul). **MVP-1 total 398.3 dev-days (3,186 h) → 9.1 FTE, 24.5 in W7** *(re-baselined 18 Aug 2026, `D-32`; previously 411.5 d / 3,292 h / 9.4 FTE)* *(both scopes 457.5 d / 3,660 h → 10.4 FTE; the figure below predates the 11 Aug 2026 correction of a stale TOTAL row)*. Total **465.6 dev-days** → **10.6 FTE sustained**.
...
|---|---|---|---|---|
| ~~FW-001~~ | ~~Existing-schema column renames~~ — **CANCELLED, `D-32`** | 1 | — | — |
| ~~FW-002~~ | ~~`INFLAT` coil status~~ — **CANCELLED, `D-32`** | 1 (used 4) | — | — |
...
**Effort roll-up per phase (dev-days).** Points are **not** the delivery unit — the schedulable estimate is in dev-days, derived in `../60-delivery/CapacityAndEffortModel.md` §2–§3 from each phase's own deliverable inventory. Points appear below only as the cross-check basis.
...
By stream, contingency included: FE **162.7** · BE **108.8** · QA **77.4** · RT **53.9** · DB **52.3** · BA **10.5**. Parenthesised figures are reserves excluded from the total (Phase 4: OI-39/G2 cross-DB recovery; Phase 9: OI-45/OQ-10 weight basis) — those two phase estimates are **provisional until their open issue closes**.
...
**Backlog housekeeping still outstanding:** `05-SprintPlanAndBacklog.md` still tags the 14 upstream stories as in-scope Critical/High Sprint 2–3, still uses the dead 5-sprint model with no sprint→phase crosswalk, still declares `IsActive (bool)` in FW-010 instead of the three-value `State` enum, and still prints the 20-table count. *(It also had no change log; since 12 Aug 2026 that is **correct** — the repository has one change log, [`../CHANGELOG.md`](../CHANGELOG.md), and no document carries its own. What is owed is a **section** for it there, since its history was never recorded anywhere.)* Reconcile it up to this document rather than maintaining two plans.
...
| OQ-3 / OQ-14 / OQ-15 unresolved | High | High | The stub routes around them (single-schedule assumption); schedule an explicit **de-stub pass** when they close; escalate to Tim O. / Jaspreet |
...
| ~~FW-001 column renames break existing reports~~ | — | — | **RETIRED — `D-32`, 18 Aug 2026.** There is no migration, so there is nothing to break. The 40 h audit and the QA4 regression pass are cancelled with it |
| OQ-10 (footage→weight) unresolved | Medium | High | Dashboard 7 shows "pending confirmation" with an operator override; keep the factor table-driven |
...
| ~~Roles are not existing JWT claims~~ ✅ **Retired 15 Aug 2026** | ~~Medium~~ — | ~~High~~ — | **Did not materialise.** All six exist on `ClaimTypes.Role`; no provisioning story was needed (G6). ⚠ **A smaller risk replaces it:** the claim **values** are coded rather than labelled and the mapping is unsupplied, so the authorization matrix cannot be *verified* until it lands — **Low / Medium**, mitigated by routing all six through one constants class |
...
> **`D-29` to `D-32` are recorded in [`Architecture.md`](../20-architecture/Architecture.md) §13.1, not here** — and ⚠ **that register also RESTATES `D-01`–`D-17`**, which this note did not say until 25 Aug 2026. The two are different views, not a split: this table carries **Date** and **Supersedes**, `§13.1` carries **Rationale**. **This table wins on the decision itself, its date and its supersession chain**; `§13.1` wins on architectural rationale and is the **sole** home of `D-29`–`D-32`. `D-18`–`D-28` exist **only** here — that register carries the design decisions taken after the ProjectPlan restructure. Most recent: **`D-32` (18 Aug 2026) — there is no shared-schema migration; `FW-001` and `FW-002` are cancelled.** See §5.13, `FR-044`, `FR-077` and **`OI-111`**.
...
| **D-04** | **`Rod` is retained** as a `FlatWireDB`-local master mirroring the shared `coils` record, with **enforced** rod-alpha FKs | Jul 2026 ("Hybrid foundation") | 28 tables; referential integrity for all rod references in-database | **Supersedes** `../20-architecture/Architecture.md` §13.1 `D-04` and `phase-01c`, which dropped `Rod` and made every rod-alpha reference an unenforced cross-DB link (21–22 tables) |
...
| **D-27** *(6 Aug 2026)* | **The edgers are inter-stand, and edging is a forming operation.** The two edging devices sit **between S1 and S2** and **between S2 and S3**; there is **no edging capability on the exit side of the mill**. They are **vertical profiling rollers** — a roll-forming capability, **not a knifing or cutting operation**, so nothing is trimmed and there is no scrap stream at the edger. Passing an edger narrows width and pushes the displaced material into **length and a slight centre bulge**; the following stand flattens it and the edge widens again. **The first edger also converts the section from round to square.** The last 6" pass at **S3 is a skim pass** — light reduction, **no edging** — sized for a uniform cross-sectional area edge to edge | Pass schedule generation needs **a separate formula subset for the edger, treated as its own operation** — see `PassScheduleGenerationSpec.md` §3.3.5 and `PSG-D24`–`PSG-D26`. Confirms the *"edgers at S2 and S3"* reading as **entry-side**, which leaves the proposed `…S2.Edger.*` / `…S3.Edger.*` tag paths unchanged (**G29**). Register pointer **PSG-D24** | Any description of the edger as knifing, cutting or trimming; any exit-side edger |
| **D-28** *(6 Aug 2026)* | **FM2 carries two dancers, each with two modes.** They sit in the edger positions — between S1/S2 and between S2/S3 — and run in either **regular dancer mode** (compensating speed control) or **tension mode**, as on the current mills | **Nothing in the design models a dancer on FM2** — no lookup row, no `PassScheduleComponent`, no tag element — and **tension mode qualifies `PSM012`** (§3.4, *"tension is derived from speed, never entered manually"*), which is true in dancer mode only. Open as **Q32**, gap **G35** | The implication that FM1's dancer is the only one in the module |
| **D-34** *(1 Sep 2026)* | **The delay-code model is authoritative for downtime reasons, replacing the pause taxonomy outright.** Four *time* buckets — `Setup` · `RunTime` · `Handling` · `Downtime` — with `SET##`/`RUN##`/`HDL##`/`DWN##` codes and per-code `Nonprod Time`, `Status`, `Delay Buffer`, plus `Supervisor Override` on `Downtime` only. **72 codes in scope: 36 inherited, 36 new.** Supersedes the 15 reasons in 5 semantic categories | `DowntimeReason` in `01_Lookup`, seeded inline as production reference data; `RunPauseEvent.ReasonCode`/`ReasonCategory` re-domained; `pause_run.js` reworked from tile columns to a two-level picker | **Supersedes** the `EquipmentMechanical`/`MaterialHandling`/`QualityMeasurement`/`Operational`/`Safety` vocabulary everywhere. ⚠ Four old reasons — `OperatorBreak`, `ShiftChangeover`, `AwaitingSupervisor`, `SafetyObservation` — have **no successor code** and are owed back to the client |
| **D-35** *(1 Sep 2026)* | **Line downtime is a separate event from a run pause.** The `Downtime` bucket's 25 codes are recorded against the **line**, not a run | New `LineDowntimeEvent` in `04_Runs` with a **nullable** `RunId`; `CK_RunPauseEvent_Bucket` excludes `Downtime` | `RunPauseEvent.RunId` is `NOT NULL`, `FootageAtPause` is `NOT NULL` and every `Outcome` value presumes a run — but *Power Outage*, *Fire Drill* and *Waiting for Spool From Previous Operation* happen when no run is open. `WipRejection.RunId` is nullable for the same reason |
| **D-36** *(1 Sep 2026)* | **Dancer mode is machine-program controlled and is not adjustable by anyone.** Each dancer holds a position **range**; tension mode exists on **FL2 only** and only for heavier/larger product | `Dancer` keeps equipment capability only — **no write surface, no pass-schedule column**; the `[PLC]` dancer element stays **read-only** | **Resolves the `C6` / `D-28` conflict** held unapplied since 12 Aug 2026 — a 1 Sep statement is later direction than the 6 Aug call, and both readings are true at once. `Dancer.SupportsTensionMode = 0` on FM1 now means **"no"**, not "not stated". ⚠ `PSG-D27`'s tension physics stays open on the client's own engineering follow-up |
| **D-37** *(1 Sep 2026)* | **Rod weight is an INPUT at receiving, never calculated**, and the rod is treated as **cylindrical** although it is not perfectly round. Footage **is** calculated and carries a ± x ft tolerance from ovality | Weight capture stays an entry field; footage derivation carries a stated tolerance | Narrows `Q10` / `OI-45` to the **dimensional basis** only. ⚠ The formula worksheet is **still owed** |
| **D-38** *(1 Sep 2026)* | **No dedicated Supervisor Monitor for flat wire.** The client's answer is a flat "No" | No supervisor screen is built | **Supersedes `C13`** (*"desired but not required for initial implementation"*). ⚠ It does **not** settle `Q20`'s unacknowledged-100 %-milestone mirroring, nor what sets the `Supervisor Monitor` **IT inhibit reason**, which the client's list still marks as applying |
...
| **D-57** *(8 Sep 2026)* | **`SpoolOrder` is RE-GRAINED from the spool to the SEGMENT — one row per (segment, order).** It was keyed `(SpoolAlpha, OrderNo, RelLetter)` and referenced the rod **nowhere**, so it could say *that* a spool crossed an order boundary and where the boundary sat on the spool, but **not which rod's material went to which order**. Reaching the rod meant joining `SpoolTraceability`, and the two tables' ranges were in **different units** — this one in pounds, that one in **feet and nullable** — so they could not be joined at all. ⛔ **A literal merge of the two was REJECTED:** it breaks `UQ_SpoolTraceability_Seq` (an order split inside one physical segment shares a `SeqNo`) and the **UNIQUE** `UX_SpoolTraceability_ChildAlpha`, because **one physical segment has one child alpha** (`Q57`) — and demoting that index leaves no uniqueness guard while `FW-231` is blocked (`G54`). ⭐ **The row now carries TWO ranges in TWO FRAMES:** segment-local **pounds**, which makes `CK_SpoolOrder_WeightRange` an exact single-row check against `AllocatedWeightLb`; and spool-local **feet**, the frame **FL2 actually cuts in** — one order spread over two segments reads as two unrelated pound intervals and as **one contiguous footage run**, and contiguity is visible only in the spool frame. ⛔ **Pounds are authoritative; feet are a recorded convenience** — no constraint can tie the two frames together | `SpoolOrder` re-keyed on `SpoolTraceabilityId`; `AllocatedWeightLb` `NOT NULL` replaces nullable `PlannedWeightLb`; `SegmentWeightFrom`/`To` replace `SpoolWeightFrom`/`To`; **`SpoolFootageFrom`/`To` added**; `SupersededByOrderId` + `IsActive` make re-planning additive. `FK_SpoolOrder_SpoolProcessing` → **`FK_SpoolOrder_SpoolTraceability`** (renamed, because the parent is a different table — one out, one in, **no count change**). Uniqueness moves from the inline `UQ_SpoolOrder_Key` to the **filtered `UX_SpoolOrder_Active`**, which a plain `UNIQUE` could not replace: additive re-planning means a superseded row and its replacement share the key. ⛔ **The guarded `ALTER` block adding `SpoolWeightFrom`/`To` was DELETED** — its guard is *"the column is absent"*, true by design after the re-grain, so a fresh teardown would have bolted two stray columns on. Domain: `AddOrder` moves to `SpoolTraceability` and finally **asserts invariants** (`SegmentOrdersMustNotOverlapRule`, `SegmentAllocationMustFitRule`, 9 tests — the table had none); `SpoolProcessing` stays the aggregate **root** with `Orders` as a projection. Counts **47 · 69 · 91** with `D-58`; **308 backend tests**. `FW-N20` | ⛔ **`Q43` IS OPEN AND RATIFICATION IS OWED — this builds ONE OF TWO client readings and says so.** The 20 Aug 2026 multi-voice call says a spool carries many orders; **Tim O'Brien said one, by e-mail, on 3 Sep 2026**. If his position is ratified, `SpoolOrder`'s purpose does not arise and the table dissolves, with `G48`/`OI-119`/`OI-123`/`OI-124`. ⚠ **`G48` is now FULLY implemented and stays OPEN** — a contradicted-premise gap does not close. ⛔ **New `G109`**: nothing writes this table. ⛔ **New `G110`**: a straddle cannot be *derived*, only allocated — it needs a rod-local anchor no table holds — so a straddle is `Planned`, never `Derived`. ⚠ Unverified against a server |
| **D-58** *(8 Sep 2026)* | **`SpoolConfiguration` is SPLIT BACK OUT of `Spool`. SUPERSEDES `Q60`'s merge.** `Q60` merged it away on 23 Aug 2026 because it was a size class holding exactly **one** meaningful row against 30–45 articles — sound reasoning, and this does not call it a mistake. ⛔ **But the merge named its own reversal condition and that condition has NOT fired:** *"it is worth it only while 'every article is one size' holds. If the client confirms a second size, revisit the merge."* The client's 20 Aug 2026 *"all one standard size"* **still stands unsuperseded**, and TKUP-1's 3,500 lb against TKUP-2's 1,100 lb are **machine positions, not article sizes** — FL2's output is a **coreless** oscillated coil, wound on no reusable article at all. ✅ **So the split is made on NORMALISATION grounds and on direction**, not on a client change: eight values repeated across 45 rows, a second size costing an `UPDATE` of many rows where the split shape needs one `INSERT`, and a size-class name that lost `UQ_SpoolConfig_Name` the moment it moved onto `Spool`. ⛔ **Do not record this as a client change** | New `SpoolConfiguration` in `01_Lookup` — `Name` (unique), the six limits **`NOT NULL` again**, `IsDefault`, `IsActive`. `Spool` drops to five columns (`Id`, `SpoolNo`, **`SpoolTypeId`**, `IsActive`, `Notes`). ⭐ **The three band CHECKs get SIMPLER:** on `Spool` each carried an all-or-nothing `IS NULL` clause, forced only because the merge made the columns nullable (*"a CHECK accepts UNKNOWN, so a half-populated band would be admitted"*); they revert to `CK_SpoolConfig_*` and to `Min < Max`. **`FK_Spool_SpoolConfiguration` restored** — ⛔ but **not** `FK_SpoolProcessing_SpoolConfiguration`, which `Q60` also dropped: a material row reaches its configuration through `SpoolProcessing.SpoolId` → `Spool.SpoolTypeId`, and a second direct path is how the two disagree. Seed: **one** configuration row, because the client has stated one. Counts **47 · 69 · 91** with `D-57`; ⚠ **zero code cost** — the article is not modelled in C# at all, and the only touch is one comment in `HubContracts.cs`. `FW-N21` | ⛔ **The split BREAKS a documented fallback, and replaces it.** `SpoolProcessing.SpoolId` is nullable (`Q42` open), so a material row may have no article and no limits to validate against; `Q60`'s answer was *"any active `Spool` row's limits"*, well-defined **only** because all articles were one size — which splitting is precisely what stops being true. `IsDefault` + `UX_SpoolConfiguration_Default` make exactly one row the default at any number of configurations. *(The alternative — refuse to validate when the article is unknown — is more honest but changes behaviour, so it was not taken silently.)* ⚠ **`Q42` untouched and still open**; the seed keeps its one-expression format so answering it stays a one-line change. ⚠ **`Q60` keeps its full text** — superseded by a note, never edited. ⛔ **`47` is now the correct `[DEP]` `V1` answer**, and that gate's old *"47 means `Edger` survived"* diagnostic is retired: **`46` now means this change did not land** |
| **D-59** *(10 Sep 2026)* | **The Active Run address is `#/flat-wire/flatline/:machineName`, and three further `D-55` details are settled as built.** ⭐ **(1) The segment is `flatline`, not `home`** — the built router composes it, so the address an operator bookmarks is `#/flat-wire/flatline/fl1|fl2|fl3`. `D-55` chose `home` in passing, in a decision whose subject was the four profile rows, and it was never argued; `[UIC §5.0]` already holds that *"`'home'` is a location, not a screen"*, which is the same reasoning that keeps `screenKey` off the route. ⚠ **The cost accepted:** `#/flat-wire/flatline/…` repeats a word, weighed and taken because the repetition costs a reader nothing while `home` costs them a wrong expectation about what the screen is. ⭐ **(2) `LineContextService` is a PURE MODULE, not an injectable** — `models/line-context.model.ts`, three functions (`resolveMachineName` · `stationFor` · `machineIndexFor`), no state: the route already holds which line is in scope, so a service holding it again would be a second copy of the same fact. What `D-55` required is a **single site**, and a single site is what exists. ⛔ **(3) The terminal-registration resolve is STRUCK, not deferred** — `projects/shared` carries no terminal, workstation or host-machine service (measured 10 Sep 2026), no story owns building one, and the requirement was inherited from `D-55`'s prose rather than from any specification of a mechanism. Terminal-derived defaulting would need its own story and its own source of truth. ⭐ **(4) `#/flat-wire` is NOT stranded — it lands on DB1.** The empty-path child redirects to `supervisor-dashboard`, which shows all three lines with their live state, so an operator arriving with no line in the address gets **more** than a picker would give them, from a screen that has to exist anyway. ⛔ **This does NOT decide the sibling route grammar** — `[CMP §5.2]`'s five line-first routes against this one screen-first route stays that note's own flag, and the next screen to land is still free to settle it either way | ⛔ **No code changes** — `FLAT_WIRE_CONSTANTS.FLATLINE` and `ACTIVE_RUN_SCREEN_KEY = 'active-run'` both stand, and `screenKey` still does **not** follow the route, because it is stamped into every element id. **The documents are what move:** `D-55`'s row, `[CMP §5.2]` (route list, the `D-55` note heading, the line-first/screen-first paragraph and the *stranded*/terminal-registration sentence), `[CMP §5.4]`, `[SCR]`'s DB3 row, `[UIC §5.0]`, `[UIC §1.1]`'s default-child row, `[UIC §3.22]`'s element-id example, `ActiveRunMonitor.md` §1.4a (**superseding note only — that document is issued for client sign-off**), `[FS-08]`'s FE scope row, `FW-062`, `FW-N19`'s handoff and `FW-204`. ⚠ **`FW-204`'s premise changes** — its two-tile picker is no longer the answer to a lineless address; its retirement condition stays its own to decide. ⛔ **Nothing costed moves** — `[TB §7]`'s `FW-N15` card keeps its hours, priority, sprint, phase, stream, dependency and blocker byte-identical. `FW-N15` | `D-55`'s **address** (`#/flat-wire/home/:lineId`) and its ***stranded*** prediction, which named `FW-204`'s picker or a terminal-registration resolve as the only two ways out. ⛔ **`D-55`'s three-fact split, its four profile rows and its station rule are NOT reopened**, and `D-56`'s `:lineId` → `:machineName` rename stands untouched. Also supersedes `[CMP §5.4]`'s `line-context.service`. ⚠ **The route-grammar disagreement was raised in `FW-N19`'s Handoff, not in `D-56`** — `D-56` carries no open-questions line |
| **D-60** *(10 Sep 2026)* | ⭐ **Check-In is ONE shared component for FL1, FL2 and FL3 — FL1 rod, FL2 spool, FL3 rod — addressed at `#/flat-wire/checkin/:machineName` and driven by a per-station profile.** This is the same move `D-55` made on the Active Run monitor, applied to the second screen where the stations diverge. It **supersedes DB2 and DB5 as separate screens** in `[SCR §7.1]`, `[SCR]` Appendix B, `[CMP §5.1]`'s component tree and `[CMP §5.2]`'s two routes — including `/flat-wire/line/FL2/checkin/spool`, which hard-coded FL2 in the path. ⚠ **Two things pointed at it already:** `FR-094` asks for *"the same mandatory confirm bar as Dashboard 2"* in its own words, and `[RCI]` was already ONE specification over two screens — its §4 opens *"The same five steps, with a different material and a different tag set"*. **What is shared:** the route, the layout, the wizard chrome and unlock machine, the gate model, the **Pass Schedule** step and the **Pre-Run SPC** step. **What is station data:** the step list, the material, the payoff selector, the SPC measurands, the endpoint and the optional panels. ⛔ **`FR-095` STANDS — there is no visual inspection at FL2**, and `TC-114` still asserts the section is absent; FL2's profile lists three steps (Spool Origin · Pass Schedule · Pre-Run SPC) against FL1/FL3's six. ⛔ **It supersedes DB5's mockup note that removed the wizard** (item 5 of its 2 Aug 2026 redesign) — the concern behind that note, that tabbing would hide `FR-092`/`FR-093`, is answered by giving them a step of their own. ⚠ **It does NOT touch the three-stand FM2 table**, which is the item that mockup says *"Do not revert this"* about. | ✅ **`FW-064` is RETIRED into `FW-061`** (16 h → 0 h), keeping its id as a forwarding address per `[SCM §1]`; `FW-061` goes **36 h → 48 h** and gains `FW-081`, `FW-179` and `FW-181` as dependencies plus `G55` and `OI-115` as inherited blockers. ⚠ **`FW-124` is not retired.** ⛔ **A real sequencing cost:** `FW-061` is Phase 4 and its FL2 steps need Phase-8 stories, and **no rule in `check_docs` checks dependency phase ordering** — verified against rules 2 and 5 — so nothing warns. ⚠ `[TB]`'s requirement-coverage row for FR-090–096 must be repointed off the retired id |
| **D-61** *(10 Sep 2026)* | ⭐ **EDGE PROFILE IS A FOUR-VALUE VOCABULARY, AND THE TOOL TAKES ONLY THREE OF THEM.** The client: *“**Natural Round Edge (no edging), Full Round Edge, Square Edge, Broken Edge**”*. **`Natural Round Edge` means no edging**, so no physical edger set is ever ground for it — and the client's own roll table carries no such row, only `-S`, `-R` and `-B` suffixes. So `CK_PSC_EdgeType` (the **product** edge the order specifies) takes **four** and `CK_TIE_EdgeType` (the **tool's** ground profile) takes **three**. ⛔ **The two constraints must not be merged or aligned** — they now differ in **cardinality** as well as meaning, which makes the separation the DDL already required load-bearing rather than stylistic. A schedule specifying *no edging* leaves the `EdgeSet` component `Bypass` or `Skip`, which `CK_PSC_State` already expresses, so `CK_PSC_EdgeTypeReq` does not fire and needs no relaxation. ⭐ **This also settles `G76`'s `Flat`-versus-`Square` half by the client's own hand:** his tolerance heading reads *“Flat Wire Rolled Flat Edge **(Square Edges)**”*, so the order screen's `Flat Edge` and the database's `Square` are the **same profile**, canonically `Square Edge`. | Both CHECK constraints re-valued and both `EdgeType` columns widened `varchar(10)` → `varchar(20)` — **every new value overflows 10 characters**; `G76` **closed**; verified on `DEV00164-001`. ⛔ **The C# enum and the TypeScript union are NOT yet changed** — `FW-147`'s three-way mirror is gated on `OI-05` (is `Bevel` the same as `Broken Edge`? `Q110` item 1), and widening a shipped enum twice is the cost of guessing | Supersedes the `Round`/`Square` pair and the `Round Edge`/`Flat Edge` pair; `OI-05` is still open on `Bevel`; the order-screen vocabulary is upstream Web epic `E06` — **`G128`** |
| **D-62** *(10 Sep 2026)* | **`ToolingInventoryEdger.EdgeType` IS `NOT NULL` AGAIN, AND THE NATURAL KEY IS `(MachineName, SetNumber)`.** `Q95` legs 5 and 2. An edger set **is** classified by profile — *“edger sets are distinguished from one another by gauge band and profile (edge type)”*, *“a set will NOT share different profiles”*, profile defined *“per set”* — and set numbers are **encoded and shop-unique**: `400-S` / `401-S` / `402-S` / `400-R` / `400-B`, stem carrying the gauge band, suffix the profile. ✅ **This settles `D-53`'s relaxation limb exactly as `D-53` provided for** — it relaxed `EdgeType` to `NULL` *“because the grid does not classify a set by profile at all”* and **named `Q95` as the send-back**. The absorption, the gauge child table and the grind life model all **stand**; `D-56` had already reversed the rename limb. **Only the relaxation limb moves.** ⭐ **And `GrooveNo` is dropped** (leg 3): rolls are identified *“by groove gauge range and profile”*, not by position, which is what that leg said would decide it. | `EdgeType NOT NULL`; `MachineName` and `SetNumber` `NOT NULL`; **`UQ_ToolingInventoryEdger_Set`** added — ⛔ as a **table constraint, not a second unique index**, because that duplication is what moved the index figure `91 → 90`; `GrooveNo` dropped; `AllocatedPosition` (`E1` / `E2`) added beside `Location` rather than replacing it. **Object counts unchanged at 47 · 69 · 90**, measured. **`G104`** all but closed — only `SerialNo` remains | ⛔ **Roll-level tracking is NOT built** — *“I am open to suggestions and alternatives, lets discuss”* is a discussion, and building on one sentence is the `G87` mistake. ⚠ **His own sample breaks his encoding** on two rows (`400-B` where the stem says `401`) — `Q110` items 2 and 3 |
| **D-63** *(10 Sep 2026)* | **THE TOOLING LIFECYCLE'S FOURTH VALUE IS `Inactive`, NOT `Retired` — A STORED-TOKEN CHANGE ONLY.** The client named it: a set worn out after its last available grind *“shall be taken out of service and **marked as ‘In Active’**”*. Read as `Inactive`, which is **already how this schema glosses the word** — `FlatWire_DDL_02_Schedule.sql`'s `[CONFIRMED]` status block reads *“`Inactive`  Retired.”* So this is **one vocabulary across two domains, not a new word**. Applied identically to all three tooling registers, which the DDL keeps deliberately alike. ⛔ **`FR-250`'s action is still named *Retire Die* and still requires a retire reason code, and `FR-253`'s display band is still `Retired`** — `DieChangeAndManagement.md` §5 remains the single copy of the five display bands. **Do not rename a label to match the token.** | `CK_ToolingInventoryDie_LifecycleStatus`, `CK_TIRS_LifecycleStatus` and `CK_TIE_LifecycleStatus` all re-valued; the three schema documents and the ERD restated. ✅ **No seeded row used `Retired`** — every die, roll set and edger seeds `'In Service'` — so the change is safe against existing data, and it was verified by insert on `DEV00164-001` | ✅ **No application code changes**: `LifecycleStatus` appears nowhere in the FlatWire C#, because the edger register has no code yet (`FW-269` / `FW-270` not started). `FW-269`'s enum-mirror AC must be written against `Inactive` |
### 10.3 Business decisions from the open-questions register

| OQ | Decision | Date | Consequence |
|---|---|---|---|
| ~~**SCADA chart layout**~~ *(register question withdrawn 12 Aug 2026; the surviving tag half is **`PLC-Q02`**)* | ~~SCADA chart layout is **defined as Dashboard 14**~~ — **SUPERSEDED 4 Aug 2026.** DB13, DB14 and the Machine View are descoped, so the chart-layout half is moot. Its *other* subject — **machine tags for flat wire** — survives, is specified in `../20-architecture/PLCTagSpecification.md`, and remains unconfirmed as **`PLC-Q02`**. *(Raised, not asked: DB14 was also the answer to the legacy .NET SCADA report.)* | ~~15 May 2026~~ | ~~Unblocked SCADA report development. Tag paths still need commissioning confirmation |
| **OQ-61** | Mid-run pass-schedule change — **five cases**: (1) same-spec tooling swap → **single alpha** + die-change event at footage; (2) size/product-config change → **new child alpha** at the footage breakpoint; (3) edge-type change → **new child alpha**; (4) roll-gap adjustment within tolerance (AGC) → **single alpha, no change**; (5) roll-gap change to a new target → **new child alpha** | 4 May 2026 | Determines when `FW-…-C##-A` children are minted |
...
| **OQ-63** | Component failure mid-run — build an **unplanned component bypass** as a distinct transaction from a planned bypass: captures component, time, footage at failure, reason, operator; **alpha split** at the bypass point; **supervisor acknowledgement** required; a **disposition step** for pre-bypass material | 4 May 2026 | A new event type not yet in the schema — see OI-43 |
| **OQ-77** | Maximum finished coil weight = **1,100 lb** (TKUP-2 equipment limit) | 28 Apr, revised 4 May 2026 | Customers set their own lower limit in orders/quotes; orders exceeding a single rod/spool split into multiple stops, each with its own alpha. **Revised up from 1,000 lb** |
| **OQ-80** | **No interleave separator** between oscillate layers — UA has no capability to provide one | 4 May 2026 | No pack-specification field needed |
| **OQ-81** | **Camber** SPC measurement available where the customer specifies it — conditional, not mandatory | 4 May 2026 | Field present but optional |
| **OQ-82** | **Edge burr not currently measured** | 4 May 2026 | No system implementation |
| **OQ-83** | **System-level die-life tracking required**: cumulative footage per unique die ID from the existing PLC footage counter (no new sensor); configurable threshold per die type; **passive banner, no hard block**; Maintenance can acknowledge and extend with a reason; mid-run swap closes the outgoing counter and starts the incoming one; manual reset through the die-management screen logs who and when | 4 May 2026 | Threshold value **deferred** until failure data exists from production |
| **OQ-66** | **Line speeds unknown**, to be determined by trial; added to a configuration table by UA once known | 4 May 2026 | The scheduling algorithm must be **table-driven** for this input. Not a blocker |
| **OQ-67** | **FL1 and FL2 can run independent orders simultaneously** as separate machines with separate bookings, alphas and check-ins; throughput ratio ≈ **3:1** (FL1 faster); **FL3 cannot run if FL1 or FL2 have scheduled orders** | 4 May 2026 | Two concurrent dashboard instances must be supported |
| **OQ-74** | **Mid-run rod checkout requires supervisor approval** | 4 May 2026 | Operator submits; supervisor approves. Mirrors the WIP-rejection disposition flow |
| **OQ-75** | **Partial-run material disposition requires supervisor approval** via a notification-driven remote model: operator confirms → Pending Disposition created → material locked, **no alpha** → SignalR notifies the supervisor → supervisor reviews gauge trace, footage, reason, operator, timestamp → **Accept / Hold / Reject** → disposition record written | 4 May 2026 | No partial spool alpha exists until approval |
| **OQ-64** | Pass-schedule data is **not printed on the coil label**; the schedule ID, version and effective configuration **are logged against the coil record at creation** | 4 May 2026 | Auditable even after the schedule is later edited (`CoilOutput.PassScheduleSnapshot`) |
| **OQ-78** | An **anneal modifies the existing spool alpha** — no child alpha. **Re-passing a spool through FL1 is not applicable** (no capability) | 4 May 2026 | The anneal is recorded as an event against the existing alpha |
| **OQ-65** | Post-die-change SPC is a **hard block on full production**, not a soft queue. **Thread mode is permitted** while SPC is taken. Confirm routes to the SPC Checkpoint screen for Gauge drift and Size change. Every toggle-off is audited and surfaces as a flagged exception | 4 May 2026 | Override authority (who may toggle it off) remains open — Operations Manager minimum is the standing recommendation |
| **Spool weight — part 3** *(the register question was withdrawn 12 Aug 2026; the residue is **OI-56**)* | An **out-of-tolerance spool weight must not block spool creation.** The completion is **authorised, not prevented**: a supervisor override appears (reason + badge/ID + PIN, remote-approval fallback) and the commit control stays enabled throughout | 29 Jul 2026 | The override, the authorising supervisor and the reason land on the spool record. **The PIN is never stored** |
| **OQ-24 (off-schedule)** | ~~A rod whose order is scheduled on **another line is not refused.** The operator is notified and a **supervisor override** authorises staging it here~~ **SUPERSEDED 30 Jul 2026: no message and no override — the system auto-selects the correct station**, at both pre-check-in and check-in. The columns below are dropped | ~~29 Jul 2026~~ → 30 Jul 2026 | ~~Recorded as `OffScheduleOverride` + `ScheduledLineId` + the credential stamp. Rod→order resolves from `planning_routings`, so the order is *revealed* by the scan rather than chosen — which is what makes even the first rod on a cold line validatable |
...
| **G19** | **Pre-check-in is delivered.** The SRS specified it with numbered, testable requirements, but it had no analysis note, no mockup, no data model, no API and no phase owner — and the WIP-station script explicitly declined to create its station | 29 Jul 2026 | Delivered as `screens/RodPreCheckin.md`, Dashboard 2A, the `RodStaging` table, `/staging/**`, `PayoffStateChanged` and the `FL1PO` station. Two items still need business sign-off (OI-01, OI-44) |

### 10.4 Superseded decisions, preserved

| Superseded position | Replaced by | Why it matters |
|---|---|---|
| Drop the `Rod` table; rod lives only in `coils`; 21–22 tables; all rod refs unenforced cross-DB links | **D-04** — `Rod` retained with enforced FKs, 28 tables | Anyone reading `../20-architecture/Architecture.md` §13.1 `D-04` or `phase-01c` in isolation will build the wrong schema |
...
| the interim single-page layout (then `dashboard_2_rod_checkin.html`) and `- Old.html` | **D-08** — the wizard, `- New.html` until it was renamed `dashboard_2_rod_checkin.html` on 11 Aug 2026 | Both retired files were **deleted 11 Aug 2026** (recoverable at `d79ce78`); two check-in implementation documents still describe the deleted interim layout as their source mockup |
...
| Max finished coil weight 1,000 lb | **OQ-77** — 1,100 lb | Two figures circulate; 1,100 is current |
| Coils-table inventory "22 tables" in the SRS `DM001` | **D-03** — 27 | The SRS was written before `PayoffPosition` and `RodStaging` |
| **`FR-381` / `FR-384` / `FR-385` / `FR-386` / `FR-387` — the generate-from-specs arithmetic** | **`screens/PassScheduleGenerationSpec.md` v1.5** (6 Aug 2026) | **Five direct contradictions, on physics rather than wording — see §10.5** |
...
deliverable. The reconciliation therefore lives here and in `../95-archive/design-notes/REVIEW.md`, not in it.
...
| **OI-45** | **OQ-10 — footage-to-weight: the formula and the density source are now settled; the *dimensional basis* is not.** §5.4 fixes the calculation as `A × 12ρ` per foot with a round-edge area correction, and ρ as `united_db..alloys.alloy_density` (unit verified lb/in³). **Four things still need sign-off:** (a) the basis — **target** dimensions, **measured-at-completion**, or **integrated over `RunReading`** (integration is recommended, because target-derived weight carries a ±2.6 % tolerance stack that exceeds the ±2 % variance threshold in `FR-153`); (b) whether the round edge is a true semicircle or a partial radius, which changes the 0.2146 coefficient; (c) Process Engineering confirmation of the density values; (d) the tail-loss treatment in `FRT011`'s `Net = Gross − Tare`. ⚠ **A CLIENT-SIDE DATA POINT ON (b), 2 Sep 2026.** Tim's own formula document (`D-43`) uses **two incompatible flat-wire areas in the same five pages** — the **rounded-edge** area for cross-section (`F10`/`F14`, which confirm the 0.2146 coefficient) but the plain **`W × T` rectangle** for the two footage formulas (`F4`/`F17`). So the basis question is live on his side too, and his footage number and ours can differ by the 3.8 % edge correction **before** any density disagreement. The legend does settle it by *operation* — *"rounded edges for mill stands, and width × thickness for edgers"* — which is a different cut than (b) asks for | Every output weight, net weight, yield figure, remaining-weight estimate and spool-progress calculation. The basis choice also decides whether `FR-153`'s ±2 % threshold is workable | **Phases 9, 12** (and the spool alerts) | Tim O. / Bob S. / Process Engineering |
| **OI-46** | **OQ-14 — no-match path at check-in is undefined.** The attribute lookup and confirm bar are specified, but what happens when **no active schedule matches** the order's attributes is not. Block and alert Operations, or allow a credential-gated manual override? | An operator on a new product variant reaches check-in with no schedule and no defined path forward | **Phase 4** | Tim O. / Jaspreet |
| **OI-47** | **OQ-15 — FL2 check-in validation for hybrid-origin spools is undefined.** The unified single-record FL3 schedule is settled; what is not settled is whether Dashboard 5 must prevent applying a standalone FL2 schedule to material originally run under a hybrid configuration | Wrong configuration silently applied to hybrid-origin material | **Phase 8** | Tim O. / Jaspreet |
...
| **OI-125** | ✅ **Resolved 22 Aug 2026 against the live instance — no runtime risk, and the finding was inverted.** `coils` **is** a real `USER_TABLE` in CommonDB (1,710 rows), and **`proddb..coils` is a SYNONYM whose base object is `[CommonDB]..[coils]`** — so the table `GenerateCoilAlpha` sweeps through its unqualified `FROM coils` is the same table `FlatWire_CompleteCoilOnSkid` writes finished coils into. The uniqueness sweep therefore **does** see flat wire's writes, and the deployment-verification concern is discharged for both FL1 and Phase 9. The companion half is resolved too: the scrap-weight procedure exists and **does call `GenerateCoilAlpha`**, so it participates in the same sweep. ⚠ **Two residuals, both smaller than the original item.** **(1)** `CommonDB..coils` is **unscripted in `ual-database`**, which is why the repository could not answer this — a scripting gap to close, not a defect; and `united_db..coils` is a *different* table (2,421 rows), so an unqualified `coils` is database-dependent. **(2)** A third-party caller of `GenerateCoilAlpha` cannot see `FlatWireDB`, so it can be issued an alpha an FL1 spool segment already holds — the limit of the one-namespace guarantee, now **`Q59`**. **Raised and resolved 22 Aug 2026** | Discharged for deployment. What remains is a scripting gap and `Q59` | — | IT / DBA |
| **OI-126** | **`FR-075`'s per-write PLC tag audit log has no named sink.** The requirement is that every tag write is auditable, and `[PLCC §3]` states it - but no table, file or external store is named anywhere, and the schema has no candidate. Promoted 23 Aug 2026 from the retired `GapAnalysis.md` (**A5**), which proposed it as `OI-108`; that number had already been minted for the cold-start *Welds this run* question, so it is renumbered here. **Our reading:** a `FlatWireDB`-local append-only table, because the write set is small, the retention requirement is unstated, and Serilog's file sink cannot be queried per tag | The requirement cannot be verified, and a tag-push dispute has no evidence | Phase 4 | Architecture / controls |
| **OI-127** | **The recorded weld footage and the material boundary are captured at different moments, and nothing states whether they coincide.** The encoder is read when the operator makes the join **at the payoff**, but the join itself passes the counter later — after the outgoing rod's remainder has run through. `CoilTraceability` and `SpoolTraceability` bound their segments at the **material** boundary, so the two differ by however much wire sits between the payoff and the counter. ⚠ **Not a duplicate of `OI-25`:** that item is about two footage *coordinate systems* (run-cumulative against coil-local) and the missing offset between them; this is about the *moment of capture* within one system. Both must be settled to attribute footage correctly, and settling either does not settle the other. Raised 25 Aug 2026 from [`RodOrderAllocation_WorkedExamples.md`](../95-archive/design-notes/RodOrderAllocation_WorkedExamples.md) §12 finding 3, which recorded it without minting an id | Source-rod attribution is wrong by the payoff-to-counter distance on every welded run. For a **welding-wire customer certificate**, which is the reason the genealogy exists, that is a wrong statement about which heat produced which footage | Phase 6 / Phase 9 | Architecture / controls |
...
| **OI-37** | ✅ **Largely closed 15 Aug 2026 — `G6`: all six roles exist as JWT claims**, on the standard `ClaimTypes.Role`. Operator / Supervisor / Operations Manager / Maintenance / QA / Admin are all present in `Login`; **no provisioning is needed** and no addition to `UA.Framework.Common`'s `JWTClaims` struct. ⚠ **Residual: the six claim *values* are abbreviated or coded**, not the matrix's labels, and the mapping is unsupplied — it gates verification, not construction. ⚠ **Unaffected:** `Engineering/Maintenance` and `QA` remain **inferred** definitions — a claim existing does not make the capabilities attributed to it correct | ~~Authorisation may block the build outright~~ — **spent.** Now: the matrix cannot be *verified* against the running system until the value mapping lands | Phase 1 | Security / Login owner |
...
| ~~**OI-42**~~ | ~~**`Rod` ↔ `coils` synchronisation is unspecified.**~~ ✅ **CLOSED 19 Aug 2026 — and it was worse than "unspecified": nothing populated `Rod` in production at all.** Rod receiving (`FW-020`–`FW-022`) is upstream, was removed from this backlog as another team's work, and writes `proddb..coils` — not `FlatWireDB`. The only thing that ever put a row in `Rod` was `FlatWire_SampleData_Materials.sql`. With `FK_RodCheckin_Rod` and `FK_RodStaging_Rod` **enforced**, the first staging or check-in on a clean production database would have failed on a foreign key. **Both halves are now answered** — *when*: `FlatWireDB.dbo.sp_IngestRodFromCoils` projects the rod on the **first write that names it** (staging or check-in, whichever comes first), inside that operation's transaction, and never on a read; *which side is master*: the column-ownership split is tabulated in `[INT §7.9]`, and the refresh touches **shared-mastered columns only** — `Status`, `FootageRunToDate` and `RemainingWeightEstimateLb` are locally mastered, and overwriting them would un-mark a running rod and destroy the carry-forward evidence `FR-043` depends on. `FR-529`–`FR-532`, story `FW-223`. **Two columns `coils` cannot supply:** the rod diameter (no such column — the operator's measurement is used, which is itself an argument for ingesting at first use rather than at receipt) and the supplier heat (**`OI-117`**) | ~~Two sources of truth for rod material with no reconciliation~~ | Phase 4 | Architecture |
| **OI-117** | **`Rod.SupplierHeat` has no source, and the welding-wire certificate chain is documented as tracing through it.** `FlatWireSchema_Materials.md` states that the rod record *"links material certification data (supplier heat) to the finished output coil via `CoilTraceability`"* — an MVP-1 obligation. But `proddb..coils` has **no heat column**, no check-in or staging payload carries one, and `coil_origin_code` is a one-character origin flag, not a heat number. `sp_IngestRodFromCoils` leaves the column `NULL` **deliberately**. `[INT §8]` lists *"`Lots` / chemistry — the far end of the cert chain — Read"*, which is the likely source and is unmapped | A welding-wire customer certificate cannot state the supplier heat of the material it certifies | Before the first certificate is issued | Quality / Architecture |
| **OI-115** | **The shared write set for FL2 spool check-in is undefined, and it blocks building rather than merely deploying.** `[API §4.6a]` says `POST /checkin/spool` has *“the same shape as §4.6”* and then lists **only `FlatWireDB` writes**. A spool has **no `proddb..coils` row**, so the reqsum, `actual_start_date` and the WIP-station claim are all undefined for it — and parking `SP-00021` in `WIPStations.CoilNo`, a column every legacy reader treats as a coil number with **no FK to stop it**, needs an explicit answer rather than an inference. `@lineId` refuses FL2 until it closes. **Raised 19 Aug 2026 and registered here 20 Aug 2026** — it was cited in `G45`, `[INT §8.0]`, `[REQ §5.26]` and `FW-220`’s plan for a day without existing in this register (gap **`G46`**). ⚠ **Narrowed by the 20 Aug client call**: FL2 scans **a coil code off the spool**, not the spool alpha (*“they’re going to scan in the coil code that’s on the spool”* — the furnace-plate behaviour), so the natural station-claim value is the **lead rod alpha**, which every legacy reader would read correctly. That is the identifier half; the reqsum and `actual_start_date` halves are still open | The FL2 half of the check-in write-back cannot be built, and a wrong answer puts a non-coil identifier into a column the whole plant reads as a coil number | Phase 4 | IT / Architecture |
| **OI-116** | **Whether flat wire owes `coil_mill_processing` a row.** `PreCheckIn_CopyPlanningData` writes it alongside `routings`, together with `coil_slitter_processing` and `slitter_head_setup`. Whether the rolling-processing table needs a flat wire row is open and affects mill reporting. **Copying it speculatively is the worse error** — adding rows later is additive, removing wrong ones is not. **Raised 19 Aug 2026, registered here 20 Aug 2026** (gap **`G46`**) | Mill reporting may omit flat wire, or may carry rows nothing should have written | Phase 4 | IT / Architecture |
| **OI-118** | ⚠ **Schema half closed 22 Aug 2026 — `SpoolStaging` is built** in `04_Runs`: one row per spool, no payoff position, no inspection columns, **no station claim**, `QueuePosition DECIMAL(9,3)` deliberately **not** unique (a drag-and-drop swap must be able to hold a transient duplicate) and `UX_SpoolStaging_LiveSpool` filtered on `Status='Queued'` so a spool can re-enter the queue after check-in. **What remains open is the requirement text, the endpoints and the screen** — `FR-031` is still written as a refusal. The original finding: **`RodStaging` cannot host an FL2 pre-check-in, so the 20 Aug reversal of `FR-031` adds a table.** The client now wants pre-check-in at FL2 — validate the next spool rather than discover it wrong at check-in — but **`PCI002`’s physical premise was never contradicted**: FL2 has one traversing payoff and no floor space. And `RodStaging` is rod-shaped throughout: oxidation / surface-defect / water-stain inspection columns that `RocCheckin.md` §4.3 says are **not performed** on a spool, plus `IsWelded`, `UnstageKind`, two-bay states, `PayoffPosition NOT NULL` and `CK_RodStaging_LineId IN (FL1, FL3)`. Widening that CHECK would admit FL2 rows that cannot populate half the table. **Raised 20 Aug 2026** | ✅ **The published count was re-derived and swept on 22 Aug 2026.** It was **34 tables · 57 FKs · 69 index statements · 1 procedure · 1 trigger** on that date, from a real teardown-and-deploy plus an idempotent re-run; the 23 Aug `SpoolConfiguration` merge (`Q60`) then took it to **33 · 55 · 69 · 1 · 1**, which is the live figure — `[DBD §6.2]`. ⚠ **This clause’s own estimate of “28 → 31” was short** — it counted the two genealogy tables but not `Spool` (`OI-120`) or `SpoolStaging` (`OI-118`). And its warning **never incrementally** proved literal: measured against a database that still held `sp_IngestRodFromCoils` from an earlier session, `sys.procedures` returned **2**, and the figure was briefly published as “2 procedures” before a real teardown showed **1** — that procedure ships in `Database/Scripts/`, outside `RunAll` | Phase 4 — blocked on **`Q41`** for shape | Architecture |
| **OI-119** | **`SpoolProcessing.OrderNo` and `SpoolCheckin.OrderId` are scalar, against a spool the client has confirmed carries many orders.** `SpoolProcessing.OrderNo` is `VARCHAR(50) NULL`; `SpoolCheckin.OrderId` is `VARCHAR(20)` **`NOT NULL`**. The 20 Aug call settled that a spool off FL1 may carry two or more orders while **FL2 makes one order at a time**, so the set and the selection are different things and only the second has a column. **The `NOT NULL` also contradicts a published rule already**: `SpoolQueue.md` **SQ-6** states an unallocated spool *“may still be checked in”*, which the constraint forbids — a pre-existing defect this decision exposes rather than creates. **Raised 20 Aug 2026.** ⚠ **PREMISE CONTRADICTED 3 Sep 2026, STAYS OPEN.** This item's whole basis is *"a spool the client has confirmed carries many orders"* (20 Aug). Tim O'Brien now: *"in both cases the system should require a check-out/check-in of the Rod, and the check-in of a new empty spool. **I do not believe that we want to have multiple orders on a spool.** The process would become very complex and the room for error at coil generation/skidding would be substantial."* **If ratified, scalar `SpoolProcessing.OrderNo` and `SpoolCheckin.OrderId` are CORRECT and this item dissolves** — but the `SpoolQueue.md` **SQ-6** `NOT NULL` defect below is independent and survives either way. ⛔ **Not closed:** one voice by e-mail against a multi-voice call, per `ClientCall_2026-08-24_SyncPlan.md` §3.1's `D1` precedent. Ratification owed (audit record [`ClientEmail_2026-09-03_RodOrderAllocation_SyncPlan.md`](../95-archive/source-documents/ClientEmail_2026-09-03_RodOrderAllocation_SyncPlan.md)) | Order allocation on a spool cannot be represented; and the one documented case of a spool with no order cannot be checked in at all  ⚠ **`D-57` (8 Sep 2026) BUILT the multi-order reading** — `SpoolOrder` is re-grained onto the segment — **which does not resolve `Q43`**; ratification is still owed, and this item dissolves if it resolves the other way. | Phase 4 / Phase 7 — **`Q43`** | Architecture / Planning |
| **OI-120** | ⚠ **Schema half closed 22 Aug 2026 — `Spool` is built** in `01_Lookup` (the schema was 32 tables at that point; it is now 34), keyed on the stencil string with `UQ_Spool_No` and an `IsActive` soft delete matching the other lookups. **What remains open is the format and the count (`Q42`) and seeding it from the physical inventory — nothing populates it yet**, and `Spool.SpoolId` is a nullable FK precisely so the schema does not pretend otherwise. The original finding: **no physical spool carrier entity existed anywhere in the schema.** The 20 Aug call confirmed the carrier is a **reusable stenciled article like a furnace plate** — 30 purchased, a further 15 under decision, **all one standard size, all identical except for identity** — entered as validated free text rather than picked from a list. `SpoolConfiguration` looks like the home and is not: it is a **size class** (`Name` e.g. 15lb / 30lb, with min/max weight, core and outer diameter), so under the client’s confirmation it holds **exactly one row** while the carriers number 30 to 45. And **no column links a material `SpoolProcessing` row to the carrier it is wound on** — `SpoolProcessing.Alpha` (`SP-#####`) is the material identity, which is precisely the distinction `SpoolQueue.md` open item 1 was raised to force. **Raised 20 Aug 2026** | The carrier cannot be validated on entry, cannot be tracked FL1 → furnace → cooling → FL2, and cannot be returned to the pool after FL2 — all three of which the client described as the operating cycle | Phase 4 — **`Q42`** | Architecture |
| **OI-121** | **Weight per source alpha on a spool has no home, and the number it would be derived from is itself open.** Tim, 20 Aug: *“It’s not a matter of how many coils are on a spool. It’s how many pounds are on the spool and **how many pounds for each alpha**.”* `G42`’s proposed `SpoolTraceability` child carries source rod alpha, sequence, footage range and a weld reference — **footage, not weight**. Converting one to the other is **`Q10`**, the footage-to-weight dimensional basis, which is `Critical`, deliberately carries **no recommendation**, and is the most widely depended-on open number in the build. So a derived weight-per-alpha inherits `Q10` wholesale, and a stored one needs a source. **Raised 20 Aug 2026.** **Update 24 Aug 2026 — the creation *moment* is now agreed by all three UA voices, the *content* still is not.** Asked at the planning end whether alphas are generated at runtime as spools are actually created, Srikanth: *“dynamically is fine and I think we should do it at the **transaction time, not at the check-in time**”*; Bob: *“you have to wait till the transaction”*; Tim: *“I also agree.”* On 20 Aug only Srikanth held this and Tim's *pounds-per-alpha* alternative was unadopted; it is now three-way agreement, and it newly rules out **check-in** as the moment. It settles **when**, not **what** — this item's weight column is untouched, and action `A1` (the Excel walkthrough) is still the only thing scheduled against the replacement design. The recording renders “alphas” as *“L pass”* / *“L plus”* — a transcription artefact, **confirmed with Yash on 24 Aug 2026** as *“alphas are generated dynamically at the actual transaction”* | The welding-wire customer certificates state material provenance by quantity; a per-alpha weight that silently rests on an unconfirmed conversion is worse than one that is absent | Phase 7 / Phase 9 — **`Q10`**, **`G42`** | Quality / Architecture |
| **OI-123** | **`FlatWireRun.OrderId` stops meaning what its readers think at an order boundary.** The rod ↔ order design keeps a rod mounted across the boundary — one check-in, so **one run spans two orders** (the schema forces this: `RodCheckin.RunId` is a single `NOT NULL` column, so one check-in cannot point at two runs). `FlatWireRun.OrderId` is a scalar `VARCHAR(20) NOT NULL`, so it narrows to *“the order this run was checked in against”* while the run outlives that order. **Raised 22 Aug 2026** | ⚠ **This is an existing-reader problem, not a new-table problem.** Anything reading `FlatWireRun.OrderId` as *“the order this run produced”* — reports, screens, the shared write-back — becomes wrong at a boundary, silently. Per-order truth moves to the consumption record and per-output truth is already on `CoilOutput.OrderId`. ⚠ **PREMISE CONTRADICTED 3 Sep 2026, STAYS OPEN.** This item rests on the rod staying mounted — *"one check-in, so one run spans two orders."* Tim O'Brien requires the opposite: *"in both cases the system should require a check-out/check-in of the Rod, and the check-in of a new empty spool."* **A check-out and check-in at the boundary means two check-ins, therefore two runs, therefore scalar `FlatWireRun.OrderId` is right and this item dissolves.** ⛔ **Not closed** — same ratification caveat as `OI-119` (audit record [`ClientEmail_2026-09-03_RodOrderAllocation_SyncPlan.md`](../95-archive/source-documents/ClientEmail_2026-09-03_RodOrderAllocation_SyncPlan.md))  ⚠ **`D-57` (8 Sep 2026) BUILT the multi-order reading** — `SpoolOrder` is re-grained onto the segment — **which does not resolve `Q43`**; ratification is still owed, and this item dissolves if it resolves the other way. | Phase 4 — audit the readers | Architecture |
| **OI-124** | **The order boundary is lost at the spool hop, and the window to fix it cheaply is open now.** A spool wound across an O1→O2 boundary records *that* it carries two orders and **not where the boundary is**: the spool↔order association carries a sequence and a planned weight but **no positional column**. FL2 makes one order at a time and must cut at that point — and cannot compute it. Separately, that table's header documents its order set as *“read from the shared `planning_routings` rod→order allocation”* — a workaround written **because the rod↔order table did not exist**; it should now derive locally, which also removes a shared-schema read from the FL1 path (`D-32`). **Raised 22 Aug 2026** | FL2 cannot place the cut, so a two-order spool cannot be finished correctly. ⚠ **Time-sensitive on the `G42` argument:** the table was created 22 Aug and **nothing writes it yet**, so today this is two columns in a DDL file; afterwards it is a migration plus a backfill against certificate data. ⚠ **PREMISE CONTRADICTED 3 Sep 2026, STAYS OPEN.** *"The order boundary is lost at the spool hop"* presupposes a spool that spans one. Tim O'Brien: *"**I do not believe that we want to have multiple orders on a spool**"*, with a rod check-out/check-in and a new empty spool at the boundary instead. **If ratified there is no boundary inside a spool, and this item and `G48` both dissolve.** ⚠ **The time-sensitivity argument now cuts the other way** — adding the positional column *before* ratification risks building a column for a case that cannot occur. ⛔ **Not closed** — same caveat (audit record [`ClientEmail_2026-09-03_RodOrderAllocation_SyncPlan.md`](../95-archive/source-documents/ClientEmail_2026-09-03_RodOrderAllocation_SyncPlan.md))  ⚠ **`D-57` (8 Sep 2026) BUILT the multi-order reading** — `SpoolOrder` is re-grained onto the segment — **which does not resolve `Q43`**; ratification is still owed, and this item dissolves if it resolves the other way. | Phase 8 — **`Q54`**, gap **`G48`** | Architecture |
| **OI-52** | **OQ-1 — roll-gap validation before run start.** Three options open: operator measures and enters, PLC encoder readback compared to setpoint, or no readback (the current implied design, rated HIGH risk). A secondary question follows: is a supervisor override enough to bypass an out-of-tolerance gap, or is it a hard stop? | The run may start on gaps that were never verified | Phase 4 | Jaspreet / Tim O. + engineering |
| **OI-53** | **OQ-12 — the full carry-forward design** (persistent rod record, `footage_run_to_date`, `remaining_weight_estimate`) is confirmed in principle but not fully. Also open: whether a **payoff-side scale** exists or is required for weighing partial rods returning to the warehouse | Partial-rod accounting; the same scale question feeds OQ-33 and OI-56 | Phase 7 | Tim O. + Scott/Bob/Shannon |
| **OI-54** | **OQ-13 — PLC tag behaviour on checkout** is In Progress. The proposed rule (never send stop, clear only when confirmed stopped, block while running) awaits engineering confirmation, and the exact stopped-state definition is unconfirmed (see OI-35) | Phase 7 build cannot be finished | Phase 7 | Jaspreet / Tim O. + engineering |
| **OI-55** | **OQ-17 — the spool status state machine** is In Progress. Unique IDs and the FL1 → furnace → cooling → FL2 workflow are confirmed; the full valid-transition set is not. Also unreconciled with OI-06 | The system cannot prevent invalid progressions (a COMPLETE spool re-opened for FL2, a spool planned for two orders) | Phase 8 | Tim O. / Jaspreet |
| **OI-56** | **Scale-vs-calculated spool weight — the remaining parts. Sole tracking home** *(the register question was withdrawn 12 Aug 2026 as ours to answer, not the client's)*. Open: the variance **tolerance value** (±2 % proposed), the **default basis** (scale pre-selected proposed), and **whether a scale exists at the take-up at all** — if none does, the capture ships optional and the calculated weight stands, but then the OD→weight formula never gets validated against measured data. The out-of-tolerance authority is already decided — §10.3, *Spool weight — part 3* — and the PIN source is **OI-38**. **Our reading:** ±2 %, the scale winning once entered with an operator override back to calculated | The spool completion step's central reconciliation | spool completion feature | Tim O. / Shannon R. |
| **OI-57** | **Published tolerance bands** per alloy and temper (ASTM B236, customer PO, or UA internal) are undefined ✅ **PARTLY ANSWERED 10 Sep 2026** — the client supplied fifteen bands across four product forms, and the flat-wire square-edge **width** table is explicitly temper-qualified (*"All Tempers Except O, F, and H112"*), which is the dimension this item names. ⛔ **But the grain is size band × edge profile, not alloy**, so it cannot be seeded where this item assumed — see `G121`, and note the standard cited is **`B211/B211M`**, not this item's `B236` (`Q110` item 5). Transcribed at the 10 Sep sync plan §3.1. **Sole tracking home** *(register question withdrawn 12 Aug 2026)*. **Our reading:** ASTM B236 as the baseline, overridden per order where the customer purchase order is tighter, held in the same lookup the dimensional-tolerance change adds (**OI-07**) rather than a second table — Mick to confirm the standard, the structure being unchanged if it is a different one | SPC control limits cannot be configured, and the gauge-trace report produces no meaningful alarms | Phases 6, 11 | Tim O. / Mick |
| **OI-58** | **OQ-5 / OQ-8 — certificate granularity and frequency.** What traceability unit welding-wire customers require (coil / lot / heat), whether it must appear explicitly on the C of C, and whether certs are per coil, per order or per heat | Gates the cert query and report format before first shipment | Phase 11 | Tim O. / Mick |
| **OI-59** | **OQ-6 / OQ-7 — weld attribution and limits, plus the rework-weld certificate question.** The footage-split rule at the exact weld point; the **maximum weld joints per finished coil** (a validation rule the system must enforce); and whether a re-weld after a break must appear on the cert. **This is the sole tracking home for the rework-weld part** *(its register question was withdrawn 12 Aug 2026)*, and that part has two routes which may deserve different answers: a weld that **failed its quality check at capture** put no accepted material through the join, whereas a weld that **broke mid-run** did. **Our reading:** the failed attempt is an annotation on the successful weld, the mid-run break is cert-visible, both persist as `WeldEvent` rows either way — and whatever is decided must **not** be implemented by preventing the second row, since no uniqueness constraint exists on `(RunId, OutgoingRodAlpha, IncomingRodAlpha)` and none should. On the joint limit: build the validation now as a **per-order configurable limit defaulting to unlimited**, and do not count a superseded attempt toward it. ✅ **JOINT-LIMIT HALF ANSWERED 3 Sep 2026.** Tim O'Brien: *"**ASTM standards do not set a limit to the number of welds, or welding across LOT#s/Heats, only across differing alloys, tempers, & drastically different chemistries.**"* So there is **no standards-derived cap**, and the real constraint is a **compatibility** rule on alloy / temper / chemistry — a different validation from the one this item assumed. The recommendation above (a per-order configurable limit defaulting to unlimited) is **confirmed as the right shape**. ⚠ **What is still open is the value and where it lives:** a customer may specify a weld count and *"we did not capture this in any fields in the quotes & orders applications"* — tracked as **`G89`**. The footage-split and rework-weld halves of this item are **untouched** by the answer (audit record [`ClientEmail_2026-09-03_RodOrderAllocation_SyncPlan.md`](../95-archive/source-documents/ClientEmail_2026-09-03_RodOrderAllocation_SyncPlan.md)) | `WLD016` cannot be implemented; exceeding a customer's joint limit causes wire jams in their welding equipment | Phases 6, 11 | Tim O. / Sales / Mick |
| **OI-60** | **Expected metallic yield per route** (rod → flat direct, rod → round wire → flat, flat → flat re-pass) is undefined. **This is the sole tracking home** — the open-questions register entry was removed as out of shopfloor scope on 12 Aug 2026. Also open whether the flat → flat re-pass route is real, UA being unable to run a spool back through FL1 | Planning cannot size rod input weight per order | upstream Planning; Phase 12 | Tim O. / Jeff G. |
| **OI-61** | **OQ-11 — per-pass scrap allowance** (die entry crop, edge trim, end crop, weld scrap) for planning rod input sizing | Planners systematically under-order rod and discover the shortage at the machine | upstream Planning | Tim O. / Margo |
| **OI-62** | **OQ-9 — twist and torsion tolerance for welding wire.** Maximum allowable twist per foot for feedability through automated welding equipment | A common first-shipment field failure; may need to be a system-enforced checkpoint | Phase 11 | Tim O. / Technical |
| **OI-63** | **OQ-2 — FL3 scheduling representation.** ⛔ **THE CLIENT HAS NOW ANSWERED THIS TWICE, DIFFERENTLY.** On **8 Sep 2026** he agreed the recommendation — *"I agree, a single FL3 booking should reserve FL1/2 capacities"* — and on **10 Sep 2026**, asked again, he deferred: *"This one is tough, I would like to see what Stephen has to say."* **The agreement stands; nothing new was raised against it.** ⚠ Reconcile this item with `Q2`, whose header still reads `Open` while its body records the 8 Sep answer — all three sites together, not one at a time (`Q110`). Partly decided (FL3 cannot run if FL1/FL2 have scheduled orders; all three tracked as separate machines); how FL3 is represented as a **booking unit** is still open | Capacity planning logic | upstream Scheduling | Tim O. / Stephen |
| **OI-64** | **Anneal rules and shared furnace capacity. Sole tracking home** *(both register questions withdrawn 12 Aug 2026)*. No timeline for the rod/wire anneal rules the scheduler needs; and whether flat-wire furnace slots share the existing coil anneal schedule. **Our reading:** **decouple the flat wire build from the rules** — `OQ-78` already settles the only flat wire behaviour that depends on an anneal (the alpha carries through it unchanged), so no flat wire phase should depend on a date; if the rules are late, flat wire schedules the anneal as a manual step. On capacity, assume **shared, and model furnace contention**, because that is the assumption that breaks if it is wrong — dedicated capacity is a simplification that can be applied later. ⚠ **EXTENDED 3 Sep 2026 — the anneal *types* are now an open question in their own right, and the answer is partly already in our code.** Asked of UA on 2 Sep 2026: is there **only plain anneal** for flat wire, or also **Back Anneal, QC Anneal, Upfront Anneal**? Unanswered. ⚠ **Answer it against the evidence, not from scratch:** *QC Anneal* and *Upfront Anneal* appear nowhere in this repository, but `40_FlatWireDB_Proc_FlatWire_CheckInRod.sql` already routes through `Common_GetProgramNo`, which its own comment says *"exists to handle `program_no = 171` (back anneal)"* — so **the flat wire check-in path already accommodates a back anneal**. If back anneal is in scope, part of the path exists; if it is not, that helper's rationale needs restating. `united_db..alloy_anneal_cycle` already exists upstream (§5.12). Audit record [`ClientEmail_2026-09-03_ProcessLetters_SyncPlan.md`](../95-archive/source-documents/ClientEmail_2026-09-03_ProcessLetters_SyncPlan.md) | The scheduler cannot sequence jobs with an intermediate anneal; and the **anneal type vocabulary** is undefined, so the SMP route cannot say which anneal it means | upstream Scheduling | Dan F. / Tim O. / Srikanth P. |
| **OI-65** | **Coreless coil OD/ID limits. Sole tracking home** *(register question withdrawn 12 Aug 2026)*. The structure is settled (4 May 2026): **UA equipment defines the outer bounds and the customer specifies within them** through orders/quotes. What is outstanding is only the **UA equipment figures for TKUP-2**. **Our reading:** seed them as nullable configuration and treat them as **blocking for the coil-split calculation**, which cannot be validated without them — chase the numbers rather than re-opening the model | Determines how continuous output splits into coil alphas | Phase 9 | Tim O. / Sales |
| **OI-66** | **OQ-33 — the OD → weight conversion formula** for a spool measured by outer diameter at the take-up (using OD, ID, coil width and alloy density) | Spool weight tracking and "assign as-is" stock handling | Phase 8 / spool completion | Tim O. |
| **OI-67** | **OQ-4 — coreless coil skid labeling rules.** Whether skid labeling, alpha assignment and packaging records follow UA's existing coil packaging rules unchanged or need flat-wire adjustments | Skid records and labels may need rework after build | Phase 9 | Tim O. / Shannon R. |
| **OI-68** | **Standard times per machine, and costing standards / industry codes. Sole tracking home** *(both register questions withdrawn 12 Aug 2026)*. **Our reading:** **use the existing industry codes unchanged by route** — nothing in flat wire costing behaves differently enough to justify a new code, and a new one fragments the historical comparison the reports depend on; a new code should be justified by a report that cannot otherwise be produced. For standard times, derive **provisional figures arithmetically from the throughput rates owed as OI-82**, publish them visibly marked provisional, and refine from actual run data after the trial — the same table-driven pattern already agreed for line speed in `OQ-66`. Waiting for authored standard times blocks the yield module for the whole build; a visibly provisional figure does not | The yield and cost-ledger modules cannot be completed | Phase 12 | Tim O. / Jeff G. |
...
| **OI-103** | **No bound exists on a roll-gap change.** Neither a maximum single-step change nor a cumulative limit is specified, and the entered value is written straight to the machine, so a mistyped gap reaches the mill. Raised 11 Aug 2026 | Validation before apply on the roll adjust dialog | Phase 6 | Process Engineering |
| **OI-104** | ✅ **CLOSED — 18 Aug 2026. The skid table is `united_db..wip_skids`, linked to coils through `proddb..wip_skid_coils`.** Skid numbers come from `proddb..generate_new_skid_no` (`order_no(6) + rel_letter + seq(2)`, `char(9)`), taking `MAX` over both `wip_skids` and `wip_skids_hist` — which is exactly what **`FR-339`** requires, so `FR-339` becomes testable for the first time and its *"blocked on `G36`"* note is lifted. Written by `FW-219` / `FR-515`; specified in `[INT §8.1]`. ⚠ **`wip_skids.IsComplete` is `NOT NULL` with no default and is absent from the `proddb..wip_skids` view, so the skid row must be inserted into `united_db..wip_skids` directly** — the view cannot be inserted through. *Original text, retained as the audit trail:* **The skid table `CoilOutput.SkidId` points at has never been located or verified.** The schema and `FlatWireSchema_Mapping.md` both describe it as "the existing skid table", but **no document names it, no story creates it, and nothing confirms it exists** in `united_db` or the planning schema. Raised 11 Aug 2026 when Phase 9 returned to MVP-1 | **Closed.** `phase-09`'s 222 h assumed the table already existed; it does, and `FW-219` is the work of writing to it | Phase 9 | **Closed — IT / Planning to confirm `Q34`–`Q36` before a shared environment** |
| **OI-105** | **Which of three weights is authoritative on the coil record is undecided.** §3.3 calculates net weight from footage and density, the DB7 operator may override it with a scale reading, and `FR-346` now adds a **third** figure — a physical scale weight at the packing station. Three rules are possible and not equivalent: the packing scale replaces the record, it is recorded alongside with the calculated figure governing, or a variance beyond a threshold blocks skid closure. **The coil label is printed before packing sees the coil**, so any rule that changes the weight afterwards forces a reprint. Raised 11 Aug 2026 | The coil record, label accuracy, and every downstream yield figure. **Compounds `OI-45`** — the variance cannot be judged until the dimensional basis is fixed, and Phase 9's 16–32 h reserve was scoped before this second capture point was in scope | Phase 9 | Quality / Operations |
| **OI-106** | **Staging locations are undefined.** `FR-351` requires the packing station to assign a staging location when a skid closes, but **no list of valid locations, no capacity per location and no selection rule** exists in any source document. A free-text field would defeat the purpose of recording it. Raised 11 Aug 2026 | Skid closure at DB7b; without it the staged-location record is unusable and the skids-this-shift table has an empty column | Phase 9 | Operations / Shipping |
| **OI-100** | **Valid rework stages per material state are unspecified.** Rework requires a named return stage, and whether a partially flattened spool may return to the flattening mill is a process question with no answer. Raised 11 Aug 2026 | The rework return-stage list on the WIP rejection dialog | Phase 7 | Process Engineering / QC |
| **OI-98** | ✅ **CLOSED 3 Sep 2026 — ship the skid with one coil.** Tim O'Brien: *"The planners **should be planning the orders in even numbers of coils** to ensure that we ship complete skids, however in the instance that an odd number was produced **we would ship the skid with one coil, as it completes the order**."* ⚠ **Read the two conditions he attaches**, and the order of control: the answer opens *"This would depend on whether or not the customer allowed multiple orders to be combined on skid and whether they had a second order in cue. That is a lot of ‘Ifs’."* So the **primary control is a planning rule** — target even coil counts — and the single-coil skid is the **fallback when planning did not achieve it**, not the design. Neither *hold for another order* nor *close short* is chosen. *Original text, retained as the audit trail:* **The odd final coil has no disposition.** An order producing an odd number of coils leaves a skid holding one coil, against a rule of two per skid. Part skid, held for another order, or closed short? Raised 11 Aug 2026 | Skid closure and the packing queue | Phase 9 | **Closed** — audit record [`ClientEmail_2026-09-03_RodOrderAllocation_SyncPlan.md`](../95-archive/source-documents/ClientEmail_2026-09-03_RodOrderAllocation_SyncPlan.md) |
| **OI-102** | **Shift report export format and recipients are unspecified** — spreadsheet, PDF, or a scheduled e-mail at shift end, and to whom. Raised 11 Aug 2026 | The export action on the shift summary | Phase 12 | Operations / IT |
| **OI-138** | ⛔ **NOTHING WRITES AN FL1 SEGMENT ALPHA TO `proddb..coils`, and the whole 26 Aug alpha design assumes something does.** The design registers **every** alpha in the shared coil master so that `GenerateCoilAlpha`'s own sweep finds prior siblings and **no mint passes an ignore list**. `50_FlatWireDB_Proc_FlatWire_CompleteCoilOnSkid.sql` is the only script that writes `coils`; **no script touches `SpoolTraceability`.** ⚠ **Until the writer ships, a blank-list mint at FL1 reissues `R00001A` on every spool** — `[RodOrderAllocation.md §2.8]`'s FL1 rows are the *design*, not current behaviour. **Four sub-questions the writer must settle: (a)** which `coil_status` a segment row carries — `D-32` bars a new shared value (`INFLAT` is `FlatWireDB`-local) and the coil row's own `ONSKID` is already open as `Q35`; **(b) tonnage multiplication** — rod, segments and coils all group flat under the six-character root via `coil_link_master_coil`, so a report summing children **triple-counts**; **(c) atomicity** — mint and insert in one transaction or a crash leaks a name (the `THROW 51011` pattern); **(d)** whether **`OI-115`**'s narrowing of the spool to one shared face is still wanted once every segment has its own shared identity, the way `Q89` retired `D6`'s narrowing at the coil hop. ✅ **`D-32` is NOT breached** — a row through existing columns is not a schema change. **Raised 26 Aug 2026** — originally `OQ-T` in `[RodOrderAllocation.md §9.2]` | **Build it: `FW-231`, phase-08** (which owns `POST /spool/complete` and `SpoolCompletionNotification.md`). ⚠ Confirm that attribution — that phase is titled *FL2 Spool Check-In* while this write happens when **FL1 produces** a spool | **the entire 26 Aug alpha scheme; `G54`** | Nagarro / IT |
| **OI-139** | ⛔ **THE ROD FALLBACK LIST MAY BE INCOMPLETE — FL2-standalone is not in it.** `[RodOrderAllocation.md §2.8]` names the rod-rooted fallback for *"FL1-standalone and FL3-from-rod"*, i.e. coils with no source segment. **FL2-standalone is spool-fed**, and **nothing enforces that a `SpoolProcessing` row has `SpoolTraceability` children** — the schema carries child→parent FKs only, with no trigger and no guard. So a spool with **zero** traceability rows is schema-legal, and a coil cut from it has **neither a segment alpha nor a rod alpha to root on**. ⚠ **Either the case is operationally impossible — in which case say so and add the guard — or the design has no mint root for it.** **Raised 26 Aug 2026** during the propagation review of the segment-rooting change | **Answer before `FR-561`'s mint rule is finalised.** Two candidate resolutions: forbid an FL2 spool with no traceability rows (a check-in validation), or define a third root for externally-sourced spools | **`FR-561`, `FR-561e`** | Nagarro / Tim O. |
| **OI-140** | ⛔ **A COMPLETED OUTPUT COIL IS NEVER BROADCAST — there is no hub member to send it on.** `FW-207` raises `CoilCompleted` and `FW-208` assigned it the post-commit lane, but `[SIG §5.2]`'s fourteen events and `[SIG §5.4]`'s six markers were counted member by member on 29 Aug 2026 and **none carries a completed coil** — so the event is raised, dispatched, and reaches no handler. **Effect:** the operator who completes the coil on DB7 sees the result through their own request/response; **every other client on the line learns nothing until it refreshes.** Adding a member is a **breaking** change under `[API §8]` and would have to move `[SIG §5.2]`, `IFlatWireClient`, the Angular mirror `[SIG §5.6]` and `FW-136` in one pass — which is cheap **now**, while `FW-136` does not yet exist, and dear later. **The question for the client: does a coil completion need to reach the other screens live, or is a refresh acceptable?** Raised 29 Aug 2026 by `FW-208`'s step-8 build (`P-137`) | Coil completion is invisible to every client but the one that caused it | Phase 9 | Operations / IT |


| **OI-141** | ⛔ **DIE MANAGEMENT AND THE MACHINES-APP TOOLING INVENTORY DESCRIBE ONE TOOL WITH TWO DIFFERENT FIELD SETS, AND `§4.10` SAYS THEY ARE THE SAME SCREEN.** `[MSP §4.10]` states Die Management's **Access: Machines Application → Tooling Inventory tab**, and **`FR-254`** makes it *the source of truth* the Die Change screen reads for alpha → size/type/condition, the accumulated footage counter and the scheduled-life threshold. The client's 31 Aug 2026 Tooling Inventory grid for that tab carries **Machine Name · S/N · P/N · Type · Location · ID(") · ID(MM) · Max Imput Dia. · Pitch · Max ID(") · Lubrication Type · In Use** — **no alpha, no life threshold, no footage, no die material, no status**: none of the four things `FR-254` says Die Change reads from it. `FR-247`'s registration set overlaps it on **three** fields. ⚠ **As of 6 Sep 2026 this spans THREE built tooling registers** — `ToolingInventoryDie`, `ToolingInventoryRollSet` and `ToolingInventoryEdger` (`D-53`). **Two of the three carry an alpha of our invention** (`D-`, `RS-`) because **no client tooling grid has ever shown one**; ⭐ **the edger deliberately carries none** — `EdgerToolAlpha` was removed on 7 Sep 2026, because the client identifies a set by `Machine Name` + `Set Number` + `P/N` and, unlike `DieAlpha`, nothing reads it at runtime. **That is the precedent this `OI` should follow**: an alpha where a caller needs one, not everywhere. ⚠ `Q95` leg 2 supplies the replacement key. ⚠ **The die split sharpens this rather than resolving it:** it puts the die register in `FlatWireDB` as `ToolingInventoryDie` and keeps Die Management as the Angular screen `dashboard_die_management.html` — **but a screen cannot both be an Angular dashboard and a tab in the Machines Application.** Either `§4.10`'s access line is stale, or there are **two die inventories** and `FR-254`'s source-of-truth claim is contested. Extends to edgers and straighteners, for which no ownership decision exists at all (**`G77`**), and now to **roll sets**, whose register is built but whose column set is `[PROPOSED]` (**`G87`** / **`Q92`**) | Die Change is specified against a register whose field list does not contain what it reads; **ownership of tooling inventory is undecided across **four** tool types (`D-42`, 3 Sep 2026)** | Die Management build; `G77` | **Ours — Nagarro design.** Not a client question until we have settled which register is which ⚠ **UPDATED 2 Sep 2026 — the die split (`Q91`) has landed and resolved the FIELD half only.** `ToolingInventoryDie` is built carrying the **union** of both field sets: `FR-247`/`FR-254`’s `DieAlpha`, `DieType`, `LastGrindingFeet`, `TotalFeetAllowed` **and** the client grid’s `S/N`, `P/N`, `Location`, `ID(")`, `Max Imput Dia.`, `Pitch`, `Max ID(")`, `Lubrication Type`, `In Use`. The status clash is resolved by **derivation**: `LifecycleStatus` stores the client’s three values plus `Retired`, while `FR-253`’s `Nearing`/`Overdue`/`Spare` are percentage bands and are computed, never stored. `ID(MM)` is not stored, being derived. ⛔ **THE OWNERSHIP HALF IS UNTOUCHED AND STILL OPEN**: whether this is the only die register or the `FlatWireDB` one of two, and whether `§4.10`’s access line or `FR-254`’s source-of-truth claim is the stale one. If two, `FR-254` needs restating — the table stands either way. |
| **OI-142** | **THE LEGACY-TABLE APPENDIX IN `FlatWireSchema_Mapping.md` DOES NOT RECONCILE WITH ITS OWN SOURCE.** The source workbook `95-archive/source-documents/flatwire tables.xlsx` names the pass-schedule sheet **`FlatLinePassSchedule`**; this repository calls it **`FlatLineSetup`** throughout (`D-13`, `[DBD §188]`, `[MSP §1473/1487/2164/3140/3362]`, the DDL) — **`FlatLinePassSchedule` returns zero hits here** and the rename is unrecorded. The appendix's *"Removed from `FlatLineSetup`"* table names six columns to remove of which **only three exist in that sheet** — `SpoolId`, `MfgOrderNo`/`HomeMfgOrderNo`, `StopNo`/`SequenceNo` and `PlanId` are **`FlatLineProcessing`** columns — while three source columns are **never accounted for**: `RollerInfoId`, `PassNo`, `SpoolTypeId`. It is also the **only** legacy table with no `Current columns` section, in an appendix whose stated purpose is to be *"the only surviving inventory of the legacy columns."* Two further unexplained gaps: the repo lists `Stand` and `SpoolProcessing` as original names where the workbook has **`RollerInfo`** and no `SpoolProcessing` sheet, and the workbook's **`SpoolCoilMapping`** appears in no mapping row | The one document preserving the legacy column inventory is wrong about which columns the table had; anyone sizing the `OI-31` migration works from it | `OI-31` | Database / Plan team |
| **OI-143** | ⛔ **IS `EDGE` AN SMP PROCESS STEP IN ITS OWN RIGHT, OR AN ATTRIBUTE OF THE FLATTEN IT SITS INSIDE?** The client's FL2 and FL3 Flattening Line Schedules show **`EDGE` as a first-class operation row** — `FL2-S1` FLAT · `E1` EDGE · `FL2-S2` FLAT · `E2` EDGE · `FL2-S3` FLAT — with its own entry and target widths, which is what edgers exist to change. The SMP route model in `Analysis/Planning System/tables.md` lists `EDGE` as one of four operations, **and then folds every post-anneal `FLATTEN` / `EDGE` into a single combined `FLATTEN` temp-routing row**, so the operation it names is never emitted as itself. Asked of UA on 1 Sep 2026, re-sent 3 Sep 2026, and ⛔ **unanswered a THIRD time in the 10 Sep 2026 reply**, which discussed edger positioning at length without touching the operation-row question, in two parts: *"Do we need to add edge as a part of SMP?"* and *"Do we need a process letter for Edge as well **regardless of if it's a part of SMP or not**."* Both unanswered. ⚠ **The letter half is `OI-27`, not this item** — and `E` is already taken either way. ⚠ **Distinct from `D-27`**, which settled the *physical* model (the edgers are inter-stand vertical profiling rollers, a forming operation, between S1/S2 and S2/S3): that decision says what an edger **is**, not whether the planning route models it as a step. Raised 3 Sep 2026 | Decides whether the SMP route gains a fourth operation or edging becomes an attribute of the flatten. Either answer changes the route strings and the temp-routing grouping rules, and the SMP build is in progress now | **the SMP build; `G86`** | Srikanth P. / Planning |
...
| **OI-05** | **`Bevel edge` has no domain value.** The Dashboard 9 / 9A Generate modal offers Round / Flat / **Bevel**; the domain and the DB CHECK allowed only `Round` / `Square` until 10 Sep 2026 and now allow the client's **four** — `Natural Round Edge`, `Full Round Edge`, `Square Edge`, `Broken Edge`. ⭐ **`Bevel` is still not among them, but `Broken Edge` is, and they may be the same profile** — one line from the client closes this item (`Q110` item 1). ⛔ **AND IT NOW GATES REAL WORK:** `FW-147` is `done` and its enforcement **is** the two-member C# enum (*"the absence is the enforcement"*), so the enum widening waits on this answer rather than being done twice | Either a fourth vocabulary or dead UI | Phase 2 | Tim O. / Technical |
...
| **OI-09** | **⚠️ UNRESOLVED** — Does an FL2 check-in occur at all in hybrid mode? `./EndToEndProcess.md` says hybrid/FL3 has **no discrete FL2 check-in step** (continuous feed); `RocCheckin.md` and Dashboard 5 define a **hybrid-mode FL2 check-in validation** (`CHK013`) | Determines whether `CHK013` is implementable | Phases 8, 10 | Tim O. / Jaspreet |
...
| **OI-12** | **Die-life colour bands differ between two screens.** Die Change uses green < 60 % / amber 60–85 % / red > 85 %; Die Management uses Active < 65 % / Nearing 65–79 % / Overdue ≥ 80 %. Both are as-specified in their own source | Inconsistent operator signal for the same die | Phases 6, 13 | Maintenance / Tim O. ⚠ **ESCALATED 2 Sep 2026 by the die split (`Q91`).** Both screens are now **MVP-1** and both derive their bands from **one table**, `ToolingInventoryDie`, so this is no longer a cross-scope curiosity — two MVP-1 screens will state different statuses for the same die. Needs a decision before the Die Management build, not a default. |
| **OI-13** | ~~**Wire break has three requirements, no screen, no table and no phase owner.**~~ **Half-closed 6 Aug 2026 — the flow is decided, the persistence target is not.** The client specified the whole sequence: the machine stops, a physical component (*"a dancer or something"*) trips a tag, the system prompts **why the machine stopped**, and on *wire break* it offers **WIP reject** or **weld and continue**. **Weld-and-continue keeps the same alpha** — Bob Scott: *"if it's not actually splitting and you're welding back together, it's the same alpha, no new alpha"*; Shannon Riotte: *"it's coming off the machine as one piece"* — and the weld is recorded at its **footage position** with the material flagged in SCADA. Consistent with **Q61** Case 1. **Still undefined:** where the stop reason, the break confirmation and the `WBK002`/`WBK003` verification results are persisted | `WBK001`–`WBK003` are still unimplementable — the flow exists, the sink does not. Now tracked as gap **G34**; the no-weld-customer branch is **Q31** | unassigned | Jaspreet |
...
| **OI-24** | **Lot number has no column and no generator.** `GET /coil/{alpha}/label` returns `lotNumber` and the label prints it. **Escalated 11 Aug 2026:** Phase 9 is now wholly MVP-1, so this sits on the critical path rather than behind a deferred screen, and the label is physically applied at the packing station (DB7b). Read with **`OI-99`**, which asks how the lot resolves when a coil has more than one source rod — the normal case under welded feed | The label cannot be rendered | Phase 9 | IT / Tim O. |
...
| **OI-26** | **Which WIP station does FL3 pre-check-in post to? Sole tracking home** *(register question withdrawn 12 Aug 2026)*. The schema, DDL and API all scope staging to FL1 **and FL3**, but only `FL1PO` is seeded and no `FL3PO` exists. Working assumption: FL3 posts to `FL1PO` (one physical VPS). Also: should staging rows record `LineId='FL3'` when in hybrid mode even though the station is `FL1PO`? **Our reading:** **one station — `FL1PO` covers FL1 and FL3**, because there is one physical VPS and a second station code would describe a bay that does not exist; still record **`LineId = 'FL3'`** in hybrid mode so the traveler and the run can distinguish the modes. That combination requires `UX_RodStaging_Bay` to be keyed on the **station** rather than `LineId` — which is the same fix **G21** needs, so answer the two together | Determines whether the FL3 Traveler Queue is a distinct queue | Phases 4, 10 | Tim O. / IT |
| **OI-27** | **`machine_type` for the flattening lines is undecided**, and `AccountingDB.dbo.GetMachineTypeFromOpLetter` has **no case for the flattening letter `F`** — it returns NULL for flat wire today. Whichever type is chosen, that function needs an `F` case. ⚠ **UPDATED 3 Sep 2026 — the letter half now has a shortlist, a second and third letter, and a live blocker.** The registration surface is named for the first time: a letter lives in **`dbo.c_process_id.process_letter`** and **`dbo.lookups.display_name` where `lookup_category_id = 930`**, and a query unioning the two returns only **four unused letters: `F`, `K`, `Q`, `U`**. That settles a namespace question this register never asked: the scheduling `OpLetter` `F` and the SMP `FLATTEN`=`F` are **the same letter in one vocabulary**, not two coincidences. ⛔ **`DRAW` also needs a letter and `D` is already taken** — **`K` is proposed and unconfirmed, and carries no client authority**; `EDGE` may need one too and `E` is taken (that half is **`OI-143`**). ⚠ **This is no longer Medium in effect** — the SMP build is in progress and the committed route analysis assigns both unavailable letters (gap **`G86`**); the row stays in §11.3 only because a register id is never re-homed. Asked 1 Sep 2026, re-sent 3 Sep 2026, unanswered — audit record [`ClientEmail_2026-09-03_ProcessLetters_SyncPlan.md`](../95-archive/source-documents/ClientEmail_2026-09-03_ProcessLetters_SyncPlan.md) | FL1/FL2/FL3 appear (or fail to appear) in every query filtering on `machine_type`; and **two of the four SMP operation letters cannot be assigned**, blocking the route-string logic | Phase 13 / FW-003 — **and the SMP build now** | Accounting / IT / Srikanth P. |
...
| **OI-31** | **G8 — no data-migration deliverable for legacy `FlatLineSetup` / `FlatLineProcessing`.** Both are renamed into the new model, but there is no mapping, migration, validation or drop-criteria plan. ⚠ **RE-EXAMINED 2 Sep 2026 — THIS MAY BE PHANTOM, AND THE EVIDENCE POINTS THAT WAY.** Three independent lines: **(1)** `CHANGELOG.md` describes the source workbook, twice, as ***"a May 2026 proposed design that was never built"***; **(2)** the mapping appendix marks its column types *"(inferred)"* and warns they are *"an artifact of the source, not a specification"*; **(3)** a **complete** recursive search of `ual-database` — all 22 databases, run to completion — returns **zero** matches for `FlatLineSetup`. If these tables were only ever a spreadsheet design, **there is nothing to migrate**. ⛔ **Not closed on this analysis.** Absence outside this repository is not proven — the tables could live in an OEM or vendor database. **Confirm with the DBA, then close as not-applicable rather than budgeting a migration.** See also **`OI-142`**, which finds the surviving column inventory does not reconcile with its own source | Legacy data stranded when the old tables are dropped — **or effort budgeted against tables that never existed** | Phase 1C | DBA / Jaspreet |
...
| ~~**OI-41**~~ | ✅ **CLOSED 2 Sep 2026 by the die split (`Q91`).** The per-tool die inventory now exists: **`ToolingInventoryDie`** carries identity (`D-{size×1000}-{seq}`), serial, hole size, type, `LifecycleStatus` and the two die-life columns, and **`DieHistory`** carries registration, reset, retirement and per-run footage. `DieChangeEvent` gained `OldDieId` / `NewDieId`, so a run event can attribute footage to a **tool** for the first time and **`FR-255`** is implementable. **Phase 6 no longer depends on Phase 13 — it depends on Phase 1**, where both tables seed. **`FR-233` / `D4` revert to their pre-11-Aug per-tool form**, and the accepted consequence *"two dies of one diameter share a counter"* is **retired**. ⚠ Open in its place: **`OI-141`** (one die register or two) and **`OI-12`** (now a live band conflict on one table). | — | — | Resolved — see `Q91` |
| **OI-43** | **The OQ-63 unplanned-component-bypass event has no home.** The decision specifies a distinct transaction with component, time, footage, reason, operator, an alpha split, supervisor acknowledgement and a pre-bypass disposition step — **no table, endpoint, screen or story exists for it** | A decided requirement is unimplemented | Phase 6 | Architecture / Jaspreet |
| **OI-44** | **RESOLVED (client, 30 Jul 2026) — approval depends on the weld.** ~~Does pre-check-out require supervisor approval?~~ **Not welded:** operator-only, reason captured — the interim design was right. **Welded:** **supervisor override required**, documented reason, rod to **`HOLD`** — removal means cutting the material, so it is a rejection, not a return. This also resolves **OQ-72 / `WLD011`** for the un-staging direction and reverses the 31 Jul removal of the welded-rod Unstage control. **Build consequence:** `RodCheckout` has **no supervisor columns at all** (OQ-74's mid-run approval is equally unpersisted) — Mode P needs `ApprovedBy` / `ApprovedAt` / `OverrideReason` plus a constraint tying them, and `NewRodStatus='HOLD'`, to the welded case. **Residual requirements gap:** pre-check-out still has **no SRS requirement ID** — §4.17 covers only post-check-in removal, so a new `PCI`-series block is needed | Phase 4/7 build detail; and a requirements gap | Phase 7 | Tim O. / Shannon R. |
...
| **OI-71** | **RESOLVED, INVERTED (client, 30 Jul 2026) — a rod may carry more than one order.** ~~Can a rod be pre-checked-in against a future order? Leaning "no", validation scoped to the *current* order.~~ Srikanth and Tim confirmed the multi-order rod (finishing order 1 on a 7,000 lb A-rod and starting order 2, same alloy), handled in **planning** in multiples of the ~900 lb outgoing coil. The "current order only, else refuse" rule is therefore **wrong for the same-rod successor**, and the recorded consequence *"continuous feed cannot cross an order boundary"* is wrong with it. **Sequencing is not decided** — Srikanth is checking his notes; Shray's one-order-at-a-time proposal is on the table; **MVP2 deferral to be confirmed**. Tracked as **OQ-73 / OI-95**; until it closes the staging validation is a known-wrong rule, not a rule to edit | Whether a rod's second order can be run without stopping the line | Phase 4 | Tim O. / Planning / Srikanth |
| **OI-72** | **`RodSeqno` scope. Sole tracking home** *(register question withdrawn 12 Aug 2026; its decided half is `OI-72 (partly)` in §10.3)*. Per line is the interim choice; per order reads more naturally on a traveler but makes two concurrent orders on one line show duplicate positions. Does per-line numbering need a reset point? Should `PlannedSeqno` be displayed as "3 of 5" rather than a bare ordinal? Does un-staging **release** the actual position for reuse (interim: yes) or leave a gap — reuse means the same number can be issued twice, which a traceability audit may object to. **Our reading:** keep `RodSeqno` **per line with no reset** (a monotonic number is a traceability key, and a reset point makes it ambiguous in the very audit it exists to serve), scope **`PlannedSeqno` per order** and display it as *3 of 5*, and **leave a gap rather than reuse a released position** | Traveler ordering and audit defensibility | Phase 4 | IT |
| **OI-73** | **OQ-24 — off-schedule is REVERSED (client, 30 Jul 2026); the rest stands.** ~~A rod whose order is booked on another line is notified and authorised by supervisor override.~~ **No blocking message and no override — the system auto-selects the correct station** (an FL3-planned rod scanned on the FL1 tab switches the tab), at both pre-check-in and check-in. The `OffSchedule*` column group and `CK_RodStaging_OffSched` are **dropped** (project decision, 1 Aug 2026); `OverrideBy`/`OverrideAt`/`OverrideReason` survive because the out-of-sequence override shares them. Scheduling correction and other-line notification are **moot**. **Newly raised by the reversal:** what happens to a **part-completed wizard** when the station switches mid-transaction, and whether an FL3 tab exists on the FL1 panel at all (**OI-26 / OI-26 / G21**). **Still open:** the mid-order cross-order refusal, now reframed by OI-71 — a same-rod successor order must pass, a genuinely unrelated order need not; and whether the out-of-sequence panel also applies at **Dashboard 2** (only 2A carries it today) | Consistency of the deviation model across screens | Phase 4 | Tim O. / Shannon R. |
| **OI-74** | **OQ-18 basis RESOLVED (client, 30 Jul 2026) — the customer weight range, not a default.** ~~Order *Max Wgt of Spool* versus take-up capacity, with a 2,000 lb default assumed.~~ The customer specifies a **min–max weight** (e.g. 900 max / 800 min) and completion is graded against **that range, by weight**; spools are sized ~**1,800 lb** so two finished coils can be cut at FL2. **The 2,000 lb default is withdrawn** and must come out of the mockup and `spool_notification.js`. **⭐ THREE OF THE FOUR OPEN LEGS CLOSED ON 8 SEP 2026** — ~~*which order field* carries the customer min/max; whether an **over-target M4 state** is required; whether the ladder applies to **finished coils at TKUP-2** (OQ-19); and supervisor mirroring plus the acknowledgement audit record's home (OQ-20).~~ **`Q18`** settles the first two: the minimum and maximum ride on the **order**, reusing the existing *Max Wgt of Spool* field for the maximum and **adding a matching minimum**, and the **over-target M4 state IS required**. **`Q20`** settles the fourth: **only the unacknowledged 100 % milestone** mirrors to the supervisor, and the acknowledgement is recorded in the **existing run-event stream**, not a new record type. Both are in [`Decisions.md`](../90-registers/Decisions.md) — a decided question moves rather than changing state in place — and both are built into [`SpoolCompletionNotification.md`](screens/SpoolCompletionNotification.md) as `D14` and `D15`. **The third leg is the one that did not close cleanly; see immediately below.** ⚠ **THE TKUP-2 HALF HAS AN ANSWER SHAPE, 3 Sep 2026.** Asked what FL2 should do when an order boundary lands a few pounds past a coil cut, Tim O'Brien: *"**Cut at 900 lb. and keep the remainder, a WIPREJ would be forced for the overage**, essentially a peel-back of the coil to put with in the allowable range."* So at a **finished coil** the rule is cut at the maximum and reject the excess, and that mechanism has no reason code or requirement (**`G88`**). ⛔ **THE *"NOT AN ALERT LADDER"* INFERENCE DRAWN FROM THIS IS SUPERSEDED — 8 Sep 2026.** This item previously concluded that *"whether the ladder applies to finished coils at TKUP-2"* was thereby answered **no**. **`Q19` answers that question directly and answers it YES** — the same 75 / 90 / 100 ladder runs at TKUP-2 with coil wording and the coil target weight, agreed on the returned questions workbook without qualification. ⚠ **The 3 Sep statement was an answer to a different question** — what to do when an *order boundary* lands past a coil cut — and the *"no ladder"* reading was **our inference from it**, not the client's words; `Q19` is the direct, later, written answer to the question as asked, and it governs. **The two mechanisms coexist:** the ladder is **advisory**, warning the operator as the coil fills; the peel-back WIPREJ is **disposition**, applied at an order boundary once the maximum is passed. Recorded as `D16` in [`SpoolCompletionNotification.md`](screens/SpoolCompletionNotification.md). ⚠ **Residual, and it is the only thing still open on this leg:** if the client intends these as **alternatives** rather than complements, `D16` is wrong. That is one sentence to put back to him — *does the fill ladder still run on a finished coil, given the overage is handled by a forced WIPREJ at the order boundary?* — and it should go with `G120`'s sentence in the same reply. ⚠ **He did not set the threshold** — our proposal was *"keep the remainder below a threshold you set"* and no number came back. The other three halves of this item are untouched (audit record [`ClientEmail_2026-09-03_RodOrderAllocation_SyncPlan.md`](../95-archive/source-documents/ClientEmail_2026-09-03_RodOrderAllocation_SyncPlan.md)) | The spool alert's central comparison basis | spool completion feature | Tim O. / Operations / IT |
| **OI-75** | **OQ-79 RESOLVED (client, 30 Jul 2026) — a short close is an unplanned stop; the stop-prompt arbitration is still open.** ~~Is a short close a real case, and should it prompt?~~ It is real and is handled as an **unplanned stop on the mill 10-90 pattern** with a reason code, graded against the **customer min/max weight**: inside the range continue; outside it, **supervisor override + production hold** or **offer to the customer under concession** before planning a remake (offer first). **The spool is run off either way** — FL2 has no spool stripper, so it must return to FL1 emptied. **Mid-run coil break:** the stop is removed and a **new stop starts from zero**; weight does not resume from the break point; leftover material is welded to the next coil on FL1, or on FL2 run to a finished stop and offered, or scrapped. That last rule is a **run/stop model** change — check it against `FlatWireRun`/`CoilOutput` footage accumulation and `CoilTraceability`'s coil-local footage (**OI-25**). **The 10-90 SOP document is not in the repository and must be obtained.** **Still open, and this is now its sole tracking home** *(the register question was withdrawn 12 Aug 2026 as ours to answer)*: is the spool-removal confirmation strictly an operator-at-the-HMI decision, or does the **supervisor** see that a prompt is pending — particularly one left unanswered with the line stopped at target, which means production is halted with no transaction recorded; and with several operators signed in on the shared screen, does the prompt appear **once per line with first-answer-wins** or once per operator session? **Our reading:** once per line, first-answer-wins, with the answering operator recorded — a shared screen must not raise the same physical decision several times — and surface a **pending** prompt to the supervisor on Dashboard 1 only once it has gone unanswered with the line stopped, the same rule and reason as the unacknowledged completion milestone | Multi-operator behaviour; a now-specified operational case | spool completion feature | Tim O. / Operations |
| **OI-95** | **OQ-73 — multi-order rod sequencing and MVP scope.** OI-71 established that a rod can carry two orders; the sequencing rule when the operator departs from the planned order across such a rod is unanswered (Srikanth checking his notes), as is whether a rod may be staged against the **later** order while the earlier is still running, and whether the whole case is **MVP2**. Minimum change when it lands: the `planning_routings` lookup returns **orders (plural)**, order membership becomes an **ordered set**, and `RodStaging.OrderId` needs a defined meaning for a rod that spans two | The Phase-4 staging validation is knowingly wrong until this closes | Phase 4 | Srikanth / Tim O. |
| **OI-76** | **OQ-16 — should planning/scheduling warn when a job is booked on a line with no active pass schedule** for that alloy/gauge/width/edge combination? | Without it, operators arrive ready to run and the line is blocked until Operations creates a schedule | upstream Scheduling | Tim O. / Stephen |
| **OI-77** | **Edger blade profiles and roll regrind.** Whether blade profiles are standardised or custom per edge type/alloy, who maintains the library, and whether it belongs in the system alongside the pass schedule; and whether rolls are reground in-house or out, the turnaround, the spare-set count, and whether the system must track roll condition, footage and location. ✅ **PARTLY ANSWERED 3 Sep 2026 (`D-42`)** — the **roll** half moves. *"We should include mill rolls for traceability"* settles that the system **must** track them, and `ToolingInventoryRollSet` is built: condition as `LifecycleStatus`, location as `Location`, and **no footage counter** because a roll is reground to a minimum OD rather than consumed by feet. The client added *"they can be refurbished"* of the capstans and a spare count of **2 going to 3**. ⛔ **Still open:** the regrind **turnaround**, whether it is in-house or out, whether *"refurbished"* is the same operation as the edger's *In Grinding*, and **the whole edger-blade-profile half**. ✅ **FURTHER ANSWERED 6 Sep 2026 (`D-53`)** — the **edger regrind** half now has schema: `ToolingInventoryEdger` carries `StdRemovalFromOdIn` (`.100`), `OdIn` (`6.00`), `MinOdIn` (`4.75`) and `DateOfLastGrind`, which is the regrind model measured rather than described — about twelve grinds. ✅ **THE PROFILE HALF IS ANSWERED 10 Sep 2026, and it lands elsewhere.** The client gave the four-value **edge**-profile vocabulary and confirmed profile is defined *"per set"*, one profile per set. ⚠ **That is not a *blade*-profile library, which `Q28` correctly retired** — grooved rolls have no profile to select; a groove is **cut** to an edge profile. So the vocabulary lands on `G76` and `EdgeType` (`D-61`), and **this item must not be reopened for it**. ⛔ **What is still open here:** the regrind **turnaround** and whether it is in-house or outsourced — though *"refurbished"* is now settled as the same operation as `In Grinding`, and so are the turnaround and in-house-or-out questions | Tooling management scope. The roll and edger-regrind halves now have schema; the **profile** half still has none, and the **straightener** has no table at all (`G77`) | Phase 13; the roll columns are `G87` / `Q92` | Tim O. / Maintenance |
...
| **OI-79** | **The digital traveler is orphaned.** Foundations declares it "fully digital", the SRS gives it ten requirements (`TRV001`–`TRV010`), but **no phase owns building, storing or displaying it** as an artifact — only OQ-3 touches it | A `Must`-priority feature with no owner | unassigned | Programme management |
...
| **OI-107** | **Dashboard 1's "WIP Rejections" nav item — is a rejection *list* screen in scope?** Surfaced 1 Aug 2026 when DB8 became a dialog. The line status board carries a nav item labelled in the **plural**, which reads as a list of recorded rejections; it points at the rejection **entry form**, and as a page that mismatch was invisible. Three possibilities, costing very different amounts: **(1)** the label is wrong and a rename settles it; **(2)** a list is wanted and already exists elsewhere — held/rejected material has a queue implied by the Suspend disposition and by the supervisor shift summary — in which case DB1 should link to it; **(3)** a list is wanted and is **new work**: a rejection register filterable by line, shift, disposition and reason, with the QA disposition action per row, which is a screen, an endpoint and a phase owner that no phase carries. **Our reading:** option 1 for MVP-1 — rename the nav item to the singular so the label matches the form it opens, and raise the register separately as its own scoped request; note that the queue option 2 points at is the shift summary, itself outside MVP-1. Until this closes the nav item opens the rejection dialog, which is what it did as a page. **Sole tracking home** *(register question withdrawn 12 Aug 2026)* | A nav item on the supervisor's overview board opens a blank entry form; option 3 is the only one with schedule impact | Phase 7 | Tim O. / Shray |
...
| **OI-112** | **Nothing releases `wip_stations.coilno` at run end.** `FR-077` **sets** the station's coil reference on successful check-in and **no requirement anywhere clears it**; `POST /checkout` lists no `wip_stations` side effect at all. `wip_stations_k1` is a **UNIQUE** index on `CoilNo`, and the legacy release pattern parks the station's *own name* in the column as the idle sentinel (quoted verbatim from `SlitterInterface_CheckoutCoil` in the **withdrawn** WIP-station seed (kept at `95-archive/design-notes/`)). Raised 18 Aug 2026 while specifying the run-end write-back, which deliberately does **not** cover it — the release belongs to *run* completion, not *coil* completion | **The second coil at a station collides on a unique index.** This is a defect, not a design gap: FL1/FL2/FL3 each become unusable after their first run until the column is cleared by hand | Phases 7, 9 | BA / Development — **specify the release; the pattern already exists** |
| ~~**OI-113**~~ | ✅ **CLOSED 26 Aug 2026 by `Q89`.** Every part alpha now gets its own `proddb..coils` row, so each is its own `child_coil_no` — `ins_coil_gen_history`'s guard is **per child**, so N distinct children pass N independent tests and each carries **one correctly-parented row**. The shared tree and `CoilTraceability` **stop disagreeing.** ⚠ **Conditional, and the condition is the whole close: each alpha must pass ITS OWN parent rod.** All N sharing one `@primaryRodAlpha` would say *"this rod produced N coils"* — not multi-rod genealogy — so `TC-796` asserts N **different** `parent_coil_no`. ⚠ **The item's own instruction *"do not resolve it by inserting several rows"* was about several rows for ONE child; it never forbade several children.** *Superseded text:* **The shared genealogy can hold one parent per coil; a welded flat wire coil has many.** `coil_gen_history` is written through `ins_coil_gen_history`, whose guard is `IF NOT EXISTS (… WHERE child_coil_no = @ChildCoil)` — one row, one parent — and its readers assume that shape. Induction welding means the normal FL2/FL3 coil has **several** source rods. `FR-512` therefore records the **primary** rod (earliest footage) in the shared tree and leaves `CoilTraceability` as the authoritative multi-rod chain. Raised 18 Aug 2026 | **Anyone reading the shared tree alone sees one rod and will believe it is the whole story** — including a legacy report built for slitter genealogy. The welding-wire certificates are safe because they are built from `CoilTraceability`, but the two records now disagree by design and that must be documented wherever the shared tree is consumed. Do **not** resolve it by inserting several rows: the table's own guard forbids it | Phases 9, 12 | IT / Quality — **decide whether any shared consumer needs the full chain** |
| **OI-114** | **The cut-record sentinels for a product that is never slit are undecided, and the legacy writers disagree with each other.** `FR-514` creates one `coil_slit_cuts` row per output coil, but `stop_no` is `1` in `CoilReceiving_InsertCoilSlitCutDataOnSkid` and `0` in `ConveyorInterface_PrepareDataForCreateSkid`, `TollingManager_InsertIntoCoilSlitCuts` and `DirectShipment_CreateSkidDataTransaction`; `mfg_order_no` is `99999` in three and `0` in the fourth; and **all five hard-code `skid_coil_seq_no = 1`, which flat wire cannot copy** because two coils share a skid and need 1 and 2. Raised 18 Aug 2026 | **It cannot be settled by copying a template** — there is no majority to follow on `stop_no`. The procedure derives `skid_coil_seq_no` and parameterises the other two with the majority defaults, so a wrong answer is a one-line change rather than a rebuild; but a report filtering on `stop_no` would see flat wire inconsistently | Phase 9 | IT — **confirm against any query filtering `stop_no` or `mfg_order_no`** |
| **OI-122** | **The 20 Aug call is entirely about the welded multi-rod spool, and weld capture was descoped on 14 Aug 2026.** `[TRP §1.5]` records the single-rod spool as a deliberate **scope** consequence — *“with no welds the chain is one rod long”* — and the trial demonstrates exactly that. The client has now confirmed the multi-rod, multi-order spool as the **normal** FL1 case, so the requirement has **no demonstration anywhere in the trial**. ⚠ This is also what makes `G42`’s window *“free”*: nothing writes the genealogy table yet, so adding it now is a DDL file and a domain type, and once weld capture returns it is a migration plus a backfill against certificate data. **The window closes at `S1`, 24 Aug 2026.** Raised 20 Aug 2026 | A confirmed `Must` requirement (`FR-172`) goes to UAT unexercised, and the cheapest moment to build its schema passes unused | Before `S1` | Delivery lead |
| **OI-130** | **`PlanningDB.dbo.GenerateCoilAlpha` filters the wrong column on two of fourteen branches.** Its `coil_mill_processing` and `coil_slitter_processing` *outgoing* branches read `WHERE incoming_coil_no LIKE @root + '%'` where `CommonDB`'s read `WHERE coil_no` — so the sweep can miss an outgoing coil whose root differs from its parent's. Found 26 Aug 2026 while evaluating, and rejecting, a cutover to that fork | ⛔ **Not flat wire's exposure — we do not call it** (the cutover was cancelled; `Q57` stands and minting stays on `CommonDB`). But it is a real defect in a shared function **four PlanningDB procedures depend on**, and it has no owner until someone is told | — | IT / DBA — **a finding handed over, not an item against flat wire** |
| **OI-132** | ⚠ **`OI-114` and the procedure's `D9` both state something false about the legacy writers.** Both assert *"all five hard-code `skid_coil_seq_no = 1`"*. **One of the four writers they name by name does not** — `ConveyorInterface_PrepareDataForCreateSkid` writes **`NULL`** — and **two further procedures actively clear it to `NULL`** (`ConveyorInterface_MoveCutsBackToLiftTable`, `CreatePlate_MoveCutsFromAnotherPlate`). The column is `int NULL` with **no uniqueness constraint** anywhere, and there is a live consumer ordering by it **in a write path**, not a report | **It makes `OI-114` worse, not better:** flat wire's `MAX+1` derivation was being layered onto a column that is **not a reliable slot number today**. ✅ **`Q89`'s decision sidesteps it** — `skid_coil_seq_no` is now set from `@skidAssignment` — but the two register/comment claims are still wrong and should be corrected | Phase 9 | IT / DBA |
| **OI-133** | **The 26 Aug table rename leaves the schema half-prefixed, and one lookup is why.** 23 of the 33 `FlatWireDB` tables take a `FlatWire_` prefix; 10 stay bare — six lookups, the three `PassSchedule*` tables (read, never authored — `OI-110`) and `Rod` (a mirror of shared `coils`). ⚠ **`PayoffPosition` is a lookup and IS renamed**, so the exclusion set is not *"transactional prefixed, reference bare"* but that split plus one named exception. **Raised 26 Aug 2026** — originally minted as `Q91` and moved here the same day, because it is **ours to answer, not the client's** | Six bare lookups beside one prefixed lookup reads as an oversight unless a document says so. **Either move the remaining six in the same pass — every FK is already being touched — or write the exception into `[DBD §6.2a]`** | Phase 1C | Architecture |
| **OI-134** | **`FlatWire_` now names both `FlatWireDB` tables and stored procedures.** `united_db` already held `FlatWire_CheckInRod` and three siblings, so the prefix was a **procedure** convention first. The 26 Aug relocation of those four into `FlatWireDB` removes the cross-*database* ambiguity but leaves a narrower one: inside one database `FlatWire_RodCheckin` is a **table** and `FlatWire_CheckInRod` a **procedure**, and the prefix distinguishes neither. **Raised 26 Aug 2026** — originally `Q92` | Nothing can mis-resolve — different object classes — but `grep FlatWire_` stops discriminating and the prefix is now redundant on **everything** in the database, which weakens `OI-133`'s case rather than strengthening it. **Record the two object classes in `[DBD §6.2a]`** | — | Architecture |
| **OI-135** | **The alpha budget is TIERED, not one shared 702 per rod — restated 26 Aug 2026 by the segment-rooting change.** Segments draw **single** letters off the rod (26 of them); each coil draws **double** letters off *its own* segment, so coils no longer compete with segments or with coils off other segments. *(This entry previously described one shared 702-suffix budget divided by N; that framing is superseded.)* `CommonDB.dbo.GenerateCoilAlpha` walks `A`…`Z`, then `AA`…`ZZ` — **702**. ⚠ **The six-character root is the exclusion sweep's `LIKE` filter, NOT the string the suffix is appended to** — the stem is `@CoilNo` verbatim, so a seven-character input returns a **child** (`R00002A` → `R00002AA`), not a sibling; `[RodOrderAllocation.md §9.1]` `F13` carries the measured walk. *(This entry said **26** and *"appends `A`…`Z` to a six-character root"* in its headline until 26 Aug 2026 while its own body said 702 — self-contradictory in one row.)* Four flat wire paths draw on that budget per rod — spool segments (~3), output coils (~6), the scrap-weight path (`Q56`), and since `Q89` **one alpha per source rod per coil**, which makes the draw proportional. ⚠ **`F4` records that exhaustion is an infinite LOOP, not an exception** — past 702 suffixes the overflow wraps and re-tries forever, and a scalar UDF cannot `RAISERROR`; the caller's `THROW 51010` catches only a blank return. **Raised 26 Aug 2026** — originally `Q93` | Headroom is real and the tiering widens it further — a rod needs **46,800 lb to reach 26 segments** against the 4,000–8,840 lb in play (`OI-97`) — so **accept and monitor**. ⚠ **But *"only readability is lost"* past 26 segments is not universally true:** at that point segments are 8 characters and their coils **9 — the entire `char(9)` budget, with no headroom** — and past **26 coils per segment** the overflow flattens to a **rod-level sibling**, which lands in the segment namespace. That needs 26 × 26 to bite, so it is unreachable in practice, but the bound is a collision bound and not merely a legibility one. *(Stated as "~13 of 26" until 26 Aug 2026; the ratio was against the wrong denominator, which understated the headroom by a factor of 27.)* Recorded because **`OI-97` has three competing rod weights in circulation** and the largest roughly doubles coils per rod, which is the one input that could make this bite | — | IT / DBA |
| ~~**OI-136**~~ | ✅ **RESOLVED 26 Aug 2026 by the universal-registration rule — the blank list is CORRECT and every design document was wrong.** The contradiction was real and this is the direction it resolved in: rather than adding an ignore list to the procedure, **every alpha flat wire mints is now written to `proddb..coils`**, so `GenerateCoilAlpha`'s own sweep sees every sibling and **no mint anywhere passes a list**. `[RodOrderAllocation.md §2.8]` is the design of record. ⚠ **The shipped line 483 needs no change — it needs the rationale**, which `[N]`/`N5` adds. ⛔ **Conditional on `OI-138`:** the FL1 writer that makes registration true does not exist, so the blank list is correct *by design* and still unsafe *in fact* until `FW-231` ships. *(Superseded issue text follows.)* ⚠ **The shipped coil-completion procedure passes a BLANK ignore list; every design document says it passes one. They cannot both be right.** `50_FlatWireDB_Proc_FlatWire_CompleteCoilOnSkid.sql` line 483 reads `SET @sharedCoilNo = [CommonDB].[dbo].[GenerateCoilAlpha](@primaryRodAlpha, '')`, while `FlatWire_DDL_05_QualityOutput.sql`'s `ChildAlpha` comment, the welded-coil ledger §2.1 and `[RodOrderAllocation.md]` §2.4/§2.8 all specify `GenerateCoilAlpha(RodAlpha, @ignoreList)`. **Neither is a typo — the blank is correct for the sweep and wrong for `FlatWireDB`.** `coils` is swept (and without `NOLOCK`), so a blank list cannot reissue a *coil* alpha; what it cannot see is `SpoolTraceability.ChildAlpha`, which nothing writes to the shared schema. Evidence and measurements: **`F14`** in `[RodOrderAllocation.md §9.1]`. **Raised 26 Aug 2026** | ⚠ **Was: *LATENT, NOT LIVE*.** That framing is superseded — the blank list is no longer a defect to fix but the design. **What survives is the timing:** it is harmless while nothing mints FL1 segment alphas, and becomes a live duplicate the moment FL1 mints its first one **without** the registration writer. So the hazard did not go away; it moved from `S5` to **`OI-138`/`FW-231`**, and the two must ship together. | **`OI-138` / `FW-231`** — no longer wave `S5` | Dev |
| **OI-137** | ⚠ **Two parts of ONE coil rooted on the SAME segment would be issued the same alpha — the only remaining way the 26 Aug design reissues a string.** Registration makes the sweep authoritative for **committed** rows only. Two parts minting blank inside one transaction off the **same** segment therefore both get the same answer; `[RodOrderAllocation.md §2.8]` Scenario B is safe **only because its two parts root on different segments**. `CoilTraceability`'s own comment concedes the precondition — *"nothing yet forbids one rod contributing two segments to one spool"* — and `SourceSegmentAlpha` exists precisely because `(RodAlpha, SpoolAlpha)` does not always imply the segment. **Raised 26 Aug 2026** — originally `OQ-S` | **Forbid it by constraint.** A coil's footage over one segment is contiguous under LIFO, so two rows off one segment should not arise; a per-transaction accumulator for that one case is the fallback. Test it (`TC-800`+) rather than assume it | `ORD025` | Nagarro |

### 11.4 Low

| ID | Issue | Impact | Owner |
|---|---|---|---|
| **OI-81** | **Web-application details:** flat-wire pricing auto-population on Orders/Quotes; whether the Finish field lock is temporary or permanent; how the Bundle Width Min/Max range is determined; and whether any flat-wire product makes edge quality contractual, contradicting the automatic "5 — Edge not a consideration" default | upstream Web | Sales / Tim O. / Technical |
| **OI-82** | **FL1/FL2/FL3 output throughput rates** (lbs/hr or ft/min) are undocumented; only equipment capacities are known | Scheduling run-time estimates | Bob S. |
| **OI-83** | **Baler maximum dimensions** and **scrap banding material** (steel versus aluminium alloy) | Scrap handling detail | Plant / Tim O. |
| **OI-84** | **WIP REJ report column definitions** are undocumented. **Sole tracking home** *(register question withdrawn 12 Aug 2026)*. **Our reading:** derive the columns from **what the WIP rejection dialog actually captures** — line, run, rod/spool alpha, footage position, reason, disposition, operator, timestamp — and send that list to Shannon R. to add to or strike from; the report cannot show what is not captured, so the capture set is the natural starting point | The report cannot be modified | Shannon R. |
| **OI-85** | **Rod storage system-side location tracking.** Are bay/row/position fields required, or is storage managed physically? *(The Dashboard 2A queue's Location column was removed pending this.)* | Whether a location column returns to the queue | Naj / Chuck |
| **OI-86** | **Tolled flat-wire cert liability.** For customer-supplied rod, who holds chemistry cert liability. **Sole tracking home** *(register question withdrawn 12 Aug 2026)*. **Our reading:** on tolled material the **customer holds it** — UA captures and passes the supplied chemistry through without re-certifying, receiving stores the customer's chemistry document against the rod, and the cert cites it as supplied data. That is the position requiring no new testing capability, so Legal is being asked to confirm the wording rather than the model | What the receiving module must capture and retain | Tim O. / Legal |
| **OI-87** | **Planning grid versus a dedicated flat-wire section.** Existing grid with column renames and a filter dropdown, or an isolated flat-wire planning view | UI scoping and regression risk for non-flat-wire planning | IT / team |
...
| **OI-91** | **`screens/LineStatusOverview.md` is an empty file** (0 bytes) | A named analysis document with no content; the Dashboard 1 spec lives in `FlatWireShopfloorDashboards.md` and the SRS instead | Analysis team |
...
| **OI-12** | The **die-life colour bands differ between the two die screens** — 60/85 % on Die Change versus 65/79/80 % on Die Management — for the same underlying die. Both are internally consistent with their own source, so neither document flags it. ⚠ **ESCALATED 2 Sep 2026 by the die split (`Q91`).** Both screens are now **MVP-1** and both derive their bands from **one table**, `ToolingInventoryDie`, so this is no longer a cross-scope curiosity — two MVP-1 screens will state different statuses for the same die. Needs a decision before the Die Management build, not a default. |
...
| `README.md` | — | current (a 1-line pointer to `../00-overview/Roadmap.md`) | §1.6 |
| ~~`Shopfloor_Flat_wireSRS_Consolidated_v2.docx`~~ | — | **deleted 26 Aug 2026** — recoverable at `285bc2e`. Was superseded by v3; v3 itself left the repo 1 Aug 2026 (`6096921`) | cross-checked against v3 before deletion; **no unique content carried**. Requirement text is authoritative in [`BusinessRequirements.md`](./BusinessRequirements.md) as `FR-###` |
...
|---|---|---|---|
| `../90-registers/Questions.md` | 11 Aug 2026 | **current — the authoritative register of *open* questions (73 items).** Split on 11 Aug 2026: answered questions moved to `../90-registers/Decisions.md`. **The two share one numbering space and nothing was renumbered**, so a `Q`/`OQ` reference resolves against whichever file holds it. `OI-88` and `PLC-Q02` are in neither — see that file | §10, §11 (every `Q`/`OQ` reference) |
| `../90-registers/Decisions.md` | 11 Aug 2026 | **current — the 24 answered questions with their decisions** (Apr 28 → Aug 6 2026), split out of the register on 11 Aug 2026. Absorbs the former `ClientQuestionsEmail.md`: its provenance and the client-facing wording of the fifteen questions it covered. **Ten `In Progress` questions are *not* here** — a partly-answered question stays in the open register with its decided portion | §10, §11 (every `Q`/`OQ` reference) |
| `FlatWireShopfloorDashboards.md` | 11 Aug 2026 | **NO LONGER A REQUIREMENTS SOURCE.** Retained as the internal UX/wireframe record, the navigation map, the design principles and the **alloy seed-data table**. Carries an authority banner and a Dashboard Inventory naming each screen's owning document; **the owning document wins on any disagreement** | §4, §7 — *now via the owning documents* |
...
| `./EndToEndProcess.md` | 28 Apr 2026 | **partially superseded** — its equipment description predates the May 21 and Aug 4 corrections (edger on FL1, FM2 as 8"→S1→S2, laser welding) | §3 (stage structure, scrap, capacities); equipment facts taken from the corrections instead |
| `./ProcessWalkthrough.md` | 1 Aug 2026 | current — the 45-step sequential overlay, and the `Known Source Conflicts` arbitration table | §3 |

> **Six rows, six files — the table is now exhaustive.** It stopped being so on 1 Aug and was reconciled on 11 Aug, when the last three non-listed files left: `Spool.md` and `PartialRodReCheckin.md` **moved to `Business/Screens/`** (both are noted at the foot of **A.2a**, and neither is one of the seventeen specifications), and `OperationsManager.md` was **deleted**, noted below. **The count held through the register split later that day** — `ClientQuestionsEmail.md` was merged into the new `../90-registers/Decisions.md` and removed, so one row left as one arrived.

> ~~`OperationsManager.md`~~ — **DELETED 11 Aug 2026** (recoverable at `d79ce78`, which holds the full pre-consolidation role document). Nothing unique was lost: the role definition and the Supervisor comparison are in `PassScheduleManagement.md` §3.3–§3.4, the alloy-table restriction in §3.4, revert authority in `RollAdjust.md` §8, the mid-run schedule-change acknowledgement in `ActiveRunMonitor.md` §2.5, and the permission matrix in `./BusinessRequirements.md` §8 — which is strictly more granular than the table it replaced. **The defect it carried is preserved at `Q74` in the register:** it cited *"open question OQ-B"*, an identifier in no register, and stated the question as open although `Q74` decided it on 4 May 2026. *(Formerly fed §8.1 role content.)*
...
> **Every dashboard has an owner as of 11 Aug 2026 — including DB7b, which never had one.** The Packing Station had a mockup and `FR-345`–`FR-352` but **no owning document and no inventory row**, so it was invisible to the check that is meant to guarantee coverage. `OutputCoilCompletion.md` **v1.1** now owns **DB7 and DB7b together** (§8), because *Confirm & Move to Packing* on DB7 is what creates DB7b's arrival and the two-per-skid rule spans both screens. **Four of DB7b's eight requirements are mockup-derived and were never client-reviewed**, and are tagged `[PROPOSED]` accordingly.

| Screen | Owning document | Note |
|---|---|---|
| DB1 Line Status Overview | `LineStatusOverview.md` | Written 1 Aug 2026 — **supersedes the "empty file, 0 bytes" record (OI-91), which was stale** |
| DB2A Rod Pre-Check-in | `RodPreCheckin.md` | |
| DB2 + DB5 Check-in | `RocCheckin.md` | Covers rod and spool check-in; §4.3 added 11 Aug |
| **DB3 Active Run Monitor** | **`ActiveRunMonitor.md`** | **New 11 Aug 2026** |
| ~~DB4 Weld Event Logger~~ | `WeldEvent.md` | Screen **retired 1 Aug 2026**; capture is DB2A *Mark as welded*. Gaps **G27**/**G28** record two capabilities not rehoused |
| DB5A FL2 Spool Queue | `SpoolQueue.md` | New 2 Aug 2026 |
| DB6 SPC Checkpoint | `SPCCheckpoint.md` | |
| **DB7 Output Coil Completion** | **`OutputCoilCompletion.md`** | **New 11 Aug 2026** |
| **DB8 WIP Rejection** | **`WipRejection.md`** | **New 11 Aug 2026** |
| DB9 + DB9A Pass Schedule | `PassScheduleManagement.md` | |
| **DB10 Supervisor Shift Summary** | **`ShiftSummary.md`** | **New 11 Aug 2026** |
| **DB11 Roll Adjust** | **`RollAdjust.md`** | **New 11 Aug 2026.** Its FM2 gap table was **rebuilt**, not copied — the source showed five rows for three stands after the 4 Aug correction |
| DB12 Rod Checkout | `RodCheckout.md` | §7.2 added 11 Aug |
| DC Die Change | `DieChangeAndManagement.md` | |
| Weight milestones / stop confirmation | `SpoolCompletionNotification.md` | Covers a slice of DB3, not the whole screen |
| *(no screen)* | `PLCTagSpecification.md` | **Authoritative for the PLC/OPC tag surface.** Supersedes §6.8 and the tag map formerly in `HMIAndSCADALayout.md`; carries the `PLC-Q##` series |
| *(no screen)* | `PassScheduleGenerationSpec.md` | v1.5 — the engineering basis for `FW-013`. See §10.5 |

> ~~`HMIAndSCADALayout.md`~~ — **DELETED 4 Aug 2026** with the DB13/DB14 descope. Its machine tag map, including the FM2 S3 row that no other copy carried, was rescued into `PLCTagSpecification.md` first. *(Formerly fed §4.21, §4.22, §6.8.)*

> **`Spool.md` also lives in this folder and is not one of the seventeen.** Moved here from `Analysis/` on **11 Aug 2026** to sit beside the four documents that own its screen content — `RocCheckin.md` §4.3 (DB5 and the DB2/DB5 comparison), `SpoolQueue.md` (DB5A), `OutputCoilCompletion.md` §4 (DB7 source traceability) and `SpoolCompletionNotification.md` (weight milestones and short close). It is the **domain reference for what a spool is** — physical form, the 3,500 lb equipment ceiling against the ~1,800 lb working target, material flow, the coils-table mapping, the lifecycle and the stakeholder concepts — and *that* content is authoritative. Its **screen** rules are not, and its **FM2 description is superseded** by `D-26`: the file carries its own banner saying so. Its *Spool-Related Open Questions* section is a **May 2026 snapshot** subordinate to the register.

> **`PartialRodReCheckin.md` also lives in this folder but is not one of the seventeen.** Moved here from `Analysis/` on **11 Aug 2026** to sit beside the two documents that own its rules, it is **internal design rationale**, not a client-facing specification: no reading convention, no confirmed-decisions table, no sign-off sheet, and **nothing in it is citable as a requirement**. It is the audit trail for **`Q12`**, which remains open on the payoff-scale question. Two reading traps are flagged in the file itself — its worked examples use non-canonical `ROD-`/`SPL-` alphas (**gap G14**), and its snake_case column names are May 2026 proposals whose *design* was delivered on 26 Jul 2026 as `Rod.FootageRunToDate`, `Rod.RemainingWeightEstimateLb` and `SpoolProcessing.SourceRodAlpha`. Rules → `RodPreCheckin.md` §7 and `RodCheckout.md` §7.2; requirement text → `FR-043`.

**Three further open items were raised on 11 Aug 2026 when Phase 9 returned to MVP-1** — **OI-104** (the skid table `CoilOutput.SkidId` points at has never been located — ✅ **closed 18 Aug 2026**, it is `wip_skids` + `wip_skid_coils`), **OI-105** (which of three weights is authoritative on the coil record, now that `FR-346` adds a packing-station scale reading) and **OI-106** (staging locations undefined). All three are DB7b dependencies that were tolerable only while that screen was deferred; they are collected as gap **G36** in `../90-registers/Gaps.md`, and **OI-24** was escalated alongside them.
...
**Three candidate items were drafted and withdrawn on checking the existing register — worth recording, because each looked like a gap and was not:**

| Withdrawn candidate | Why |
|---|---|
| *Alloy density factor is held nowhere* | **Settled.** `FR-332` fixes the formula and names the density source; the genuine remaining question is the **dimensional basis**, which is the existing **OI-45** — now cited by `OutputCoilCompletion.md` rather than duplicated |
| *Gross weight — scale-entered or calculated?* | **Settled** by the same requirement: net weight is calculated and **never read from a scale during rolling**, with an operator scale override permitted |
| *Which lines have Roll Adjust?* | **Answered 2 Aug 2026.** `FR-107`–`FR-109` and all three active-run mockups agree on **FL2 and FL3, not FL1**; the dashboard design reference stated it three contradictory ways and is simply stale. Verified against the mockups directly — FL1's has no Roll Adjust control |

### A.3 `MVP-1/ProjectPlan/` — plan, contracts and backlog

| File | Last Updated | Status | Fed into |
|---|---|---|---|
| `../00-overview/Roadmap.md` | 26 Jul 2026 | **current — the roadmap index** | §1.5, §9 |
...
| File | Last Updated | Status | Fed into |
|---|---|---|---|
| `../20-architecture/Architecture.md` | 26 Jul 2026 | **current except decision 3** (the `Rod` drop, superseded by the DDL) | §2, §6.7, §8.2, §10 |
| `../90-registers/Gaps.md` | 26 Jul – 6 Aug 2026 | current — dependency chain, calendar, gaps G1–G35 | §9, §11 |
| `../00-overview/Roadmap.md` | 26 Jul 2026 | superseded by the 1A/1B/1C splits; its 21-table statement is stale | §9.2 |
...
| `../30-database/sql/FlatWire_DDL_00_Database.sql` | — | **current — authoritative** | §5.1, §5.11 |
| `../30-database/sql/FlatWire_DDL_01_Lookup.sql` | — | **current — authoritative** | §5.4 |
| `../30-database/sql/FlatWire_DDL_02_Schedule.sql` | — | **current — authoritative** | §5.5 |
| `../30-database/sql/FlatWire_DDL_03_Materials.sql` | — | **current — authoritative** | §5.2, §5.6 |
| `../30-database/sql/FlatWire_DDL_04_Runs.sql` | — | **current — authoritative** | §5.7 |
| `../30-database/sql/FlatWire_DDL_05_QualityOutput.sql` | — | **current — authoritative** | §5.8 |
| `../30-database/sql/FlatWire_DDL_06_ForeignKeys.sql` | — | **current — authoritative; all foreign keys in one script** (`06b` folded in 23 Aug 2026). Count: `[DBD §6.2]` | §5.9 |
| `../30-database/sql/FlatWire_DDL_07_Indexes.sql` | — | **current — authoritative** | §5.10 |
| `../30-database/sql/FlatWire_DDL_08_Programmability.sql` | — | **current — authoritative** | §5.10 |
| `../30-database/sql/FlatWire_DDL_99_Teardown.sql` | — | current | §5.11 |
| `../30-database/sql/FlatWire_DDL_RunAll.sql` | — | current — SQLCMD runner | §5.11 |
| `../30-database/sql/FlatWire_SampleData_Lookup.sql` | — | current | §5.11 |
| `../30-database/sql/FlatWire_SampleData_Schedule.sql` | — | current; **header coverage comment wrong** (says 3/7, actual 4/6) | §5.11 |
| `../30-database/sql/FlatWire_SampleData_Materials.sql` | — | current | §5.11 |
| `../30-database/sql/FlatWire_SampleData_Runs.sql` | — | current | §5.11 |
| `../30-database/sql/FlatWire_SampleData_QualityOutput.sql` | — | current | §5.11 |
| `../95-archive/design-notes/10_CommonDB_Insert_WIPStations_FlatWire.sql` | 28 Jul 2026 (D2 revised 29 Jul) | ⛔ **WITHDRAWN 6 Sep 2026** — archived, never ran; the seeding is `FW-241` | §5.12, §11 |

### A.6 `MVP-1/ProjectPlan/Frontend/Mockups/` — 33 files

| File | Status | Fed into |
|---|---|---|
| The **20 approved screens** *(DB13 and DB14 removed 4 Aug 2026)* — `dashboard_1_line_status` · `dashboard_2a_rod_precheckin` · `dashboard_2_rod_checkin` · `dashboard_2_rod_checkin_fl3` · `dashboard_3_active_run` · `dashboard_3_active_run_fl2` · `dashboard_3_active_run_fl3` · `dashboard_5_spool_checkin` · `dashboard_6_spc_checkpoint` · `dashboard_7_coil_completion` · `dashboard_7b_packing_station` · `dashboard_8_wip_rejection` · `dashboard_9_pass_schedule` · `dashboard_9a_schedule_list` · `dashboard_10_shift_summary` · `dashboard_11_roll_adjust` · `dashboard_12_rod_checkout` · `dashboard_die_change` · `dashboard_die_management` · `dashboard_oee` (all `.html`) | **current — approved visual baseline** | §4 (field lists, states, validations), §7 |
| ~~interim 8-step layout (then `dashboard_2_rod_checkin.html`)~~ | **superseded** — **deleted 11 Aug 2026**, recoverable at `d79ce78`; the filename was reassigned to the approved wizard the same day | §7.1 (listed as retired) |
| ~~`dashboard_2_rod_checkin - Old.html`~~ | **retired** (grid + progress ring, 9-step) — **deleted 11 Aug 2026**, recoverable at `d79ce78` | §7.1 (listed as retired) |
...
> **Two exceptions to this folder's read-only convention, moved here from `Analysis/` on 11 Aug 2026.** The two `ClientCall_*_SyncPlan.md` ledgers below are **Nagarro-authored working documents**, not business inputs, and they are **live**: the 6 Aug ledger's waves **W3–W8 are documented and not executed**. They sit here because they belong with the call transcript they were written from, but they are **maintained, not frozen** — update them as their waves complete.
...
| `Scheduling System  Wire Flattening Process 04132026.docx` | 13 Apr 2026 | 0.9 KB | §5.13 (**the definitive list of FW-001 renames**, `INFLAT`, operation letter `F`, the Flattening Lines tab) — ⚠ the renames and `INFLAT` are **cancelled** by `D-32`; the operation letter and the Flattening Lines tab stand |
...
| `../50-frontend/mockups/mainlogo.gif` | Binary image asset |
