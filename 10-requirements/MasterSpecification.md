...
| **OI-57** | **Published tolerance bands** per alloy and temper (ASTM B236, customer PO, or UA internal) are undefined. **Sole tracking home** *(register question withdrawn 12 Aug 2026)*. **Our reading:** ASTM B236 as the baseline, overridden per order where the customer purchase order is tighter, held in the same lookup the dimensional-tolerance change adds (**OI-07**) rather than a second table — Mick to confirm the standard, the structure being unchanged if it is a different one | SPC control limits cannot be configured, and the gauge-trace report produces no meaningful alarms | Phases 6, 11 | Tim O. / Mick |
...
| **OI-63** | **OQ-2 — FL3 scheduling representation.** Partly decided (FL3 cannot run if FL1/FL2 have scheduled orders; all three tracked as separate machines); how FL3 is represented as a **booking unit** is still open | Capacity planning logic | upstream Scheduling | Tim O. / Stephen |
