...
flowchart TB
  ...
  subgraph DATA["SQL Server"]
    FWDB[("FlatWireDB<br/>NEW — 27 tables")]
...
1. **Clone Dashboard 12's skeleton, not Dashboard 2's**, when starting a new screen — DB2's `- New.html` inlines its own app bar and omits `flat-wire-topbar.js`.
...
**27 tables.** This figure was taken by counting `CREATE TABLE` statements across `FlatWire_DDL_01` … `_05`, not copied from any document:
...
> **The stale side, named explicitly so nobody builds from it:** `DevelopmentPlan/ShopfloorPlan/00-foundations.md` decision 3 and `DevelopmentPlan/ShopfloorPlan/phase-01c-database-foundation.md` both describe dropping `Rod`. **Anyone reading either in isolation will build the wrong schema.**
>
...
| **Unplanned component bypass** | OQ-29, a **decided** requirement | **No table, endpoint, screen or story** — **OI-43** |
...
The full model does not read at 27 tables in one diagram. This section publishes an **overview** of the five groups and their inter-group edges, then **one detailed diagram per group**. No table is omitted from either level.

### 7.1 Overview — groups and the edges between them

```mermaid
erDiagram
    LOOKUP["Group 1 — Lookup (6)"]      ||--o{ SCHEDULE["Group 2 — Schedule (3)"] : "AlloyProperty→PassSchedule; Stand/Drawer/Edger→Component"
...
| **D-03** | The schema is **27 tables** | The as-built count | See §6.2 for the full supersession history |
...
| **D-08** | **Dashboard 2 is the `- New.html` 6-step tab wizard** | Progressive unlock and tolerance-viz replaced the retired progress ring | Both earlier DB2 layouts retired |
