...
**Sources:** [`../FlatWire_MasterSpecification.md`](../FlatWire_MasterSpecification.md) §1, §2, §9.6, §10, §11 · [`../../DevelopmentPlan/CapacityAndEffortModel.md`](../../DevelopmentPlan/CapacityAndEffortModel.md) · [`../../DevelopmentPlan/ShopfloorPlan/back-matter.md`](../../DevelopmentPlan/ShopfloorPlan/back-matter.md) · [`../../DevelopmentPlan/TechStackRecommendation.md`](../../DevelopmentPlan/TechStackRecommendation.md) · `BaseDocuments/` business inputs
...
No current UAL application can represent **rod as an input**, a **flattening operation**, a **bundle width**, an **edge type**, or a **coreless oscillated coil**. Approximately **24 existing applications** need extension, catalogued in `BaseDocuments/New Flat Wire Machine - Impact on Applications 041726.xlsx`. The largest single blast radius is the shared-schema rename (story `FW-001`) described in §10.3 and `[HLD §8]`.
...
**Fourteen backlog stories** (FW-020/021/022, FW-030/031, FW-040–043, FW-050–053, FW-055) sit in this out-of-scope set. They remain Critical/High **for their own teams**. See `[SP §7]`.
...
Written so `[TP]` can test them and `[DR]` can gate a release on them.

| ID | Success criterion | Acceptance measure | Verified by |
|---|---|---|---|
| **SC-01** | A rod can be checked in on FL1 and the line configured from the acknowledged pass schedule | One check-in writes the run, check-in, inspection and pre-run SPC records, sets `coils.coil_status = INFLAT`, and pushes the full tag set for the selected payoff — in that order | `[TP]` FL1 E2E (FW-120) |
| **SC-02** | An FL1 run produces an intermediate spool with a complete profile | Spool alpha issued only after per-spool SPC passes; gauge profile with weld markers stored and retrievable | `[TP]` FL1 E2E |
| **SC-03** | An FL2 run finishes that spool into a coreless coil | FL2 check-in validates against the FL1 record, shows the historical profile, pushes FM2 tags, completes a coil with a label | `[TP]` FL2 E2E (FW-121) |
| **SC-04** | FL3 hybrid runs rod to finished coil in one pass | One acknowledgement pushes FM1 **and** FM2 tags; no spool alpha; real-time trace end to end | `[TP]` FL3 E2E (FW-122) |
| **SC-05** | Weld genealogy is complete and certifiable | For a coil made from ≥ 2 rods across ≥ 1 weld, the traceability rows cover 100 % of coil footage, do not overlap, and resolve to supplier heat | `[TP]` weld genealogy suite |
| **SC-06** | The line board is live and correct | All three lines render concurrently; every alert rule fires on its stated condition; readings arrive by push with **no polling** | `[TP]` real-time suite |
| **SC-07** | Exceptions have formal off-ramps | WIP rejection sets material status and alerts a supervisor; all three checkout modes complete with the correct status transition and PLC tag treatment | `[TP]` exception suite |
| **SC-08** | Every override is attributable | Every supervisor override, pass-schedule change and PLC tag write/clear is retrievable with who, when, why and old→new value | `[TP]` audit suite |
| **SC-09** | The screens work on the panel | Every screen renders complete at 1280 × 1024 at 1:1, no text below 14 px (except the documented SVG axis exception), every tap target ≥ 48 px | `[TP]` UI conformance |
| **SC-10** | A network drop is survivable | Client shows "Reconnecting…" over cached last-known state — **never a blank screen** — and re-joins its line group automatically | `[TP]` resilience suite |
| **SC-11** | The release can be rolled back | A rehearsed rollback returns every component to its prior version, with the data-loss position stated in advance | `[DR §6]` |
...
Stay entirely within the existing UAL stack — Angular 18.2+ (delivered as a PWA), .NET 8 with Clean Architecture and MediatR CQRS, SQL Server, SignalR, Chart.js, JWT and Serilog. Detail and rationale in `[HLD §3]` and `[HLD §12]`.
...
The design is therefore **records first, PLC second**, with **compensating writes** rather than rollback. The recovery path when one side succeeds and the other fails is **not yet specified** — gap **G2** / **OI-39**, and it blocks Phase 4. Full treatment in `[HLD §10]`.
...
- The mockups in `Mockups/` are final; no UI design time is costed.
...
|---|---|---|---|---|---|---|
| **RISK-01** | **The 30 Sep date is unreachable as scoped.** The plan is **3,727 hours / 465.9 dev-days** against **44 working days** (32 post-gate) — **352 h per person** → **10.6 FTE sustained**, a **10.7-FTE Phase-1 gate**, and an arithmetically impossible **27.2 FTE in W7** | **Certain** — measured, not forecast | **Critical** | Programme management | **Not mitigable by parallelism or descoping.** The full descope ladder recovers **12 %** (448 h), leaving 9.3 FTE. A programme decision is required: **staff to ~11 FTE**, **move the date** (6 FTE → 18 Nov 2026; 8 FTE → 22 Oct 2026, both inside the already-planned Q4 window), or **cut below the critical path**. See `[SP §1]` | **G1** / **OI-51** |
...
| **RISK-12** | **Phase 6 depends on Phase 13.** Die-change validation needs the die inventory Die Management creates, and there is **no die master table** in the schema at all | Medium | Medium | Programme management | Pull a minimal die reference forward into Phase 6, or resequence | **OI-41** |
...
Twenty-five design, equipment and business decisions are closed and recorded in [`../FlatWire_MasterSpecification.md`](../FlatWire_MasterSpecification.md) §10, with the superseded position preserved in each case. **Do not re-open them.** The ones with the widest reach:
...
| **D-03 / D-04** | The schema is **27 tables**, and **`Rod` is retained** as a local master with enforced rod-alpha FKs — superseding the earlier "drop `Rod`, 21–22 tables" position |
...
These block the phase named. The full register (Critical, High, Medium, Low — 90+ items) is in [`../FlatWire_MasterSpecification.md`](../FlatWire_MasterSpecification.md) §11 and is carried into `[SRS §11]` and `[SP §10]`.
