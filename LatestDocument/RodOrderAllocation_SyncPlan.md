...
**Propagates:** [`RodOrderAllocation.md`](RodOrderAllocation.md) into `MVP-1/`, `MVP-2/` and [`FlatWire_MasterSpecification.md`](FlatWire_MasterSpecification.md)
...
*into* `MVP-1/ProjectPlan/Backend/APIs.md` §4.15–§4.16 for the coil endpoints — and that pointer
...
| Target | Change |
|---|---|
[`Analysis/FlatWireOpenQuestions.md`](../Analysis/FlatWireOpenQuestions.md) | Mint **`Q48`–`Q59`** from `OQ-A`…`OQ-K`, `OQ-M`, `OQ-O`. Each carries a `Recommendation:` per the register's rule |
[`Analysis/FlatWireDecidedQuestions.md`](../Analysis/FlatWireDecidedQuestions.md) | **`OQ-L`** and **`OQ-N`** land here as decided, with their full text — a decided item is never deleted |
[`FlatWire_MasterSpecification.md`](FlatWire_MasterSpecification.md) §11 | **`OI-123`+** for the `FlatWireRun.OrderId` narrowing, the `SpoolOrder` boundary loss, and `GenerateCoilAlpha`'s unresolved `coils` reference |
[`Development/GapsRegister.md`](../MVP-1/ProjectPlan/Development/GapsRegister.md) | **`G47`** (`FlatWireRun.OrderId` reads wrong at an order boundary) · **`G48`** (the `SpoolOrder` boundary, with its closing window) |
...
| Target | Change |
|---|---|
[`Business/BusinessRequirements.md`](../MVP-1/ProjectPlan/Business/BusinessRequirements.md) | New `FR-541`+ block for allocation, sequencing, the handoff and fulfilment, carrying rule codes **`ORD003`–`ORD017`**. ⚠ **`FR-533`–`FR-540` are reserved by `W3`** for the FL2 pre-check-in — do not use them |
[`Business/BusinessRules.md`](../MVP-1/ProjectPlan/Business/BusinessRules.md) | §3 gains the rod ↔ order cardinality and the four-tier sequence rule. **§3.3's alpha table is untouched** (§2) |
[`Business/ProcessFlows.md`](../MVP-1/ProjectPlan/Business/ProcessFlows.md) | The order-boundary handoff as a flow: threshold → notification → acknowledgement → next order on the same mount |
[`Business/Screens/RodPreCheckin.md`](../MVP-1/ProjectPlan/Business/Screens/RodPreCheckin.md) · [`RocCheckin.md`](../MVP-1/ProjectPlan/Business/Screens/RocCheckin.md) | The sequence validation at both entry points (`Q73` item 7), and the order-set membership check |
[`Business/Screens/ActiveRunMonitor.md`](../MVP-1/ProjectPlan/Business/Screens/ActiveRunMonitor.md) | The allocation-reached notification and the **operator's mark-complete action** — rule 9's acknowledgement has no screen today |

### `S3` — Schema and DDL ⟳ *(the count-bearing wave)* ✅ **Applied 22 Aug 2026**

| Target | Change |
|---|---|
`Database/Schema/SQL/FlatWire_DDL_03_Materials.sql` | `RodOrderAllocation`; `SpoolTraceability.ChildAlpha` with its **replacement** comment block; `SpoolOrder.SpoolWeightFrom`/`To`; `SpoolOrder`'s derivation comment re-pointed |
...
`Database/DatabaseDesign.md` · `Schema/FlatWireSchema_Materials.md` · `_Runs.md` · `_QualityOutput.md` · `_Lookup.md` | The two tables written up; the rename; and the **three stale schema documents** closed (`Spool`, `SpoolTraceability`, `SpoolOrder`, `SpoolStaging` were never written up) |
...
| Target | Change |
|---|---|
[`Backend/APIs.md`](../MVP-1/ProjectPlan/Backend/APIs.md) | New endpoints for the acknowledgement and the allocation read; `GET /rod/{alpha}` returns an order **set**. `"coilAlpha"` payloads untouched |
[`Architecture/SignalR.md`](../MVP-1/ProjectPlan/Architecture/SignalR.md) §5.2 | Events **13 and 14**, both durable and re-delivered on group re-join, following event 11's contract. ⚠ `PP-04` records the count in the master spec and `BusinessRules.md` §3 too — **change all three or none** |
[`Backend/Services.md`](../MVP-1/ProjectPlan/Backend/Services.md) | `IFootageWeightConverter` and the allocation/consumption services |
[`Architecture/Integration.md`](../MVP-1/ProjectPlan/Architecture/Integration.md) | FL1 spool completion becomes a **cross-database caller** of `CommonDB.dbo.GenerateCoilAlpha` — and therefore **not testable on LocalDB** |

### `S5` — Plans, tests, effort ⟳ ⚠ **Applied 22 Aug 2026 — except the six enforcement test cases, written 25 Aug 2026**

| Target | Change |
|---|---|
[`Development/TaskBreakdown.md`](../MVP-1/ProjectPlan/Development/TaskBreakdown.md) | **`FW-225`+**, additive. ⚠ **`FW-224` is reserved by `A6`.** Story rows are parsed by three `.xlsx` generators — plain rows, no strikethrough |
[`Development/CapacityAndEffortModel.md`](../MVP-1/ProjectPlan/Development/CapacityAndEffortModel.md) | **A new additive sheet, never an in-place edit of a total** — the 3,186 h baseline is quoted in ~20 files |
[`Testing/TestCases.md`](../MVP-1/ProjectPlan/Testing/TestCases.md) | **`TC-730`+** for the four verification blocks in the design: the legal-sequence counts, the handoff invariants, the two-partition intersection, and the alpha ignore-list |
`Development/Phases/phase-04-rod-checkin-plc-config.md` | Owns the handoff, the sequence validation and the acknowledgement |
...
Measured surface in `FlatWire_MasterSpecification.md`: **`32 tables` ×7 · `CoilAlpha` ×7 ·
