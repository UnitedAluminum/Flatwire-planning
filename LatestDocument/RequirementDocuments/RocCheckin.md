...
> `[CLIENT INPUT REQUIRED]` The **min/max values** applied at check 3 are the same four-attribute set as at pre-check-in — gauge, width, diameter and ovality per alloy. The shape is agreed; the values are owed (Q71). Until they arrive, nothing is seeded and this check cannot be enforced.
...
> `[CLIENT INPUT REQUIRED]` The treatment of a **part-completed wizard** when the station switches mid-transaction must be confirmed (Q73/Q76), as must the case of a rod scheduled on **neither** line (Q78).
...
| [Pass Schedule Management](PassScheduleManagement.md) | Where schedules are authored, approved and overridden |
| [Pass Schedule Generation](PassScheduleGenerationSpec.md) | The engine that drafts a schedule from product specifications |
