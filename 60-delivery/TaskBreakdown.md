...
> [`FW-140`](../10-requirements/features/FS-02-backend-service-foundation.md).
...
> the plan at [`FW-220`](../10-requirements/features/FS-15-shared-schema-boundary.md).
...
> `[INT §7.9]`; plan at [`FW-223`](../10-requirements/features/FS-15-shared-schema-boundary.md).
...
**Rate-card basis:** **3 h**, quoted from [`FW-138`](../10-requirements/features/FS-02-backend-service-foundation.md) §8 and `P-50` — *"recommends `OrderController` as a sixteenth (**+3 h**)"*. ⚠ **Not a `[CE §2]` rate-card unit**: the card has no controller-shell row, and the command endpoint's 5 h is `FW-227`'s, not this story's
