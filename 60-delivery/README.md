...
| [`phases/phase-02-pass-schedule-management.md`](phases/phase-02-pass-schedule-management.md) | Phase 2, complete | **Moved whole, with its 231 h** |
| [`phases/phase-11-mvp2-shift-summary.md`](phases/phase-11-mvp2-shift-summary.md) | DB10 and `GET /shiftsummary` | **Partial** — carved at bullet level |
| [`phases/phase-13-mvp2-die-management.md`](phases/phase-13-mvp2-die-management.md) | Die Management screen and lifecycle service | **Partial** — carved at bullet level |
...
So each carved deliverable was **re-priced from the rate card** in [`./CapacityAndEffortModel.md`](./CapacityAndEffortModel.md) §2, and QA and contingency **re-derived** from the reduced base:
...
**One item remains open** and is not dissolved by any of the above: `PassScheduleManagement.md` §3.3–§3.4 holds the **only Operations Manager role definition**, and MVP-1 enforces that role — `FR-212` restricts reverting a roll-gap override to it on **DB11 Roll Adjust**. See [`../95-archive/design-notes/MVP-2-scope-note.md`](../95-archive/design-notes/MVP-2-scope-note.md).
...
| Artifact | Why |
|---|---|
| `../00-overview/Roadmap.md` | The roadmap index — one plan, both scopes. Its phase table now flags which rows are split |
| `../20-architecture/Architecture.md` §2.2 | §0.2 reference map, §0.3 domain cheat-sheet, §0.4 real-time architecture. Carries an explicit **do-not-restate** rule |
| `../90-registers/Gaps.md` | Dependency chain, milestone calendar, **gaps register G1–G35** |
...
| `../20-architecture/PLCCommunication.md` | The internal half of the PLC tag surface — MVP-1, and it carries **no tag path strings** by rule |
...
| `../tools/deliverables/build_docx.py` | Renders client deliverables in **both** scopes |
