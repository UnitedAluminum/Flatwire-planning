...
|---|---|---|---|---|---|---|---|---|
| **FW-010** | Pass Schedule data model + API | E02 | 2 | BE | 5 | Critical | FW-006 | **Given** an Operations user, **when** they create a schedule, **then** it starts `Draft`, **and** at most one `Active` schedule exists per `(MachineName, Alloy)`. **Component state is the three-value enum, not `IsActive` bool.** |
