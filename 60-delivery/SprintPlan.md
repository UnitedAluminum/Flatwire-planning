...
**Last Updated:** September 7, 2026 (`G23` closed — the canvas is 1920 × 1080) — **The §9.2 UI-conformance DoD bar moves to 1920 × 1080 at 1:1** — client-confirmed as `Q26`, closing gap `G23`. ⚠ **The mockups are still composed at 1280 × 1024** and supply **content, not composition** (`F-15`), so conformance is checked against the *canvas*, not against the mockup's geometry. *(previously September 5, 2026 (`D-50`) — ⭐ **§9.2's Definition of Done now REQUIRES .NET xUnit coverage** of `[TS §1.2]`'s four layers, reversing the 15 Aug 2026 withdrawal for the **unit level only**. ⛔ **No numeric .NET coverage bar** — Angular's 95 % does not transfer. ⚠ **Both former obligations stay** (code review against `[API]`, and the `TC-###` evidence line), and ⚠ **stories already `done` are NOT reopened**. Gap `G101`; carded `FW-263`–`FW-267`. *(previously August 18, 2026 — **`D-32`: there is no shared-schema migration.** The `FW-001` cross-feature impact and **RISK-05** are retired and replaced by the narrower **`OI-111`** *(previously August 15, 2026 — §9.2 DoD requires **`data-testid` on every element an automated test must reach**, the one E2E obligation that lands inside the trial window *(otherwise August 13, 2026 — split out of `05-SprintPlanAndBacklog.md` in the ProjectPlan restructure. **Section numbers are unchanged**, so every `§n` citation still resolves; numbering inside this file is deliberately non-contiguous)*)*)*)*
...
- **Must be sequential:** 4→5→6→7, 8→9, 9→10, and 14 last. *(The old `2→4` edge is now an external dependency, not a phase edge.)* **Phase 6 no longer waits on Phase 13** — since the 2 Sep 2026 die split (`Q91`) its die validation reads **`ToolingInventoryDie`**, the per-tool register seeded in Phase 1, and `D4` is enforced per tool. *(Until then it read the `Drawer` size catalogue, not a die inventory (`REVIEW.md` #34 / `OI-41` closed).
...
- [ ] **.NET: code in `FlatWire.sln` ships with xUnit coverage of the four layers in `[TS §1.2]`** — the validators, the domain rules and value objects, `PLCTagService`'s simulate branch, the line models and simulators, and the reachable parts of the DbContext-backed services. **Reversed 5 Sep 2026, `D-50`** *(the 15 Aug 2026 withdrawal stood until then)*. ⛔ **There is NO numeric .NET coverage bar** — the 95 % on the line above is Angular's and does **not** transfer: `FlatWire.sln` contains `Program.cs`, the EF entity configurations and controller plumbing, and a bar nobody can meet is suspended rather than met. ⚠ **Both of the old obligations STAY**, because a unit suite proves neither: **code review against `[API]`'s contracted shape and status codes**, and the manual test evidence on the next line. ⚠ **Stories already `done` under the 15 Aug DoD are NOT reopened** — this is a standing obligation from 5 Sep 2026 forward. ⛔ **Do not withdraw this again without reversing `D-50` in `[TS §1.2]`** — the 15 Aug decision was reversible precisely because it was taken *in the strategy* rather than inside a leaf story, and `[TS §1.2]`'s reinstatement is the **unit level only**: the integration and contract levels remain withdrawn.
...
- [ ] UI conformance checked against the mockup at **1920 × 1080 at 1:1** *(`Q26`, 7 Sep 2026; the mockups are composed at 1280 × 1024 and supply content, not composition — `F-15`)* — no text below 14 px (bar the documented SVG-axis exception), tap targets ≥ 48 px, no hover-dependent action.
