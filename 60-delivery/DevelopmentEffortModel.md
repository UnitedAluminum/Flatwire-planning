...
**Last Updated:** August 29, 2026 — **§1 gains an *FE — standalone desktop client (WinForms)* row at 0.70 (`D-33`).** ⚠ **The 0.62 screen factor is not borrowable** — its basis is that mockup → Angular conversion against one token system *“is largely mechanical”*, which is exactly what the move gives up. Sole consumer today: `FW-214` *(previously August 18, 2026 — **`D-32`: there is no shared-schema migration.** The `FW-001` 40 h rename impact audit and its 0.75 AI-assist factor leave this sheet entirely *(previously August 13, 2026 — **Phase 12 removed** to its own sheet; every total re-derived *(initial publication same day)*)*)*
...
| **FE — standalone desktop client (WinForms)** *(new, `D-33`, 29 Aug 2026)* | **0.70** | ⚠ **The 0.62 row above does not apply and must not be borrowed.** Its basis is that mockup → Angular component conversion *"against one existing token system … is largely mechanical"*, and a WinForms port gives up exactly that: no token system, no shared chrome, hand-laid-out designer code. The 0.70 is a blend — the SignalR client sits in the **RT 0.75–0.85** band, the project scaffold / auth / REST client are template-exact at ~**0.60** (`Tools/ConfigureAPI` and `Mobile Apps/UalApp/Repository/GenericRepository.cs` are the templates), and the form layout falls between. **Sole consumer today: `FW-214`** |
