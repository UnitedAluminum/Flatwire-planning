...
|---|---|---|---|---|---|---|---|
| 1 | **DB2** Rod Check-in (FL1) | `dashboard_2_rod_checkin.html` | [`RocCheckin.md`](../10-requirements/screens/RocCheckin.md) | 4 | `FW-061` | FR-063–084 | TC-070…109 |
| 2 | **DB3** Active Run (FL1) | `dashboard_3_active_run.html` | [`ActiveRunMonitor.md`](../10-requirements/screens/ActiveRunMonitor.md) | 5 | `FW-062` | FR-100–120 | TC-130…159 |
| 3 | **DB6** SPC Checkpoint *(dialog)* | `spc_checkpoint.js` | [`SPCCheckpoint.md`](../10-requirements/screens/SPCCheckpoint.md) | 6 | `FW-065` | FR-180–196 | TC-220…244 |
| 4 | **DB8** WIP Rejection *(dialog)* | `wip_rejection.js` | [`WipRejection.md`](../10-requirements/screens/WipRejection.md) | 7 | `FW-067` | FR-290–299 | TC-355…369 |
| 5 | **DB5** Spool Check-in (FL2) | `dashboard_5_spool_checkin.html` | [`RocCheckin.md`](../10-requirements/screens/RocCheckin.md) §4.3 | 8 | `FW-064` | FR-090–096 | TC-110…118 |
...
| Shell layout and the 1920×1080 shopfloor canvas | 16 | `FW-130` | 10 |
...
> ([`FW-138`](../10-requirements/features/FS-02-backend-service-foundation.md) `P-54`).
...
> [`MachineSimulator.md`](../20-architecture/MachineSimulator.md) `[SIM]`, stories **`FW-210`–`FW-215`** (111 h
...
> that converges on the pushed pass schedule), `FW-213` (the fault catalogue beyond one fault), `POST /sim/{machineName}/run`,
...
> `(MachineName, PayoffPosition)` to `(Station, PayoffPosition)`, for **`G21`**. **The trial column is deliberately
...
`FW-155`'s **`FlatWireRun(MachineName, Status)` index** (check-in's *single active run per line* rule and
...
| Blocker | Why it goes |
|---|---|
| **`G21`** — `(MachineName, PayoffPosition)` uniqueness across FL1/FL3 | It **blocked the Phase-4 schema freeze**, and it is a defect *in `RodStaging`*. No staging table, no gap in trial scope. **It remains open for MVP-1** |
...
> resource**, not 8. See [`ClientCall_2026-08-24_SyncPlan.md`](../95-archive/source-documents/ClientCall_2026-08-24_SyncPlan.md) §3.4.
>
...
| Artifact | Content |
|---|---|
| [`BusinessRequirements.md`](../10-requirements/BusinessRequirements.md) §5.5 | **`FR-130`–`FR-155` — 26 requirements.** Part A the advisory milestone ladder (`Should`); **Part B the PLC-confirmed stop confirmation (`Must`) — *"it is the gate on the spool completion transaction"*** |
| [`TestCases.md`](../70-testing/TestCases.md) §5.5 | **`TC-160`…`TC-184` — 25 cases**, eleven of them **P1** |
| [`SpoolCompletionNotification.md`](../10-requirements/screens/SpoolCompletionNotification.md) | The owning specification, Parts A/B/C |
| [`SignalR.md`](../20-architecture/SignalR.md) | Two hub events — `SpoolCompletionPromptDue`, `SpoolCompletionPromptResolved` — **server-owned state, persisted against the run, re-delivered on reconnect** |
...
✅ **`G23` is CLOSED (7 Sep 2026) — the canvas is 1920×1080, confirmed by the client (`Q26`).** All
25+ mockups are *composed* at 1280×1024 and `flat-wire-fit.js` calibrates its 14 px floor against that box, so
**§7's DoD bar and T4's UAT now assert conformance at 1920×1080 at 1:1** — both moved in the same pass.
**1920×1080 is 1.5× width and 1.05× height: a re-layout of every screen, not a rescale**, and `data-fit="fill"` had always made
a wider panel the cheap direction, so the exposure was bounded in the direction it went. ⚠ **What is left is not this
item's:** the panel's **physical diagonal** is recorded nowhere and the 14 px floor tracks dpi rather than resolution, so a
same-size denser panel would scale every size ×1.5 (`[P1A §6.10]`). **Ask for the diagonal before T1 closes.**
...
- **DoD bar is unchanged in every respect but one** (`[SP §9.2]`): Jest 95 % coverage, UI conformance at
  **1920×1080** at 1:1 (`Q26`, 7 Sep 2026), **no text below 14 px**, tap targets ≥ 48 px, no hover-dependent action, no new `--fw-*`
...
SELECT ScheduleId, RouteMode, Status FROM PassSchedule WHERE MachineName = 'FL2';
...
WHERE  p.MachineName = 'FL2' AND p.RouteMode = 'Standalone'
  AND  c.ComponentName = 'FM1' AND c.State = 'Active';
```

> ⚠ **Do not generalise (b) into *"a component's `Stand.MachineName` must equal its schedule's `MachineName`"*.** That
...
> an operator screen.** The operator drives the application; a test engineer drives the machine, from `DB-S1`. **`POST /sim/{machineName}/steer`** produces
> step 3's out-of-spec run, **`DELETE /sim/{machineName}/run`** produces step 7's stop edge at a chosen instant, and
> **`POST /sim/{machineName}/fault`** (dropped readings) is the only way to reach the interlock's condition 5.
...
([`PLCCommissioning.md`](../70-testing/PLCCommissioning.md) — `C1` tag paths, `C8` AGC latency, `C11` FM2 station
...
| [`../90-registers/Gaps.md`](../90-registers/Gaps.md) `[GAP]` | `G37` (this plan's finding), plus `G2` and `G6` — the blockers in §6 — and `G21`, `G26`, `G28`, `G31`, which the 14 Aug removals took out of trial scope while leaving them open for MVP-1 |
| [`Phases/`](Phases/) | The per-phase deliverable inventories this plan subsets |
| [`../70-testing/TestCases.md`](../70-testing/TestCases.md) `[TCS]` | The `TC-###` ranges in §1.1, the 75 cases §1.5 puts out of scope, and the acceptance run in §8 |
| **`FlatWire_TrialRunPlan.xlsx`** *(this folder)* | **This document as a workbook** — ten sheets, filterable, one row per work item. Generated by [`../tools/deliverables/build_trial_run_xlsx.py`](../tools/deliverables/build_trial_run_xlsx.py); **every figure in it is parsed from this file, so change this document and re-run.** Never edit the workbook. Five fatal guards, including one that refuses to build if any total here stops reconciling |
