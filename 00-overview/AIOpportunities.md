...
| `AI-40` | ⛔ **WITHDRAWN 9 Sep 2026 — the blind spot it existed to fill does not exist.** ~~⭐⭐ **Vision-based gauge and width on FL2** — the one place vision fills a *structural* blind spot rather than replacing a human. FL2 standalone has **no live gauge or width measurement at all**: it broadcasts `null`, and its trace is the historical profile from the FL1 pass that made the spool (`FR-120`, `[PLC §5.2.2]`).~~ **FL2 has gauging stands downstream of S3 that measure thickness and width continuously** — the devices driving S3's automatic position control. Assumption `A3` of `[PLC §14]` claimed otherwise, was never supported by any client source, and is retired; `FR-120` is superseded. ⚠ **The premise was ours, not the client's — which is exactly why this entry rated ⭐⭐.** The observation it rested on is still half-true and worth keeping: **FL2 is the pass that sets final dimensions, on the tightest tolerance.** But instrumenting it needs two OPC tags, not computer vision |
