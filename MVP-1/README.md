...
|---|---|
| The roadmap index, phase files, foundations, back matter, gaps register | now **here**, `DevelopmentPlan/` (moved and divided 11 Aug 2026) |
...
`PLCTagSpecification.docx` — generated output. **The `.md` in `RequirementDocuments/` is the source** — edit it and re-render with [`DevelopmentPlan/Tools/build_docx.py`](ProjectPlan/Tools/build_docx.py).

`FlatWire_ClientQuestions.xlsx` — generated output, the client questions workbook. Rendered by [`DevelopmentPlan/Tools/build_questions_xlsx.py`](ProjectPlan/Tools/build_questions_xlsx.py) from **two** sources: structure from the `Analysis/` question registers, prose from [`RequirementDocuments/ClientQuestionsContent.md`](RequirementDocuments/ClientQuestionsContent.md). Edit whichever of the two owns the field and re-run.
