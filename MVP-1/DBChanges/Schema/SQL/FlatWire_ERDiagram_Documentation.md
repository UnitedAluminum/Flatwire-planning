...
**Table count:** 28 tables across five domains — Lookup (7), Schedule (3), Materials (3), Runs (9), Quality/Output (6). Build scripts run in order `00 → 08` plus the two seed scripts (Lookup seed before Schedule seed). The full set has been validated on SQL Server 2019 (clean build + idempotent re-run + constraint/genealogy tests).
...
| **Drawer** | Draw box die configurations (DB1, DB2) | Id (PK), Name, DiameterIn (output wire size), MinDiameterIn, MaxDiameterIn, **LastGrindingFeet** (feet run *since* the last grind — a resettable counter, not the reading at it), **TotalFeetAllowed** (scheduled life; NULL until thresholds arrive, OQ-83), IsActive |
| **Edger** | Edger tooling configurations (Round/Square edge profiles) | Id (PK), Name (UK), EdgeType (Round\|Square), ToolingSetNo, IsActive |
| **Dancer** | Tension-management rollers. **FM1 carries one; FM2 carries two**, between S1/S2 and S2/S3 (`D-28`). Equipment capability only — no schedule column, pending `OQ-32` | Id (PK), Name (UK), LineId, Position (FM1\|FM2_S1_S2\|FM2_S2_S3), Ordinal (1=upstream), SupportsTensionMode, DefaultMode (Dancer\|Tension), IsActive |
...
| **PassScheduleChangeLog** | Immutable override/edit/acknowledgment audit trail (OQ-62) | FK → PassSchedule |
...
| **CoilOutput** | Finished output coils (one per completed run segment). Adds PassScheduleId + PassScheduleSnapshot (OQ-64), NetWeightOverrideLb + ScaleWeightLb (OQ-10 / packing), StagingLocation, audit + RowVersion | FK → FlatWireRun, PassSchedule; 1-N with CoilTraceability |
...
| **RodCheckout** | Rod removal from a payoff position (Mode P: pre-check-out, never checked in; Mode A: pre-run; Mode B: mid-run emergency). Carries the **supervisor approval stamp** — required for Mode B (OQ-74) and for a **welded** Mode P removal, which is a rejection to HOLD (OQ-69) | FK → FlatWireRun (nullable), Rod |
...
PassSchedule (1) ──┬──→ (N) PassScheduleComponent ──┬──→ Stand
                   ...
                   ├──→ (N) RodCheckin
                   ├──→ (N) SpoolCheckin
                   └──→ (0-N) CoilOutput   (schedule effective at coil creation, OQ-64)
```
