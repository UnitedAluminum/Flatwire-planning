...
| Concern | Story |
|---|---|
| The live telemetry stream that follows the snapshot | [`FW-150`](FW-150-Broadcast-Loop.md) — built |
| `LineStatus` transitions and markers | [`FW-172`](FW-172-Run-Event-Markers.md) |
| `GET /run/active` and `gaugetrace` | [`FW-164`](FW-164-Run-Queries-And-RunQueryService.md) — ⚠ **the story the header comment confuses this with** |
| The alert lifecycle behind `ActiveAlertDto` | `FW-N06` — ⚠ this endpoint **reads** alerts, it does not raise them |
| Bay staging commands | [`FW-158`](FW-158-PayoffStaging-Commands-And-Queries.md) |
...
The board goes live from [`FW-150`](FW-150-Broadcast-Loop.md)'s stream after this snapshot, and
the two must agree (`P-181`). `FW-N06` owns the alert lifecycle `ActiveAlertDto` reads.
[`FW-158`](FW-158-PayoffStaging-Commands-And-Queries.md) writes the bay occupancy this reads,
and `FW-160` broadcasts its changes. `FW-N12` deletes the stub when the last throw goes.
⚠ **DB1 left MVP-1 scope**, so [`FW-164`](FW-164-Run-Queries-And-RunQueryService.md) — not this —
