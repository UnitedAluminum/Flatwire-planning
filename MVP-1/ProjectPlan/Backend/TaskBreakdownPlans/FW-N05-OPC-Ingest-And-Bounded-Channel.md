...
**Last Updated:** August 28, 2026 — ✅ **EXECUTED, AND VERIFIED BY HARNESS AND BY BOOT. The contract is built and the hosted service is built, registered off, and blocked exactly where `P-120` said it would be.** Built in `ual-api`: **`Reading`** (`FlatWire.Domain/Models/RealTime/Reading.cs`), **`IReadingChannel`** + **`ReadingChannel`** (bounded, `DropOldest`, `SingleReader`, `SingleWriter = false`), **`OpcIngestService`** (`FlatWire.Infrastructure`), three new options (`SimulateOpcFeed`, `ChannelCapacity`, `IngestLines`) with validation, and the registration seam `AddFlatWireOpcIngest`. **0 errors, 13 warnings on a clean rebuild — 5 code warnings, all pre-existing, none in these files.** ✅ **The headline behaviour is verified: 9 of 9 harness assertions pass.** A 1,524-snapshot burst against a stalled drain stayed **bounded at 1,024**, the **oldest 500 were dropped and the freshest survived**, `written − drained = 500` made the resolution loss observable, and the whole burst allocated **349.7 KB** — ~235 bytes a snapshot, so `P-28`'s 1024 default costs ~240 KB. ⛔ **EXECUTION FOUND A DEFECT THE REVIEW DID NOT: the configuration binder APPENDS to a collection that already holds C# defaults.** `IngestLines = ["FL1","FL2"]` in code plus the same two in `appsettings` bound to **`FL1, FL2, FL1, FL2`** — four poll loops reading every tag **twice a second**, which the boot log printed as *"OPC ingest started: FL1, FL2, FL1, FL2"*. Worse than the doubling: a deployer setting `["FL3"]` would get `FL1, FL2, FL3` and **could not remove the first two**, making this card's `{FL1,FL2}`-or-`{FL3}` rule impossible to honour. **Fixed by removing the C# default** (`P-123`) — the default lives in `appsettings.json`, where it can be overridden — **plus a dedupe guard** in the service, since a deployer listing a line twice is the same bug by hand. ✅ **Re-booted: `FL1, FL2` once each.** ✅ **`G59`/`G60` reproduced by boot, by name:** with the flag flipped, both lines log *"cannot resolve … `FlatWireOpc:OpcModuleId` is not configured (gap G60 …)"* — **once per line, twice in 15 seconds of ticking**, and the host **stayed up** (health `200`, `database.reachable: true`). That last property is the one that matters: a deferred, blocked ingest does not take the API down. ⚠ **Two of this card's own numbers are corrected by the build: the read list is 14 of FL1's 17 paths and 11 of FL2's 22 — 25 tag reads in 2 POSTs a second**, not the ~37 this card estimated, because the dancers, the two edger keys and the write-only interlock have nowhere to land (`P-122`). ⚠ **And one is now stale: `Polly` IS in `FlatWire.Infrastructure`'s package set** — `FW-151` added it on 28 Aug. What was missing instead was **`Microsoft.Extensions.Hosting.Abstractions`**, which AC 1's placement requires and which was **not** centrally pinned; added at **8.0.1** to match the net8.0 shared framework (`P-121`). ⚠ **One review finding withdrawn:** the *"no fault tag on any FM2 stand"* item is **already tracked** — `TagNames.Fm2S3Faulted` carries a derived path and its own note citing `PLC-Q02` / `[PLC §5.4]`. It was not a new gap. *(earlier the same day)* ⚠ **RE-REVIEWED BEFORE EXECUTION against the built `FlatWire` projects: ten corrections, and three would have shipped a defect.** ⛔ **(1) The quantity value objects must NOT be used in the channel item, and the reason is a throw rather than a preference: `Gauge` and `Width` are `RequirePositive`, so `new Gauge(0)` throws — and `0` is exactly what an idle, unconfigured or mis-read tag returns.** Constructing them in the poll loop makes a bad tag value an exception **every second**; `Footage`/`WeightLb`/`SpeedFpm`/`RollGap` are `RequireNonNegative` and would not. So: **primitives in the channel, value objects downstream**, with the mapping rule stated — gauge/width `≤ 0` → `null`, speed/footage/weight **keep `0`** (a stopped line is not a missing reading). ⛔ **(2) FL3 must not be polled.** The built `FlatWireOpcOptions.Lines` says it is *"configured and inert"* until `PLC-Q08`/`G30`, and under one of that question's two answers FL3's stands **are** `FL2.FM2.*` — so polling both reads the same load cell and the same three stands twice a second, invisibly. **Two lines, not three; {FL1,FL2} *or* {FL3}, never all three.** ⛔ **(3) `DropOldest` reports nothing** — `TryWrite` returns `true` either way, so *"degrades resolution"* was unobservable as written; a written count here plus `FW-150`'s drained count is the whole fix. ✅ **Four things the built code already provides, now cited instead of prescribed:** `ITagPathResolver.LogicalNamesFor(LineId)` is documented as *"the ingest subscription list"*; `OpcInfoCacheSeconds` (default 300) is the `GetOPCInfo` cache; `LineId`/`PayoffPosition`/`ComponentName` are built enums the record should use; and `IFlatWireBroadcaster.Line(LineId)` confirms **per line** is the unit the whole RT spine is keyed on. ✅ **`G31` is more decidable than its wording** — `[PLC]`'s own *"used in"* column names consumers for `Status.IsActive`, `RollGap`, `Diameter` and even `Status.IsFaulted` (*the Critical component-fault alert*, which is why it is absent from `[SIG §5.2]`'s payload); **the one genuine orphan is `FL1.EdgeSet.Status.IsActive`**, on the line with no edger. ✅ **FL2's null gauge/width needs no branch at all** — `[PLC §5.2.2]` publishes **no `AGC` row**, so there is nothing to suppress; never write `if (line == FL2)` here. ⚠ **Resolve the path list once at boot, not per tick** — `Resolve` throws by design. ⚠ **Exactly one publisher may be registered**, and `FW-203`'s *"flag pair"* has only one half built. ⚠ **And the batch's win is stated honestly:** `OPCConnection` still reads **tag by tag** inside the handler, so the saving is **2 HTTP requests instead of ~39**, not fewer OPC operations. ⚠ **One finding raised, not fixed:** no `Status.IsFaulted` exists on any **FM2 stand**, so the component-fault alert cannot fire for the three stands the coil is made on. *(earlier the same day)* ⚠ **REVIEWED against the built `ual-api` and the 26–28 Aug changes. `P-118`–`P-120` minted; three citations corrected.** ⛔ **Headline: this story is the largest `G59` case in the plan and no document said so.** `RestClient.SetHeadersAsync` takes its bearer token from `context.HttpContext`, and an `IHostedService` **has no `HttpContext`** — so the call fails **before the network**, on the **first** call (`GetOPCInfo`), with *"Object reference not set to an instance of an object."* `G59` names `SetITInhibit` and hold/restore; **the ingest is the same shape running once a second forever.** ⛔ **`G60` sits behind it** — `OPCModules` has no flat wire member and no script registers one, so `GetOPCInfo` returns an empty list even *with* a token. **Both are build-order step 0 (`P-120`), not commissioning detail.** ⚠ **The read is a batched POST, and that is the performance decision in this story:** `ReadTag` takes `OPCInfo.Tags` as a **list** and returns it — **one POST per line per interval (2/s at `NFR005`'s default, FL3 inert), never one per bound path (~39/s)** — `P-118`. ✅ **"Coalesce" needs no coalescer.** A **per-line snapshot** plus `BoundedChannelFullMode.DropOldest` is the whole mechanism: a newer snapshot supersedes an older one field for field, so dropping the old one *is* the coalesce. **A keyed dictionary under a lock on this path is the overengineering to avoid** — `P-119`, which also **defines `Reading` at last** and discharges `P-29`'s standing requirement. ⚠ **A failed read is invisible unless you make it visible:** both managers return the tag **unchanged** on a null read or an `OpcException`, and **`OPCUAManager.ReadTag` never sets `IsGood`** — so send `Value = null`, treat a null return as *no reading*, and **never carry the previous tick's value forward** (§2.4). ⚠ **The `41`-path figure is struck** — `FW-144` measured **72** bound (FL1 17 · FL2 22 · FL3 33); re-baselining the literal is `G33`'s, not a leaf plan's. ⛔ **`[SIG §5.2]` was miscited** as the contract this story shares with `FW-203`: §5.2 is `FW-149`'s **client** contract on the far side of `FW-150`, and the shared surface is **this card's channel**. *(previously August 15, 2026 — first issue)*
...
| **FL2's `null` gauge/width suppression** | [`FW-150`](FW-150-Broadcast-Loop.md) / [`FW-181`](FW-181-FL2-Null-Gauge-Contract.md) — a **broadcast-boundary** rule (`[SIG §5.3]`, `FR-120`). **The ingest has no FL2 branch** |
| Resolving a line's **active run** | [`FW-150`](FW-150-Broadcast-Loop.md), which persists `RunReading` — §2.5 |
...
| Hub drain cadence | ~100 ms / 10 Hz — `FlatWireSignalROptions.DrainCadenceMs` | [`FW-150`](FW-150-Broadcast-Loop.md) |

They sit on opposite sides of the channel. **Never derive one from the other**
([`FW-144 §2.1`](FW-144-Configuration-Binding.md), which built and validated both keys —
...
[`FW-144`](FW-144-Configuration-Binding.md) built the map behind **`ITagPathResolver`**
...
[`FW-151`](FW-151-PLCTagService.md), which found them on the write path; **three bite harder
...
[`FW-203`](FW-203-OPC-Feed-Simulator.md) write to the same channel, so registering both
...
9. Resilience: **Polly** on the outbound call, sharing
   [`FW-151`](FW-151-PLCTagService.md)'s policy — **retry on `Result.IsFailure`, not on an
