...
**Status:** ⚠ **Part A only.** Part B moved to [`FW-202`](FW-202-FL1-Spool-Completion.md) — ⛔ **the card's ACs still describe both**
...
> [`FW-202`](FW-202-FL1-Spool-Completion.md) took Part B. ⚠ **Its acceptance criteria still
...
| Concern | Owner |
|---|---|
| ⛔ **The machine-stop confirmation** (AC 2) | [`FW-202`](FW-202-FL1-Spool-Completion.md) — **Part B** |
...
| The broadcast loop | [`FW-150`](FW-150-Broadcast-Loop.md) — ✅ built |
...
4. The **milestone ladder** as a threshold evaluator over `payoffWeight$`, following
   [`FW-N06`](FW-N06-Alert-Rules-Engine.md)'s raise/clear shape (§2.3).
...
[`FW-202`](FW-202-FL1-Spool-Completion.md) owns Part B — the PLC-confirmed stop confirmation, the
completion transaction and the `SpoolProcessing` write — and **shares AC 5's physical
constraint**. [`FW-149`](FW-149-IFlatWireClient.md) recorded `targetLb`'s absence on the hub
member and is waiting on this story's answer (`P-238`).
[`FW-N06`](FW-N06-Alert-Rules-Engine.md) is the pattern to follow.
