...
| Concern | Story |
|---|---|
| Exception broadcasts and supervisor notification | [`FW-177`](FW-177-Exception-Broadcasts.md), Phase 7 — ⚠ **it consumes `AlertRaised`** |
| The telemetry loop | [`FW-150`](FW-150-Broadcast-Loop.md) — built, and ⛔ these events stay out of its batch |
| `PayoffStateChanged` itself | `FW-160`, Phase 4 — this story **consumes** it |
| Bay occupancy writes | [`FW-158`](FW-158-PayoffStaging-Commands-And-Queries.md), Phase 4 |
| Reading alerts for page load | [`FW-154`](FW-154-Lines-Status-And-LineStatusService.md)'s `ActiveAlertDto` |
| The `ITInhibit` interlock | [`FW-205`](FW-205-ITInhibitService.md) — ⚠ **a different mechanism**; do not conflate (§2.4) |
...
**Take the second**, the same shape as [`FW-206`](FW-206-ITInhibit-Conditions-1-2.md)'s `P-179`.
...
4. **Rule 2** — gauge outside ±tolerance → Warning. ⚠ **Reuse `FW-150`'s per-run cached band**
   ...
   built and unexercised, like [`FW-245`](../../Database/TaskBreakdownPlans/FW-245-SpcMeasurement-InSpec-Asymmetric-Band.md).
...
[`FW-177`](FW-177-Exception-Broadcasts.md) (Phase 7) consumes `AlertRaised` for supervisor
notification — ⚠ **and `FW-175` is deferred, so that notification is transient.**
[`FW-154`](FW-154-Lines-Status-And-LineStatusService.md)'s `ActiveAlertDto` reads the current
alert set for page load and must return an empty list until this story lands. `FW-160` supplies
`PayoffStateChanged`; [`FW-158`](FW-158-PayoffStaging-Commands-And-Queries.md) writes the
