...
**Last Updated:** August 27, 2026 — ✅ **EXECUTED. `TC-020` has been run** (new §6.1) and step 4's `P-19` handoff verified as already in `FW-207`. **12 of 14 enums pass C# ↔ DDL with zero mismatches** — legs extracted mechanically, all 87 value-list `CHECK`s parsed, 24 constraints compared. ⚠ **It is a TWO-way diff: the TypeScript leg does not exist in any `ual-angular` checkout** (`FW-132` unbuilt), and `LineState`/`AlertSeverity` have **no DB leg either**, so two enums are asserted by nothing — **`G56`**, and `TC-020` is now signed off **per leg** (**`P-84`**). One real defect found: **`G55`** — `CK_SpoolCheckin_PayoffPos` pins FL2's spool to payoff position `1` (`Payoff1`, a rod-fed VPS bay) while this story's enum and the pinned lookup both make FL2's payoff `3` (`TraversingTakeup`), on a column with **no FK**; the *membership* diff passes because the disagreement is about **meaning**. Earlier the same day: **reviewed against the built code; four factual corrections and `P-83` minted.** ⚠ **The one that changes what a developer does today: `TC-020` is NOT defective and never needed fixing.** `[TCS]` corrected it on **14 Aug 2026** — the day *before* this plan first issued — and its expected result matches `CanonicalEnums.cs` value for value. `phase-01b` L163's *"`TC-020` is stale"* was the stale artifact and **is struck at source the same day** — rewritten to the same line count so every `phase-01b` L-number citation still resolves — so **this story's one outstanding step is runnable now**. Also: **fourteen validators were built, not thirteen** (`UnstageRodRequestValidator`; **six places in `FW-139` and one in `Orchestration.md` corrected at source**, along with `FW-139`'s `POST /payoff/stage`, a route that exists nowhere — it is `POST /staging/rod`); **an FL2 `lineId` does not fail enum membership** — `LineId` carries all three lines by construction and **five explicit line-eligibility shape rules were built** (`P-83`); and on thirteen of the fourteen the `400` reaches the client through **model binding**, not `ValidatorBehavior` (`P-60`). §3 now names all fourteen enums and the fifteen string vocabularies beside them; §5 points at `P-57`/`P-58`, which govern this story's code and are minted in `FW-138`. Earlier: August 25, 2026 — the validators target request DTOs while `ValidatorBehavior` resolves on the **command**; `FW-139` `P-59` bridges them and nothing here is rewritten. Earlier: ⚠ **the `lineId = FL2` rejection is withdrawn** (`FR-533`, 20 Aug 2026): §2.1 flagged *(previously August 15, 2026 — `G41` recorded — `CK_PSC_FM1NotBypassable` is line-blind; the fix lands in `FW-207`, not here *(first issue, same day)*)*
