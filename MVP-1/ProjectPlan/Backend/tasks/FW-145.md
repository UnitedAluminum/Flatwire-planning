...
| Hub auth | A hub connection with `?access_token=` succeeds; without it, fails. ⚠ **The handler is this story's; the evidence is not available until [`FW-080`](FW-080.md) maps `/hubs/flatwire`** — until then there is nothing to connect to. Carry it to `FW-080`'s *Auth* verification row rather than recording an untested pass |
