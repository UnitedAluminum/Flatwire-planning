...
> **Scope legend — the map spans both scopes deliberately** *(added 27 Aug 2026; Appendix B marked scope, this map did not)*. **MVP-2, not built in MVP-1:** `DB9` · `DB9A` · `DB10`. Everything else on the map is MVP-1. The map is kept whole rather than halved for the same reason `Analysis/FlatWireShopfloorDashboards.md`'s is — the navigation itself crosses the boundary.
>
