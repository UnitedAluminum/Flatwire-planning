...
status_note: "✅ **Buildable now — `FW-N03` landed 28 Aug 2026** (wave 1). ⚠ The app `styles` array slot is empty and waiting for this story's stylesheet; `FlatWirePlaceholderComponent` is the temporary landing component **this story replaces with the shell**"
owner:
jira:
mvp: 1
phase: "1A"
...
3. ⚠ **The shell component needs a spec on the day it is written.** `collectCoverageFrom` globs `*.component.ts` and the gate is **95 %** (`F-08`); `FW-N03` hit exactly this — its first suite run failed at **36 %** because a guard stub had no spec ([`FW-N03 §6.2`](FW-N03.md) item 6).
