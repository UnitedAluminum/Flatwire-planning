...
> [`TrialOrchestration.md §1`](../../Backend/tasks/TrialOrchestration.md) uses —
...
> [TrialOrchestration.md](../../Backend/tasks/TrialOrchestration.md) sequences all
...
> [TrialOrchestration.md](../../Backend/tasks/TrialOrchestration.md) and `[TB §7.2]`,
> not by wave here.
>
...
|---|---|---|---|---|---|
| `FW-N03` | Angular library scaffold, routing, configuration | 24 | **0** | [`[CMP §5.1]`](../Components.md) · [`phase-01a`](../../Development/Phases/phase-01a-angular-foundation.md) · [`plan`](FW-N03.md) | ✅ **BUILT 28 Aug 2026** — builds, lints, 3 tests at 100 %, all eleven integration points verified ([`plan §6`](FW-N03.md)). **Wave 1 is open** |
...
[`TrialOrchestration.md §1.2`](../../Backend/tasks/TrialOrchestration.md), which is
...
| **`P-58`** / **`P-83`** | `FW-132` | **Fourteen canonical enums, mirrored in all three layers, and `LineId` is never narrowed** — line eligibility is a per-endpoint *shape* rule, so no screen gets an FL2 refusal free from enum membership. Settled in [`FW-147`](../../Backend/tasks/FW-147.md); the TypeScript leg is **owed by `FW-132`** (`G56`) |
| **`P-53`** 🔴 / **`P-54`** | `FW-061`, `FW-N01` | **The service hosts no rod-receiving surface**, so **DB2's rod scan has no endpoint** — `GET /rod/{alpha}` was *"everything staging and check-in need in one round trip"*. `[API]`'s call, not a plan's. [`FW-138 §8.1`](../../Backend/tasks/FW-138.md) |
...
| **Before `FW-131`'s role half** *(was: before QA0)* | ⛔ **`G6` — and it is worse than this row said.** The residual was filed as *coded claim values*; measured 27 Aug 2026, **the client has no role source at all** — no role field on `LoginStatusDetails`, no role vocabulary in `shared.constants.ts`, no JWT decoder in the repository | ⛔ **Construction, not verification.** `FlatWireRoleGuard` **cannot be built** until a source is chosen; `FlatWireAuthGuard` is unaffected and builds today. Three options and a recommendation in [`P1A`](Phase-01A-ImplementationPlan.md) `F-12`; either way bind to one constants class, as `FW-145` does server-side | `[SEC §8]` · [`FW-145 §5`](../../Backend/tasks/FW-145.md) · [`P1A §6.1`](Phase-01A-ImplementationPlan.md) |
| **Before T2** | **`G10`** — IIS WebSockets on the target, and MessagePack as a new client dependency | Transport **silently** falls back to long-poll. **A provisioning task, not a build one**; `[SIG]` treats the package as measure-first | [`FW-080`](../../Backend/tasks/FW-080.md) · `[DEP §4.4]` |
...
| | **MVP-1** | **Trial (30 Sep)** |
|---|---|---|
| `FW-133` | **120 h** — all six composite controls | **75 h** ([`TrialOrchestration §2`](../../Backend/tasks/TrialOrchestration.md)) |
...
> and [`Database/tasks/Orchestration.md`](../../Database/tasks/Orchestration.md)
...
6. **1A's stated verification exercises the mock path only.** `phase-01a`'s *Testing* section
   ...
   ([`FW-138 §6.1`](../../Backend/tasks/FW-138.md),
