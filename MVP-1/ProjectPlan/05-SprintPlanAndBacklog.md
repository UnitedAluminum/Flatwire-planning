...
| Total effort — both scopes | 3,660 h = 457.5 dev-days *(the **3,727** quoted before 11 Aug 2026 was a stale TOTAL row — see `CapacityAndEffortModel.md` §3)* |
...
| **Sustained requirement** | **≈ 9.4 FTE** (MVP-1) · ≈ 10.4 FTE (both scopes) |
| Phase-1 gate (1,027 h in 12 working days, 96 h/person) | **10.7 FTE on Phase 1 alone** |
| W7 (**587 h** MVP-1 in 3 working days, 24 h/person) | **24.5 FTE — arithmetically impossible** *(653 h / 27.2 FTE both scopes)* |
...
|---|---|---|
| **A — Staff to the plan** | **~9.4 concurrent people (75 h/day) for MVP-1**, ~10.4 (83 h/day) for both scopes, across the six streams, from W0 | The only option that holds 30 Sep. Requires ~3.7 FE, ~2.5 BE, ~1.8 QA, ~1.2 RT, ~1.2 DB, ~0.2 BA. **Ramp-up time is not in the 3,727 h** |
...
| **QA** | Test / E2E / UAT | All phases + Phase 14 | **701** | 19.2 % | **2.0** |
| **RT** | Real-time / PLC | `FlatWireHub`, OPC ingest, `PLCTagService` | **408** | 11.1 % | **1.2** |
| **DB** | SQL Server | New `FlatWireDB` + the shared-schema FW-001 renames | **404** | 11.0 % | **1.1** |
| **BA** | BA / Ops liaison | Pass-schedule content, OQ closure, UAT coordination | **96** | 2.6 % | **0.3** |
| | | **Total (both scopes)** | **3,660** | 100 % | **10.4** |

> **Recomputed 11 Aug 2026.** The previous row (FE 1,302 · BE 870 · QA 621 · RT 432 · DB 418 · BA 84 = 3,727) was wrong twice: the total came from a **stale TOTAL row** in the effort model — the 4 Aug DB13/DB14 descope cut Phase 5 by 67 h and never reached it — and **QA was the raw column with no contingency loaded**, which is why QA moves most (621 → **701**). For **MVP-1 only, scale to 3,292 h / 9.4 FTE**: see `CapacityAndEffortModel.md` §3b.
...
| Unit | Hours | Notes |
|---|---|---|
| New dashboard screen | **24 h** | `MVP-1/Mockups/*.html` is the approved visual spec — **no design time is included** |
...
| Footage→weight basis | **16–32 h** | 9 | **OI-45** / OQ-10 — integrating over `RunReading` is materially more work than a target-derived weight |
...
\* Rungs 1–4 of the descope ladder. **The `Hours` column sums to 3,660** (both scopes) — allocated with no leakage. **On MVP-1 scope the total is 3,292**, and the weekly shape changes: W2/W3 fall to 95 h each (Phase 2 left MVP-1), W4 to 409, W6 to 517 and W7 to 587. See `CapacityAndEffortModel.md` §3b.
...
> **Seven stories are MVP-2 as of 11 Aug 2026** — `FW-010`, `FW-011`, `FW-012`, `FW-013`, `FW-068`, `FW-069`, `FW-N09` — and one more spans both scopes (`FW-014`). **`FW-066` returned to MVP-1** with Phase 9, and **`FW-N07` is now wholly MVP-2** — the die master table left MVP-1 with the die inventory and lifecycle. **Their rows are marked in place, not removed.** Deleting them would tear holes in **§11, the coverage matrix that proves every `FR-###` reaches a story**, and that matrix is the only artifact demonstrating requirement coverage. Full rows are also reproduced in [`../../MVP-2/ProjectPlan/05-Backlog-MVP2.md`](../../MVP-2/ProjectPlan/05-Backlog-MVP2.md).
>
> **`FW-010` is the one to look at twice.** It builds the pass-schedule data model and API — now MVP-2 — while `FW-061` (rod check-in) and `FW-082` (PLC tag push), both **Critical MVP-1**, list it as a dependency. The dependency did not disappear with the scope line.
...
| **FW-007** | Event/output tables | E01 | 1C | DB | 5 | Critical | FW-006 | **Given** the core tables, **when** `04_Runs` … `08_Programmability` run, **then** all **28 tables**, **43 FKs**, **64 non-clustered indexes**, the non-overlap trigger and both read procedures exist and pass the post-run checks. *(Counts re-measured against the deployed database 6 Aug 2026; the earlier 41/46 were a 30 Jul DDL count that had drifted.)* |
| **FW-010** | Pass Schedule data model + API | E02 | 2 | BE | 5 | Critical | FW-006 | **Given** an Operations user, **when** they create a schedule, **then** it starts `Draft`, **and** at most one `Active` schedule exists per `(LineId, Alloy)`. **Component state is the three-value enum, not `IsActive` bool.** **⚠ MVP-2 — deferred; see [`../../MVP-2/ProjectPlan/05-Backlog-MVP2.md`](../../MVP-2/ProjectPlan/05-Backlog-MVP2.md). Row retained so §11 coverage stays whole.** |
| **FW-011** | Dashboard 9A — schedule list | E02 | 2 | FE | 3 | High | FW-010 | **Given** schedules exist, **when** search, alloy, line and status filters are applied, **then** they apply simultaneously, the stats strip updates, and any non-"All" filter renders amber. **⚠ MVP-2 — deferred; see [`../../MVP-2/ProjectPlan/05-Backlog-MVP2.md`](../../MVP-2/ProjectPlan/05-Backlog-MVP2.md). Row retained so §11 coverage stays whole.** |
| **FW-012** | Dashboard 9 — schedule management | E02 | 2 | FE | 8 | Critical | FW-010 | **Given** a schedule open for edit, **when** the operator toggles components, **then** the mandatory final stand is locked on, bypassed rows read "Bypassed · no parameters", edger rows offer an edge-type selector, **and** nothing pushes to the PLC. **⚠ MVP-2 — deferred; see [`../../MVP-2/ProjectPlan/05-Backlog-MVP2.md`](../../MVP-2/ProjectPlan/05-Backlog-MVP2.md). Row retained so §11 coverage stays whole.** |
| **FW-013** | Generate-from-Specs algorithm | E02 | 2 | BE | 8 | High | FW-004 | **Given** alloy 1100, rod 0.375″, gauge 0.125″, width 0.875″, **when** generate runs, **then** it returns `preflattenDiameterIn 0.3732`, `areaReductionPct 0.95`, `drawPasses 0`, `routeMode Hybrid` with the two warnings — **not the published 0.265 / 50.1 / Standalone.** Apply stays enabled on errors; **no PLC write ever occurs**. **⚠ MVP-2 — deferred; see [`../../MVP-2/ProjectPlan/05-Backlog-MVP2.md`](../../MVP-2/ProjectPlan/05-Backlog-MVP2.md). Row retained so §11 coverage stays whole.** |
| **FW-014** | Pass-schedule override logging | E02 | 2 *(used 6)* | BE | 3 | High | FW-010 | **Given** a mid-run override, **when** it is saved, **then** `PassScheduleChangeLog` records parameter, old→new, user, timestamp and reason, **and** the Active Run Monitor raises an alert that **cannot be passively dismissed**, with the line continuing on the **previous** PLC values until acknowledged. **⚠ Spans both scopes — log table is MVP-2, trigger is MVP-1.** |
...
| **FW-066** | Dashboard 7 — coil completion *(and DB7b packing)* | E07 | 9 | FE + BE | 5 | High | FW-100 | **Given** a completed coil, **when** the operator confirms, **then** the alpha is issued, traceability rows cover 100 % of coil footage without overlap, gauge/width show the **target** value when SPC confirms in tolerance, and **exactly two coils close a skid**. **MVP-1 — returned 11 Aug 2026 with Phase 9; covers DB7 and DB7b.** |
...
| **FW-068** | DB9/9A shopfloor integration | E02 | 2 | FE | 2 | High | FW-011, FW-012 | **Given** the shopfloor shell, **when** a user opens Pass Schedules from the More Options popup, **then** DB9A opens with role-appropriate actions and operators see read-only. **⚠ MVP-2 — deferred; see [`../../MVP-2/ProjectPlan/05-Backlog-MVP2.md`](../../MVP-2/ProjectPlan/05-Backlog-MVP2.md). Row retained so §11 coverage stays whole.** |
| **FW-069** | Dashboard 10 — shift summary | E07 | 11 | FE + BE | 5 | Medium | FW-007 | **Given** a shift window, **when** a machine tab is selected, **then** the KPI tiles reflect **that machine only**, the utilisation timeline shows one line (or all three on All Lines), and runs that resumed without a completed SPC checkpoint are **flagged as exceptions**. **⚠ MVP-2 — deferred; see [`../../MVP-2/ProjectPlan/05-Backlog-MVP2.md`](../../MVP-2/ProjectPlan/05-Backlog-MVP2.md). Row retained so §11 coverage stays whole.** |
...
| **FW-N09** `[NEW]` | OEE dashboard | E07 | **unassigned** | FE + BE | **Could** | **PP-03.** The OEE dashboard has an approved mockup and 17 source requirements (`OEE001`–`OEE017`, carried as `FR-500`–`FR-508`) and **no story, no phase and no owner** | **Given** shift data, **when** OEE renders, **then** A·P·Q per line, MTBF/MTTR, the 7-shift trend and Six Big Losses display against the configurable 85 % target. **Or: formally record OEE as out of scope** **⚠ MVP-2 — deferred; see [`../../MVP-2/ProjectPlan/05-Backlog-MVP2.md`](../../MVP-2/ProjectPlan/05-Backlog-MVP2.md). Row retained so §11 coverage stays whole.** |
...
| **G12** | Source artifacts still reference `united_db` and the dropped `Rod` | Open — DDL/ERD agree at 28 tables |
