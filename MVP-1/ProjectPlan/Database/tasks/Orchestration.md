...
| **[`FW-245`](FW-245.md)** | `G51` — `SpcMeasurement.InSpec` on an asymmetric band | 1C | DB 6 | [`FlatWire_DDL_05_QualityOutput.sql`](../Schema/SQL/FlatWire_DDL_05_QualityOutput.sql) | ⛔ A **`PERSISTED`** symmetric verdict against `AlloyProperty`'s min/max pairs — **the wrong answer is on disk** |
| **[`FW-246`](FW-246.md)** | `G50`/`G52`/`G41`/`G55` — constraint and RI repairs | 1C/4 | DB 12 | [`03_Materials`](../Schema/SQL/FlatWire_DDL_03_Materials.sql) · [`04_Runs`](../Schema/SQL/FlatWire_DDL_04_Runs.sql) · [`06_ForeignKeys`](../Schema/SQL/FlatWire_DDL_06_ForeignKeys.sql) | ⛔ `G41` means **a correct FL2 pass schedule cannot be authored**; `G55` is a disagreement about *meaning* on a column with no FK, so the membership diff passes |
...
| **Before the Phase-4 schema freeze** *(owner: **`FW-243`**)* | 🔴 **`D-30`** — `ROWVERSION` absent on `WeldEvent`, `RodCheckout`, `WipRejection` | 3 of the 7 aggregate roots, all mutated after insert | [`FW-207`](../../Backend/tasks/FW-207.md) · [`FW-142`](../../Backend/tasks/FW-142.md) |
| **Before T2** | **`G2` / `OI-39`** — cross-DB check-in recovery | Compensation design; carries the **24–64 h** reserve. **Phase 4 is provisional until it closes.** ⚠ `D-32` **narrowed** it — `INFLAT` left the cross-boundary set, taking it from four shared writes to three — but did not close it | [`FW-157`](../../Backend/tasks/FW-157.md) · `[INT §8.0]` |
| **Before T2** | ⛔ **`Q22`** — the four min/max tolerance pairs are **owed by e-mail** and `AlloyProperty` is **deliberately unseeded** | `CHK007` is a band check at **both** stations and cannot be exercised. Diameter and ovality are `NULL` by design — **do not seed them** | [`FlatWireSchema_Lookup.md`](../Schema/FlatWireSchema_Lookup.md) · [`FW-168`](../../Backend/tasks/FW-168.md) |
...
> [`Backend/tasks/Orchestration.md`](../../Backend/tasks/Orchestration.md)
> and [`Frontend/tasks/Orchestration.md`](../../Frontend/tasks/Orchestration.md)
