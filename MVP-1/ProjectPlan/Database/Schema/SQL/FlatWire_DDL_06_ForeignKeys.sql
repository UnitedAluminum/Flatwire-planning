...
WHERE fk.name LIKE 'FK_FlatWire%'
   OR t.name IN (
       -- MVP-1 tables only. PassSchedule, PassScheduleComponent and
       -- PassScheduleChangeLog are NOT MVP-1 tables: pass schedule
       -- generation and management are owned by a separate track.
       --
       -- There is therefore NO PassScheduleId foreign key to add or to
       -- restore here. PassScheduleId on FlatWireRun, RodCheckin,
       -- SpoolCheckin and CoilOutput is a DOCUMENTED EXTERNAL
       -- REFERENCE -- the same class as PlanId, CoilOrderPlanId and
       -- CoilOutput.SkidId, none of which has a local parent either.
       -- See phase-01c-database-foundation.md, "Cross-DB logical FKs".
       --
       -- CoilOutput and CoilTraceability ARE MVP-1 (Phase 9 is wholly
       -- MVP-1) and are listed below.
       'FlatWireRun','Spool','FlatWireRunDetail',
