...
| `build_development_plan_xlsx.py`'s multi-stream bug | [`FW-250`](FW-250-Development-Plan-Xlsx-Multi-Stream.md) — a sibling tooling defect, same `S1` |
...
| Thing | State |
|---|---|
| [`verify_schema_counts.py`](../../Tools/verify_schema_counts.py) | ✅ Built — six checks `C1`–`C6` |
...
| **Contradiction 1** | `Database/DatabaseDesign.md` **`:34`** — *"the current scripts measure … **69** index statements"*, against **`:24`** and **`:327`** which both say **70**. ⛔ **Same file, three claims, two values** |
...
| ⛔ **A live `C2` failure** | **`FLatWire_DDL_02_Schedule.sql` creates tables but has no `"Tables :"` header block** — the run **FAILS** today. See below |
...
[`FW-244`](FW-244-G49-Nine-Requirements-No-Column.md) and `FW-234` both **move the counted
...
one re-opens the same blind spot. [`FW-250`](FW-250-Development-Plan-Xlsx-Multi-Stream.md) is
