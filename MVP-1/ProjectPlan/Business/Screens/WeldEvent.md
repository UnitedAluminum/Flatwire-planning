...
| **Maximum weld joints per coil** — a validation rule the system must enforce | `[CLIENT INPUT REQUIRED]` — the limit is unknown (Q7) |
| **Re-weld traceability** — if a weld is remade, both events recorded | `[CLIENT INPUT REQUIRED]` — whether both must appear on the certificate (OI-59). **Two routes produce this now:** a weld that breaks mid-run, and (since D7) a weld that fails its quality check before anything runs through the join |
| **Certificate frequency** — per coil, per order, or per heat | `[CLIENT INPUT REQUIRED]` (Q8) |
...
| **OI-59** | Medium | **Re-welds on the certificate** — if a weld breaks and is re-welded, must both events appear? | Whether a re-weld is a new record or an annotation |
| **Q8** | Medium | **Certificate frequency** — per coil, per order, or per heat | Certificate trigger and report format |
...
| Q6 | Footage attribution rule at the weld point | | ☐ |
| Q7 | Maximum weld joints per finished coil | | ☐ |
| OI-59 | Re-weld representation on the certificate | | ☐ |
| Q8 | Certificate frequency | | ☐ |
