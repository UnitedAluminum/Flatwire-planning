...
The per-phase owner, hours, days and window table is owned by [`Development/Roadmap.md`](Roadmap.md), which indexes the 15 phase specifications in [`Development/Phases/`](Phases/). It was duplicated here until 13 Aug 2026.
...
The prerequisite chain from [`Development/GapsRegister.md`](GapsRegister.md), expressed sprint-to-sprint:
...
1. **`Development/GapsRegister.md` draws Phase 8 → 9 → 10**, implying FL3 hybrid depends on the FL2 **spool check-in**. **It does not — FL3 has no intermediate spool.** FL3 depends on Phases 4, 5, 6 and 9. Corrected above.
...
The `G##` register is owned by [`Development/GapsRegister.md`](GapsRegister.md). The blockers that stop specific stories are in §7.4.
