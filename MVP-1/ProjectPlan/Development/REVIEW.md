...
The DevelopmentPlan is thorough and largely self-aware — its own [`ShopfloorPlan/back-matter.md`](GapsRegister.md) **Gaps register** (`G1`–`G36` at the time of review; `G1`–`G52` today) already tracks many of the issues below as "Open." The dominant problem is **doc split-brain**:
...
3. **Persist AGC readings** (Tier 2 / **G3**) — add a `RunReading` table; FL2 profile + Gauge-Trace/Cut-Traceability reports currently have no data source. *(Done in [1C](Phases/phase-01c-database-foundation.md).)*
4. **Add FK/`RunId` indexes and correct the doc↔DDL precision drift** (Tier 3) — hub-join performance + a regeneration hazard. *(Indexes/precision addressed in [1C](Phases/phase-01c-database-foundation.md).)*
...
50. ⚠ **RE-CLOSED 25 Aug 2026, against a baseline that has moved twice since.** The 13 Aug closure pointed at **25 MVP-1 / 28 in the full design**; `D-31` then took it to **34** and the 23 Aug `SpoolConfiguration` merge (`Q60`) to **33 tables · 55 FKs · 69 index statements · 1 procedure · 1 trigger**, which is the live figure. ⚠ **A closure that names a figure goes stale when the figure moves** — this one read as closed while pointing at a number wrong by eight. The counts are now asserted at **exactly one site, `[DBD §6.2]`**, and checked by [`verify_schema_counts.py`](../Tools/verify_schema_counts.py); the 25 Aug sync corrected ~35 live restatements across 24 files. **Do not re-state the figure here.** ~~**Table-count drift**~~ — 20 (`FlatWireJiraStories.md:1466`) vs 21 (`back-matter.md`, post-`Rod`-drop) vs 22 (ERD); the roadmap change log is itself self-inconsistent (58/58 vs 44/44; 22 vs 21). *(With the new `RunReading` table the current count is **22**.)*
...
[04-APIContract.md](../Backend/APIs.md) attributes to this document. A read-only **Welds this run** dialog was built on
...
| Gap | Outcome |
|---|---|
| ~~**G25**~~ | **WITHDRAWN 1 Aug 2026 — built rather than deferred** (the *Welds this run* dialog). It was withdrawn before it was ever registered, so nothing cited it, and **the ID was reused on 13 Aug 2026** for the requirement-coverage gap now in [`back-matter.md`](GapsRegister.md) |
