...
> longer exists. Current layout: [`RodPreCheckin.md`](../RequirementDocuments/RodPreCheckin.md) — the owning specification since 1 Aug 2026. `Analysis/FlatWireShopfloorDashboards.md` § *Dashboard 2A* is the design record only and is **no longer a requirements source**.

---

## The prompt

> Analyse `@MVP-1/Mockups/dashboard_2a_rod_precheckin.html` and produce a plan for how this UI could be
...
> 2. `MVP-1/DevelopmentPlan/ShopfloorPlan/00-foundations.md` — §0.2 reference-codebase rules, §0.3 domain
...
> 3. `MVP-1/DevelopmentPlan/ShopfloorPlan/phase-04-rod-checkin-plc-config.md` — the phase that owns this
>    screen — plus `phase-02-pass-schedule-management.md` (its gating dependency).
> 4. `MVP-1/DevelopmentPlan/ShopfloorPlan/back-matter.md` — the **G1–G18 gap register**.
> 5. `MVP-1/RequirementDocuments/RodPreCheckin.md` — **the dedicated analysis for this exact screen; read it first of
>    the five.** Also `MVP-1/RequirementDocuments/WeldEvent.md`, `Analysis/FlatWireShopfloorDashboards.md` (the
...
> 6. `MVP-1/DevelopmentPlan/REVIEW.md` — tells you which specs carry known contradictions.
> 7. `MVP-1/Mockups/flat-wire-shopfloor.styles.scss` and `MVP-1/Mockups/flat-wire-fit.js` — the design tokens
...
>   (`dashboard_2_rod_checkin.html`), Dashboard 4, or Dashboard 8 — but do say when a problem
...
>    `MVP-1/DBChanges/Schema/` (`RodStaging`, `RodCheckin`, `WeldEvent`, `FlatWireRun`) and
>    `MVP-1/DevelopmentPlan/APIContracts.md`. Flag every field the UI shows that has no column, and every
...
**Comparative pass** — add: "Read `MVP-1/Mockups/dashboard_2_rod_checkin.html` and
`MVP-1/Mockups/dashboard_2_rod_checkin_fl3.html` alongside it, and say where 2A duplicates, contradicts,
