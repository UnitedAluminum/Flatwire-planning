...
2. **25 tables** exist (incl. `RunReading` **and `Rod`** — `D-04` retains it, superseding the "drop `Rod`" position in [Foundations](./00-foundations.md) decision 3); all FKs and recommended indexes present. The three `PassSchedule*` tables are **not** built here — the pass schedule is owned outside MVP-1.
