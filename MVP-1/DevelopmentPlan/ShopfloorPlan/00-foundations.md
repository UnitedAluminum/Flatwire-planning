...
> 3. ~~**Rod uses the existing `coils` table** — the designed `Rod` table is **dropped** … `FlatWireDB` drops to **21 tables**.~~ **SUPERSEDED by master-spec `D-04` (29 Jul 2026, the "Hybrid foundation" decision).** `Rod` **is retained** as a `FlatWireDB`-local master mirroring the shared `coils` record, with **enforced** rod-alpha FKs, and the schema is **27 tables** — **24 of them MVP-1**, the other three being the MVP-2 pass-schedule group. A clean deploy on 11 Aug 2026 confirms 24 tables / 33 FKs / 1 procedure / 1 trigger. `coils` remains the source of truth for the rod *lifecycle* (receipt, status incl. `INFLAT`, chemistry/heat, lot); `Rod` mirrors it locally so the FKs can be enforced. **Anything still saying "21 tables" or "`Rod` is dropped" is stale** — that was gap `G12`, and this is its resolution.
