...
- [ ] FL3 (hybrid) is represented as a single schedulable entity that simultaneously blocks FL1 and FL2 when scheduled (see OQ-2 / OQ-67 — confirm with Tim O. before build)
...
**Open Questions:** OQ-2 (FL3 scheduling representation), OQ-67 (FL1/FL2 simultaneous independent operation) must be resolved before implementation.
...
**Dependencies:** FW-040
**Open Questions:** OI-60 (metallic yield per route) and OQ-10 (footage-to-weight conversion) must be resolved — these drive how weight is converted to stops.
...
- [ ] Pricing auto-population method to be confirmed (OI-81) — placeholder for now
...
- [ ] "Acknowledge Pass Schedule & Begin Check-in" button: validates all fields → records inspection → pushes PLC tags → starts run timer → **returns to Dashboard 2A** to stage the next rod (`FR-079a`, revised 1 Aug 2026; was Dashboard 3, which stays reachable from the app bar and Line Status — client confirmation pending, **OI-109**)
- [ ] PLC tags are only pushed after explicit acknowledgment — no automatic push
- [ ] Rod status updated to `INFLAT` on acknowledgment
- [ ] Pass schedule selection: system loads the schedule linked to the active job; OQ-14 (selection mechanism) must be resolved before this story is built

**Dependencies:** FW-010, FW-020, FW-080, FW-002
**Open Questions:** OQ-14 (pass schedule selection at check-in), OQ-15 (FL3 hybrid — one or two schedules)
...
**Blocked by:** **OQ-17** (which statuses count as available), **OQ-76/OI-02** (identifier and format), **OI-06** (two status vocabularies), and — critically — confirmation that **`Spool.OrderNo` is populated from planning**, without which the order cannot be resolved at all
...
- [ ] Net Weight: calculated from footage × footage-to-weight conversion factor (OQ-10 must be resolved)
...
- [ ] Shannon R. to confirm final list of rejection reason groups and reasons before build (OI-84)
...
- [ ] Approval authority for mid-run checkout must be confirmed (OQ-74)

**Dependencies:** FW-061, FW-071
**Open Questions:** OQ-12 (partial-rod re-check-in), OQ-74 (checkout authorisation level), OQ-13 (PLC tag behaviour on checkout), OQ-75 (partial-run material disposition authority)
...
- [ ] Placeholder configuration deployed; full activation after OI-68 and OI-68 are decided

**Dependencies:** FW-003
**Open Questions:** OI-68 (costing standards), OI-68 (standard times)
...
**Dependencies:** None
**Open Questions:** OI-83 (baler maximum dimensions), OI-83 (scrap banding material)
...
- [ ] All Critical open questions (OI-88, OQ-2, OQ-3, OQ-76, OI-49, OQ-61, OI-60, OQ-10, OQ-67, OQ-14, OQ-15) resolved before UAT begins
...
|----|----------|--------|-------|
| OI-88 | Pass schedule management — UI vs. table? | FW-012 | Tim O. |
| OQ-2 | FL3 scheduling representation | FW-030 | Tim O. / Stephen |
| OQ-3 | Traveler screen fields per station | FW-061/064 | Jaspreet / Tim O. |
| OQ-76 | FL2 spool check-in identifier | FW-064 | Jaspreet / Tim O. |
| OI-49 | Inventory type for rods in coils table | FW-020 | Tim O. / Jeff G. |
| OQ-61 | Mid-run pass schedule change — alpha handling | FW-061 | Jaspreet / Tim O. |
| OI-60 | Expected metallic yield per route | FW-041 | Tim O. / Jeff G. |
| OQ-10 | Footage-to-weight conversion factor | FW-100, FW-066 | Tim O. / Bob S. |
| OQ-67 | FL1/FL2 simultaneous independent operation | FW-030 | Tim O. / Stephen |
| OQ-74 | Mid-run checkout authorisation level | FW-072 | Tim O. / Shannon R. |
| OQ-13 | PLC tag behaviour on checkout | FW-072, FW-082 | Jaspreet / Tim O. |
| OQ-14 | Pass schedule selection mechanism at check-in | FW-061 | Tim O. / Jaspreet |
| OQ-15 | FL3 hybrid — one or two pass schedules? | FW-061, FW-030 | Tim O. / Jaspreet |
