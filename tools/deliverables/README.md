...
| [`build_questions_xlsx.py`](build_questions_xlsx.py) | both `Analysis/` question registers + `ClientQuestionsContent.md` | `../../deliverables/FlatWire_ClientQuestions.xlsx` | coverage · drift · team names · leakage |
| [`build_development_plan_xlsx.py`](build_development_plan_xlsx.py) | `../../60-delivery/TaskBreakdown.md` + `../../60-delivery/StaffedSprintPlans.md` + `DevelopmentPlanContent.md` | `../../deliverables/FlatWire_DevelopmentPlan.xlsx` | coverage · drift · team names · leakage |
| [`build_trial_run_xlsx.py`](build_trial_run_xlsx.py) | `../../60-delivery/TrialRunPlan.md` + `../../60-delivery/TaskBreakdown.md` + `TrialRunContent.md` | **`../../60-delivery/FlatWire_TrialRunPlan.xlsx`** | reconciliation · coverage · title · drift · **abbreviation** |
| [`build_allocation_examples_xlsx.py`](build_allocation_examples_xlsx.py) | `../../95-archive/design-notes/RodOrderAllocation.md` §0.1 and §7 + `../../10-requirements/BusinessRequirements.md` §5.28 + `[DBD §6.6]` + `AllocationExamplesContent.md` | `../../deliverables/FlatWire_OrderAllocationExamples.xlsx` | **arithmetic** · coverage · drift · team names · leakage |
| [`build_formula_review_xlsx.py`](build_formula_review_xlsx.py) | nothing — the content is inline, and the arithmetic is computed from the client's own formulas rather than quoted | `../../deliverables/FlatWire_FormulaReview.xlsx` | leakage |
| [`build_coverage_matrix.py`](build_coverage_matrix.py) | `../../10-requirements/BusinessRequirements.md` + `../../70-testing/TestCases.md` + `../../60-delivery/TaskBreakdown.md` | nothing — reports | a requirement with no case and no §10.4 entry |
...
Nothing is recreated. `../../deliverables/PassScheduleGenerationSpec.docx` is opened as a template and its body is stripped, **keeping only the final `<w:sectPr>`** — which is the element that carries `headerReference` and `footerReference`. The output therefore inherits, byte for byte:
...
- **Structure** — effort, phase, discipline, sprint, dates, working days, utilisation, finish
  dates — is parsed from [`../../60-delivery/TaskBreakdown.md`](../../60-delivery/TaskBreakdown.md)
  and [`../../60-delivery/StaffedSprintPlans.md`](../../60-delivery/StaffedSprintPlans.md), so it
...
- **Structure** — every figure, story identifier, sprint, date, phase and blocker — is parsed
  from [`../../60-delivery/TrialRunPlan.md`](../../60-delivery/TrialRunPlan.md), so it cannot drift.
  Titles come from [`../../60-delivery/TaskBreakdown.md`](../../60-delivery/TaskBreakdown.md), which
