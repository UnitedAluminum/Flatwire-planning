...
HOW THE BRANDING WORKS
    Nothing is recreated. MVP-2/SRS/PassScheduleGenerationSpec.docx is opened as a
    template and its body is stripped, keeping only the final <w:sectPr> - which carries
...
GOTCHA
    ...
    in MVP-1/SRS/ is the tell. Close it and re-run.
"""
...
SRC = os.path.abspath(_args[0]) if len(_args) > 0 else \
    os.path.join(ROOT, '20-architecture', 'PLCTagSpecification.md')
OUT = os.path.abspath(_args[1]) if len(_args) > 1 else \
    os.path.join(ROOT, 'deliverables', 'PLCTagSpecification.docx')
