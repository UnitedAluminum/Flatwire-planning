...
def render_block(rows, cid, derived, expected):
    """expected: how many stories the map assigns to this category.
    ...
    if expected == 0:
        ...
        return "\n".join(L)
    if derived:
        L.append('> ⚙ **Generated by `tools/build_features.py` from the stories’ '
                 'own front-matter.** '
    ...
    if not rows:
        ...
        L.append(END)
        return '\n'.join(L)
    L.append('**%d stor%s in this category, each retained as its own file.** One activity per '
             'story. **Activities are expected to '
...
def selftest():
    ...
    if want_total and n > want_total:
        print('build_features: SELFTEST FAILED - round-tripped %d activities, the map '
              'the map assigns only %d' % (n, want_total))
        return 1
    if want_total and n < want_total:
